...
def _map_reference_ids(map_info: BpfMapInfo, entries: Sequence[Mapping[str, str]]) -> list[int]:
    if int(map_info.map_type) not in RECURSIVE_REFERENCE_MAP_TYPES:
        return []

    referenced_ids: set[int] = set()
    for entry in entries:
        value_hex = str(entry.get("value_hex", "") or "")
        try:
        ...
        if len(value) < 4:
            continue
        referenced_id = int.from_bytes(value[:4], byteorder="little", signed=False)
        if referenced_id > 0:
            referenced_ids.add(referenced_id)
    return sorted(referenced_ids)


def _inline_entries_by_map_id(
    inline_capture_program: InlineCaptureProgram | None,
) -> dict[int, list[dict[str, str]]]:
    grouped: dict[int, list[dict[str, str]]] = {}
    if inline_capture_program is None:
        return grouped
    for entry in inline_capture_program.inlined_map_entries:
        grouped.setdefault(int(entry.map_id), []).append(
            {
                "key_hex": str(entry.key_hex),
                "value_hex": str(entry.value_hex),
            }
        )
    return grouped


def _capture_map_record(
    map_id: int,
    *,
    selected_entries: Sequence[Mapping[str, str]] | None = None,
) -> tuple[dict[str, object], list[int], list[str]]:
    map_fd = bpf_map_get_fd_by_id(map_id)
    try:
        map_info = bpf_map_get_info(map_fd)
        capture_source = "live_dump"
        if selected_entries:
            entries = [dict(entry) for entry in selected_entries]
            capture_error = None
            capture_source = "inline_capture"
        else:
            try:
        ...
        if capture_error is not None:
            ...
            return map_record, [], [f"map {map_id}: {capture_error}"]
        return map_record, _map_reference_ids(map_info, entries), []
    finally:
        os.close(map_fd)
...
def capture_program(
    ...
    *,
    ...
    try:
        prog_info, root_map_ids = bpf_prog_get_map_ids(prog_fd)
        kernel_prog_name = _decode_name(prog_info.name) or (
            inline_capture_program.kernel_prog_name
            if inline_capture_program is not None
            else f"prog_{prog_id}"
        )
        qualified_prog_name = (
        ...
        inline_entries_by_map_id = _inline_entries_by_map_id(inline_capture_program)
        pending_map_ids = sorted(
            {
                *(int(map_id) for map_id in root_map_ids),
                *(int(map_id) for map_id in inline_entries_by_map_id),
            }
        )
        while pending_map_ids:
            map_id = pending_map_ids.pop(0)
            if map_id in seen_map_ids:
                continue
            seen_map_ids.add(map_id)
            map_record, referenced_ids, capture_errors = _capture_map_record(
                map_id,
                selected_entries=inline_entries_by_map_id.get(map_id),
            )
            maps_payload.append(map_record)
            map_errors.extend(capture_errors)
            for referenced_id in referenced_ids:
                if referenced_id not in seen_map_ids:
                    pending_map_ids.append(referenced_id)

        maps_payload.sort(key=lambda item: int(item.get("map_id", 0)))
        fixture_payload = {
            "prog_id": int(prog_id),
        ...
        return {
            ...
            "inline_entry_count": len(inline_capture_program.inlined_map_entries)
            if inline_capture_program is not None
            else 0,
...
def capture_all_programs(
    *,
    ...
    live_program_ids = iter_prog_ids()
    live_program_id_set = set(live_program_ids)
    programs_seen = len(live_program_ids)
    ...
    for prog_id in requested_program_ids:
        if prog_id not in live_program_id_set:
            skipped_programs.append(
                {
                    "prog_id": int(prog_id),
                    "reason": "requested program ID was not found among live programs",
                }
            )
            continue
        ...
        spec = (
            ...
            if program_specs is not None
            else _default_program_spec(
                prog_id,
                program.kernel_prog_name if program is not None else f"prog_{prog_id}",
            )
        )
        try:
            results.append(
                capture_program(
                    ...
                    inline_capture_program=program,
                )
            )
        except Exception as exc:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Capture live BPF map state into per-program corpus fixtures."
    )
    ...
    parser.add_argument(
        "--inline-capture-json",
        help="Optional JSON array describing per-program daemon optimize results; used for program selection metadata only.",
    )
    parser.add_argument(
        "--manifest-json",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    inline_capture_programs = (
        load_inline_capture_programs(Path(args.inline_capture_json).resolve())
        if args.inline_capture_json
        else {}
    )
