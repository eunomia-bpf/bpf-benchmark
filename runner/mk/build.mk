...
$(X86_RUNNER_RUNTIME_IMAGE_TAR): $(X86_RUNNER_RUNTIME_CONTAINERFILE) $(DOCKERIGNORE_FILE)
	@mkdir -p "$(dir $@)"
	$(CONTAINER_RUNTIME) build --platform linux/amd64 -t "$(X86_RUNNER_RUNTIME_IMAGE)" -f "$(X86_RUNNER_RUNTIME_CONTAINERFILE)" "$(ROOT_DIR)"
	$(CONTAINER_RUNTIME) save -o "$@" "$(X86_RUNNER_RUNTIME_IMAGE)"

$(ARM64_RUNNER_RUNTIME_IMAGE_TAR): $(ARM64_RUNNER_RUNTIME_CONTAINERFILE) $(DOCKERIGNORE_FILE)
	@mkdir -p "$(dir $@)"
	$(CONTAINER_RUNTIME) build --platform linux/arm64 -t "$(ARM64_RUNNER_RUNTIME_IMAGE)" -f "$(ARM64_RUNNER_RUNTIME_CONTAINERFILE)" "$(ROOT_DIR)"
