...
TRACEE_BUILD_TIMEOUT ?= 90m
ACTIVE_TRACEE_GO_ENV := $(if $(filter arm64,$(RUN_TARGET_ARCH)),GOMAXPROCS=1 GOFLAGS=-p=1,)
PREBUILT_TRACEE_BPF_ROOT ?=
...
$(ACTIVE_TRACEE_REQUIRED) &: $(TRACEE_SOURCE_FILES) $(BUILD_RULE_FILES)
	...
	if [ -n "$(PREBUILT_TRACEE_BPF_ROOT)" ]; then \
		make -C "$$repo_root" -j"$(JOBS)" \
			...
			CMD_CLANG=clang \
			CMD_GO=/bin/false \
			"$$dist_root/libbpf/obj/libbpf.a"; \
		install -m 0644 "$(PREBUILT_TRACEE_BPF_ROOT)/tracee.bpf.o" "$$dist_root/tracee.bpf.o"; \
		mkdir -p "$$dist_root/lsm_support"; \
		install -m 0644 "$(PREBUILT_TRACEE_BPF_ROOT)/lsm_support/kprobe_check.bpf.o" "$$dist_root/lsm_support/kprobe_check.bpf.o"; \
		install -m 0644 "$(PREBUILT_TRACEE_BPF_ROOT)/lsm_support/lsm_check.bpf.o" "$$dist_root/lsm_support/lsm_check.bpf.o"; \
		touch "$$dist_root/tracee.bpf.o" "$$dist_root/lsm_support/kprobe_check.bpf.o" "$$dist_root/lsm_support/lsm_check.bpf.o"; \
	fi; \
	timeout --kill-after=2m "$(TRACEE_BUILD_TIMEOUT)" \
		env $(ACTIVE_TRACEE_GO_ENV) \
...
$(ACTIVE_TETRAGON_REQUIRED) &: $(TETRAGON_SOURCE_FILES) $(BUILD_RULE_FILES)
	...
	find "$$bpf_output_root" -maxdepth 1 -type f \( -name '*.o' -o -name '*.o.gz' \) \
		-exec ln -f {} "$$artifact_root"/ \; ; \
