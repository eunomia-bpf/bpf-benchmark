...
from ..corpus import materialize_katran_packet
...
def ip_command(command: Sequence[str], *, check: bool = True, timeout: int | float | None = 30) -> subprocess.CompletedProcess[str]:
    return run_command([ip_binary(), *_normalize_ip_command(command)], check=check, timeout=timeout)


def ip_json_command(command: Sequence[str], *, timeout: int | float | None = 30) -> object:
...
def ns_exec_command(
    ...
    *,
    ...
    return run_command([ip_binary(), "netns", "exec", namespace, *_normalize_exec_command(command)], check=check, timeout=timeout)
...
def set_ns_sysctl(namespace: str, key: str, value: int) -> None:
    completed = ns_exec_command(namespace, ["sysctl", "-q", "-w", f"{key}={value}"], check=False, timeout=15)
    if completed.returncode == 0:
        return
    proc_key = key.replace(".", "/")
    ns_exec_command(namespace, ["sh", "-c", f"printf '%s' '{value}' > /proc/sys/{proc_key}"], timeout=15)
...
def get_link_json(namespace: str | None, iface: str) -> dict[str, object]:
    ...
    if namespace is not None:
        payload = run_json_command([ip_binary(), "netns", "exec", namespace, ip_binary(), *command], timeout=30)
    else:
...
def parse_wrk_output(stdout: str, stderr: str) -> dict[str, object]:
    ...
    for pct_match in WRK_LATENCY_DIST_RE.finditer(stdout):
        latency[f"p{pct_match.group(1)}"] = parse_wrk_latency_ms(pct_match.group(2))
...
def run_wrk(*, namespace: str, duration_s: int | float, connections: int, threads: int) -> dict[str, object]:
    binary = wrk_binary()
    if binary is None:
    ...
    completed = run_command(command, check=False, timeout=max(30, int(float(duration_s) * 4) + 10))
...
def run_katran_prog_test_run(session: KatranDirectSession) -> dict[str, object]:
    ...
    try:
        result = api.prog_test_run(prog_fd, packet, data_out_size=max(256, len(packet) + 64))
    finally:
        os.close(prog_fd)
    result.update(
        {
