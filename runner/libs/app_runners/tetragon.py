...
def inspect_tetragon_setup() -> dict[str, object]:
    artifact_root = repo_artifact_root() / "tetragon"
    tetragon_artifact_binary = artifact_root / "bin" / "tetragon"
    bpf_artifact_dir = artifact_root
    ...
    if tetragon_binary is None:
        return {
            ...
            "tetragon_bpf_lib_dir": None,
            "stdout_tail": "",
            "stderr_tail": f"missing repo-managed Tetragon binary under {tetragon_artifact_binary}",
        }

    bpf_lib_dir = bpf_artifact_dir if bpf_artifact_dir.is_dir() else None
    if bpf_lib_dir is None or not any(bpf_lib_dir.glob("*.o")) and not any(bpf_lib_dir.glob("*.bpf.o")):
        return {
            ...
            "tetragon_bpf_lib_dir": None,
            "stdout_tail": "",
            "stderr_tail": f"missing Tetragon .bpf.o artifacts under {bpf_artifact_dir}",
        }
