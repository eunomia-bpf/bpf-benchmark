...
SIMPLE_E2E_DEFAULT_DURATION_S = 5


def run_simple_app_e2e_case(
    args: object,
    *,
    runner_name: str,
    workload_kind: str,
    ...
    """Run an e2e measurement on a single-runner / single-workload app.

    Used for apps whose only e2e behaviour is "start runner, measure throughput,
    stop". Multi-tool / multi-policy apps (bcc, bpftrace, tracee, tetragon) keep
    custom case modules because their iteration shape differs.
    """
    ...
    prepared = getattr(args, "_prepared_daemon_session", None)
    if prepared is None:
        raise RuntimeError("prepared daemon session is required")
    duration_s = int(int(getattr(args, "duration", 0) or 0) or SIMPLE_E2E_DEFAULT_DURATION_S)
    runner = get_app_runner(runner_name, workload=workload_kind, **dict(runner_kwargs or {}))

    def measure(lifecycle: CaseLifecycleState, _phase_name: str) -> dict[str, object]:
        before = sample_bpf_stats(lifecycle.prog_ids)
        result = runner.run_workload(float(duration_s))
        after = sample_bpf_stats(lifecycle.prog_ids)
        return {
            "status": "ok",
            "measurement": {
    ...
    def build_state(active_runner: AppRunner, started_prog_ids: list[int]) -> CaseLifecycleState:
        prog_ids = [int(value) for value in started_prog_ids if int(value) > 0]
        if not prog_ids:
            raise RuntimeError(f"{repo_label} runner did not expose any live prog_ids")
        programs = program_records(active_runner.programs)
        if not programs:
            raise RuntimeError(f"{repo_label} runner did not expose any live programs")
        return CaseLifecycleState(
            runtime=active_runner,
            prog_ids=prog_ids,
            artifacts={
                "programs": programs,
                "rejit_policy_context": {"repo": repo_label, "level": "e2e"},
            },
        )
    ...
    payload: dict[str, object] = {
        ...
        "daemon": str(Path(getattr(args, "daemon", "")).resolve()),
        "duration_s": duration_s,
        "workload_spec": {"kind": workload_kind},
        "host": host_metadata(),
...
def simple_app_e2e_markdown(title: str) -> Callable[[Mapping[str, object]], str]:
    def build(payload: Mapping[str, object]) -> str:
        return "\n".join([
            f"# {title} End-to-End Benchmark",
            "",
            "```json",
            json.dumps(payload, indent=2, sort_keys=True, default=str),
            "```",
