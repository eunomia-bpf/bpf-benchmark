from __future__ import annotations

import os
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from runner.libs.run_contract import RunConfig


PYTHON_IMPORT_MAP = {
    "PyYAML": "yaml",
    "pyelftools": "elftools",
}

def _contract_scalar(contract: "RunConfig | None", name: str) -> str:
    if contract is None:
        return os.environ.get(name, "").strip()
    return contract.scalar(name)


def env_csv(name: str, *, contract: "RunConfig | None" = None) -> list[str]:
    value = _contract_scalar(contract, name)
    if not value:
        return []
    return [token for token in value.split(",") if token]


def python_import_name(package_name: str) -> str:
    try:
        return PYTHON_IMPORT_MAP[package_name]
    except KeyError as exc:
        raise RuntimeError(f"unsupported python package contract: {package_name}") from exc


def inside_runtime_container() -> bool:
    return os.environ.get("BPFREJIT_INSIDE_RUNTIME_CONTAINER", "").strip() == "1"


def active_python_bin(contract: "RunConfig | None" = None) -> str:
    if inside_runtime_container():
        return _contract_scalar(contract, "RUN_RUNTIME_PYTHON_BIN") or _contract_scalar(contract, "RUN_REMOTE_PYTHON_BIN")
    return _contract_scalar(contract, "RUN_REMOTE_PYTHON_BIN")


def runtime_container_enabled(contract: "RunConfig | None" = None) -> bool:
    return bool(_contract_scalar(contract, "RUN_RUNTIME_CONTAINER_IMAGE"))


def required_commands(*, contract: "RunConfig | None" = None) -> list[str]:
    commands: list[str] = []
    if runtime_container_enabled(contract) and not inside_runtime_container():
        tokens = (
            _contract_scalar(contract, "RUN_REMOTE_PYTHON_BIN"),
            _contract_scalar(contract, "RUN_CONTAINER_RUNTIME") or "docker",
        )
    else:
        tokens = (
            _contract_scalar(contract, "RUN_BPFTOOL_BIN"),
            active_python_bin(contract),
            *env_csv("RUN_REMOTE_COMMANDS_CSV", contract=contract),
            *workload_tool_commands(contract=contract),
        )
    for token in tokens:
        if token and token not in commands:
            commands.append(token)
    return commands


def workload_tool_commands(*, contract: "RunConfig | None" = None) -> list[str]:
    if runtime_container_enabled(contract) and not inside_runtime_container():
        return []
    if _contract_scalar(contract, "RUN_NEEDS_WORKLOAD_TOOLS") != "1":
        return []
    return ["hackbench", "sysbench", "wrk"]
