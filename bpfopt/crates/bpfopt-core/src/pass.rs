...
pub struct BpfProgram {
    ...
    /// Populated externally (e.g. from map-values.json). Used by MapInlinePass
    /// instead of live kernel map lookups.
    pub map_values: HashMap<(u32, Vec<u8>), Vec<u8>>,
    /// Pre-loaded map metadata: map_id → MapMetadata.
    /// Populated externally. Used by MapInfoAnalysis and MapInlinePass
    /// instead of live kernel map info lookups.
    pub map_metadata: HashMap<u32, MapMetadata>,
}

/// Pre-loaded map metadata (replaces live kernel bpf_map_get_info_by_id).
#[derive(Clone, Debug)]
...
pub struct PassContext {
    /// Available kinsn targets and their descriptor metadata.
...
pub struct KinsnRegistry {
    ...
    /// Per-target call offsets used when emitting kinsn calls.
    /// `0` means no extra offset metadata is required.
