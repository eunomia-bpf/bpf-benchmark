...
fn apply_profile(program: &mut BpfProgram, io_args: &IoArgs) -> Result<()> {
    ...
    for (pc_str, counts) in &profile.per_insn {
        let pc: usize = pc_str.parse()
            .with_context(|| format!("invalid PC in profile: {}", pc_str))?;
        if pc < program.annotations.len() {
            program.annotations[pc].branch_profile = Some(
                bpfopt_core::pass::BranchProfile {
...
fn apply_map_values(program: &mut BpfProgram, io_args: &IoArgs) -> Result<()> {
    ...
    for map in &mv.maps {
        program.map_metadata.insert(map.map_id, MapMetadata {
...
fn apply_side_inputs(program: &mut BpfProgram, ctx: &mut PassContext, io_args: &IoArgs) -> Result<()> {
...
fn run_single_pass(pass_name: &str, io_args: &IoArgs) -> Result<()> {
    ...
    use bpfopt_core::analysis::{BranchTargetAnalysis, CFGAnalysis, LivenessAnalysis, MapInfoAnalysis};
