...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Auto-tune corpus per-program v5 policy allowlists via census, "
            "all-apply screening, leave-one-out ablation, and pairwise combo search."
        )
    )
    parser.add_argument(
        "--inventory-json",
...
def render_policy_text(
    *,
    ...
    project = relative.parts[0] if relative.parts else "unknown"
    lines = [
        "version: 1",
        "",
        "match:",
        "  layer: corpus",
        f"  project: {project}",
        f"  object: {object_policy_stem(object_path)}",
        f"  program: {program_name}",
        "",
        "selection:",
        "  mode: allowlist",
        "  families: [" + ", ".join(canonical_families(allowlist)) + "]",
        "",
        "provenance:",
        "  source: auto-tuner",
