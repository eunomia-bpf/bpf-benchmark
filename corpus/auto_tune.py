...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Auto-tune corpus per-program version 2 per-site policy artifacts "
...
def render_policy_text(
    *,
    ...
    sites.sort(key=lambda item: (int(item["insn"]), str(item["family"])))
    return render_policy_v2_text(
        program_name=program_name,
        default_action="skip" if sites else "stock",
        sites=sites,
        comments=[
