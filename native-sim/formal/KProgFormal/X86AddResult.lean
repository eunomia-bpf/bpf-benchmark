...
theorem x86_add_step_refines (a b sign : BitVec 64) (width:X86Width) :
    generatedX86AddFlags (GeneratedX86Width.narrow a width)
      (GeneratedX86Width.narrow b width)
      (GeneratedX86Width.narrow (GeneratedX86Adc.result a b false) width) sign =
    x86AddFlagsSpec (x86NarrowSpec a width) (x86NarrowSpec b width)
      (x86NarrowSpec (x86AddResultSpec a b) width) sign := by
  rw [x86_add_flags_refine, x86_add_result_refines]
