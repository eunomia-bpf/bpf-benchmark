...
replaced from the generated result. -/
def generatedX86NegLaneHandler (state : X86RegAluState) (width : X86Width)
    (dstLane : X86ByteLane) : X86RegAluState :=
  let value := GeneratedX86RegRead.readAt state.dst.bits width dstLane
  let result := GeneratedX86SbbResult.result 0 value false
  { dst := generatedX86RegWriteAt state.dst result width dstLane
    flags := generatedX86SubFlags 0 (GeneratedX86Width.narrow value width)
      (GeneratedX86Width.narrow result width)
      (GeneratedX86Width.signMask width) }

def x86NegLaneHandlerSpec (state : X86RegAluState) (width : X86Width)
    (dstLane : X86ByteLane) : X86RegAluState :=
  let value := x86RegReadAtSpec state.dst.bits width dstLane
  let result := x86NegResultSpec value
  { dst := x86RegWriteAtSpec state.dst result width dstLane
    flags := x86NegFlagsSpec (x86NarrowSpec value width)
      (x86NarrowSpec result width) (x86WidthSignMaskSpec width) }

theorem x86_neg_lane_handler_refines (state : X86RegAluState)
    (width : X86Width) (dstLane : X86ByteLane) :
    generatedX86NegLaneHandler state width dstLane =
      x86NegLaneHandlerSpec state width dstLane := by
  simp only [generatedX86NegLaneHandler, x86NegLaneHandlerSpec]
  rw [x86_reg_read_at_refines, x86_neg_step_refines,
    x86_reg_write_at_refines, x86_neg_result_refines]

theorem x86_neg_high8_signed_min_overflow :
    generatedX86NegLaneHandler
      { dst := { bits := 0x1122334455668001, tag := .packet },
