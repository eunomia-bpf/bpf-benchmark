...
the ADD- and ADC-immediate lane theorems additionally compose that decoded
value through their generated result, flags, selected-lane writeback, and tag
scalarization. ADC consumes one immutable pre-state carry in both its result
and flags. Other immediate opcodes and textual parsing remain outside this
composition.
