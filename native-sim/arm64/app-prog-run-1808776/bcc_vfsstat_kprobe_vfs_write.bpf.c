...
static const struct arm64_sim_compact_step __arm64_sim_steps_bcc_vfsstat_kprobe_vfs_write[] = {
...
SEC("xdp")
int bcc_vfsstat_kprobe_vfs_write_arm64_sim_xdp(struct xdp_md *ctx)
{
	...
	for (__u32 __a64_c_guard = 0; __a64_c_guard < __a64_c_guard_limit; __a64_c_guard++) {
		if (__a64_c_pc >= 11U)
			break;
		const struct arm64_sim_compact_step *__a64_c_step = &__arm64_sim_steps_bcc_vfsstat_kprobe_vfs_write[__a64_c_pc];
