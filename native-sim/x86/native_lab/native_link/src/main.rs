...
//!      offset arithmetic).
//!
//! We keep the compiler-emitted prologue/epilogue intact. The blob runs
//! inside a BPF JIT frame, not as a normal userspace call target, so
//! post-link ABI surgery is only safe when proven for a concrete pattern.
//!
