...
3. **Tail-call counter (r9)**. The BPF JIT reserves r9 as a tail-call
   ...
   `native-sim/test/*.bpf.c` use tail calls, so this is deferred; the fix
...
`native-sim/test/Makefile` produces, per source `<name>.bpf.c`:
...
Helper declarations live in a new `native-sim/test/include/native_helpers.h`
...
Today's `native-link` (Rust, `native-sim/x86/native_lab/native_link/`) only
...
Test programs live at `native-sim/test/`:
...
1. **Phase A — design + test sources** (this doc + `native-sim/test/*.bpf.c`
...
3. **Phase C — native-link reloc extension** (~250 LOC in
   `native-sim/x86/native_lab/native_link/src/main.rs`: parse ELF
