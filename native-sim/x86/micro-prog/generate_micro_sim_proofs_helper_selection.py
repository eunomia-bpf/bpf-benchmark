#!/usr/bin/env python3
"""Generate helper-selection x86-simulator verifier artifacts.

This backup keeps the current generator semantics, but restores the older
Python-side x86_exec_* helper selection so clang compile cost can be compared
against the C/clang constant-propagation path.
"""
...
OUT_DIR = Path(__file__).resolve().parents[1] / "build" / "helper-selection-src"
...
PACKET_HELPERS = {
    "X86_OP_MOV_LOAD": "x86_exec_mov_load_packet",
    "X86_OP_MOVSX_LOAD": "x86_exec_mov_load_packet",
    "X86_OP_MOV_STORE_IMM": "x86_exec_mov_store_imm_packet",
    "X86_OP_MOV_STORE_REG": "x86_exec_mov_store_reg_packet",
    "X86_OP_ALU_MEM": "x86_exec_alu_mem_packet",
    "X86_OP_CMP_MEM_IMM": "x86_exec_cmp_mem_imm_packet",
    "X86_OP_TEST_MEM_IMM": "x86_exec_test_mem_imm_packet",
    "X86_OP_CMP_MEM_REG": "x86_exec_cmp_mem_reg_packet",
}

NON_PACKET_BASES = {"X86_RDI", "X86_RBP", "X86_RSP", "X86_REG_NONE"}
...
def packet_mem_base(encoded: EncodedInsn) -> str | None:
    if encoded.op in {"X86_OP_MOV_LOAD", "X86_OP_MOVSX_LOAD", "X86_OP_ALU_MEM"}:
        return encoded.src
    if encoded.op in {
        "X86_OP_MOV_STORE_IMM", "X86_OP_MOV_STORE_REG",
        "X86_OP_CMP_MEM_IMM", "X86_OP_TEST_MEM_IMM", "X86_OP_CMP_MEM_REG",
    }:
        return encoded.dst
    return None


def op_helper(encoded: EncodedInsn) -> str:
    base = packet_mem_base(encoded)
    if base is not None and base not in NON_PACKET_BASES:
        return PACKET_HELPERS[encoded.op]
    try:
        return OP_HELPERS[encoded.op]
    except KeyError as err:
        raise ValueError(f"missing simulator helper for {encoded.op}") from err


def append_step(lines: list[str], insn: NativeInsn, indent: str = "\t",
                step_macro: str = "X86_SIM_RUN_OP") -> None:
    encoded = encode(insn)
    helper = op_helper(encoded)
...
def append_branch_or_ret(lines: list[str], insn: NativeInsn, addrs: set[int],
                         ...
                         ret_statement: str = "X86_SIM_RET_RAX();") -> None:
    if insn.mnemonic in CC_AUX and insn.mnemonic.startswith("j"):
        lines.append(f"{indent}/* 0x{insn.addr:x}: {c_comment(insn.raw)} */")
        target = branch_target(insn.operands[0]) if insn.operands else 0
        if target in addrs:
            lines.append(f"{indent}if (x86_eval_cc(&__x86_sim_state, {CC_AUX[insn.mnemonic]}))")
    ...
    if insn.mnemonic == "call":
        ...
        if call_functions and target in call_functions:
            lines.append(f"{indent}X86_SIM_RUN_CALL({call_functions[target]});")
            return
...
def append_loop_exit(lines: list[str], target: int, indent: str) -> None:
    lines.append(f"{indent}X86_SIM_LOOP_EXIT(0x{target:x});")
...
def append_loop_branch_or_ret(lines: list[str], spec: LoopSpec, insn: NativeInsn,
    ...
    if insn.mnemonic in CC_AUX and insn.mnemonic.startswith("j"):
        lines.append(f"{indent}/* 0x{insn.addr:x}: {c_comment(insn.raw)} */")
        target = branch_target(insn.operands[0]) if insn.operands else 0
        cond = f"x86_eval_cc(&__x86_sim_state, {CC_AUX[insn.mnemonic]})"
    ...
    if insn.mnemonic == "call":
        ...
        if target not in call_functions:
            lines.append(f"{indent}X86_SIM_LOOP_FAIL();")
            return
        lines.append(f"{indent}X86_SIM_LOOP_CALL({call_functions[target]});")
        append_loop_fallthrough(lines, spec, next_addr, indent,
                                physical_next=physical_next)
        return
    if insn.mnemonic == "ret":
        lines.append(f"{indent}/* 0x{insn.addr:x}: {c_comment(insn.raw)} */")
        lines.append(f"{indent}X86_SIM_LOOP_RET();")
        return
    append_step(lines, insn, indent, "X86_SIM_LOOP_OP")
    append_loop_fallthrough(lines, spec, next_addr, indent,
                            physical_next=physical_next)
...
def render_loop_callback(spec: LoopSpec, insns: list[NativeInsn],
    ...
    lines = [
        ...
        "{",
        "\tstruct x86_sim_loop_ctx *loop = ctx;",
        "\tvoid *__x86_sim_data = loop->data;",
        "\tvoid *__x86_sim_data_end = loop->data_end;",
        "\tstruct x86_insn __x86_sim_insn = {};",
        "\t#define __x86_sim_state loop->state",
        "",
...
def append_loop_call(lines: list[str], spec: LoopSpec,
    ...
    done_statement = "return 1;" if in_callback else "X86_SIM_RET_RAX();"
    save_name = f"__x86_loop_save_{spec.region_start:x}_{spec.entry:x}"
    lines.extend([
        f"{indent}struct x86_sim_reg_save {save_name}_rdi = {{}};",
        f"{indent}x86_sim_loop_prepare({ctx_name}, __x86_sim_data, "
        f"__x86_sim_data_end, &{save_name}_rdi);",
    ])
    lines.extend([
        ...
        f"{indent}}}",
        f"{indent}x86_sim_loop_restore_rdi({ctx_name}, &{save_name}_rdi);",
    ])
    lines.extend([
...
def render_x86_subfunction(symbol: str, insns: list[NativeInsn]) -> str:
    ...
    for reg in ("rbx", "r12", "r13", "r14", "r15"):
        lines.extend([
            f"\t__u64 __save_{reg} = __x86_sim_state.{reg};",
            f"\tvoid *__save_p_{reg} = __x86_sim_state.p_{reg};",
            f"\t__u8 __save_tag_{reg} = __x86_sim_state.tag_{reg};",
        ])
    lines.extend([
        "\t#define X86_SIM_SUB_RETURN() do { \\",
        "\t\t__x86_sim_state.rbx = __save_rbx; \\",
        "\t\t__x86_sim_state.r12 = __save_r12; \\",
        "\t\t__x86_sim_state.r13 = __save_r13; \\",
        "\t\t__x86_sim_state.r14 = __save_r14; \\",
        "\t\t__x86_sim_state.r15 = __save_r15; \\",
        "\t\t__x86_sim_state.p_rbx = __save_p_rbx; \\",
        "\t\t__x86_sim_state.p_r12 = __save_p_r12; \\",
        "\t\t__x86_sim_state.p_r13 = __save_p_r13; \\",
        "\t\t__x86_sim_state.p_r14 = __save_p_r14; \\",
        "\t\t__x86_sim_state.p_r15 = __save_p_r15; \\",
        "\t\t__x86_sim_state.tag_rbx = __save_tag_rbx; \\",
        "\t\t__x86_sim_state.tag_r12 = __save_tag_r12; \\",
        "\t\t__x86_sim_state.tag_r13 = __save_tag_r13; \\",
        "\t\t__x86_sim_state.tag_r14 = __save_tag_r14; \\",
        "\t\t__x86_sim_state.tag_r15 = __save_tag_r15; \\",
    ...
    for insn in insns:
        ...
        append_branch_or_ret(lines, insn, addrs, subroutine=True,
                             ...
                             ret_statement="X86_SIM_SUB_RETURN();")
    lines.extend([
        "\t#undef X86_SIM_SUB_RETURN",
        "\t#undef __x86_sim_state",
...
def render_program(name: str, insns: list[NativeInsn],
    ...
    lines.extend([
        '#include "../../x86_sim_bpf_helper_selection.h"',
        "",
    ])
    for symbol, fn_insns in subfunctions.items():
    ...
    if loop_specs:
        lines.extend([
            ...
            "\tstruct x86_sim_loop_ctx __x86_loop = {};",
            "\tstruct x86_insn __x86_sim_insn = {};",
            "\t#define __x86_sim_state __x86_loop.state",
            "\tx86_init_state(&__x86_sim_state, (void *)ctx);",
        ])
    else:
        lines.append("\tX86_SIM_DECLARE_XDP(ctx);")
    if synthetic_entry_frame:
        lines.extend([
            "\t__x86_sim_state.rbp = 0;",
            "\t__x86_sim_state.p_rbp = 0;",
            "\t__x86_sim_state.tag_rbp = X86_PTR_STACK;",
            "\t__x86_sim_state.rsp = 0;",
            "\t__x86_sim_state.p_rsp = 0;",
            "\t__x86_sim_state.tag_rsp = X86_PTR_STACK;",
        ])
    if call_returns:
        lines.extend([
    ...
    lines.extend([
        "\t#undef __x86_sim_state" if loop_specs else "",
        "\treturn XDP_ABORTED;",
        "}",
        "",
        "X86_SIM_LICENSE();",
