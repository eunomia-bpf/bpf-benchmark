...
| Native call return-address model | in progress | `X86_SIM_X86_CALL` now writes the next native instruction address into the modeled stack slot. Remaining work: `ret` control flow still assumes the callee does not modify the return-address slot. |
...
| Correctness-first micro compile/verify check | latest: 4/29 verifier ok | `results/README-20260518-235117.md`: all 29 compiled; `bitmap_popcount_scan`, `sorted_rule_binary_search`, `packet_checksum_fold`, and `tc_packet_checksum_fold` loaded/tested. Most failures are verifier loss of correlation between runtime tag fields and scalar pointer types or verifier stack spills beyond `fp-512`; `simple` now exposes this same tag/scalar join after removing checked-simulator shortcuts. |
