...
| Correctness-first micro compile/verify check | latest: 5/29 verifier ok | `results/README-20260519-014225.md`: all 29 compiled; `simple`, `simple_packet`, `bitmap_popcount_scan`, `sorted_rule_binary_search`, and `packet_checksum_fold` loaded/tested. Logs are stored as `results/*-20260519-014225.verifier.log`; dominant failures are pointer-typed x86 GPR integer operations, byte-wise pointer stack stores, scalar/pointer subtraction, direct `pkt_end` arithmetic, and packet accesses whose native guard is now represented only through flags. |
| Local-register macro state experiment | open | Replace `struct x86_state` field traffic with entry-function local variables for architectural GPRs/flags/ghost metadata, and convert helper functions into scoped C macros or C-authored templates. Goal: reduce BPF stack spills, improve constant propagation, and make the proof object look more like hardware registers. Constraint: Python still emits one native instruction macro; macro expansion must not add branch assertions, bounds checks, safety guards, or benchmark-specific logic. |
