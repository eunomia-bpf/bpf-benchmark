...
from runner.libs.app_runners.cilium import CiliumRunner  # noqa: E402
from runner.libs.benchmark_net import BENCHMARK_IFACE  # noqa: E402
...
def run_cilium_case(args: argparse.Namespace) -> dict[str, object]:
    ...
    def build_state(active_runner: CiliumRunner, started_prog_ids: list[int]) -> CaseLifecycleState:
        prog_ids = [int(value) for value in started_prog_ids if int(value) > 0]
        if not prog_ids:
            raise RuntimeError("Cilium runner did not expose any live prog_ids")
        programs = program_records(active_runner.programs)
        if not programs:
            raise RuntimeError("Cilium runner did not expose any live programs")
        artifacts: dict[str, object] = {
            "programs": programs,
            "rejit_policy_context": {"repo": "cilium", "level": "e2e"},
        }
