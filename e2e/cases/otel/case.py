...
from runner.libs.app_runners.otel_profiler import OtelProfilerRunner  # noqa: E402
...
WORKLOAD_KIND = "stress_ng_cpu"


def measure_workload(
    runner: OtelProfilerRunner,
    duration_s: int,
    prog_ids: list[int],
) -> dict[str, object]:
...
def build_markdown(payload: Mapping[str, object]) -> str:
    return "\n".join(
        [
            "# OpenTelemetry eBPF Profiler End-to-End Benchmark",
            "",
            "```json",
...
def run_otel_case(args: argparse.Namespace) -> dict[str, object]:
    ...
    runner = OtelProfilerRunner(workload_kind=WORKLOAD_KIND)
    ...
    def build_state(active_runner: OtelProfilerRunner, started_prog_ids: list[int]) -> CaseLifecycleState:
        prog_ids = [int(value) for value in started_prog_ids if int(value) > 0]
        if not prog_ids:
            raise RuntimeError("Otel runner did not expose any live prog_ids")
        programs = program_records(active_runner.programs)
        if not programs:
            raise RuntimeError("Otel runner did not expose any live programs")
        artifacts: dict[str, object] = {
            "programs": programs,
            "rejit_policy_context": {"repo": "otelcol-ebpf-profiler", "level": "e2e"},
        }
