...
from runner.libs import run_command, which  # noqa: E402
...
def ensure_required_tools() -> None:
    ...
    if version is None or version < MIN_BPFTRACE_VERSION:
        raise RuntimeError(
            f"bpftrace {version_text or 'unknown'} does not meet the required >= "
            f"{MIN_BPFTRACE_VERSION[0]}.{MIN_BPFTRACE_VERSION[1]}"
        )
def measure_workload(runner: BpftraceRunner, duration_s: int, prog_ids: list[int]) -> dict[str, object]:
    ...
    with enable_bpf_stats():
        for spec in SCRIPTS:
            phase_data = run_phase(
                spec,
                duration_s=duration_s,
                attach_timeout_s=DEFAULT_BPFTRACE_ATTACH_TIMEOUT_S,
