...
def test_prepare_daemon_session_rejects_partial_kinsn_module_load(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        case_common,
        "_capture_kinsn_module_snapshot",
        lambda expected_modules: {
            "expected_modules": list(expected_modules),
            "resident_expected_modules": [],
            "missing_expected_modules": list(expected_modules),
        },
    )
    monkeypatch.setattr(
        case_common,
        "_run_kinsn_module_loader",
        lambda expected_modules, *, before_snapshot=None: (_ for _ in ()).throw(
            RuntimeError(
    ...
    with pytest.raises(RuntimeError, match="did not load all expected modules"):
