...
- **YAML 只列 app，不列 object/program**：YAML 定义 app（loader instance），每个 app ��定 runner + workload。启动 app 后通过 bpf_stats/get_next_id 自动发现所��� BPF 程序，全部测量。不需要在 YAML 里枚举 .bpf.o 或 program name。这和 daemon apply-all 模���完全一致。Object 是编译产物的��包格式，和测量无���。
