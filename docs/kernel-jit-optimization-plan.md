...
| **467f** | repo 控制面验证与旧路径清理 | 🔄 | 当前验证：repo manifest 解析 ✅、`make corpus-fetch REPOS='bcc'` ✅、`python3 runner/scripts/build_corpus_objects.py --repo bcc --max-sources 2` ✅、相关 Python 文件 `py_compile` ✅、`make -C runner e2e-prep` ✅。fetch 路径已支持把 legacy `corpus/repos/<name>` checkout 自动迁移到 `runner/repos/<name>`（本轮实测迁移并对齐 `scx`）。遗留旧路径清理仍未完成。 |
...
| **469c** | Benchmark 执行性能瓶颈分析与提速 | 🔄 数据已采集 | 本轮全量数据表明：1) `vm-micro` 当前主要耗时不是 VM 启动，而是 guest 内 `kernel-rejit compile`；`wide_mem` verifier rollback/retry（见 #457）仍是明显放大项。2) `vm-corpus` 在 single-VM batch 后已消掉 per-target VM 启动，但总时长仍主要花在 guest 内逐 object compile/verify。3) `vm-e2e` 的长尾主要来自 case/workload 本身，不是 VM 包装层；`scx` 的 host-side repo/binary 前置已收口到 `runner`，重复运行时可直接 skip。4) 仍有非阻塞基础设施噪音：`runner.libs.workload` HTTP handler 偶发 `BrokenPipeError` 日志、tracee/tetragon setup 仍会在每次 e2e 前执行存在额外固定成本。下一步提速重点：#457、e2e setup cache/skip 判定、以及剩余 repo 控制面收口。 |
