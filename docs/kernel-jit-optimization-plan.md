...
| 231 | **Docker ARM64 交叉编译（2026-03-19）** | ✅ | `docker/arm64-crossbuild.Dockerfile` + Makefile targets: `make selftest-arm64`（Docker 内交叉编译 test_recompile）、`make vm-arm64-selftest`（QEMU guest 内执行）。ARM64 ELF binary OK，但 ARM64 kernel 返回 EOPNOTSUPP 18/19 fail（无 `bpf_jit_supports_recompile()` override）。commit `0c7f0f1`。 |
...
| 277 | **Post-refactor cleanup（2026-03-20）** | ✅ | 完成 post-refactor 审计与清理：`micro/driver.py` help 路由修复，当前源码 `py_compile` 无错误，scripts/docker 归属确认保留，旧 archive / scratch 目录均确认已删除，当前文档中的旧路径引用已收敛到历史归档材料。报告：`docs/tmp/benchmark_post_refactor_audit_20260320.md`。 |
...
| 298 | **ARM64 c7g.large 正式 benchmark（2026-03-20）** | 🔄 | codex 执行中（AWS，不占本地 VM）。 |
| 299 | **x86 新优化机会调研（2026-03-20）** | 🔄 | 调研 Tracee/Tetragon/Katran 的 JIT gap，找 top-5 新 form 候选。codex 执行中。 |
