...
| 74 | **Tetragon 真正端到端** | ✅ | 已实现 `e2e/cases/tetragon/`：`setup.sh` + `case.py` + `guest_smoke.sh`，复用 `e2e/common/` 做 workload / metrics / scanner / agent plumbing。Framework-kernel VM smoke（2026-03-11, `7.0.0-rc2-g2a6783cc77b6`）因 guest 无可用 `tetragon` binary 走允许的 manual fallback，但完成真实应用吞吐 + CPU + BPF runtime 对比，并成功对 3 个 Tetragon 对象程序应用 `BPF_PROG_JIT_RECOMPILE`。总体均值：app `340697 -> 354538 ops/s`（`+4.06%`），controller CPU `50.56% -> 48.01%`（`-5.04%`），BPF avg ns `443.34 -> 409.96`（`-7.53%`）。per-program：`event_execve` 37 sites `1.014x`，`security_file_open` 2 sites `1.001x`，`security_socket_connect` 2 sites `1.075x`。结果：`e2e/results/tetragon-real-e2e.json`，`e2e/results/tetragon-real-e2e.md`，报告：`docs/tmp/tetragon-real-e2e-report.md` |
