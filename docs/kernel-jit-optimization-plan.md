...
| **ADDR_CALC (LEA)** | 可选但不推荐 | 🔽 scoped down（no first-wave kInsn; no core-JIT peephole） | x86 `lea` / ARM64 shifted-add。2026-05-13 combined census changes the old native-C conclusion: native x86 has many LEAs, but most are address-mode materialization, while BPF bytecode exposes almost only adjacent `MOV dst, base; ADD dst, src`. A packed-sidecar kinsn would not reduce `bytes_xlated` for the dominant 2-insn form and does not capture Katran's native LEA gap. A core kernel-JIT peephole would be the direct lowering, but that is outside the project constraint because kinsn is the mechanism for avoiding core kernel JIT edits. Defer LEA unless it becomes a deliberate low-priority scalar-only kinsn experiment. | `docs/tmp/lea_kinsn_design_census_20260513.md`：testbin strict **13,321** sites（all pattern `a`; static-scalar **10,922**）；testobject strict **6,999** sites / native x86 **42,153** LEAs（BPF/native **16.6%**）；Katran native **225** vs BPF strict **4**（**1.8%**）；scaled+disp / add-imm-chain **0** |
...
Status: do not implement the `bpf_lea64` / `bpf_lea32` bpfopt kinsn pass from the initial sketch in the first wave. Preserve the sketch in the report as archived design context. Also do not pursue the core kernel-JIT peephole path under the current project constraint: kinsn is the chosen mechanism precisely to avoid editing the core kernel JIT.
