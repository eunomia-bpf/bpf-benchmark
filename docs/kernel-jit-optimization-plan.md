...
| **480** | **Corpus 全 prog type 覆盖（2026-03-25）** | ✅ | **(1)** C++ runner `run-kernel-attach`（libbpf attach + bpf_enable_stats）。**(2)** batch plan `kernel-attach` 分支。**(3)** 删除 65K 行 inventory.json，改为 `macro_corpus.yaml` 驱动。**(4)** YAML 131→764 条目（322 test_run + 390 attach + 52 compile_only），覆盖 2019/2019 程序 100%。**(5)** **结果**：`make vm-corpus` 从 152 targets/69 applied → **764 targets / 291 measured / 113 applied**。 |
