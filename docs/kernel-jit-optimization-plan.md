...
> 上次更新：2026-03-20e。**权威数据**（#256 rerun）：micro **1.057x** / applied-only **1.193x**；corpus **0.983x**；Tracee **+8.1%**；Tetragon **+20.3%/+32.2%**；Katran BPF **1.108-1.168x**（harness 升级后）；gap **0.581x**。vm-selftest **35/35**。vm-micro validation **62/62 valid, 1.100x**。**完成**：P2 trampoline leak fix、ARM64 E2E smoke（Katran recompile 跑通）、kernel -137 LOC 精简、表驱动 validator substrate（+333 LOC 投资，边际成本 ~500→~80-150 LOC/form）、用户态 -32k 行精简（legacy/ 删除 + driver 合并）、framework 散乱清理、Katran harness 升级（throughput/PPS/p99）、ARM64 bpftool kernel_btf fallback、emitter 成本分析（BRANCH_FLIP 条件码反转不可行）。**进行中**：commit+push。
...
| # | 任务 | 状态 | 关键数据 / 文档 |
|---|------|:---:|------|
| 1 | v3 可扩展 JIT pass 框架设计 | ✅ | 8/10 review。`docs/tmp/2026-03-09/jit-pass-framework-v3-design.md`，`docs/tmp/2026-03-09/jit-pass-framework-v3-final-verdict.md` |
| 2 | v4 Post-load re-JIT 框架设计 | ✅ | `docs/tmp/2026-03-09/jit-pass-framework-v4-design.md` |
| 3 | v5 声明式 Pattern 框架设计 | ✅ | 7.5/10 review。`docs/tmp/2026-03-09/jit-pass-framework-v5-design.md`，`docs/tmp/2026-03-09/jit-pass-framework-v5-review.md` |
| 4 | Design-v7 Hybrid 设计 | ✅ | `docs/tmp/2026-03-08/bpf-jit-advisor-v7.md`，`docs/tmp/2026-03-08/bpf-jit-advisor-v7-review.md` |
| 5 | Interface 详细设计 | ✅ | syscall/blob/CPU contract/安全/部署。`docs/tmp/2026-03-08/interface-design-detail.md`，review 6/10 `docs/tmp/2026-03-08/interface-design-review.md` |
| 6 | Verifier-rewrite 方案分析 | ✅ | 结论：JIT lowering 为主，verifier-level 属 future scope。`docs/tmp/2026-03-08/verifier-rewrite-approach.md` |
| 7 | Policy decision layer 设计 | ✅ | 3 级策略（static/profile/AB）。`docs/tmp/2026-03-10/policy-decision-layer-design.md` |
| 8 | Directive 发现 + 分类 | ✅ | 11 候选排名，5 新 benchmark。`docs/tmp/2026-03-08/directive-discovery-analysis.md` |
| 9 | OSDI readiness review | ✅ | 4/10。`docs/tmp/2026-03-08/osdi-readiness-review.md` |
| 10 | 跨文档交叉验证 | ✅ | `docs/tmp/2026-03-08/cross-document-review.md` |
| 11 | POC v1: wide_load verifier rewrite | ⚠️ 方向错误 | BPF 指令重写 = 错误层（userspace 也能做）。`docs/tmp/2026-03-09/poc-design-review.md` |
| 12 | POC v2: cmov_select JIT lowering | ✅ | xlated 7480→7480 不变，jited 4168→4167。`dd81852`。`docs/tmp/2026-03-09/poc-v2-design-conformance-review.md` |
| 13 | v4 POC 实现（4 directives） | ✅ | CMOV+WIDE+ROTATE+LEA，BPF_PROG_JIT_RECOMPILE，~850 LOC kernel。branch `jit-directive-v4` |
| 14 | v4 correctness bug 修复 | ✅ | flag-clobber + subprog boundary + interior edge。`docs/tmp/2026-03-09/v4-bug-diagnosis.md` |
| 15 | v4 implementation review | ✅ | 6/10，10 gaps (R1-R10)。`docs/tmp/2026-03-09/v4-implementation-review.md` |
| 16 | v4 Round 1 修复 + 重测 | ✅ | R1/R3/R5/R6/R7 修复。ADDR_CALC: 4 prog 5 sites, stride_load_16 -12%。COND_SELECT: 1 prog 6 sites, log2_fold +28%（policy-sensitivity）。ROTATE/WIDE 仍 0 覆盖。`docs/tmp/2026-03-09/v4-round1-test-results.md` |
| 17 | v4 Round 2: WIDE_MEM 突破 | ✅ | WIDE_MEM 0→11 sites（3 prog），load_byte_recompose -13%，stride_load_4 -14%。ROTATE 仍 0。`docs/tmp/2026-03-09/v4-round2-test-results.md`，`docs/tmp/2026-03-09/v4-round2-implementation.md` |
| 18 | v4 Round 3: ROTATE 突破 | ✅ | ALL 4 families active。ROTATE 0→126 sites（2 prog），rotate64_hash -28.4% exec -32.3% code，packet_rss_hash -10.5%。ROTATE+WIDE combined -36.6% exec -35% code（composability 验证）。kernel `3d3587b8c`。`docs/tmp/2026-03-09/v4-round3-test-results.md` |
| 19 | Kernel-fixed baselines 设计 | ✅ | `docs/tmp/2026-03-10/kernel-fixed-baselines-design.md` |
| 20 | Kernel-fixed baselines 测试 | ✅ | **Go/no-go PASSED**。fixed-all: rotate64_hash -16.8%, load_byte_recompose -13.0%, stride_load_16 +14.2%（LEA 组合回归），**log2_fold +28.3%**（CMOV 伤害）。结论：ROTATE/WIDE substrate 可固定，CMOV 必须 policy-controlled。kernel `1572a4ddb` on `jit-fixed-baselines`。`docs/tmp/2026-03-10/kernel-fixed-baselines-test-results.md` |
| 21 | Per-directive 隔离测试 | ✅ | CMOV-only 只改 log2_fold（+19.7%），code size 可加但 exec time 不可加。`docs/tmp/2026-03-10/kernel-fixed-baselines-per-directive.md` |
| 22 | v5 声明式 pattern POC | ✅ | rotate64_hash v5=v4（115 sites, jited 2409）。`docs/tmp/2026-03-10/v5-minimal-poc-results.md` |
| 23 | v5 迭代改进 | ✅ | CMOV 上 v5（6 sites, +76%）。scanner 统一。`docs/tmp/2026-03-10/v5-iteration-results.md` |
| 24 | v5 canonical binding table | ✅ | COND_SELECT + ROTATE 验证通过。v5 extensibility: v4 1018 LOC vs v5 32 LOC (32x)。`docs/tmp/2026-03-10/v5-canonical-binding-results.md`，`docs/tmp/2026-03-10/extensibility-quantification.md` |
| 25 | Scanner 独立化到 scanner/ | ✅ | kernel_runner.cpp 删除全部重复代码。`docs/tmp/2026-03-10/scanner-cleanup-results.md` |
| 26 | v5 design gap analysis | ✅ | 6.5/10。`docs/tmp/2026-03-10/v5-design-gap-analysis.md` |
| 27 | Framework architecture review | ✅ | 4/10。`docs/tmp/2026-03-10/framework-architecture-review.md` |
| 28 | cmov_select subprog scan 修复 | ⚠️ | scanner 0→1 site ✅，但 emitter correctness bug 待修。`docs/tmp/2026-03-10/cmov-subprog-fix-results.md` |
| 29 | Pure-JIT benchmark 审计 | ✅ | 49/50 OK，`map_lookup_repeat` 移到 runtime。`docs/tmp/2026-03-08/pure-jit-benchmark-audit.md` |
| 30 | Host 重跑 50+11 | ✅ | pure-jit L/K=0.797x, runtime L/K=0.875x。`micro/results/pure_jit_with_cmov.json` |
| 31 | VM 重跑 (7.0-rc2) | ✅ | kernel VM/host=1.003x (parity)。`docs/tmp/2026-03-08/vm-rerun-analysis.md` |
| 32 | Real program directive census | ✅ | 98 EM_BPF 扫描，67/98 有 sites，1029 总 sites。`docs/tmp/2026-03-11/real-program-directive-census.md`，`micro/directive_census.py` |
| 33 | Corpus v5 recompile | ✅ | 37/40 apply 成功。xdp_synproxy +15%/+31%。`docs/tmp/2026-03-10/corpus-v5-recompile-results.md` |
| 34 | Production corpus 编译 | ✅ | Cilium 3 prog（658 CMOV），Katran 5，xdp-tools 13。总 493。Cilium 无法加载（Operation not supported，需 kfunc）。`docs/tmp/2026-03-10/production-program-compilation.md` |
| 35 | Production corpus v5 recompile | ✅ | 15/15 成功。katran -0.2%，xdp_vlan_swap -5.7%。`docs/tmp/2026-03-10/production-corpus-v5-rerun-results.md` |
| 36 | Corpus 扩充 Round 2 | ✅ | +21→529 总 .bpf.o（22 个项目）。loxilb 912 sites/prog，Calico 314 sites。`docs/tmp/2026-03-10/corpus-expansion-round2.md` |
| 37 | Scanner rotate gap 修复 | ✅ | core_kern 0→818，test_verif_scale2 0→992。`docs/tmp/2026-03-10/scanner-gap-fix.md` |
| 38 | 严格 benchmarking | ✅ | 10×1000, CPU pin#23, perf governor, turbo off, Wilcoxon BH-adjusted。CMOV 0.655x (p=0.002), ROTATE 1.193x/1.235x (p=0.002), LEA 1.052x (p=0.012), WIDE 不显著 (p=0.164)。`docs/tmp/2026-03-10/rigorous-benchmark-results.md` |
| 39 | 微架构实验 | ✅ | CPU 频率敏感性：binary_search 31.7%，load_byte_recompose 33.2%。输入分布：branch_layout predictable 0.225x vs random 0.326x（差 44.6%）。PMU：llvmbpf IPC 普遍高于 kernel（binary_search 6.86 vs 2.41）。`docs/tmp/2026-03-08/microarch_experiments.md` |
| 40 | ARM64 CI 全流程 | ✅ | 3 轮修复。commits `1c17332`, `d2bd851`, `d8edc80` |
| 41 | ARM64 跨架构分析 | ✅ | pure-jit L/K=0.656x（vs x86 0.797x）。`docs/tmp/2026-03-08/arm64-cross-arch-analysis.md`，review `docs/tmp/2026-03-11/arm64-analysis-review.md` |
| 42 | Debug BPF_PROG_JIT_RECOMPILE EINVAL | ✅ | production programs 的 EINVAL 问题 |
| 43 | Benchmark 基础设施审计 | ✅ | `docs/tmp/2026-03-11/benchmark-infrastructure-audit.md` |
| 44 | Benchmark 基础设施分析（重构方案） | ✅ | P0: 5 个 bug，P1: 10 项结构问题。`docs/tmp/2026-03-10/benchmark-framework-analysis.md` |
| 45 | 论文 LaTeX 初稿 | 🔄 | ACM sigconf。R1 4/10 → R2 6/10 → R3 6.5/10。`docs/paper/paper.tex`，`docs/tmp/2026-03-10/paper-review-r3-post-rewrite.md` |
| 46 | **COND_SELECT 上 v5 path** | ✅ | v5 pattern descriptor + canonical binding + parameterized emitter。scanner 找到 1 cmov site，VM 验证 result 一致，xlated 7480 不变，jited 4168→4167。`docs/tmp/2026-03-10/cmov-v5-migration-report.md`，包含在 `jit-directive-v5` commit `2a6783cc7` |
| 47 | **cmov_select emitter bug 修复** | ✅ | 根因：absolute-vs-local site index 混用。修复：`bpf_jit_rule_local_site_start()` rebases site_start 到 subprog 本地。验证：cmov_select baseline=recompile，log2_fold 无回归。`docs/tmp/2026-03-10/cmov-emitter-fix-report.md`，包含在 `jit-directive-v5` commit `2a6783cc7` |
| 48 | **Benchmark 基础设施重构** | 🔄 | P0 5/5 修复完成：call-overhead category、macro repeat、默认路径、按需 root/scanner、stats 统一 median。`docs/tmp/2026-03-10/p0-bugfix-report.md`。P1 结构问题待做 |
| 49 | **端到端部署评估** | 已取消 | 用户决定不再追求独立 PPS throughput 数据，现有 Tracee/Tetragon/bpftrace E2E 已足够 |
| 50 | **论文更新** | 已取消 | 用户决定移除 |
| 51 | jit_recompile 加 log_level/log_buf | ✅ | framework kernel 已加 `log_level/log_size/log_buf`，scanner `apply` 默认携带 16 KiB log buffer，失败时直接回显 kernel 诊断。`docs/tmp/2026-03-11/v6-tier1-implementation.md` |
| 52 | Shape whitelist 扩展 | ✅ | v5 active path 保留 generic `site_len <= 64` + `pattern_count == site_len`，删除 per-form 固定长度 gate 的约束效果；scanner ABI 常量同步到 64。`docs/tmp/2026-03-11/v6-tier1-implementation.md` |
| 53 | Overlap/priority 语义 | ❌ | 当前 kernel 不拒绝也不仲裁重叠 rule |
| 54 | Subprog 一致性 | ❌ | v4 分支与 fixed-baselines 分支的 subprog 处理不同步 |
| 55 | K2/Merlin/EPSO 对比实验 | ✅ | **正交非替代**。backend-only gap 占 surplus 89.1%。dual-pass spot check: size geomean 0.995x（IR 侧几乎碰不到 backend gap）。K2/Merlin 代码可获取但不兼容当前 pipeline，EPSO 无公开 artifact。`docs/tmp/2026-03-11/k2-merlin-epso-comparison.md`，`micro/results/llvm_dual_pass_boundary_spotcheck.md` |
| 56 | Re-JIT 原子性 / RCU 安全说明 | ✅ | POC benchmark-only safe（单线程 test_run）。Production 需：per-prog 串行化 + shadow image + RCU retirement + tailcall/trampoline 刷新。旧 image JIT text leak 未回收。`docs/tmp/2026-03-11/rejit-rcu-safety-analysis.md` |
...
| 61 | **Benchmark 多样性** | 🟡 | 67 micro: 64 XDP + 2 tc + 1 cgroup_skb。Corpus recompile paired: 61 tc + 16 xdp + 2 cgroup_skb = 79。Census 覆盖全类型。审计报告: `docs/tmp/2026-03-11/benchmark-diversity-audit.md` |
...
| 64 | **Corpus 可运行性盘点 + 自动化 recompile** | ✅ | 532 objects, 1836 progs, 1214 loadable, 274 有 sites, 381 truly runnable。79 paired baseline/recompile (sched_cls 61, xdp 16, cgroup_skb 2)。3069 total sites (CMOV 840, WIDE 389, ROTATE 1840)。新脚本 `micro/run_corpus_runnability.py`。`docs/tmp/2026-03-11/corpus-runnability-report.md`, `docs/tmp/corpus-runnability-results.json` |
| 65 | **Corpus 批量 recompile 实验** | ✅ | 78 measured pairs，7 项目。原始 exec geomean 0.813x，**排除噪声后 0.847x**（32/78 sub-resolution <50ns）。CMOV-only 可靠程序 0.718x（11/11 回归，验证 policy-sensitivity）。CMOV+WIDE 0.848x。噪声严重（控制组 ±36%）。`docs/tmp/2026-03-11/corpus-batch-recompile-results.md`，`docs/tmp/2026-03-11/corpus-recompile-regression-analysis.md` |
| 66 | **非网络端到端场景研究** | ✅ | **最佳 OSDI 方案**：Phase 1: Tracee（169 progs, 555 sites）+ Tetragon（kprobe/tracepoint）+ bpftrace（--emit-elf）。Phase 2: scx_rusty（11 struct_ops hooks）+ scx_lavd（22 hooks）。Corpus 已有大量非网络程序未利用：kprobe 508, tracing 442, tracepoint 151, lsm 75, struct_ops 51。核心差距不是"有没有程序"而是"有没有 honest attach+trigger 路径"。`docs/tmp/2026-03-11/non-networking-evaluation-plan.md` |
| 67 | **Benchmark 框架重构 Step 1-3** | ✅ | 早期共享 Python 层（commands/results/environment/inventory/__init__ 5 模块）落地。`run_micro.py` 迁移到共享库。`UnifiedResultRecord` schema。README 55+11 修正。`directive_census.py` 动态计数。commit `c000ed3`。`docs/tmp/2026-03-11/framework-improvement-step1.md` |
| 68 | **Corpus 扩展 Round 3** | ✅ | 560 objects（23 项目）, 2010 progs, 1280 loadable, 293 有 sites, 391 truly runnable（由 #64 baseline + Round 3 targeted reruns 合并）。新增/扩充：`scx` 4、`tetragon` 23、`systemd` 6 source-backed rebuild、5 个 selftests；`netbird` source-backed rebuild 但仍 `bpf_object__open_file` fail。新增 manifest `corpus/config/corpus_manifest.yaml`。`docs/tmp/2026-03-11/corpus-expansion-round3.md`, `docs/tmp/2026-03-11/corpus-expansion-round3-priority-runnability.md`, `docs/tmp/2026-03-11/corpus-expansion-round3-selftests-runnability.md` |
| 69 | **Selective CMOV policy + corpus rerun** | ✅ | `micro_exec`/`run_corpus_v5_vm_batch.py` 新增 `--skip-families`，可在 v5 auto-scan policy 中排除 CMOV。blind-all rerun（repeat=200）原始 geomean `0.826x`；selective `--skip-families cmov` 原始 geomean `0.813x`，**未**在 full corpus 上胜出。但在 shared non-sub subset（46 measured rows, all four exec_ns >=50ns）selective `0.870x` > blind-all `0.828x`；Calico raw `0.865x` > `0.828x`，Calico non-sub CMOV-only `0.886x` > `0.811x`，CMOV+WIDE `0.846x` > `0.819x`。结论：global skip-cmov 只在目标 Calico predictable-branch slice 上证明有效，不足以给 raw corpus 带来净收益。`docs/tmp/2026-03-11/corpus-batch-recompile-results.md`, `docs/tmp/2026-03-11/corpus-batch-recompile-selective-results.md`, `docs/tmp/2026-03-11/selective-policy-results.md` |
| 70 | **bpftrace 端到端 benchmark driver** | ✅ | 新增 `micro/run_e2e_bpftrace.py`：自动检查/安装 `bpftrace` + `stress-ng`，构建 `micro_exec`，跑 5 个代表性 bpftrace 脚本，支持 `--emit-elf` artifact-first 路径（原始 ELF candidate site census + libbpf-style section rename + `micro_exec --compile-only` load probe）和 live attach+trigger 路径（`bpftool prog show` 发现 bpftrace-owned programs，libbpf 读 `run_cnt/run_time_ns`，scanner `scan/apply --v5`，workload 前后比较 avg exec ns）。当前 host 全量实测：`--emit-elf` 5/5，live attach 5/5，检测到 9 个 CMOV sites（`open_latency` 5，`scheduler_latency` 4），generic artifact load 0/5；live `BPF_PROG_JIT_RECOMPILE` 在有 sites 的脚本上仍返回 `EINVAL`，因此 `rejit_ns/speedup` 为 `n/a`。结果：`docs/tmp/bpftrace-e2e-results.json`, `docs/tmp/2026-03-11/bpftrace-e2e-results.md` |
| 70 | **Tetragon attach+trigger harness** | ✅ | 新脚本 `micro/run_e2e_tetragon.py`：优先尝试 Tetragon daemon + `TracingPolicy`，默认 fallback 到本地 `.bpf.o` whole-object load + explicit attach（`event_execve -> syscalls/sys_enter_execve`，`generic_kprobe_event -> security_file_open/security_socket_connect`），scanner v5 site census + JSON/Markdown 输出。Smoke（2026-03-11, host kernel `6.15.11-061511-generic`）：stock 3/3 OK，sites `event_execve=37`、`generic_kprobe_event=2`；avg_ns `2670/222/685`。当前 host 上 `BPF_PROG_JIT_RECOMPILE` 返回 `EINVAL`，re-JIT 对比待 framework kernel 重跑。输出：`docs/tmp/tetragon-e2e-results.json`，`docs/tmp/2026-03-11/tetragon-e2e-results.md` |
| 70b | **BPF-side VM 测试** | ✅ | Tracee VM: sched_process_exec **2.71x**，security_file_open 1.17x，lsm_file_open 1.10x，security_socket_connect 0.66x（CMOV 回归）。bpftrace VM: open_latency 0.947x，scheduler_latency 0.965x（pure CMOV 回归，geomean 0.956x）。Tetragon VM: 0/3 失败（kernel 缺 CONFIG_FTRACE_SYSCALLS + CONFIG_BPF_KPROBE_OVERRIDE）。注意：这些都是 BPF run_time_ns 测量，**不是真正的端到端**。`docs/tmp/2026-03-11/tracee-e2e-vm-summary.md`，`docs/tmp/2026-03-11/bpftrace-e2e-vm-summary.md`，`docs/tmp/2026-03-11/tetragon-e2e-vm-summary.md` |
| 70c | **Benchmark 框架重构 Steps 2-7** | ✅ | 统一入口 `micro/driver.py`（5 子命令），新增 `orchestrator/catalog.py`（统一 manifest loader）+ `orchestrator/reporting.py`（共享报告层）+ `orchestrator/corpus.py` + `orchestrator/rigorous.py`。旧 `run_*.py` 降级为兼容 wrapper → `_driver_impl_*.py`。`summarize_rq.py` / `generate_figures.py` 不再硬编码旧 benchmark 数。`docs/tmp/2026-03-11/framework-improvement-steps2-7.md` |
| 71 | **仓库层次重组** | ✅ | 三层划分已落地：`micro/`（隔离微基准）、`corpus/`（真实程序收集+测量+分析）、`e2e/`（端到端部署+分析）。corpus 测量脚本和 `_driver_impl_*` 已从 `micro/` 移到 `corpus/`，Tracee/Tetragon/bpftrace harness 已移到 `e2e/`，`corpus/config/macro_corpus.yaml` 和 `corpus/config/corpus_manifest.yaml` 已移到 `corpus/config/`，`user_bpf_benchmark/` 已移到 `legacy/`，README/driver/import 路径已修正，验证通过（`py_compile`、`--help`、`make -C micro micro_exec programs`、`micro/run_micro.py` smoke）。报告：`docs/tmp/2026-03-11/repo-restructure-report.md` |
| 72 | **E2E 骨架 + Tracee 真正端到端** | ✅ | 新增早期共享 E2E helper 层（agent/workload/metrics/recompile/vm）+ `e2e/cases/tracee/`（`setup.sh`/`config.yaml`/`case.py`）+ 统一入口 `e2e/run.py`。Host smoke（2026-03-11, Tracee `v0.24.1` via `/tmp/tracee-bin/tracee`）成功采集真实 app throughput + Tracee events/s + drops + CPU；stock host kernel 上 `BPF_PROG_JIT_RECOMPILE` 仍 `EINVAL`。Framework-kernel VM smoke（`vendor/linux-framework/arch/x86/boot/bzImage`, kernel `7.0.0-rc2-g2a6783cc77b6`）发现 15 个 Tracee 程序、扫描出 21 个 CMOV sites、成功 recompile `8/15` 程序；真实端到端对比：`exec_storm` app `+21.65%` / events/s `+21.16%` / BPF avg ns `-2.63%`，`file_io` app `+5.06%` 但 events/s `-10.05%`，`network` app `+1.32%` / events/s `+1.26%`。结果：`e2e/results/tracee-e2e-real.json`，`e2e/results/tracee-e2e-real.md`，`docs/tmp/tracee-real-e2e-host-smoke.json`，`docs/tmp/tracee-real-e2e-vm-smoke.json`，报告：`docs/tmp/2026-03-11/tracee-real-e2e-report.md` |
| 73 | **Framework kernel 重编** | ✅ | `vendor/linux-framework/.config` 已启用 `CONFIG_FTRACE_SYSCALLS=y`、`CONFIG_BPF_KPROBE_OVERRIDE=y`、`CONFIG_SCHED_CLASS_EXT=y`（并补齐依赖 `CONFIG_FUNCTION_ERROR_INJECTION=y`），`bzImage` 重编完成（131.43s）。VM 验证：syscall tracepoints 存在、`/sys/kernel/sched_ext` 存在、`micro_exec` v5 recompile 恢复工作、Tetragon direct-object smoke 3/3 stock+reJIT OK。`docs/tmp/2026-03-11/kernel-rebuild-report.md` |
...
| 75 | **bpftrace 真正端到端** | ✅ | 已实现 `e2e/cases/bpftrace/`：5 个 `.bt` 脚本 + `case.py`，复用共享 E2E helper 层做 attach / workload / metrics / scanner plumbing，把 `bpftrace` 本身当作 tracing agent，测应用吞吐 + `bpftrace` CPU + BPF avg ns。Framework-kernel VM 全量实测（2026-03-11, `7.0.0-rc2-g2a6783cc77b6`, `bpftrace v0.20.2`）：5/5 baseline 成功，2/5 有 eligible CMOV sites（`open_latency` 5，`scheduler_latency` 4），aggregate 9 sites。真实端到端对比：`open_latency` BPF avg ns `277.17 -> 273.77`（`1.012x`），app `95367 -> 96082 ops/s`（`+0.75%`）；`scheduler_latency` `158.59 -> 163.27`（`0.971x`），app `41.72 -> 38.09 ops/s`（`-8.70%`）。eligible-script geomean `0.992x`，验证 policy-sensitivity。结果：`e2e/results/bpftrace-real-e2e.json`，`e2e/results/bpftrace-real-e2e.md`，`docs/tmp/2026-03-12/bpftrace-real-e2e-report.md` |
| 77 | **Directive gap 分析 + 下一步方向** | ✅ | 深度审查 kernel JIT（`bpf_jit_comp.c` 5433 行）+ corpus site census + 性能 gap 分解。结论：(1) CMOV 过度关注——corpus 10% sites 且多数回归，应作为 policy-sensitivity 证据而非性能主力；(2) 4 family 机制数量够但组合偏 peephole，缺 structural policy-sensitive family；(3) 18.5% prologue bucket 完全未覆盖；(4) 下一步 P0：`bitfield_extract`（~100-300 sites，~200 LOC）、`packet_ctx_wide_load`（~300-800 sites）、加宽 `cmov_select` 识别；P1：`bounds_window`、`branch_reorder`；kernel 清理：prologue NOP、零偏移编码、imm64 stack store、div/mod liveness。`docs/tmp/2026-03-11/directive-gap-analysis.md` |
| 78 | **加宽 cmov_select 识别** | ✅ | commit `f3d7c03`。新增 JSET/JSET32、wider diamond（jcc +3/+4）、guarded-update、switch-chain patterns。结果：switch_dispatch 0→1, binary_search 0→2, bounds_ladder 0→2 cmov sites。纯用户态。`docs/tmp/2026-03-11/cmov-broadening-report.md` |
| 79 | **bitfield_extract directive** | ✅ | Scanner: 8 v5 descriptors (32/64-bit, shift→mask/mask→shift, with-copy/in-place)。Kernel: canonical validator + x86 emitter。Corpus: **544 sites across 41 objects**（top: cilium/bpf_lxc.bpf.o 138 sites）。新 benchmark `bitfield_extract.bpf.c` + input generator。`docs/tmp/2026-03-11/bitfield-extract-implementation.md` |
| 80 | **packet_ctx_wide_load 扩展** | ✅ | Scanner: odd widths 2-8 + big-endian byte-recompose + v5 descriptors。Kernel: widened validator + x86 chunked emitter (4/2/1)。Corpus: **2835 wide sites**。`docs/tmp/2026-03-11/wide-load-extension-report.md` |
...
| 83 | **Kernel JIT 清理 patches** | ✅ | commit `0496966`（kernel `8c66cec7c`）。已实现：零偏移内存编码优化 + imm64 stack store 优化。Code size：load_byte_recompose 422→418, stride_load_16 517→514。Prologue NOP / div-mod / endian fusion 待做。`docs/tmp/2026-03-11/kernel-jit-cleanup-patches.md` |
| 84 | **Interface 设计审计 + v6 提案** | ✅ | 审计 v5 UAPI 接口强度：contiguous-only、local-CFG-only、no verifier facts、kernel-owned emitters。发现 bitfield_extract x86 dispatch wiring bug。v6 提案：fact-backed region rewrites + restricted template plans（需新 kernel semantics）。branch_reorder / bounds_window 无法用当前 v5 表达。`docs/tmp/2026-03-11/interface-design-audit.md`，`docs/tmp/2026-03-11/interface-improvement-proposals.md` |
...
| 96 | **Corpus 目录整理** | ✅ | 一次性脚本曾临时下沉到 scratch 区，后续已全部删除或回迁；README 同步更新，核心脚本验证通过。15 组 --help/import 检查全部通过。 |
| 97 | **Per-program policy 配置设计** | ✅ | 调研完成。现状：scanner 默认 all-apply，micro/corpus/e2e 无 per-program policy，family surface 漂移（scanner 8 / runner 5 / e2e 4）。设计：统一 YAML policy schema（match+selection+site_overrides），自动 tuning 5 阶段（census→screening→leave-one-out→combo→site refinement），实现路径 P0-P4。→ `docs/tmp/2026-03-11/policy-configuration-design.md`（960 行） |
| 99 | **Micro 目录 Python 脚本整理** | ✅ | 一批 analysis/demo 脚本进入归档层；完整运行验证通过（llvmbpf + kernel，simple/bitcount/rotate64_hash 结果正确）。commit `741536b`。 |
| 99a | **Micro Python cleanup follow-up** | ✅ | 2026-03-11 follow-up：compatibility wrappers `run_pass_ablation.py` / `run_rigorous.py` / `run_rigorous_framework_vm.py` 与 legacy `tools/build_policy_blob.py` 进一步进入归档层；新增共享 benchmark helper 模块，把 `select_benchmarks()` / `resolve_memory_file()` 从 `run_micro` / pass-ablation / rigorous 路径抽成公共逻辑。验证：`python3 -m py_compile`（修改后的 `micro/` Python 文件）、`python3 micro/run_micro.py --list`、`python3 micro/run_micro.py --runtime llvmbpf --bench simple --iterations 1 --warmups 0 --repeat 5`、`python3 micro/driver.py --help`。 |
| 98 | **Policy design v2：程序无关自动 tuning** | ✅ | 设计完成（793 行）。核心：auto-tuner 自动生成 per-program policy，人类不需要理解程序。默认行为改为 stock（不再 blind all-apply）。scanner 升级为 policy compiler。→ `docs/tmp/2026-03-12/policy-configuration-design-v2.md` |
...
| 98e | **P3 Corpus per-program policy files** | ✅ | 已完成 corpus per-program policy artifact wiring：新增 `corpus/generate_default_policies.py`、共享 policy helper、`corpus/policies/`，并在 corpus drivers 增加 `--use-policy` lookup。**本条不再表示任何 v1 / `selection.mode` 兼容仍然存在。** scanner parser 已在 #122 切为 v2-only；steady-state artifact 形状也在 #128 从 per-site allowlist 收敛为 `default: apply` + `families:` coarse denylist。 |
| 98c | **Tracing corpus VM full run** | ✅ | `corpus/run_tracing_corpus_vm.py` 已完成 framework-kernel VM 全链路执行：host preflight `724` considered / `473` ready / `251` skipped / `88` inventory failures；isolated guest per-target subprocess runner 稳定完成，无需再回退旧单进程 guest 路径。Artifacts：`corpus/results/tracing_corpus_vm_full_20260312.json`、`corpus/results/tracing_corpus_vm.latest.json`、`docs/tmp/2026-03-11/tracing-corpus-vm-full-results.md`。此前阻塞与尝试记录保留在 `docs/tmp/2026-03-11/tracing-corpus-vm-plan.md`。 |
| 98d | **Non-network code-size systematic** | ✅ | `corpus/run_code_size_comparison.py` 创建。Dry-run 盘点：560 objects, 2010 progs, 1282 loadable（kprobe 209, tracepoint 130, sched_cls 110, fentry 88, raw_tracepoint 84）。Host 无法 recompile (stock kernel EINVAL)。VM smoke 1 prog OK (xdp_probe_prog 78→84 bytes)，全量 VM 因 OOM 被 kill。commit `d1e7ef4`。`docs/tmp/2026-03-11/code-size-by-progtype.md` |
...
| 143 | **Post-cmov-fix micro 全量重跑** | ✅ | 56/56 valid, 0 crash。**11/56 applied**（从 #139 的 8 涨到 11，新增 binary_search/switch_dispatch/branch_dense）。Overall geomean **1.007x**，applied-only **0.986x**（新加入的 3 个 benchmark cmov site 被 fail-close 回 stock）。报告：`docs/tmp/2026-03-12/post-cmov-fix-micro-rerun.md`，数据：`micro/results/post_cmov_fix_micro.json`。 |
| 144 | **Post-cmov-fix corpus 抽样重跑 + cmov 消融** | ✅ | 以可 `packet_test_run` 的 CMOV-bearing corpus 程序做 45-program / 7-source VM 抽样（84 eligible pool, 42 measured timing pairs），比较两组 v3 policy：(A) allowlist 含 CMOV vs (B) 当前 `cmov:skip`。结果：**CMOV 仍整体负面**；Group A/Group B 归一化 geomean **0.906x**，对 stock 的 exec geomean 分别 **0.859x** vs **0.947x**。wins/losses/ties = **21/20/1**，但少数重度回归主导整体结果：`xdp_fwd_fib_full` **0.133x**、`balancer_ingress` **0.187x**、`xdp_fwd_fib_direct` **0.333x**、`cgroup_skb_ingress` **0.643x**。当前 runnable sample 中所有 live CMOV 都是 **`cond-select-64`**；`calico`/`linux-selftests` 小幅正面，但被 `katran`/`xdp-tools`/`tracee ingress` 的大幅负面抵消。结论：**暂时保留 corpus steady-state 的 `cmov:skip`**。报告：`docs/tmp/2026-03-12/post-cmov-fix-corpus-ablation.md`；数据：`corpus/results/post_cmov_fix_corpus_ablation.json`。 |
| 145 | **v3 policy: per-site directive list** | ✅ | 已完成 v2→v3 迁移：policy 现为 scanner sites 子集（`version: 3`, `sites: [{insn, family, pattern_kind}]`），不在 list 中即不 apply。scanner C++ parser/filter/CLI、Python shared policy helper、`generate_default_policies.py`、`auto_tune.py`、共享 live recompile helper、`run_bflip_endian_skip_sweep.py` 已切到 v3-only；scanner README + golden tests 已更新；`micro/policies/`、`micro/policies/per_family/`、`corpus/policies/` 已重生成并用 `parse_policy_v3()` 验证 **614** 个 YAML 全部通过。验证：`cmake --build scanner/build --target bpf-jit-scanner --target test_scanner -j`、`./scanner/build/test_scanner` = **PASS 163**、`python3 -m py_compile`（shared policy helper）。 |
| 146 | **Recompile 开销测量** | ✅ | Scanner latency: **4.32ms** median (micro), **4.42ms** (corpus 50-object)。Kernel recompile syscall: **29.89μs** median, **63.31μs** p90。Scanner/syscall ratio 207.8x — overhead 完全 userspace-dominated。Blob size: 164B (1 site) → 9.6KB (86 sites) → 213KB (992 sites)。线性 scaling: 13.03ms/1k insns (R²=0.997)。报告：`docs/tmp/2026-03-12/recompile-overhead-measurement.md`。 |
| 147 | **bpftrace E2E v3 validation** | ✅ | 修复 `recompile.py` remapped_sites==0 EINVAL bug 和 bpftrace/case.py v3 parsing。VM 验证：v3 policy resolve + remap + no-op 成功，正向控制 1-site apply 成功。报告：`docs/tmp/2026-03-12/bpftrace-e2e-v3-validation.md`。 |
| 148 | **架构 Review + 图** | ✅ | Mermaid 架构图 + 流程图。关键发现：WIDE_MEM/ROTATE/ADDR_CALC 缺 canonical-site validation，scanner `pattern_kind` filtering bug，无 rollback/recompile-lock/tracepoints。报告：`docs/tmp/2026-03-12/architecture-review-and-diagrams.md`。 |
...
| 175 | **WIDE_MEM 覆盖更多 benchmark（multi_acc_4 等）** | ❌ | byte-recompose 占 gap 50.7%，是最大单 family 改进空间。当前 WIDE_MEM 只覆盖连续 byte-load+shift+or 模式，multi_acc_4 / stride_load_16 等 benchmark 未命中（可能是不同 BPF bytecode pattern）。需要扩展 scanner descriptors 或分析未命中原因。 |
| 176 | **Prologue 优化 canonical form** | ❌ | 占 pure-JIT gap 约 18.5%，当前完全未覆盖。Prologue 包括：函数 prologue NOP、callee-saved 寄存器压栈优化、frame pointer 操作。每个可独立成 canonical form，约 50-100 LOC 内核 + scanner descriptors。 |
...
| 178 | **更多 E2E workload 数据** | ❌ | 当前只有 Tracee exec_storm +5.97%（app）作为强 E2E 数据点。Tetragon connect_storm 仍需复跑确认（#140 噪声），`scx_rusty` 因 struct_ops `-EOPNOTSUPP` 缺 post-reJIT 数据；原 `xdp_forwarding +0.27%` case 因价值过低已于 2026-03-18 退役删除，后续 XDP 方向改看 Katran。目标：≥3 个 E2E workload 均正向。 |
...
| 183 | **P0.1：更多 E2E workload 正向数据** | 🔄 | 当前仅 Tracee exec_storm +6.28% / file_io +7.00%。需要 Tetragon connect_storm 复跑（#140 有噪声），`scx` 仍缺 honest post-reJIT 对比；原 `xdp_forwarding` 已退役删除，不再作为强化数据点，XDP replacement 方向转向 Katran。目标：≥3 个 E2E workload 均正向。 |
...
| 189 | **Micro policy 优化（2026-03-18）** | ✅ | 三轮迭代完成。最终 authoritative（干净环境 10iter/5warm/1000rep）：**overall 1.024x，applied-only 1.110x（7 applied，5W/2L）**。Non-applied geomean 1.002（极干净）。Top wins：rotate_dense +39.1%，rotate64_hash +34.1%，cmov_dense +15.0%。清掉了 cmov_select/mixed_alu_mem 两个 loss。数据：`micro/results/vm_micro_authoritative_20260318.json`。 |
| 190 | **结果目录整理 + result schema 升级（2026-03-18）** | ✅ | 三层 results/ 整理：顶层只留 authoritative JSON + README + dev/ + archive/；所有 *.latest.json symlinks 删除；refresh_latest_alias() 改为 no-op；Makefile 默认输出改到 results/dev/（gitignored）；vm_micro_authoritative_20260314.json 从 git HEAD 恢复。Result JSON schema 升级：per-benchmark 内嵌 policy_content + policy_sha256；provenance 新增 kernel_commit/scanner_commit/repo_git_sha/repo_dirty。验证：make smoke 输出到 dev/，policy 内嵌确认有效。 |
| 191 | **Corpus + E2E 性能分析（2026-03-18）** | ✅ | Corpus 主因不是 CMOV 是 stale policy：katran balancer_ingress policy 10 sites vs live 94 sites → 0 apply。extract 偏负（0.977x），wide 正向（1.111x）。E2E：Tracee 接近吃满非-CMOV（42 sites 全 applied）；Tetragon 缺 authoritative JSON，v3 policy stale；bpftrace 纯 CMOV 被 skip → 永远 neutral；`xdp_forwarding` 顶层 authoritative 实际是 dry-run，随后已于 2026-03-18 退役删除。报告：`docs/tmp/corpus_e2e_performance_analysis_20260318.md`。 |
| 192 | **Katran E2E 可行性调研（2026-03-18）** | ✅ | 有条件推荐替换 XDP forwarding。balancer_ingress 有 94 live sites（vs XDP fwd 3 sites），L4 LB 论文价值高。建议 balancer-only MVP：standalone attach + raw PPS + cmov:skip。**现状更新：`xdp_forwarding` 已于 2026-03-18 退役删除；Katran 成为后续 XDP 类 E2E 的首选 replacement。** 风险：stale policy 需先 refresh、需要 raw sender 不能用 iperf3、单接口 DSR 拓扑。报告：`docs/tmp/katran_e2e_feasibility_20260318.md`。 |
| 193 | **Corpus policy refresh（2026-03-18）** | ✅ | generate_default_policies.py 新增 --skip-families 参数。重新生成 582 个 v3 policy，跳过 cmov + extract。97 个 policy 变化，344 个 extract sites 被移除。非空 policy 366→343。验证：katran 8 sites(wide=4,endian=4)，calico 904 sites(endian=786,wide=108,rotate=10)，tracee 2028 sites(bflip=1791,wide=229,endian=8)。 |
| 194 | **项目卫生审计（2026-03-18）** | ✅ | 全面审计：Result JSON 缺 policy 内容（已在 #190 修复）、vm_micro_authoritative_20260314 被污染（已恢复）、v2 golden→v3 重命名、archive wrapper 修复、219 py_compile 通过。报告：`docs/tmp/project_hygiene_audit_20260318.md`。 |
| 195 | **内核修改详细文档（2026-03-18）** | ✅ | 1017 行中文文档：13 文件 +6759/-194 行，jit_directives.c 3708 行、bpf_jit_comp.c +2551 行、UAPI +258 行。8 种 canonical form emitter 实现、recompile 机制、policy blob 格式。`docs/tmp/kernel_modifications_analysis.md`。 |
| 196 | **Benchmark 框架设计文档（2026-03-18）** | ✅ | 800+ 行中文文档：Makefile 入口、三层模型、driver.py 路由、C++ runner TSC 计时、5 种 harness 宏、scanner 6 子命令、结果管理、VM 框架、ASCII 数据流图。`docs/tmp/benchmark_framework_analysis.md`。 |
| 197 | **Micro 第三轮 policy 优化（2026-03-18）** | ✅ 归入 #189 | 清掉 cmov_select + mixed_alu_mem，最终 7 applied。干净 rerun 确认 rotate64_hash +34.1%（之前 -19% 反转是并发 codex 竞态导致旧 policy 状态）。 |
| 198 | **旧 prog_fd 路径残留清理（2026-03-18）** | ✅ | 删除 _USE_ENUMERATE_PATH flag + 6 个 legacy helper 函数。scanner scan/apply 删除 --prog-fd。只保留 enumerate --prog-id 和 dump --prog-fd。e2e/corpus drivers 统一走 enumerate。py_compile + scanner build + make smoke 通过。 |
...
| 206 | **Sequential rerun round 2（2026-03-18）** | ✅ | 顺序完成 Tracee rerun + Corpus rerun。Tracee 修复 guest bpftool wrapper，并新增 trampoline-attached program recompile guard；rerun 成功但 `file_io` 只有 +1.68%，未替换旧 authoritative。Corpus clean rerun overall `0.934x`，低于旧 authoritative `1.046x`，同样不替换。**后续发现 trampoline guard 是 corpus 回归根因，已 revert（8c70a729d）。** 报告：`docs/tmp/sequential_rerun_round2_20260318.md`。 |
| 207 | **Tracee revert rerun（2026-03-19）** | ✅ | reverted kernel（无 trampoline guard）重跑 Tracee E2E。exec_storm +4.40%，file_io +4.69%，network +2.18%。13/15 applied 恢复。仍低于旧权威（+6.28%/+7.00%），VM 噪声导致。旧权威保持。数据：`e2e/results/dev/tracee_revert_20260319.json`。报告：`docs/tmp/tracee_revert_rerun_20260319.md`。 |
| 208 | **Corpus revert rerun + 改进分析（2026-03-19）** | ✅ | reverted kernel 完整 corpus rerun：overall 0.900x，applied-only **1.023x**（61 applied），non-applied **0.826x**（ultra-short 噪声主导）。linux-selftests 0.708x、katran 0.759x 是主 regressor。两轮 targeted validation：extract 回补 5/5 regression，calico applied rows 当前 policy 优于 stock。**结论：不改 policy，瓶颈是 non-applied ultra-short rows 测量噪声。** 报告：`docs/tmp/corpus_improvement_analysis_20260319.md`。 |
| 209 | **Trampoline regeneration 调研（2026-03-19）** | ✅ | ~100 LOC 方案：`bpf_prog_regenerate_trampolines()` + `bpf_trampoline_update()` + freplace `bpf_arch_text_poke()`。插点在 commit 后、`synchronize_rcu()` 前。unattached programs no-op。报告：`docs/tmp/trampoline_regeneration_research_20260319.md`。待实施。 |
| 210 | **Kernel deep review（2026-03-19）** | ✅ | 9 个问题（4×P1, 5×P2）+ 3 设计建议 + 6 可复用 API。P1：#1 hidden subprog kallsyms 悬挂、#2 非 x86 半实现 syscall、#3 policy_fd==0 歧义、#4 ROLLBACK 语义+snapshot 过早。P2：#5 do_jit 无 policy dispatch、#6 2-insn dead code、#7 同#1、#8 重复逻辑、#9 recompile_count。计划 3 轮 codex 修复（A: bug fix → B: 性能+去重 → C: 结构重构）。报告：`docs/tmp/kernel_deep_review_20260319.md`。 |
| 211 | **GitHub ARM64 KVM 调研（2026-03-19）** | ✅ | `ubuntu-24.04-arm` 无 /dev/kvm，larger runner 也无官方保证。ARM64 recompile benchmark 必须走租机方案（bare metal）。现有 CI（arm64-benchmark.yml）只跑 native llvmbpf+kernel。报告：`docs/tmp/github_arm64_kvm_research_20260319.md`。 |
| 212 | **Kernel fix Phase A：bug fix（2026-03-19）** | ✅ | #1+#7 func_cnt→real_func_cnt + #2 架构 gate。报告：`docs/tmp/kernel_fix_phase_a_20260319.md`。 |
| 213 | **Kernel fix Phase B：性能+去重（2026-03-19）** | ✅ | #4a snapshot 后移 + #5 无 policy 短路 + #8a/b 去重 helper。报告：`docs/tmp/kernel_fix_phase_b_20260319.md`。 |
| 214 | **Kernel fix Phase C：结构重构+能力扩展（2026-03-19）** | ✅ | C1: 2-insn validator 补齐。C2: 8 form validator/emitter params 统一。报告：`docs/tmp/kernel_fix_phase_c1_20260319.md`、`docs/tmp/kernel_fix_phase_c2_20260319.md`。 |
...
| 229 | **Kernel 5k LOC deep review（2026-03-19）** | ✅ | 6/10 评分。P0：trampoline-attached 程序 recompile 未 gate（仅拒绝 struct_ops）。P1：ZERO_EXT_ELIDE+BRANCH_FLIP emitter 读原始 insn 违反 param-only 合约；reserved flags fail-open。P2：overlapping rules 未检测；BRANCH_FLIP dead normalized fields。报告：`docs/tmp/kernel_5k_loc_deep_review_20260319.md`（codex 未写成功，结果在 task output）。 |
| 230 | **Kernel P0/P1/P2 修复（2026-03-19）** | ✅ | P0: `bpf_prog_has_active_trampoline()` + `bpf_jit_recompile_has_trampoline_dependency()` 覆盖所有 trampoline 场景。P1: ZERO_EXT_ELIDE+BRANCH_FLIP emitter 改为 param-only；reserved flags fail-closed `-EINVAL`。P2: sort 后 overlap 检测；BRANCH_FLIP dead fields 删除。selftest 19/19, smoke OK, scanner PASS 180。commit `b50fd6279`。报告：`docs/tmp/kernel_review_fixes_20260319.md`。 |
| 231 | **Docker ARM64 交叉编译（2026-03-19）** | ✅ | `runner/docker/arm64-crossbuild.Dockerfile` + Makefile targets: `make selftest-arm64`（Docker 内交叉编译 test_recompile）、`make vm-arm64-selftest`（QEMU guest 内执行）。ARM64 ELF binary OK，但 ARM64 kernel 返回 EOPNOTSUPP 18/19 fail（无 `bpf_jit_supports_recompile()` override）。commit `0c7f0f1`。 |
| 232 | **用户态 cleanup round 3（2026-03-19）** | ✅ | 修复 `corpus/_driver_impl_run_production_corpus_v5_framework.py` failure path NameError。清理 dead imports、unused locals、no-op f-strings。commit `0c7f0f1`。报告：`docs/tmp/userspace_cleanup_round3_20260319.md`。 |
| 233 | **Kernel LOC breakdown（2026-03-19）** | ✅ | 5800 行 runtime kernel delta：jit_directives.c 3114（53.7%）、bpf_jit_comp.c 2182（37.6%）、其余 504（8.7%）。jit_directives.c 67.9% essential logic、32.1% scaffolding/validation/logging。ROTATE 最大 461 行。报告：`docs/tmp/kernel_loc_breakdown_20260319.md`。 |
| 234 | **Kernel 架构 review（2026-03-19）** | ✅ | ROTATE 4 parser 合并可省 180-210 行；logging 复用 verifier log 可省 70-100 行；trivial wrappers inline 可省 50-90 行；bounds/choice 去重可省 55-105 行。保守总计 400-550 行。报告：`docs/tmp/kernel_architecture_review_20260319.md`。 |
| 235 | **Kernel 代码简化实施（2026-03-19）** | ✅ | 全部完成：Phase 1 实现 verifier log 复用、trivial wrappers / staged getters / ksym wrappers cleanup；Phase 2 实现 spec-driven ROTATE common validator + form/native-choice metadata table + bounds/feature 去重。相对简化前 touched files 净减 **447 LOC**（`jit_directives.c 3228→2797`，`bpf_jit_comp.c 6136→6091`）。验证全绿。报告：`docs/tmp/kernel_simplification_20260319.md`。 |
| 236 | **Micro rerun post-P0/P1/P2（2026-03-19）** | ✅ | 3warm/5iter/500rep 初轮 overall 0.976x（rotate_dense 回归）。debug 确认：旧 bzImage provenance 问题，非代码 bug。简化后重建 bzImage 两轮 targeted rerun 均正：rotate_dense +17.5%/+7.0%，rotate64_hash +35%/+31%。256/256 rules applied。P0/P1/P2 代码审计无 bug。报告：`docs/tmp/micro_post_p0p1p2_fix_20260319.md`、`docs/tmp/rotate_dense_regression_debug_20260319.md`。 |
| 237 | **Scaleway RV1 RISC-V 调研（2026-03-19）** | ✅ | RV1-C4M16S128 可通过 CLI/API 租用。boot 走 OpenSBI+device tree 非标准链，custom kernel 需特殊处理。SLA 0%（Labs 产品）。报告：`docs/tmp/riscv_cloudv_scaleway_research_20260319.md`。 |
| 238 | **LLVM vs kernel JIT 指令级对比（2026-03-19）** | ✅ | 8 个代表 benchmark fresh dump：`rotate_dense`、`cond_select_dense`、`load_byte_recompose`、`extract_dense`、`endian_swap_dense`、`bounds_ladder`、`large_mixed_500`、`simple`。结论：WIDE_MEM + ROTATE 是主因，ENDIAN/COND_SELECT 次之；`extract_dense` 的关键差异不是 `bextr`，而是 LLVM 的 load-width narrowing；`bounds_ladder` 主要是 wide-load collapse + 少量 `cmove`，不是纯 BRANCH_FLIP。8-form 未覆盖的差距：load-width narrowing、store coalescing、whole-function register allocation。报告：`docs/tmp/llvm_vs_kernel_jit_disasm_analysis_20260319.md`，artifacts：`docs/tmp/llvm_vs_kernel_jit_disasm_20260319_artifacts/`。 |
...
| 245 | **Corpus clean rerun（2026-03-19）** | ✅ | overall 0.816x，applied-only **1.062x**，61 applied。`linux-selftests` 拖后腿（0.577x，ultra-short non-applied 噪声主导）。`katran/balancer_ingress` applied 回归 0.404x。**不替换旧权威。** 报告：`docs/tmp/corpus_post_simplification_20260319.md`。 |
| 246 | **E2E rerun tracee+tetragon（待做）** | ⏳ | 等 VM 空闲后串行执行。 |
| 247 | **P1 masked ROTATE fix + P2 cleanup（2026-03-19）** | ⚠️ | **kernel-side 已完成并在 #263 验证**：masked ROTATE 仅接受 high-mask，dead `bpf_prog_has_active_trampoline()` 删除，新增 low-mask rejection selftest。**剩余**：scanner masked-rotate acceptance 仍比 kernel 宽，待单独对齐。报告：`docs/tmp/trampoline_regen_rotate_fix_20260319.md`。 |
| 248 | **Trampoline regeneration 实现（2026-03-19）** | ✅ | 实现 `bpf_prog_regenerate_trampolines()`：attached tracing progs refresh、target-side trampoline retarget、freplace target repoke。commit path 在 image retirement 前触发 refresh；失败 warning-only，不回滚已提交 image，并保留旧 JIT text。验证：`make vm-selftest` 25/25，`make vm-micro-smoke` pass。报告：`docs/tmp/trampoline_regen_rotate_fix_20260319.md`。 |
...
| 260 | **ARM64 micro eval（2026-03-19）** | ✅ | ARM64 micro smoke 端到端通过。关键发现：blob header 硬编码 x86 arch id → ARM64 kernel EINVAL，修复后 COND_SELECT/ROTATE recompile smoke 通过。**性能强烈负向**（待调查，可能是 QEMU 噪声/emitter bug）。cross-build 通过 docker。报告：`docs/tmp/arm64_micro_eval_20260319.md`。 |
| 261 | **Benchmark cleanup（2026-03-19）** | ✅ | adaptive-repeat 已从 micro runner 删除，`run-kernel` 直接使用 CLI `--repeat`；`micro_exec.hpp` / `common.cpp` 已清理相关 options、CLI 与校验；`main.cpp` 无相关引用无需改动。验证：`make -C micro micro_exec`、`make smoke` 通过。报告：`docs/tmp/benchmark_cleanup_261_20260319.md`。 |
| 262 | **Same-image paired measurement 改造（2026-03-19）** | ✅ | `run-kernel --recompile` 改为 same-image：load→stock warmup→stock measure→recompile→recompile warmup→recompile measure。JSON 新增 `stock_exec_ns`。删除 `run-kernel-paired` dead code。Corpus VM batch driver 改为单次 recompile run 读 `stock_exec_ns`。验证：`make -C micro micro_exec`、`make vm-micro-smoke`、guest 直接跑 `load_byte_recompose_xdp` 确认 `stock_exec_ns` 输出。Corpus sanity 1-target pass。报告：`docs/tmp/same_image_measurement_20260319.md`。 |
| 263 | **Trampoline regeneration + P1 ROTATE fix（2026-03-19）** | ✅ | 合并完成：live trampoline regeneration 落地（attached + target-side + freplace repoke），masked ROTATE low-mask rejection 固化为 selftest，trampoline dead helper 删除。验证：`make -C vendor/linux-framework -j$(nproc) bzImage`、`make vm-selftest`（**25/25**）、`make vm-micro-smoke` 全绿。报告：`docs/tmp/trampoline_regen_rotate_fix_20260319.md`。 |
| 264 | **ARM64 emitter review（2026-03-19）** | ✅ | **7/10**。P0：ARM64 staged recompile 缺 subprog first-pass staging（`jit_recompile_bpf_func` 未发布），多函数程序 recompile 可能出问题。P1：`BITFIELD_EXTRACT` 无 width 校验，非 UBFX-safe site 直接 emit 而非 fail-closed。正面：A64_EXTR/UBFX/CSEL 编码正确，ROTATE/WIDE_MEM/COND_SELECT solid，emitter param-only，不支持 form fail-closed。报告：`docs/tmp/arm64_emitter_review_20260319.md`。 |
| 265 | **ARM64 emitter P0/P1 fix + testcase（2026-03-19）** | ✅ | P0 完成：ARM64 subprog recompile first-pass staging 发布 `jit_recompile_bpf_func`，并在 extra pass `BPF_PSEUDO_CALL` 上解析 staged subprog target。P1 完成：`BITFIELD_EXTRACT` width/representability 语义移入 validator，ARM64 emitter 改为 direct `UBFX` fail-closed。验证：`make -C vendor/linux-framework -j$(nproc) bzImage`、`make vm-selftest`（**27/27**）、`make vm-micro-smoke` ✅；ARM64 QEMU targeted selftests `26/27`，剩余 `Recompile After Attach Works` 与本任务无关。报告：`docs/tmp/arm64_p0p1_fix_20260319.md`。 |
| 266 | **AWS ARM64 bare metal benchmark + Makefile（2026-03-19）** | ⚠️ | 已补齐 `make aws-arm64-*` 目标与 `runner/scripts/aws_arm64.sh`，并新增 profile-aware AWS CLI 路径（默认 `AWS_ARM64_PROFILE=codex-ec2`）。2026-03-20 在 `us-east-1` `t4g.micro` 完成真实 launch→setup→benchmark→terminate 流程：custom ARM64 kernel 安装/one-shot reboot 验证通过，native-built `micro_exec` smoke + extra benchmark 成功，最终实例确认 `terminated`。残余问题：脚本当前 `benchmark` 直接上传的本地 ARM64 `micro_exec` 与 AL2023 glibc ABI 不兼容，需 native build workaround；`make kernel-arm64` 与 AWS-compatible build dir 仍有 mismatch。报告：`docs/tmp/aws_arm64_benchmark_20260319.md`、`docs/tmp/aws_arm64_benchmark_run_20260320.md`。 |
| 267 | **Makefile + benchmark pipeline 审计（2026-03-19）** | ✅ | `make smoke` 加人可读摘要输出。`micro/Makefile clean` 不再删 authoritative JSON。清理 stale latest.json defaults、unused wrappers、hardcoded venv。Pipeline 一致，defaults 统一 10/2/200。Gap：缺 ARM64 eval targets。scanner-tests 19 FAIL（#263 ROTATE tightening 未同步 scanner）→ 另派 #268 修复。报告：`docs/tmp/makefile_benchmark_audit_20260319.md`。 |
| 268 | **Scanner ROTATE 对齐（2026-03-19）** | ✅ | `scanner/src/pattern_v5.cpp` 对齐 kernel masked 32-bit ROTATE high-mask-only 规则，显式拒绝 low-mask shift 序列；新增 5-insn/6-insn negative tests，policy/filter 期望同步更新。验证：`cmake --build scanner/build --target bpf-jit-scanner --target test_scanner -j`、`make scanner-tests`（**0 FAIL**）、`make smoke` ✅。报告：`docs/tmp/scanner_rotate_align_20260319.md`。 |
| 269 | **Validator/emitter boundary audit（2026-03-20）** | ✅ | 规则收敛为“Validator = 唯一安全门，Emitter = 无条件信任”。完成 8 个 shared form validator + x86/arm64 emitters 审计：`BITFIELD_EXTRACT` 语义/边界归 validator；`WIDE_MEM` 显式 width bound；`ZERO_EXT_ELIDE` / `BRANCH_FLIP` validator 收紧到 emitter 可线性重放子集；删除 x86/arm64 emitter-side param/native-choice rechecks 与 x86 `BITFIELD_EXTRACT` dead fallback；x86 feature gating 统一进 `bpf_jit_arch_form_supported()`。验证：`make -C vendor/linux-framework -j$(nproc) bzImage`、`make vm-selftest`（**27/27**）、`make vm-micro-smoke`、`make scanner-tests` ✅。报告：`docs/tmp/validator_emitter_boundary_audit_20260320.md`。 |
| 270 | **Katran harness fix（2026-03-20）** | ✅ | harness 改为 same-image paired measurement + warmup + map reset。smoke 1.042x 翻正。报告：`docs/tmp/katran_harness_fix_20260320.md`。 |
| 271 | **Corpus 0.983x 根因分析（2026-03-20）** | ✅ | 57/150 缺 stock_exec_ns 因 policy_mode=stock 走普通 run-kernel；applied-only 0.947x 被 wide+endian 回归拖低。报告：`docs/tmp/corpus_rootcause_20260320.md`。 |
| 272 | **Selftest negative/boundary tests（2026-03-20）** | ✅ | +9 tests（overlap/cross-subprog/invalid-choice/arch-gate/blinded/struct_ops/interior-edge/reserved-flags/concurrent）。**vm-selftest 36/36**。报告：`docs/tmp/selftest_negative_boundary_20260320.md`。 |
| 273 | **Kernel code quality audit（2026-03-20）** | ✅ | P0: BRANCH_FLIP temp buffer overflow。P1: COND_SELECT alias miscompile + ADDR_CALC self-referential alias。P2: trampoline regen memory leak。Dead code: policy->blob, BFX_PARAM_ORDER, unused params。Bloat: x86 replay 523 行给 0-demand ZERO_EXT_ELIDE。估计可省 220-350 LOC。报告：`docs/tmp/kernel_code_quality_audit_20260320.md`。 |
| 274 | **Benchmark framework refactoring（2026-03-20）** | ✅ | runner/ 顶层提取（C++ src + Python libs）。corpus 8 driver→1。旧 E2E helper 模块合并到 `runner/libs/`。20+ 旧文件删除。CLAUDE.md/Makefile/README 路径更新。报告：`docs/tmp/benchmark_framework_refactor_20260320.md`。 |
| 275 | **Kernel P0+P1 bug fix（2026-03-20）** | ✅ | P0: BRANCH_FLIP validator 加 native-byte 上限。P1: COND_SELECT 拒绝 dst==condition alias，ADDR_CALC 拒绝 base==dst self-ref。Dead code 删除。vm-selftest **36/36**。报告：`docs/tmp/kernel_p0p1_fix_20260320.md`。 |
| 276 | **ZERO_EXT_ELIDE deletion analysis（2026-03-20）** | ✅ | 可安全删除。x86 0 sites，ARM64 未实现，无 benchmark/policy 使用。共享 replay helpers 保留（BRANCH_FLIP 用）。UAPI form id 6 保留 deprecated。报告：`docs/tmp/zero_ext_elide_deletion_analysis_20260320.md`。 |
| 277 | **Post-refactor cleanup（2026-03-20）** | ✅ | 完成 post-refactor 审计与清理：`micro/driver.py` help 路由修复，当前源码 `py_compile` 无错误，旧 archive / scratch 目录均确认已删除，当前文档中的旧路径引用已收敛到历史归档材料。报告：`docs/tmp/benchmark_post_refactor_audit_20260320.md`。 |
| 278 | **ZERO_EXT_ELIDE full-stack deletion（2026-03-20）** | ✅ | 内核 -118 LOC（validator + x86 emitter + arch support）+ scanner pattern + selftest 删除。共享 replay 保留。UAPI deprecated。vm-selftest **35/35**。报告：`docs/tmp/zero_ext_elide_deletion_20260320.md`。 |
| 279 | **Cross-layer dependency fix（2026-03-20）** | ✅ | `micro/driver.py` 删除 corpus/census 转发。Makefile vm-corpus 直接调 corpus/driver.py。corpus/driver.py 用 runner/libs 共享 helpers。零跨层 import 验证通过。报告：`docs/tmp/cross_layer_cleanup_20260320.md`。 |
| 280 | **P2 trampoline regen leak fix（2026-03-20）** | ✅ | Double `synchronize_rcu()` 方案：regen 失败时先 synchronize_rcu()→free old images→再 synchronize_rcu()。`keep_old_images` gate 从 free path 移除。Dead code（`policy->blob`、unused rollback param）删除。vm-selftest 35/35。 |
| 281 | **ARM64 E2E smoke t4g.micro（2026-03-20）** | ✅ | Custom kernel boot + 网络 ✅。Native micro_exec + scanner build ✅。Tracee/Katran load ✅，Tetragon ❌（CO-RE pt_regs.bp）。Scanner enumerate + recompile blocked by AL2023 bpftool 不支持 kernel_btf（后续 #286 修复 Katran）。3 台 EC2 已 terminated。报告：`docs/tmp/arm64_e2e_smoke_20260320.md`。 |
| 282 | **Katran E2E 质量分析 + harness 升级（2026-03-20）** | ✅ | 分析：74 live sites 只 apply 4 个 wide-load-2，1.042x 是局部结果。缺 PPS/p99/CPU、流量太少（1200 events vs 50K+）、sample_count=1。升级：harness 改为 timed short-flow benchmark，新增 req/s、p50/p90/p99、PPS、CPU、BPF avg ns。VM rerun BPF **1.108-1.168x**，outer throughput 不稳（VM 噪声）。报告：`docs/tmp/katran_e2e_quality_analysis_20260320.md`。 |
| 283 | **ARM64 bpftool kernel_btf fallback（2026-03-20）** | ✅ | corpus/driver.py 加 kernel_btf 能力探测 + fallback。AWS t4g.micro 验证：Katran `loadall → enumerate → recompile` 全通（74 sites，6 applied）。Tracee 被 autoattach 阻塞（独立问题），Tetragon libbpf load failure。EC2 已 terminated。报告：`docs/tmp/arm64_bpftool_fix_20260320.md`。 |
| 284 | **Kernel -137 LOC 精简（2026-03-20）** | ✅ | 6 项全做：ARM64 a64_invert_cond（-28）、alu32/64_linearizable 合并（-25）、跨 arch 函数提共享 header（-20）、image_prog helper 提取（-18）、opcode switch→table（-30）、cond_op_valid 合并（-10）。净减 **-137 LOC**。vm-selftest 35/35。报告：`docs/tmp/kernel_low_risk_simplification_20260320.md`。 |
| 285 | **用户态代码精简（2026-03-20）** | ✅ | 删除 legacy/（85MB，317 文件）。micro/_driver_impl_*.py 3 个合并到 driver.py。删除 corpus/fetch_bcf_dataset.py + analyze_bytecode.py。净减 **-32,329 行**。make smoke + scanner-tests ✅。报告：`docs/tmp/userspace_consolidation_20260320.md`。 |
| 286 | **Framework 散乱清理（2026-03-20）** | ✅ | 删除 build-arm64/、live-xlated-test.*/、tmp/（根目录）、micro/*.sh dead scripts。e2e/run_e2e_tracee.py fallback 提取到 manual.py。.gitignore 更新。make smoke + scanner-tests ✅。报告：`docs/tmp/framework_cleanup_audit_20260320.md`。 |
| 287 | **Kernel bloat review（2026-03-20）** | ✅ | 审查 6349 行 delta。最大膨胀：x86 BRANCH_FLIP ALU replay ~420 行。低风险可省 ~131 行（已由 #284 实施）。无 dead code、无过度抽象。BRANCH_FLIP 条件码反转不可行（body 必须物理重排）。报告：`docs/tmp/kernel_bloat_review_20260320.md`。 |
| 288 | **Emitter 成本分析 + BRANCH_FLIP 可行性（2026-03-20）** | ✅ | Per-form 成本：简单 ~180-500 LOC，复杂（BRANCH_FLIP）~1132 LOC。对比 GCC peephole2 ~20-50 行，偏高 10-20x。BRANCH_FLIP 条件码反转 **不可行**（只改 jcc 交换真假语义，不改物理 fallthrough 布局；body 必须物理重排）。可共享 replay helper 降到 ~150 行。报告：`docs/tmp/emitter_cost_and_branchflip_analysis_20260320.md`。 |
| 289 | **表驱动 validator substrate（2026-03-20）** | ✅ | 轻量 compile-time pattern matcher：ADDR_CALC、BITFIELD_EXTRACT、ENDIAN_FUSION 切到声明式表。+333 LOC substrate 投资，新增简单 form 边际成本从 ~500 降到 ~80-150 LOC。ROTATE/WIDE_MEM/COND_SELECT/BRANCH_FLIP 未切入（避免重 DSL）。vm-selftest 35/35。报告：`docs/tmp/form_cost_reduction_design_20260320.md`。 |
| 290 | **vm-micro 全量验证（2026-03-20）** | ✅ | 默认参数 3/1/200。**62/62 valid**，overall **1.100x**，applied-only **1.269x**，0 crash/mismatch。Applied: cmov_dense 3.12x、bounds_ladder 2.17x、rotate64_hash 1.39x（VM 噪声大于权威 10/5/1000）。数据：`micro/results/dev/vm_micro.json`。报告：`docs/tmp/vm_micro_validation_20260320.md`。 |
| 291 | **ROTATE/WIDE_MEM 表驱动（2026-03-20）** | ✅ | ROTATE 切声明式 fixed-length pattern + finalize callback。WIDE_MEM 切有界 head/body repeat chunk matcher。jit_validators.c 2056→1990，净减 **-66 行**。vm-selftest 35/35。报告：`docs/tmp/rotate_wide_table_driven_20260320.md`。 |
| 292 | **Katran endian-swap-store-16 bug 调查（2026-03-20）** | ✅ | 根因：x86 `movbe store` 不保留寄存器副作用。Katran store 后复用 reg 算 checksum → 错误 checksum → 超时。**Emitter bug + Validator 缺 liveness check**。修复建议：方案 C（recompile 时 50 行 liveness analysis）。报告：`docs/tmp/katran_endian_store_bug_investigation_20260320.md`。 |
| 293 | **PGO-style policy 设计（2026-03-20）** | ✅ | 核心结论：discovery 错层（offline scan 看不到 live-only families）是真正 blocker。P0：切 live enumerate；P1：run_cnt/run_time_ns 热度门控；P2：perf branch miss 引入 cmov/branch-flip 决策。对比 BOLT/AutoFDO。报告：`docs/tmp/pgo_policy_design_20260320.md`。 |
| 294 | **Verifier 扩展调研（2026-03-20）** | ✅ | 三种方案：全替代（侵入 verifier，-500~800 行，低接受度）、post-fixup collector（-600 行，中接受度）、混合方案（-150 行，高接受度）。结论：保存 verifier facts 不能大幅缩减 validator（shape matching 仍需手写），方案 C（recompile 时 50 行 liveness pass）最划算。报告：`docs/tmp/verifier_extension_research_20260320.md`、`docs/tmp/verifier_facts_savings_analysis_20260320.md`。 |
| 295 | **Katran wide-load 覆盖调查（2026-03-20）** | ✅ | 结论：不是 scanner gap，是 Katran 代码形态。主 datapath 已是 LDXH/LDXW，4 个 wide 来自 MAC 拷贝。剩余 byte-recompose 是 tree shape（scanner+kernel 都不支持）。74 sites 分布：bflip=56、cmov=8、wide=4、endian=4、extract=2。报告：`docs/tmp/katran_wide_load_investigation_20260320.md`。 |
| 296 | **Katran site coverage 分析 + safe6 policy（2026-03-20）** | ✅ | 根因两层：E2E policy 只列 4 wide + offline scan 看不到 live bflip。逐 family 实测：bflip 全开功能 OK 但 0.739x；endian-swap-store 2 个 bug（破坏 datapath）。Policy 更新为 safe6（4 wide + 2 endian-load-swap-32）。BPF 1.042x。报告：`docs/tmp/katran_site_coverage_analysis_20260320.md`。 |
| 297 | **Tetragon 4/7 未 apply 调查（2026-03-20）** | 🔄 | codex 执行中。 |
| 298 | **ARM64 c7g.large 正式 benchmark（2026-03-20）** | ⚠️ 方案已改 | 改为本地交叉编译+t4g.micro 跑。见 #300。**⚠️ ARM64 benchmark 只用 t4g.micro，不用大机器（c7g/c7g.large）。** 每次 session reset 必须检查远端有没有实例还开着（`aws ec2 describe-instances --filters Name=instance-state-name,Values=pending,running,stopping,stopped`）。 |
| 299 | **x86 新优化机会调研（2026-03-20）** | ✅ | Top-5：PROLOGUE_TIGHTEN（每程序 1 site，~200 LOC）、NARROW_OPS（Katran 30+ sites，~250 LOC）、ZERO_STORE_FUSION（~150 LOC）、WIDE_MEM_TREE_BE（需改 kernel）、BOUNDS_WINDOW（需 verifier facts）。推荐先做 #1+#2。报告：`docs/tmp/x86_new_optimization_opportunities_20260320.md`。 |
| 300 | **ARM64 cross-build pipeline + t4g.micro benchmark（2026-03-20）** | 🔄 | **决策**：ARM64 benchmark 改为本地交叉编译 + scp 上传 + t4g.micro 跑。不用大机器（c7g/c7g.large）。修 Docker cross-build glibc 兼容。更新 Makefile/runner/scripts/docs。每次 session reset 检查远端实例。codex 执行中。 |
| 301 | **Tetragon 4/7 未 apply 调查（2026-03-20）** | ✅ | 根因：offline static scan 看不到 live branch-flip sites（同 Katran #296）。4 个未 apply 程序 live 有 6/3/5/1 bflip sites。VM 验证最多 5/7 可 apply。阻塞：execve_rate + tg_kp_bprm_committing_creds 的 bflip site 被 validator reject。报告：`docs/tmp/tetragon_unapplied_investigation_20260320.md`。 |
| 302 | **E2E profiler + profile-guided policy（2026-03-20）** | 🔄 | 创建 BPF profiler（run_cnt/run_time_ns/热度/branch-misses）+ profile-guided policy generator + leave-one-out 自动化。codex 执行中。 |
| 303 | **目录整理（2026-03-20）** | ✅ | 已完成：顶层 Docker 构建目录迁到 `runner/docker`，顶层 runner/helper 脚本迁到 `runner/scripts`，参考资料迁到 `docs/reference`。检查确认原顶层 `config/` 仅剩 `micro_pure_jit.yaml`，已迁到 `micro/config/micro_pure_jit.yaml`。Makefile / GitHub Actions / docs / Python 引用已同步更新，并在删除旧顶层目录前合并了旧目录中已有的用户修改。验证：`make smoke`、`make scanner-tests`、`python3 -m py_compile`（改动 Python 文件）、旧路径残留 grep 均通过。报告：`docs/tmp/directory_reorganization_20260320.md`。 |
