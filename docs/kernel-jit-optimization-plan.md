...
| **500** | **⚠️ map_inline 真实 VM 零命中调查（2026-03-26）** | 🔄 | 根因修复已提交：`d9e2ccd`（放宽 backward scan，覆盖真实 clang 指令顺序）；调查/修复报告：`map_inline_real_values_investigation_20260326.md`、`map_inline_zero_hit_root_cause_20260326.md`、`map_inline_pattern_fix_report_20260326.md`。已重跑 `make vm-static-test STATIC_VERIFY_ARGS='--mode macro' TARGET=x86`，artifact `daemon/tests/results/static_verify.json` 显示 `objects=477` / `programs=1178` / `applied=104`，但 **map_inline applied programs 仍为 0**。验证报告：`map_inline_fix_verification_report_20260326.md`。需继续定位为什么真实 macro corpus 仍无命中。 |
