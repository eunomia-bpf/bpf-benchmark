...
- **YAML 只列 app，不列 object/program**：YAML 定义 app（loader instance），每个 app 指定 runner + workload。启动 app 后通过 bpf_stats/get_next_id 自动发现所有 BPF 程序，全部测量。不需要在 YAML 里枚举 .bpf.o 或 program name。这和 daemon apply-all 模式完全一致。Object 是编译产物的打包格式，和测量无关。
- **YAML schema**：
  ```yaml
  apps:
    - name: tracee            # loader instance 名
      runner: tracee          # app runner 类名
      workload: exec_storm    # 触发方式
      # 启动后自动发现 30+ 个 BPF 程序
    - name: bcc/execsnoop
      runner: bcc
      tool: execsnoop         # BCC 的哪个 tool
      workload: exec_storm
    - name: katran
      runner: katran
      workload: network
...
| **634** | llvmbpf verifier pass 率修复 | ✅ | **Round 1**：`last insn` 清零（106→0），lift 556→**779**，lower 309→**413**，verifier 7→**16（3.9%）**。报告：`llvmbpf_roundtrip_fix_report_20260329.md`。**Round 2**：map relocation repair — guest helper 从 orig_insns 插回 pseudo-map LD_IMM64。**VM 验证通过：verifier 16→157/413（38.0%）**，超过 >20% 目标。剩余失败：166 not pinned、51 EINVAL、14 !read_ok、10 map_ptr。报告：`llvmbpf_map_relocation_fix_report_20260329.md`、`llvmbpf_map_relocation_vm_results_20260329.md`。 |
