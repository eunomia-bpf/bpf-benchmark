...
> 上次更新：2026-03-20d。**权威数据**（#256 rerun）：micro **1.057x** / applied-only **1.193x**；corpus **0.983x**；Tracee **+8.1%**；Tetragon **+20.3%/+32.2%**；Katran **1.042x**；gap **0.581x**。vm-selftest **35/35**。**完成**：ZERO_EXT_ELIDE 全栈删除（-118 kernel LOC）、P0/P1 kernel fix（BRANCH_FLIP/COND_SELECT/ADDR_CALC）、framework runner/ 提取 + cross-layer cleanup（零跨层 import）、selftest +9 negative tests、code quality audit、kernel/benchmark 设计文档。**进行中**：P2 trampoline leak fix、ARM64 E2E smoke（t4g.micro）、commit+push。
...
| 280 | **P2 trampoline regen leak fix（2026-03-20）** | 🔄 | 旧 image/stack 在 regen 失败时永不释放。codex 执行中。 |
| 281 | **ARM64 E2E smoke t4g.micro（2026-03-20）** | 🔄 | codex 执行中。 |
