...
- `bpfget` 是 daemon-owned library，负责读取 bytecode/metadata/map 信息；thin dry-run 和最终 ReJIT 由 daemon 通过 `kernel-sys` 直接执行。
- `bpfrejit-daemon` 保留 runner socket 边界和 watch/map invalidation，进程内持有 prog/map/kinsn BTF fd；字节码变换仍只通过外部 `bpfopt` CLI，主路径直接 `BPF_PROG_REJIT`。
...
5. **Fail-safe**：`BPF_PROG_REJIT` 内核 re-verify 失败 → 什么都不改，返回错误；当前 daemon 不做主路径 `BPF_PROG_LOAD` dry-run，也不在 pass 之间回滚
...
9. **Pipeline 是 bpfopt shell/file 协议 + daemon in-memory kernel state**：`bpfopt` stdin/stdout 只传 raw binary bytecode，profile/verifier states/map values/report 等 `bpfopt` side-input/output 走文件；prog info、snapshot-owned map fd、fd_array 在 daemon 进程内传递。
10. **Daemon 是 runner 稳定边界**：`bpfrejit-daemon` watch 新程序、检测 map invalidation、维护 runner socket，并进程内编排 live snapshot、自动 side-input 准备、final ReJIT；benchmark runner Python 不直接改成调 kernel-facing CLI。
...
│                                                              │
│  * PROG_LOAD dry-run only for verifier-state side inputs      │
│                                                              │
...
v3 rollback 后，runner 边界是 daemon socket；bytecode transform 仍是 CLI/file protocol，kernel-facing state 不再跨进程序列化。
...
4. daemon fd_array builder       → used_maps 顺序 map fd + kinsn BTF fd
5. daemon direct ReJIT           → BPF_PROG_REJIT；kernel re-verify → re-JIT → image swap
```
...
包含 `map_inline` / `const_prop` 的请求会在步骤 3 前自动插入一次 thin dry-run 生成 `verifier-states.json`；默认 12-pass pipeline 包含 `const_prop`，因此默认会自动准备该 side-input。
...
2026-05-01 v2-style rollback 后，v3 的权威边界是：`bpfopt` / `bpfprof` 保持 CLI，`bpfget` 是 daemon-owned in-process library，thin dry-run 和 final ReJIT 由 daemon 通过 `kernel-sys` 直接执行。`bpfverify` / `bpfrejit` crate 已删除。
...
- **kernel-facing 功能在 daemon 进程内**：`bpfget` 读取 live program bytecode/metadata/map info；daemon 从 `prog_info.used_maps` 和 target kinsn BTF 构造 in-memory `fd_array`；final ReJIT 直接调用 `kernel_sys::prog_rejit()`。
...
- **主路径不 dry-run**：daemon 不在 pass 之间或 final ReJIT 前调用 `BPF_PROG_LOAD` 接受 candidate。kernel 在 `BPF_PROG_REJIT` 内 re-verify；失败直接 surface error。
- **自动 verifier-state thin dry-run**：默认 12-pass pipeline 包含 `const_prop`。请求包含 `map_inline` / `const_prop` 时，daemon 用原始 bytecode 做一次 thin dry-run 捕获 verifier states，并且强制 `func_info = None`、`line_info = None`。缺失 states 或 parse 失败直接 error，不允许空结果 fallback。
...
- **结构化 per-pass 记录**：每个 program 的每个 pass 记录 `pass`/`changed`/`sites_applied`/`insn_delta` 等 CLI `--report` 事实字段；daemon 不伪造 per-pass verify/rollback 状态。
...
2. **Phase 2：kernel syscall libs + profile CLI**。`bpfprof` 保持 CLI；`bpfget` 归 daemon-owned library；thin dry-run 和 ReJIT orchestration 在 daemon 内直接调 `kernel-sys`。
...
- **1.3 `bpfopt` CLI 完整版**：已提供 pass 子命令、`optimize`、`list-passes`；默认 `optimize` pass list 为 12 个，包含 `const_prop`，不含 Paper B `branch_flip` 和非默认 `ccmp`。缺少 required side-input 或 kinsn 不可用时退出 1（`b0da364`、`aaaaa76`；见 `docs/tmp/bpfopt-cli-review-20260428.md`）。
...
- **1.5 direct ReJIT**：`bpfverify` / `bpfrejit` crate 已删除；final REJIT 由 daemon 直接调用 `kernel_sys::prog_rejit()`，主路径不做 `BPF_PROG_LOAD` dry-run。
- **1.6 minimal pipeline**：生产路径不再是 `bpfget | bpfopt | bpfrejit`；当前路径是 daemon snapshot → `bpfopt` CLI → daemon direct ReJIT。
- **3.3 verifier-state thin dry-run**：daemon 在请求包含 `map_inline` / `const_prop` 时，用原始 bytecode 做一次 thin dry-run 捕获 verifier states，且不传 BTF func_info/line_info。

测试现状：`bpfopt` workspace 和 daemon workspace 单独 `cargo test --workspace` 均通过；daemon workspace 包含 `bpfget` library tests、thin dry-run tests 和 daemon command tests。

### Phase 2（完整工具链）— 进行中

- **2.1 verifier-state parser 迁移**：verifier log parser 已移到 `kernel-sys`，供需要 verifier states 的 pass 的 thin dry-run side-input 使用。
...
- daemon 只 fork+exec `bpfopt` / `bpfprof`；`bpfget` 是唯一 daemon-owned kernel-facing library，thin dry-run 和 ReJIT orchestration 在 daemon 内直接调 `kernel-sys`。
...
- **3.1 daemon socket orchestration**：`optimize` 走 daemon snapshot → `bpfopt optimize`/pass CLI → daemon direct ReJIT；`profile-start/stop` 走 `bpfprof`；`status` 只保留 daemon 健康和 profiling 状态，`optimize-all` 已删除。
