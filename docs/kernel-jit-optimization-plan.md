...
| **DCE** | 否 | ✅ 已实现 | const prop / map inline 后的 unreachable block / dead store 消除 | bpfopt-core DCE 让更多条件变常量 |
...
| **寄存器重分配** | 否 | 📝 待做 | BPF 10 个寄存器的 liveness 分析→callee-saved 重分配（R6-R9）减少 spill/fill。纯 bytecode 变换。bpfopt-core 已有 liveness 分析能力 | 待调研 |
...
- **CLI-first，6 个工具**（#646 后续方向）：`bpfopt`、`bpfverify`、`bpfprof`、`bpfget`、`bpfrejit`、`bpfrejit-daemon`。`bpfopt-core` 是 workspace-internal 共享 crate，不是 runtime 入口。
...
- **daemon 是可选 watch/trigger 进程**：只能 watch 新程序、检测 map invalidation、触发外部脚本/命令、维护 session 生命周期。daemon 不跑 pass pipeline、不维护 `PassManager`、不调用 `bpfopt`、不做 profiling、不做字节码变换。
...
- **Benchmark runner 优先直接调 CLI**：corpus/e2e 不依赖 daemon socket；watch/invalidation 的长期运行场景才使用 `bpfrejit-daemon`。

实现阶段：

1. **Phase 1：核心工具最小可用**。迁移/保留 `bpfopt-core`，实现 `bpfopt optimize`、至少 `wide-mem` 子命令、`bpfget PROG_ID`/`--list`、`bpfrejit PROG_ID FILE`。
2. **Phase 2：完整工具链**。实现所有 pass 子命令、`bpfverify`、`bpfprof`、`bpfget --full` 和 side-input/report 文件协议。
3. **Phase 3：集成**。runner 从 daemon socket 切到直接 CLI，daemon 瘦身为 external-script trigger，必要时把 bpfopt-suite 独立 repo/submodule 化。
