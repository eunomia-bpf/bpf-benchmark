...
- **单一 pass 实现（#646 后续，2026-04-28 收敛）**：`daemon serve` 是唯一运行时入口，优化 pass/IR/analysis 只保留在 `daemon/src/`。旧 `bpfopt/` workspace 已删除，避免与 daemon 的 per-pass verify/rollback、REJIT BTF FD transport、live map lookup 语义分叉。离线 `bpfopt-core`/CLI 架构已废弃；如未来重建，必须从 daemon 单一实现抽取，不能复制维护第二份 pass。
