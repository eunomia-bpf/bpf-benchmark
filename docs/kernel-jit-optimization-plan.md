...
| **513** | **map_inline emit + const_prop fixed point 修复（2026-03-26）** | 🔄 | **(1)** u64 常量 fit 32-bit 时用 MOV_IMM 替代 LD_IMM64（省 1 slot/site）。**(2)** const_prop+dce 迭代到 fixed point（最多 5 轮）。codex 执行中（`b59g8avv0`）。 |
| **514** | **map_inline 可 inline 程序数统计（2026-03-26）** | 🔄 | codex 在 VM 跑 vm-static-test 统计全 corpus map_inline 命中数。codex `b3pjjq61m`。 |
