...
| **470e** | `micro/` / `corpus/` / `e2e/` VM 路径迁移 | 🔄 | `micro/driver.py` 和 `corpus/modes.py` 的执行热路径已继续收口：Python 仍负责 YAML/control-plane 选择，但实际 job 执行改为一次调用 `runner/build/micro_exec run-batch` 的 C++ batch runner；`corpus/modes.py` 仍通过 `runner.libs.vm.build_vng_command()` 生成 VM 命令；`e2e/cases/scx/case.py` 已通过 `run_in_vm(..., action=\"vm-e2e\")` 接入 runner 调度语义。剩余待迁移的是 `static_verify`、`e2e/run.py` 和其他 case 的具体执行循环。 |
