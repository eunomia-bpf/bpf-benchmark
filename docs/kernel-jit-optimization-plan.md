...
| **Dynamic map inlining** | 否 | 🔄 tracker 已实现 | JVM deopt 模型：map 稳定 → inline → invalidation → re-REJIT。**论文核心 story** | **11556 个 map_lookup site；Katran 22→2 条（-91%）；Tetragon 447→2（-99.6%）**。`MapInvalidationTracker` 已在 `daemon/src/invalidation.rs` 实现，完成依赖记录 / 批量检测 / remove / dedup；watch/serve invalidation loop 仍待接线。设计：`dynamic_map_inlining_design_20260324.md` |
...
| **474f** | Dynamic invalidation（watch/serve 模式） | ✅ tracker 已实现 | `MapInvalidationTracker` 已落地：daemon 侧完成 `(prog_id, map_fd, key, expected_value)` 记录、按 map 分桶批量检测、失效 prog_id 去重输出、`remove_prog()` 清理。测试：`cargo test --manifest-path daemon/Cargo.toml invalidation` **12/12**（其中新 invalidation tests **9/9**）通过。实现 commit：`eb9b5fe`。watch/serve 轮询接线仍待后续。 |
