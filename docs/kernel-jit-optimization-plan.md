...
10. **Daemon 是 runner 稳定边界**：`bpfrejit-daemon` watch 新程序、检测 map invalidation、维护 runner socket，并进程内编排 live snapshot、可选 `const_prop` thin dry-run、final ReJIT；benchmark runner Python 不直接改成调 kernel-facing CLI。
...
│                                                              │
│  * PROG_LOAD dry-run only for explicit const_prop states      │
│                                                              │
...
显式 `const_prop` 请求会在步骤 3 前插入一次 thin dry-run 生成 `verifier-states.json`；默认 11-pass pipeline 不触发 dry-run。
...
- **`const_prop` opt-in thin dry-run**：默认 11-pass pipeline 不含 `const_prop`。只有请求显式包含 `const_prop` 时，daemon 用原始 bytecode 做一次 thin dry-run 捕获 verifier states，并且强制 `func_info = None`、`line_info = None`。缺失 states 或 parse 失败直接 error，不允许空结果 fallback。
...
- **1.3 `bpfopt` CLI 完整版**：已提供 pass 子命令、`optimize`、`list-passes`；默认 `optimize` pass list 为 11 个，不含 Paper B `branch_flip` 和 opt-in `const_prop`。缺少 required side-input 或 kinsn 不可用时退出 1（`b0da364`、`aaaaa76`；见 `docs/tmp/bpfopt-cli-review-20260428.md`）。
...
- **3.3 const_prop thin dry-run**：daemon 只有在请求显式包含 `const_prop` 时，用原始 bytecode 做一次 thin dry-run 捕获 verifier states，且不传 BTF func_info/line_info。
...
- **2.1 verifier-state parser 迁移**：verifier log parser 已移到 `kernel-sys`，只供显式 `const_prop` thin dry-run 捕获 states 使用。
