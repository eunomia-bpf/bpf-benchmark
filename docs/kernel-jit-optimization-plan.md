...
| **666** | **Wave 3 silent-failure review（2026-04-22）** | 🔍 | 1 CRITICAL（runner 把 `applied` 当 `changed` 用）+ 4 HIGH（bpftool schema drift / counts TypeError / daemon CPU-count fallback / tracee event parse 静默丢行）+ 2 MEDIUM + 1 LOW。报告：`docs/tmp/wave3-silent-failure-review-20260422/summary.md`。待修。 |
