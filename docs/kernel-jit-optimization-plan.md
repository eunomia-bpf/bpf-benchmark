...
| **474** | **⚠️ Map Inlining + Const Prop + DCE 实现（论文核心 story）** | 🔄 | **父任务**：分阶段实现 dynamic map inlining 及其级联优化链。当前进度：**474a/474b/474c/474d 已完成**，默认 pipeline 已接入 `map_inline -> const_prop -> dce -> wide_mem -> ...`；下一步是 **474e corpus 全量验证** 和 **474f invalidation**。Corpus 数据支撑：11556 个 map_lookup_elem site；Katran 22→2 条（-91%）；Tetragon 447→2（-99.6%）。设计文档：`dynamic_map_inlining_design_20260324.md` + `verifier_constprop_dce_design_20260324.md`。 |
...
| **474e** | Corpus 全量测试 + 性能验证 | ✅ | **vm-static-test**: 62/62 verifier accepted, 53 applied, avg -36 insns, avg -161 bytes。**vm-micro**: geomean 1.00x（micro 无 map lookup，中性）。**vm-corpus**: 152 measured, 72 applied, **exec geomean 0.854x（rejit 快 ~17%，从 9% 提升到 17%）**。Tracee +30.4%, Katran healthcheck 2.0x。8 个 REJIT 失败（"call to invalid destination"）。**vm-e2e**: 运行中。结果：`vm_micro_20260325_053433` + `vm_corpus_20260325_053936`。 |
