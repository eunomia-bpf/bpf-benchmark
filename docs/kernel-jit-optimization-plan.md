...
| **467a** | `runner/repos.yaml` schema 收口 | ✅ | `corpus/repos.yaml` 已迁到 `runner/repos.yaml`。`corpus/config/macro_corpus.yaml` 已改为读取 `runner/repos.yaml` / `runner/repos` / `runner/inventory.json`。schema 已扩到支持 `cargo_packages` / `promoted_objects` / `forced_includes` 等 repo-specific 字段；本轮新增 runner 注册项：`scx`、`netbird`、`systemd`。 |
...
| **467c** | repo build/harvest 编排收口到 `runner/` | ✅ | build 入口已迁到 `runner/scripts/build_corpus_objects.py`，`runner/Makefile` 新增 `corpus-fetch` / `corpus-build-objects` / `corpus-build`。对象产物仍写入 `corpus/build/`，但 orchestration 已收口到 `runner/`。builder 本轮新增 manifest-driven `forced_includes` 能力，已用于 `netbird` legacy map-def 源和 `systemd` 源的兼容编译。 |
...
| **469b** | Runner infra 收口后的全量 VM 验证 | ✅ | 串行全量验证已完成：`make vm-static-test STATIC_VERIFY_ARGS='--mode micro' TARGET=x86` ✅（62 objects / 62 programs / 39 applied / 62 verifier accepted）、`make vm-static-test TARGET=x86` ✅（191 objects / 420 programs / 63 applied / 418 verifier accepted / 2 `xdp_synproxy` errors）、`make vm-micro TARGET=x86` ✅（`468.87s`）、`make vm-corpus TARGET=x86` ✅（`908.68s`，152 compile/measured pairs，44 applied）、`make vm-e2e TARGET=x86` ✅（`1186.39s`；tracee/tetragon/bpftrace/scx/katran/bcc 全部 OK）。 |
| **469c** | Benchmark 执行性能瓶颈分析与提速 | 🔄 数据已采集 | 本轮全量数据表明：1) `vm-micro` 当前主要耗时不是 VM 启动，而是 guest 内 `kernel-rejit compile`；`wide_mem` verifier rollback/retry（见 #457）仍是明显放大项。2) `vm-corpus` 在 single-VM batch 后已消掉 per-target VM 启动，但总时长仍主要花在 guest 内逐 object compile/verify。3) `vm-e2e` 的长尾主要来自 case/workload 本身，不是 VM 包装层；`scx` 的 host-side repo/binary 前置已收口到 `runner`，重复运行时可直接 skip。4) 已修两项基础设施固定成本/噪音：`runner.libs.workload` 和 `katran` 的轻量 HTTP helper 对客户端提早断开不再打印 `BrokenPipeError` traceback；`runner/Makefile` 新增按需 `e2e-tracee-setup` / `e2e-tetragon-setup`，warm cache 实测直接 skip（`elapsed_s=0.00`），不再每次 `vm-e2e` 都无条件跑 setup 脚本。下一步提速重点：#457、剩余 e2e setup/download 缓存细化、以及 repo 控制面继续收口。 |
...
| **470e** | `micro/` / `corpus/` / `e2e/` VM 路径迁移 | 🔄 | `corpus/modes.py` 已改为通过 `runner.libs.vm.build_vng_command()` 生成 VM 命令；`e2e/cases/scx/case.py` 已通过 `run_in_vm(..., action=\"vm-e2e\")` 接入 runner 调度语义。`micro/driver.py`、`e2e/run.py` 和其他 case 的 VM 路径仍待迁移。 |
| **470f** | 调度控制面验证 | 🔄 | 已完成基础验证：`py_compile` ✅、机器解析 ✅、命令生成 ✅、并发队列/锁串行化 ✅。完整的 `make vm-selftest` / `make vm-micro-smoke` / `make vm-corpus` / `make vm-e2e` 端到端验证仍待在 #469b 中统一跑完。 |
