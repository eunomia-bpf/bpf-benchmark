...
- Task #44 范围调整：不再修改 runner Python；只改 daemon 实施 CLI subprocess。
- Task #45（daemon 瘦身）顺延：删 `PassManager` / pass code / profiler 代码，daemon 主体只剩 socket server + watch + invalidation + CLI fork+exec。
- v3 §8 文档不动（保留两方案历史描述），plan doc 记录我们的实施选择。

- **3.1 daemon socket-to-CLI 适配**：原 runner CLI pipeline 迁移改为 daemon fork+exec CLI，历史设计见 `docs/tmp/runner-cli-migration-20260428.md`。
