...
| **SETcc/CSET** | 是 | 📝 待做（待调研） | x86 `SETcc` + ARM64 `CSET`。比较结果直接存 0/1，不需要 branch+mov。需调研 standalone boolean-set 与现有 `COND_SELECT`/CSET lowering 的覆盖关系 | P89 审计确认未找到专项调研；仅有 related `COND_SELECT`/ARM64 CSET lowering 与 P1 opportunity 记录。审计：`docs/tmp/p89_research_audit.md` |
| **ANDN** | 是 | 📝 待做（待调研） | x86 BMI1 `ANDN`（`a & ~b` 一条指令）。需先做 corpus census for mask-clear idioms 与跨架构 fallback ROI | P89 审计确认未找到专项调研；只有 Tracee native gap 里的 `sete/setne + andn` 观察，缺 corpus census。审计：`docs/tmp/p89_research_audit.md` |
| **BLSI/BLSR/BLSMSK** | 是 | 📝 待做（待调研） | x86 BMI1 isolate/reset/mask lowest set bit。需先做 bitmap / lowest-set-bit idiom census | P89 审计确认未找到当前 corpus 专项调研；历史 micro 只提到 `blsr`/`bitcount`，不足以覆盖 BLSI/BLSR/BLSMSK。审计：`docs/tmp/p89_research_audit.md` |
| **除法强度削减** | 否 | 📝 待做（待调研） | 常量除数 → shift+multiply trick。DIV 40 cycles → 3 cycles。纯 bytecode 变换，需要 verifier const prop 提供常量。已有 const-prop 泛化论证，但缺 division-specific corpus census | P89 审计确认只有 const-prop/strength-reduction 泛化论证，未找到 division-specific corpus census。审计：`docs/tmp/p89_research_audit.md` |
...
| **寄存器重分配** | 否 | 📝 待做（待调研） | BPF 10 个寄存器的 liveness 分析→callee-saved 重分配（R6-R9）减少 spill/fill。纯 bytecode 变换；需与下方 `REJIT spill-to-register` 区分，尚未找到专门调研 | P89 审计确认 `rejit_register_mapping_research` 覆盖的是受限 spill-to-register，不是纯 bytecode register reallocation。审计：`docs/tmp/p89_research_audit.md` |
...
| **Region kinsn（寄存器扩展）** | 是 | 📝 待做（待调研） | 高寄存器压力的代码段包装为 region kinsn，emit callback 内部用 native 寄存器。需先定义 region ABI、verifier proof model 与现有 kinsn v2 proof-lowering 的关系 | P89 审计确认现有 kinsn proof-lowering 文档不等于 extended-register region ABI/proof model；未找到专项调研。审计：`docs/tmp/p89_research_audit.md` |
...
```
1. bpfget PROG_ID              → stdout 只输出原始 raw bytecode；JSON metadata/profile/target/list 走 --output/--output-dir side files
...
- **kernel-facing 功能拆到独立 CLI**：`bpfget` 读取 live program bytecode/metadata/side-input；`bpfget` 的 JSON metadata/list/target 和 `bpfprof` profile 必须写 side-output 文件，不能写 stdout；`bpfverify` 做 `BPF_PROG_LOAD` dry-run 和 verifier states；`bpfrejit` 做最终 `BPF_PROG_REJIT`。
...
- **Per-pass verify loop 由外部 runner orchestration 实现**：每个 pass 产生 candidate bytecode 后调用 `bpfverify`，通过则接受，失败则保留上一段 bytecode。最终只调用一次 `bpfrejit`。**禁止为此修改内核代码**；没有真实调用方时不保留未使用的 standalone 脚本。
...
`CLAUDE.md` 的 **Fail-Fast: No Dead Code, Fallback, or Silenced Errors** 是当前实现约束：能力缺失、syscall/IO/parse 失败、无法生成完整结果时都必须 exit 1 并向 stderr 给出友好错误，不能用降级路径、空结果或 warning 继续掩盖问题。所有 fn、struct、字段、常量都必须有真实调用方；`#[allow(dead_code)]` 标注对象要删除，test-only helper 改用 `#[cfg(test)]`。当前已知违反点包括 `bpfget --target` fallback empty kinsns，必须改成缺能力时失败。`bpfprof` PMU 不可用时必须 fail-fast；P88 后 per-site branch profile 字段为必填，缺真实 site 数据时 `bpfprof`/`branch_flip` 必须 exit 1。`kernel-sys` 旧 `prog_load_dryrun` / `map_get_next_id` wrapper 没有真实调用方，应删除而不是保留兼容层。`BpfInsn` 的 `#[allow(dead_code)]` helper 要改成 `#[cfg(test)]` 或删除，`PassManager::pass_count` / `pass_names` 这类无调用方 API 也要清理。
...
4. **Phase 4：增强**。在有真实调用方时补 runner-owned per-pass verify orchestration、`bpfget --target`/target.json 自动生成、map-inline invalidation hints。
...
- **1.5 `bpfrejit` CLI**：已实现最终 REJIT 提交；dry-run 验证由 `bpfverify` 负责，`bpfrejit` 只执行 `BPF_PROG_REJIT`（`dc3abe3`、`8ca15b6`；见 `docs/tmp/bpfrejit-design-20260428.md`、`docs/tmp/bpfget-bpfrejit-review-20260428.md`）。
- **1.6 minimal pipeline 验证**：`bpfget | bpfopt | bpfrejit` 仍待跑 #41。
- **3.3 per-pass verify bash 脚本**：早期 standalone 脚本无真实调用方，已删除；如需要应由 runner/daemon 外部 orchestration 重新引入。
...
- **3.1 daemon socket-to-CLI 适配**：`optimize` 走 `bpfget --full` → `bpfopt optimize` → `bpfverify` → `bpfrejit`；`profile-start/stop` 走 `bpfprof`；`status` 只保留 daemon 健康和 profiling 状态，`optimize-all` 已删除。
