...
- **bpfopt 架构（#646，2026-04-26 收敛）**：优化 pass 已提取到独立 `bpfopt/` workspace，当前只保留纯优化 crate：`bpfopt-core`（pass library）和 `bpfopt`（CLI）。`verify` / `rejit` / `profiler` / `syscall` / `daemon` 逻辑继续留在 `daemon/`；`bpfopt-core` 不再承载 per-pass verify/rollback 或 REJIT transport 状态。
