...
> 上次更新：2026-03-19i（#239 ✅ `rotate_dense` regression debug + P0/P1/P2 audit：`docs/tmp/micro_post_p0p1p2_fix_20260319.md` 记录的 `177ns→214ns` regression **未在当前 18:09 -0700 bzImage 上复现**；同参数 `make vm-micro BENCH="rotate_dense rotate64_hash" ITERATIONS=5 WARMUPS=3 REPEAT=500` 连跑两次均为正向（`rotate_dense 208→177ns`, `199→186ns`；`rotate64_hash 92→68ns`, `88→67ns`）。直接 VM dump 显示 `rotate_dense` 仍保留 `256/256` rotate sites，native JIT size `10198→7638`，recompiled image 含 dense `ror`。结论：负向 run 最可能来自 **旧 bzImage / stale image provenance**，不是当前代码中的 overlap / branch-flip / reserved-flags / trampoline-guard live bug。`make vm-selftest` 19/19，`make vm-micro-smoke` pass。报告：`docs/tmp/rotate_dense_regression_debug_20260319.md`。）
> 前次更新：2026-03-19h（#235 ✅ Kernel 代码简化实施完成：recompile logging 复用 verifier log，trivial wrappers / staged getters / ksym wrappers 清理，ROTATE family 合并为 spec-driven common validator，form/native-choice/CPU support 改为 metadata table。相对简化前 worktree touched files 净减 **447 LOC**（`jit_directives.c 3228→2797`，`bpf_jit_comp.c 6136→6091`）。验证：`make -C vendor/linux-framework -j$(nproc) bzImage`、`make vm-selftest`（19/19）、`make vm-micro-smoke`（`simple` + `cmov_dense` on `kernel`/`kernel-recompile` 通过）、scanner `PASS 180`。报告：`docs/tmp/kernel_simplification_20260319.md`。）
> 前次更新：2026-03-19g（#238 ✅ LLVM vs kernel JIT x86_64 指令级对比：8 个代表 benchmark fresh dump + side-by-side disassembly。结论：**WIDE_MEM + ROTATE** 是主因，ENDIAN/COND_SELECT 次之；`extract_dense` 暴露 taxonomy 外的 **load-width narrowing**；`bounds_ladder` 的 gap 主要来自 wide-load collapse，而不是纯 BRANCH_FLIP。报告：`docs/tmp/llvm_vs_kernel_jit_disasm_analysis_20260319.md`，artifacts：`docs/tmp/llvm_vs_kernel_jit_disasm_20260319_artifacts/`。）
> 前次更新：2026-03-19f（#229-#235 新增。#229 ✅ Kernel 5k LOC deep review（6/10）：P0 trampoline guard gap, P1 emitter param leak + flags fail-open, P2 overlap + dead fields。#230 ✅ Kernel P0/P1/P2 修复：trampoline guard 扩展到所有 trampoline-attached 程序、ZERO_EXT_ELIDE+BRANCH_FLIP emitter param-only、reserved flags fail-closed、overlap 检测、dead fields 删除。selftest 19/19, smoke OK。commit `b50fd6279`。#231 ✅ Docker ARM64 交叉编译：`docker/arm64-crossbuild.Dockerfile` + `make selftest-arm64` + `make vm-arm64-selftest`，ARM64 binary OK 但 guest 返回 EOPNOTSUPP（ARM64 无 recompile 实现）。#232 ✅ 用户态 cleanup round 3：P1 extract_error fix、dead imports 清理、文档更新。#233 ✅ Kernel LOC breakdown：5800 行 delta，jit_directives.c 3114 行（67.9% essential）。报告：`docs/tmp/kernel_loc_breakdown_20260319.md`。#234 ✅ Kernel 架构 review：ROTATE 4 parser 合并可省 180-210 行，logging 复用可省 70-100 行，trivial wrappers 可省 50-90 行。总计可省 400-550 行。报告：`docs/tmp/kernel_architecture_review_20260319.md`。#235 🔄 Kernel 代码简化实施中。#236 🔄 Micro rerun（3w/5i/500r）验证 P0/P1/P2 修复后无回归。Scaleway RV1 RISC-V 调研已完成，见 `docs/tmp/riscv_cloudv_scaleway_research_20260319.md`。）
> 前次更新：2026-03-19e（#205 ✅ Katran E2E gRPC setup fix：不再在 benchmark 路径内编译 `katran_server_grpc`；`setup.sh` 直接发现 checked-in bundle，Katran VM path 不再 out-of-band setup + `--skip-setup`，payload 能正确记录 server binary。VM smoke + authoritative rerun 均通过；latest rerun 5/5 HTTP、30 IPIP decap packets、1/1 recompile apply，但 BPF avg_ns `407.6→507.0`（未复现旧 `603.7→351.8` 正向结果）。报告：`docs/tmp/katran_e2e_grpc_fix_20260319.md`。）
...
> 当前 active E2E cases：`tracee` / `tetragon` / `bpftrace` / `scx`。`xdp_forwarding` 已于 2026-03-18 退役并从 active pipeline 删除；未来 XDP replacement 目标是 `katran`。
...
| 252 | **Selftest 扩展 5 form（2026-03-19）** | ✅ | 5 个新 BPF 测试程序 + 5 个 test_recompile.c 用例（ADDR_CALC/BITFIELD_EXTRACT/ZERO_EXT_ELIDE/ENDIAN_FUSION/BRANCH_FLIP）。编译通过，**待 VM 跑 vm-selftest 验证**。报告：`docs/tmp/selftest_expansion_20260319.md`。 |
...
| 255 | **Corpus llvmbpf 删除（2026-03-19）** | ✅ | corpus 全路径删除 llvmbpf：`run_real_world_code_size.py`、`run_real_world_exec_time.py`、ARM64 CI corpus section。只保留 kernel + kernel-recompile。micro 的 llvmbpf 保留用于 characterization。报告：`docs/tmp/corpus_llvmbpf_removal_20260319.md`。 |
