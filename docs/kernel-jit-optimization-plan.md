...
| **500** | **⚠️ map_inline 真实 VM 零命中调查（2026-03-26）** | 🔄 | corpus 130 applied 中 map_inline 命中 **0 个**，E2E 也零命中。投机 inline 代码 unit test 全过但真实 VM 不触发。codex 正在定位 root cause（frozen 残留检查？零值 skip？map type filter？）。调查报告：`map_inline_real_values_investigation_20260326.md`。**根因确认后需修复+重跑 corpus/e2e 验证。** |
