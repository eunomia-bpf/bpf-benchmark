...
- **bpfopt-suite 架构（#646）**：优化 pass 已提取到独立 `bpfopt-suite/` workspace。Daemon 不再处理 pass pipeline；pipeline 由 bash 脚本编排（`bpfget | bpfopt ... | bpfverify | bpfrejit`）。Daemon 退化为 REJIT 代理 + watch 薄壳。详见 `docs/tmp/bpfopt_design_v3.md`。
...
| **646** | **bpfopt-suite 独立工具套件（2026-04-07）** | ✅ | 从 daemon 提取所有 pass 代码到独立 `bpfopt-suite/` workspace，Unix 风格 CLI 工具。**核心设计**：daemon 不处理 pipeline，pipeline 是 bash；每个工具做一件事。**六个 CLI**：`bpfopt`（bytecode 优化器，12 pass 子命令 + optimize + analyze，stdin/stdout 管道，--target/--profile/--map-values side-input）、`bpfverify`（dry-run 验证）、`bpfprof`（profile 采集）、`bpfget`（读内核 bytecode）、`bpfrejit`（提交 REJIT）、`bpfrejit-daemon`（watch 薄壳）。**bpfopt-core**：零内核依赖纯 bytecode 库，移除所有 `crate::bpf` 引用，map 数据通过 `BpfProgram.map_values/map_metadata` 传入。**455/455 tests pass**。kernel-sys 及其他内核 CLI 为 stub。设计文档：`docs/tmp/bpfopt_design_v3.md`。Benchmark review：`docs/tmp/benchmark_status_review_20260403.md`。Commits：`ea92e14`、`8b66471`。 |
...
| **670** | **Round 8 后续审计 + VM smoke 验证（2026-04-23，进行中）** | 🟡 | Round 8 commit 后并行派出 4 个 codex 做独立验证与后续审计：(a) `b71j00s3d` **VM smoke verify** —— `make vm-e2e` + `make vm-corpus SAMPLES=1`，已观测到 SCX `scx_20260423_233939_252543` 13 programs / applied sites=2 (extract) / baseline+post 两阶段完成；(b) `bl7qdjynz` **daemon Rust 审计** —— 10 维矩阵，重点查 coverage-gap 未验证假设 `daemon/src/commands.rs:533-547` 的 `orig_insns.is_empty()` short-circuit 是否绕过 Round 7 taxonomy；该问题已按 loud-fail 方案修复，并顺带完成 daemon SHOULD/NICE 清理，代码提交 `358da2d5` / `f0692904` / `a95c6466`，报告：`docs/tmp/daemon-orig-insns-fix-20260423/summary.md`；(c) `b6cjwaqlz` **设计整洁性审计** —— 12 维（单实现 abstraction / 重复 lifecycle / 多 surface config / artifact dict 死字段 / wrapper 只转发 / reporting 过厚 / Makefile 死 rule / 等），给 5 档干净度评分；(d) `bvhfeigla` **功能清单 + 冗余诊断** —— 完整盘点 Makefile targets / CLI 入口 / e2e case / app runner / workload / daemon pass / kinsn / 测试模式 / report 格式 / config 参数 / 环境支持，给 🔴/🟡/🟢 冗余分级。报告目录：`docs/tmp/{round8-vm-smoke,daemon-rust-audit,design-cleanliness-audit,feature-inventory}-20260423/`。 |
