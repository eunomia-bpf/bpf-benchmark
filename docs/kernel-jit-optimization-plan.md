...
> 上次更新：2026-03-19（#1-#268）。当前状态：#1-#264 全部 ✅，#265 🔄 ARM64 P0/P1 fix，#266 🔄 AWS ARM64，#267 ✅ Makefile audit，#268 🔄 scanner rotate 对齐。vm-selftest **25/25**，vm-micro-smoke ✅。权威数据：micro applied-only **1.110x**，corpus **1.046x**，Tracee **+6.28%/+7.00%**，Tetragon **+8.70%/+22.29%**。Active E2E：tracee/tetragon/bpftrace/scx，katran 替换 xdp_forwarding。详见各任务条目和 `docs/tmp/` 报告。
...
| 265 | **ARM64 emitter P0/P1 fix + testcase（2026-03-19）** | 🔄 | P0: ARM64 subprog staging 对齐 x86。P1: BITFIELD_EXTRACT width 校验 fail-closed。加 ARM64 testcase。codex 执行中。 |
| 266 | **AWS ARM64 bare metal benchmark + Makefile（2026-03-19）** | 🔄 | 在 AWS c7g 上跑 ARM64 benchmark，封装为 `make aws-arm64` 目标（含实例分配+释放）。codex 执行中。 |
...
| 268 | **Scanner ROTATE 对齐（2026-03-19）** | 🔄 | scanner pattern_v5.cpp 对齐 kernel low-mask rejection。codex 执行中。 |
