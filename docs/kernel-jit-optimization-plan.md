...
| **527** | **Katran corpus REPEAT=50 结果 + 可信度审查（2026-03-27）** | 🔄 | **结果**（`vm_corpus_20260327_152910`）：5 objects / 5 measured / 2 applied。balancer_ingress map_inline → exec_ratio **0.75**；xdp_pktcntr map_inline+const_prop+dce → size **-47%**。**可信度红旗**：healthcheck_encap（无 pass）exec_ratio 0.625、healthchecking_ipip（无 pass）0.5。无 pass applied 但 37-50% "加速"说明 REPEAT=50 噪声极大。对比 live e2e (#521) 仅 1.6%。**审查中**。 |
