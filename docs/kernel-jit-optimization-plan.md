...
| **467d** | `corpus/` 和 `e2e/` 消费层迁移 | 🔄 | active 路径已开始迁移：`bcc` / `scx` e2e 默认 repo 路径改到 `runner/repos/`，`e2e/run.py` 的 `scx` 默认参数也已切到 `runner/repos/scx`。`corpus` README / `e2e` README 已同步更新；其余 case/配置和遗留硬编码路径继续清理。 |
