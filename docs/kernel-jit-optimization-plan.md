...
- **bpfopt-core pass 源（#646 修正，2026-04-28）**：优化 pass/IR/analysis 的单一实现位于 `bpfopt/crates/bpfopt-core/`，daemon 依赖 `bpfopt-core`，不再在 `daemon/src/passes/` 维护重复 pass。daemon 仍是唯一运行时入口，保留 `serve`、raw BPF syscall、per-pass `BPF_PROG_LOAD` verify closure、final `BPF_PROG_REJIT`、runtime map/BTF FD adapters、profiler、invalidation 和错误上报。`bpfopt` CLI 仅作为 offline optimizer 使用 snapshot map metadata/value 和 static call-offset adapter；live REJIT 行为只通过 daemon。
