...
Transition errata: daemon 在 Phase 1.1 合并后短期依赖 `bpfopt` 的 lib 部分，提供 passes/analysis/insn/verifier_log。这是过渡状态。Task #45（daemon 瘦身为 event trigger）完成后 daemon 不再需要这些代码，依赖将消除。
...
- **3.1 benchmark runner 切到 CLI pipeline**：设计见 `docs/tmp/runner-cli-migration-20260428.md`。
- **3.2 daemon 瘦身**：设计见 `docs/tmp/daemon-slim-design-20260428.md`，目标从约 1500 行降到约 300 行。
