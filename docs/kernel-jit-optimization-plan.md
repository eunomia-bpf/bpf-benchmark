...
| **469c** | Benchmark 执行性能瓶颈分析与提速 | 🔄 数据已采集 | 本轮全量数据表明：1) `vm-micro` 当前主要耗时不是 VM 启动，而是 guest 内 `kernel-rejit compile`；`wide_mem` verifier rollback/retry（见 #457）仍是明显放大项。2) `vm-corpus` 在 single-VM batch 后已消掉 per-target VM 启动，但总时长仍主要花在 guest 内逐 object compile/verify。3) `vm-e2e` 的长尾主要来自 case/workload 本身，不是 VM 包装层；`scx` 的 host-side repo/binary 前置已收口到 `runner`，重复运行时可直接 skip。4) 已修一项低风险基础设施噪音：`runner.libs.workload` 和 `katran` 的轻量 HTTP helper 对客户端提早断开不再打印 `BrokenPipeError` traceback；剩余固定成本主要是 tracee/tetragon setup 仍会在每次 e2e 前执行。下一步提速重点：#457、e2e setup cache/skip 判定、以及剩余 repo 控制面收口。 |
