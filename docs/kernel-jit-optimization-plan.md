...
| **Dynamic map inlining** | 否 | 🔄 设计完成 | JVM deopt 模型：map 稳定 → inline → invalidation → re-REJIT。**论文核心 story** | **11556 个 map_lookup site；Katran 22→2 条（-91%）；Tetragon 447→2（-99.6%）**。设计：`dynamic_map_inlining_design_20260324.md` |
...
| **128-bit LDP/STP** | 可选 | 🔄 调研完成 | ARM64 LDP/STP pair load/store。**仅 ARM64，x86 不做** | **ARM64 corpus store 对密度高，当前 JIT 完全没 LDP/STP**。调研：`128bit_wide_loadstore_research_20260324.md` |
...
| **423** | **Dynamic map inlining + invalidation** | 🔄 v1 已落地，失效链路待做 | `daemon/src/passes/map_inline.rs` 已实现 **ARRAY 全 lookup 消除**，并扩展到 **struct value 多字段 fixed-offset load**；同时支持 **HASH/LRU_HASH speculative inline**（保留 runtime lookup + null check，只替换 non-null path 字段 load）。dynamic invalidation/watch-serve 仍待做。**设计报告见 #472a**。 |
...
| **474** | **⚠️ Map Inlining + Const Prop + DCE 实现（论文核心 story）** | 🔄 | **父任务**：分阶段实现 dynamic map inlining 及其级联优化链。当前进度：**474a/474b/474c 已完成**，`map_inline -> const_prop` 已接入默认 pipeline；下一步是 **474d DCE** 和 **474f invalidation**。Corpus 数据支撑：11556 个 map_lookup_elem site；Katran 22→2 条（-91%）；Tetragon 447→2（-99.6%）。设计文档：`dynamic_map_inlining_design_20260324.md` + `verifier_constprop_dce_design_20260324.md`。 |
...
| **474d** | DCEPass：dead branch + unreachable block elimination | 待做 | const prop 后的 dead branch → 删掉不可达 block → 删掉 dead store。利用 analysis/cfg.rs + analysis/liveness.rs。和 #443 设计对齐。目标：Katran pktcntr 22→2 条、Tetragon tg_conf_map 447→2 条。 |
| **474e** | Corpus 全量测试 + 性能验证 | 待做 | make vm-static-test 验证 verifier 全部接受。make vm-corpus 验证无 correctness regression。量化 map inline + const prop + DCE 的 geomean 改进。目标：corpus geomean 从 1.09x 提升到 1.15x+。 |
| **474f** | Dynamic invalidation（watch/serve 模式） | 待做 | daemon 记录 (map_fd, key, inlined_value) → 周期性检测 map 值变化 → re-REJIT。v1 用轮询（1s 间隔），v2 探索 ring_buffer 事件驱动。 |
...
| **474h** | Frozen map 特殊路径 | 待做 | 如果 map 已 BPF_MAP_FREEZE（libbpf 自动 freeze .rodata/.kconfig），可以永久 inline 不需要 invalidation。虽然 #422 调研说 hot-path frozen lookup ≈ 0，但作为 correctness 优化仍有价值。 |
