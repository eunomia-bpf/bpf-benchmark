...
| **BRANCH_FLIP** | 否 | 🧪 experimental / Paper B scaffolding | if/else body 重排。policy-sensitive；默认 benchmark profile 不启用。显式启用时要求 PMU gate + profile side-input；缺真正 per-site PGO 时仅作为 placeholder，不作为论文实验依据 | 非默认 benchmark pass |
...
`branch_flip` 属于 Paper B：基于 profile/info 的 speculative runtime optimization。当前 Rust pass、runner profile prep hooks、daemon `profile-start`/`profile-stop` 和 `bpfprof` PMU 字段只是 scaffolding；实现仍是 placeholder + 保守 PMU gate，缺少真正 per-site PGO 时不作为论文实验依据。Paper A 默认优化仍是 kinsn 主线，`bpfopt optimize` 默认 11 pass 不包含 `branch_flip`；Paper B 启动时再实现真实 per-site PGO 并重新评估是否进入策略。
...
`CLAUDE.md` 的 **Fail-Fast: No Dead Code, Fallback, or Silenced Errors** 是当前实现约束：能力缺失、syscall/IO/parse 失败、无法生成完整结果时都必须 exit 1 并向 stderr 给出友好错误，不能用降级路径、空结果或 warning 继续掩盖问题。所有 fn、struct、字段、常量都必须有真实调用方；`#[allow(dead_code)]` 标注对象要删除，test-only helper 改用 `#[cfg(test)]`。当前已知违反点包括 `bpfget --target` fallback empty kinsns，必须改成缺能力时失败。`bpfprof` PMU 不可用时必须 fail-fast；保留的 nullable branch 字段只是 Paper B attribution scaffolding，不能表示 PMU 能力缺失后继续成功。`kernel-sys` 旧 `prog_load_dryrun` / `map_get_next_id` wrapper 没有真实调用方，应删除而不是保留兼容层。`BpfInsn` 的 `#[allow(dead_code)]` helper 要改成 `#[cfg(test)]` 或删除，`PassManager::pass_count` / `pass_names` 这类无调用方 API 也要清理。
...
- **1.3 `bpfopt` CLI 完整版**：已提供 12 个 pass 子命令、`optimize`、`list-passes`；默认 `optimize` pass list 仍为 11 个，不含 experimental `branch_flip`。缺少 required side-input 或 kinsn 不可用时退出 1（`b0da364`、`aaaaa76`；见 `docs/tmp/bpfopt-cli-review-20260428.md`）。
