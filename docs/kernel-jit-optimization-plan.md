...
> 上次更新：2026-03-12（#114 per-site policy v2 ✅；#112 ✅ micro 0.609x；#113 ✅ 根因=branch-flip+endian；#108 tracing VM 🔄；此前：strict rerun ✅；corpus tuned-policy ✅ 0.898x；code-size VM ✅ 1.003x；llvmbpf inline ✅ 1.30x）
...
| 证据 | 数值 |
|------|------|
| Exec time geomean (L/K) | **0.609x** (56 pure-JIT, strict 30×1000); 0.849x (31 pure-JIT, old 3×100) |
...
| 98f | **P4 Auto-tuner** | ✅ | `corpus/auto_tune.py`：5-phase auto-tuner（census→screening→ablation→combo→generate）。支持 `--dry-run`/`--phase`/`--resume`/`--programs`。`corpus/policy_utils.py` 扩展 per-program policy path。Corpus drivers 优先查找 per-program policy。commit `318db2d`。 |
...
| 108 | **Tracing corpus VM 全量 run** | 🔄 | 473 程序 framework-kernel VM 执行。codex `bos0jwf15` 进行中。 |
...
| 114 | **Per-site policy v2 格式 + scanner 实现** | ✅ | scanner 新增 per-site v2 policy：`version: 2`, `program`, `default`, `sites[{insn,family,action}]`；保留 v1 `families:`/`selection:` backward compat。`scan --json` 现额外导出 `insn` alias，接受 `--per-site` no-op；新增 `generate-policy` 子命令输出 v2 YAML 模板；`apply` 现支持 offline object/xlated → blob 或 live `--prog-fd` recompile，两者都按 `(insn,family)` 选 site，stale site 仅 warning+skip。`micro_exec` 同步接入 detailed filter warning。验证：`cmake --build scanner/build -j$(nproc)`、`ctest --test-dir scanner/build --output-on-failure`、`scanner/build/bpf-jit-scanner generate-policy micro/programs/log2_fold.bpf.o --program-name log2_fold_xdp --default apply`、编辑 1 个 site 后 `compile-policy` 与 offline `apply` 都得到 `10/11` rules 且 blob identical、`scanner/build/bpf-jit-scanner compile-policy corpus/build/xdp-tools/xdp_trafficgen.bpf.o --config corpus/policies/xdp-tools/xdp_trafficgen.bpf.o.policy.yaml --output /tmp/policy-smoke.blob`、`make -C micro micro_exec -j$(nproc)`、`python3 -m py_compile corpus/policy_utils.py e2e/common/recompile.py`。 |
