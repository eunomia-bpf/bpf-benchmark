...
> 上次更新：2026-03-18（#189 ✅ Micro policy 优化完成，applied-only 1.118x；#190 ✅ 结果目录整理 + result schema 升级；#191 ✅ Corpus/E2E 性能分析；#192 ✅ Katran E2E 可行性调研；#193 🔄 Corpus policy refresh；§8 合并入 §7。）
...
| **Micro 62-bench overall (#189, policy-optimized, 10iter/5warm/1000rep, 20260318) ← 当前权威** | **1.024x** | **✅ non-applied geomean 0.991（接近 1.0，噪声极低）；9 applied, 7W/2L；数据：`micro/results/vm_micro_authoritative_20260318.json`** |
| **Micro 62-bench applied-only (#189, 9 benches, 20260318) ← 当前权威** | **1.118x** | **✅ 清除 regressor + 新增 rotate64_hash(+33.3%)；top wins: rotate_dense +36.2%, cmov_dense +30.2%** |
...
| 189 | **Micro policy 优化（2026-03-18）** | ✅ | 两轮迭代完成。R1：清除 6 regressor + 新增 rotate64_hash/large_mixed_1000/large_mixed_500 rotate-only。R2：进一步清除 log2_fold/switch_dispatch/branch_dense/multi_acc_4/binary_search，cmov_select 改 rotate-only。Authoritative rerun（10iter/5warm/1000rep）：**overall 1.024x，applied-only 1.118x（9 applied，7W/2L）**。Non-applied geomean 0.991（噪声极低）。Top wins：rotate_dense +36.2%，rotate64_hash +33.3%（新增），cmov_dense +30.2%。分析：`docs/tmp/micro_performance_improvement_analysis.md`，报告：`docs/tmp/micro_policy_improvement_results.md`，数据：`micro/results/vm_micro_authoritative_20260318.json`。 |
...
| 191 | **Corpus + E2E 性能分析（2026-03-18）** | ✅ | Corpus 主因不是 CMOV 是 stale policy：katran balancer_ingress policy 10 sites vs live 94 sites → 0 apply。extract 偏负（0.977x），wide 正向（1.111x）。E2E：Tracee 接近吃满非-CMOV（42 sites 全 applied）；Tetragon 缺 authoritative JSON，v3 policy stale；bpftrace 纯 CMOV 被 skip → 永远 neutral；XDP fwd 顶层 authoritative 是 dry-run。报告：`docs/tmp/corpus_e2e_performance_analysis_20260318.md`。 |
| 192 | **Katran E2E 可行性调研（2026-03-18）** | ✅ | 有条件推荐替换 XDP forwarding。balancer_ingress 有 94 live sites（vs XDP fwd 3 sites），L4 LB 论文价值高。建议 balancer-only MVP：standalone attach + raw PPS + cmov:skip。先新增验证再 retire XDP fwd。风险：stale policy 需先 refresh、需要 raw sender 不能用 iperf3、单接口 DSR 拓扑。报告：`docs/tmp/katran_e2e_feasibility_20260318.md`。 |
...
| 197 | **Micro 第三轮 policy 优化（2026-03-18）** | 🔄 | 清掉 cmov_select + mixed_alu_mem 两个 loss，新增 imm64_storm(64 rotate) + branch_fanout_32(8 rotate)。预期 applied-only 1.118x → ~1.15-1.17x。 |
| 198 | **旧 prog_fd 路径残留清理（2026-03-18）** | 🔄 | 删除 _USE_ENUMERATE_PATH flag 和 legacy scan/apply --prog-fd fallback，只保留 enumerate 路径。 |
| 199 | **Corpus rerun（refreshed policy，2026-03-18）** | 🔄 | 用 skip cmov+extract 的新 policy 跑 corpus benchmark，目标 geomean > 1.05x。如有回归迭代调整 policy。 |
| 200 | **Tetragon rerun（2026-03-18）** | 🔄 | refresh v3 policy + rerun，拿 authoritative JSON。 |
