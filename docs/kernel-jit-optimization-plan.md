...
| **470** | **Runner 统一 VM/机器调度控制面** | 🔄 | **父任务**：所有 VM/远端机器的注册、选择、排队、串行执行、guest 初始化和 VM 相关构建编排全部收口到 `runner/`。**第一批实现已落地**：`runner/machines.yaml`、`runner/libs/machines.py`、`runner/scripts/with_vm_lock.py`、`runner/libs/vm.py` 的机器解析 + 串行锁/队列状态；`corpus/modes.py` 与 `e2e/cases/scx/case.py` 已开始消费。验证：`py_compile` ✅、`resolve_machine()` / `build_vng_command()` ✅、两个并发 `with_vm_lock.py` 任务自动排队串行完成（总耗时 `2.59s`，第二个任务 `wait_seconds=1.82s`）✅。剩余 runner-only backend/Make 收口见 #470c-#470f。 |
...
| **470c** | VM backend adapter 收口 | 🔄 | `runner/libs/vm.py` 已开始接管 `vng` backend 的命令拼装、锁包装和机器解析，并对非-`vng` backend 显式拒绝，避免错误复用。`qemu-system-aarch64`、AWS SSH、CI runner adapter 仍待继续收口。 |
