...
9. **Pipeline 是 bpfopt shell/file 协议 + daemon in-memory kernel state**：`bpfopt` stdin/stdout 只传 raw binary bytecode，profile/verifier states/map values/report 等 `bpfopt` side-input/output 走文件；prog info、map fd、BTF records、fd_array 在 daemon 进程内传递。
...
2. daemon bpfget library         → in-memory ProgramSnapshot: bytecode, prog info, map fd, BTF records
3. bpfopt PASS/optimize CLI      → stdin raw struct bpf_insn[]，stdout raw bytecode；side-input/output 走文件
4. daemon bpfverify library      → BPF_PROG_LOAD dry-run；失败则 surface error
...
- **kernel-facing 功能在 daemon 进程内**：`bpfget` 读取 live program bytecode/metadata/map fd/BTF records；`bpfverify` 做 `BPF_PROG_LOAD` dry-run 和 verifier states；`bpfrejit` 做最终 `BPF_PROG_REJIT`。三者不再通过 fork+exec 和 JSON side files 协作。
...
- **1.4 daemon-owned `bpfget` library**：live bytecode/metadata/map fd/BTF snapshot 已移入 daemon workspace；kinsn target probing 由 daemon 进程内调用（见 `docs/tmp/full-matrix-20260430/v3-arch-pivot.md`）。
...
- **3.3 per-pass verify**：通过 daemon-owned `bpfverify` library 执行，不保留 standalone bash/CLI wrapper。
