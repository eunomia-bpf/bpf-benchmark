...
| **SETcc/CSET** | 是 | 📝 待做（待调研） | x86 `SETcc` + ARM64 `CSET`。比较结果直接存 0/1，不需要 branch+mov。需调研 standalone boolean-set 与现有 `COND_SELECT`/CSET lowering 的覆盖关系 | 待调研 |
| **ANDN** | 是 | 📝 待做（待调研） | x86 BMI1 `ANDN`（`a & ~b` 一条指令）。需先做 corpus census for mask-clear idioms 与跨架构 fallback ROI | 待调研 |
| **BLSI/BLSR/BLSMSK** | 是 | 📝 待做（待调研） | x86 BMI1 isolate/reset/mask lowest set bit。需先做 bitmap / lowest-set-bit idiom census | 待调研 |
| **除法强度削减** | 否 | 📝 待做（待调研） | 常量除数 → shift+multiply trick。DIV 40 cycles → 3 cycles。纯 bytecode 变换，需要 verifier const prop 提供常量。已有 const-prop 泛化论证，但缺 division-specific corpus census | 待调研 |
...
| **寄存器重分配** | 否 | 📝 待做（待调研） | BPF 10 个寄存器的 liveness 分析→callee-saved 重分配（R6-R9）减少 spill/fill。纯 bytecode 变换；需与下方 `REJIT spill-to-register` 区分，尚未找到专门调研 | 待调研 |
