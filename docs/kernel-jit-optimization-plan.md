...
- `bpfget`、`bpfverify`、`bpfrejit` 是 daemon-owned library，分别承担读取 bytecode/metadata、dry-run verify、最终 REJIT。
- `bpfrejit-daemon` 保留 runner socket 边界和 watch/map invalidation，进程内持有 prog/map/BTF fd；字节码变换仍只通过外部 `bpfopt` CLI。
...
5. **Fail-safe**：final verify 失败 → 什么都不改，返回错误；当前 daemon 对 `bpfopt optimize` 的完整 candidate 做一次 final verify，不在 pass 之间回滚
6. **BPF_PROG_REJIT 接受完整的新 BPF bytecode**，不是 patch format — daemon-owned `bpfrejit` library 提交整段新程序，kernel 走标准 verify 路径
...
8. **`bpfopt` 零 syscall 依赖**：`bpfopt` 只读写 raw `struct bpf_insn[]` bytecode；所有 raw BPF syscall 都在 `kernel-sys` 内实现，并由 daemon-owned `bpfget` / `bpfverify` / `bpfrejit` library 或 `bpfprof` CLI 调用。
9. **Pipeline 是 bpfopt shell/file 协议 + daemon in-memory kernel state**：`bpfopt` stdin/stdout 只传 raw binary bytecode，profile/verifier states/map values/report 等 `bpfopt` side-input/output 走文件；prog info、snapshot-owned map fd、BTF records、fd_array 在 daemon 进程内传递。
10. **Daemon 是 runner 稳定边界**：`bpfrejit-daemon` watch 新程序、检测 map invalidation、维护 runner socket，并进程内编排 live snapshot、dry-run verify、final ReJIT；benchmark runner Python 不直接改成调 kernel-facing CLI。
...
| **ADDR_CALC (LEA)** | 可选 | ❌ 不做第一波 | strict x86 LEA 只有 14 个命中 site，corpus geomean 预期不可见；若追求跨架构收益，应另立 `generic shifted-add`，不是 LEA pass | **14 sites / 7 对象，全部 tetragon；ROI 很低**。调研：`addr_calc_lea_research_20260324.md` |
...
│                         │         ↘ in-process verify/rejit  │
│                         │           bpfget/bpfverify/bpfrejit│
│                         ↑                                    │
│                  bpfprof profile CLI                         │
│                                                              │
...
v3 pivot 后，runner 边界是 daemon socket；bytecode transform 仍是 CLI/file protocol，kernel-facing state 不再跨进程序列化。

```
1. runner optimize request       → daemon socket JSON
2. daemon bpfget library         → in-memory ProgramSnapshot: bytecode, prog info, snapshot-owned map fd, BTF records
3. bpfopt optimize CLI           → stdin raw struct bpf_insn[]，stdout raw bytecode；side-input/output 走文件
4. daemon bpfverify library      → 对最终 candidate 做一次 BPF_PROG_LOAD dry-run；失败则 surface error
5. daemon bpfrejit library       → BPF_PROG_REJIT；kernel re-verify → re-JIT → image swap
```
...
Correctness（用户态工具链负责）：
  ...
  手段：differential testing、final verify、gradual rollout、failure artifact inspection
...
2026-05-01 pivot 后，v3 的权威边界是：`bpfopt` / `bpfprof` 保持 CLI，`bpfget` / `bpfverify` / `bpfrejit` 是 daemon-owned in-process library。

- **`bpfopt` 是纯 bytecode CLI**：不直接调用 BPF syscall；stdin/stdout 传 raw `struct bpf_insn[]` binary；`--target`、`--profile`、`--verifier-states`、`--map-values`、`--report` 等 side-input/output 全部走文件。
- **`bpfprof` 是独立 profile CLI**：PMU profiling 和 per-site branch profile 仍由外部 CLI 输出文件，daemon 只管理 lifecycle。
- **kernel-facing 功能在 daemon 进程内**：`bpfget` 读取 live program bytecode/metadata/map fd/BTF records；`bpfverify` 做 `BPF_PROG_LOAD` final dry-run 和 verifier states；`bpfrejit` 做最终 `BPF_PROG_REJIT`。三者不再通过 fork+exec 和 JSON side files 协作。
...
- **Per-pass verify loop 在 daemon 内用 library 执行**：每个 pass 产生 candidate bytecode 后调用 in-process `bpfverify`，通过则接受，失败则保留上一段 bytecode；最终调用 in-process `bpfrejit`。**禁止为此修改内核代码**。
- **Verifier-in-the-loop 仍保留**：daemon 用 in-process `bpfverify` 捕获 verifier states，再写成 `bpfopt const-prop --verifier-states` 的 side-input。缺失 states 或 parse 失败直接 error，不允许空结果 fallback。
- **安全 pass 不在 OSDI 范围**：`speculation_barrier`、`dangerous_helper_firewall`、`live_patch` 不在默认 pipeline。
- **结构化 per-pass 记录**：每个 program 的每个 pass 记录 `pass`/`changed`/`sites_applied`/`insn_delta` 等 CLI `--report` 事实字段；per-pass verify/rollback 状态不再由 daemon 伪造。
...
`CLAUDE.md` 的 **Fail-Fast: No Dead Code, Fallback, or Silenced Errors** 是当前实现约束：能力缺失、syscall/IO/parse 失败、无法生成完整结果时都必须 exit 1 并向 stderr 给出友好错误，不能用降级路径、空结果或 warning 继续掩盖问题。所有 fn、struct、字段、常量都必须有真实调用方；`#[allow(dead_code)]` 标注对象要删除，test-only helper 改用 `#[cfg(test)]`。daemon-owned target probing 缺少 kinsn/BTF 能力时必须失败，不能生成空 capability fallback。`bpfprof` PMU 不可用时必须 fail-fast；P88 后 per-site branch profile 字段为必填，缺真实 site 数据时 `bpfprof`/`branch_flip` 必须 exit 1。`kernel-sys` 旧 `prog_load_dryrun` / `map_get_next_id` wrapper 没有真实调用方，应删除而不是保留兼容层。`BpfInsn` 的 `#[allow(dead_code)]` helper 要改成 `#[cfg(test)]` 或删除，`PassManager::pass_count` / `pass_names` 这类无调用方 API 也要清理。

#### CLI 之间不交叉依赖

`CLAUDE.md` 的 **No CLI Cross-Dependencies** 是实现约束。剩余 standalone CLI binary crate（`bpfopt`、`bpfprof`、`bpfrejit-daemon`）之间不能有 runtime dependency，也不能有 compile-time path-dependency。`bpfget`、`bpfverify`、`bpfrejit` 是 daemon-owned library crate，不是 CLI crate；共享 syscall/data access 必须通过 `kernel-sys`。

当前实现状态：`bpfrejit-daemon` 不依赖 `bpfopt` 的 lib 部分；daemon 通过 `kernel-sys` 和 daemon-owned libraries 访问 live BPF state，优化变换通过 `bpfopt` CLI 完成，profile 通过 `bpfprof` CLI 完成。
...
`kernel-sys` 是 bpfopt-suite 内唯一直接调用 BPF syscall 的 crate。`bpfopt` 可以依赖 `kernel-sys` 获取 `bpf_insn` 类型、opcode 常量、prog type 枚举等纯数据 API，不再要求 `bpfopt` 完全不依赖 `kernel-sys`。`bpfopt` 仍然不能直接调用 `libc::syscall(SYS_bpf, ...)`，也不能用其它方式绕过 `kernel-sys` 调 BPF syscall。`bpfprof`、`bpfrejit-daemon`、daemon-owned `bpfget`/`bpfverify`/`bpfrejit` libraries 同样只能通过 `kernel-sys` 调 BPF syscall。`kernel-sys` 内部对标准 BPF 命令使用 `libbpf-rs`/`libbpf-sys`，只对 fork 自定义命令 `BPF_PROG_REJIT` 和 `BPF_PROG_GET_ORIGINAL` 用 `libc::syscall` 自行 wrap。

实现阶段：

1. **Phase 1：核心 bytecode CLI**。保留 `bpfopt`，实现 `bpfopt optimize` 和 pass 子命令。
2. **Phase 2：kernel syscall libs + profile CLI**。`bpfprof` 保持 CLI；`bpfget`/`bpfverify`/`bpfrejit` 归 daemon-owned libraries。
...
- **1.3 `bpfopt` CLI 完整版**：已提供 12 个 pass 子命令、`optimize`、`list-passes`；默认 `optimize` pass list 仍为 11 个，不含 Paper B `branch_flip`。缺少 required side-input 或 kinsn 不可用时退出 1（`b0da364`、`aaaaa76`；见 `docs/tmp/bpfopt-cli-review-20260428.md`）。
- **1.4 daemon-owned `bpfget` library**：live bytecode/metadata/map fd/BTF snapshot 已移入 daemon workspace，`ProgramSnapshot` 持有 snapshot 时打开的 map fd 并在 verify/ReJIT 复用；kinsn target probing 由 daemon 进程内调用（见 `docs/tmp/full-matrix-20260430/v3-arch-pivot.md`）。
- **1.5 daemon-owned `bpfrejit` library**：final REJIT 提交已移入 daemon workspace；dry-run 验证由 daemon-owned `bpfverify` library 负责，final syscall 仍走 `BPF_PROG_REJIT`。
- **1.6 minimal pipeline**：生产路径不再是 `bpfget | bpfopt | bpfrejit`；当前路径是 daemon snapshot → `bpfopt` CLI → daemon verify/rejit。
- **3.3 final verify**：daemon fork+exec 一次 `bpfopt optimize` 后，通过 daemon-owned `bpfverify` library 验证最终 candidate；不保留 standalone bash/CLI wrapper，也不在 daemon 内实现 per-pass rollback。

测试现状：`bpfopt` workspace 和 daemon workspace 单独 `cargo test --workspace` 均通过；daemon workspace 包含 `bpfget`/`bpfverify`/`bpfrejit` library tests 和 daemon command tests。

### Phase 2（完整工具链）— 进行中

- **2.1 daemon-owned `bpfverify` library + `verifier_log` 迁移**：dry-run verify、verifier log parse、verifier states in-process 捕获已接入 daemon。
...
- daemon 只 fork+exec `bpfopt` / `bpfprof`；`bpfget` / `bpfverify` / `bpfrejit` 已合并为 daemon-owned libraries。
...
- **3.1 daemon socket orchestration**：`optimize` 走 daemon snapshot → `bpfopt optimize`/pass CLI → daemon verify → daemon ReJIT；`profile-start/stop` 走 `bpfprof`；`status` 只保留 daemon 健康和 profiling 状态，`optimize-all` 已删除。
- **3.2 daemon 瘦身**：`daemon/Cargo.toml` 依赖 daemon-owned `bpfget`/`bpfverify`/`bpfrejit` libs 和 `kernel-sys`；删除旧 `pipeline.rs`、`profiler.rs`、`kfunc_discovery.rs`、`platform_detect.rs` 及对应旧测试文件。
