...
| **Stack-slot coalescing** | 否 | 🔍 未调研（待 census） | 同一物理 stack slot 服务多个 lifetime 不重叠的 logical scalar，缩 stack_depth；scalar-only 第一版（不动 dynptr/pointer/PTR_TO_PACKET spill）。承接 dce dead-def removal 的"短命 slot"。kernel 只做窄 fastcall spill/fill removal，无通用 coalescing | **未实测**。来源：`docs/tmp/const_prop_dce_overlap_and_addable_20260506.md` 候选 #1；估算 high payoff，但 corpus 实际可合并 slot 数未 census。待派 codex 扫 957 个 `.bpf.o` |
| **Strength reduction (mul/mod by const)** | 否 | 🔍 未调研（待 census） | `mul by power-of-2 → lsh`、`mod pow2 → and`、常量除数 magic multiply（与已有"除法强度削减"重叠：见上）。kernel 不做通用 bytecode rewrite | **未实测 mul/mod 部分**。除法已有 `division_reduction_research_20260430.md` （1269 sites）。待派 codex 单独 census mul/mod sites |
| **PHI-style merge simplification** | 否 | 🔍 未调研（待 census） | 不同 path 后 reg 一致的合并简化。kernel verifier 只 join state 不重写 bytecode | **未实测**。来源：overlap 报告候选 #3。待 census |
| **Loop-invariant code motion (LICM)** | 否 | 🔍 未调研（待 census） | open-coded 循环里把不变量提到循环外。kernel 只 inline `bpf_loop` helper（`verifier.c:24729-24864`），无通用 LICM | **未实测**。来源：overlap 报告候选 #5。risk 高（跨 helper/memory boundary），收益 app-dependent。待 census |
