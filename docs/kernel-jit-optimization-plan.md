...
| **507** | **E2E map state capture（2026-03-26）** | ✅ | `capture_map_state.py` 用 raw BPF syscall 遍历 live map → JSON fixture。E2E `--capture-maps` 集成。**首次收集**：Tracee 13 + Tetragon 7 = **20 个真实 map fixtures**。Katran 被 OOM kill 需加 size limit。Commit `ed9588f`（工具）+ `6c2139d`（E2E run）。 |
| **508** | **map_inline 性能验证（2026-03-26）** | 🔄 | **3 个真实命中**（pktcntr/tracee cgroup_skb x2），但全部回归：0.938x/0.813x/0.722x。**原因待查**：替换后指令数可能更多、测量噪声（13-18ns 级别 repeat=100）。**主要 skip 原因**：不支持 map type、非常量 key、非固定 offset load。报告：`map_inline_performance_validation_20260326.md`。 |
...
| **506** | **⚠️ 内核 Bug：attached EXT (freplace) REJIT 失败（2026-03-26）** | 🔄 | **表现**：`rejit_hotswap_ext` FAIL — `kernel does not support live REJIT of attached EXT programs`。**根因**：`bpf_tracing_link_release()` 清掉 `aux->dst_prog`，REJIT 时 verifier 无法重建 freplace target 上下文。fentry 能 REJIT 因为只需 `attach_btf_id`（从原 prog 继承），EXT 额外需要 `dst_prog` 指针。**第二个潜在问题**：target prog 被 REJIT 后 inbound trampoline 的 `tr->func.addr`/`tr->ip` 不更新。**调研报告**：`ext_rejit_fix_proposal_20260326.md`（方案进行中）、`rejit_kernel_bug_investigation_20260326.md`。**修复方向**：从 live trampoline/bpf_tracing_link 恢复 dst_prog。 |
