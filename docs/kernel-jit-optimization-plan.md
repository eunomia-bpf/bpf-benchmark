...
| 66 | **非网络端到端场景研究** | 🔄 | 安全（Tracee/Falco/Tetragon）、可观测（bpftrace/Hubble）、调度（sched_ext）场景评估。codex 进行中 |
| 67 | **Benchmark 框架重构 Step 1-3** | 🔄 | orchestrator/ 共享库 + stale metadata 修复 + 统一 result schema。codex 进行中 |
| 68 | **Corpus 扩展 Round 3** | 🔄 | sched_ext/Tetragon/更多项目 + corpus manifest YAML。codex 进行中 |
