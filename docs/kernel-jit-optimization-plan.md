...
| **480** | **Corpus 全 prog type 覆盖（2026-03-25）** | 🔄 | **(1)** C++ runner 新增 `run-kernel-attach` 命令（libbpf attach + bpf_enable_stats + workload trigger）✅。**(2)** `build_target_batch_plan` 对 `can_test_run=False` 用 `kernel-attach`/`kernel-attach-rejit` runtime ✅。**(3)** `load_targets` 传 `include_attach=True` ✅。**(4)** 删除 inventory.json 依赖 → 改为从 `macro_corpus.yaml` 驱动 🔄（agent 执行中）。**(5)** 给 YAML 补 kprobe/tracing 条目 🔄。目标：corpus 从 152 程序扩展到 500+。 |
