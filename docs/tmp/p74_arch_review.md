...
3. 删或收窄确认 dead public APIs：`available_passes_help`、`validate_pass_names`、`build_full_pipeline`、`PassManager::pass_at`、`KinsnRegistry::call_off_for_pass`、未用的 `BpfInsn` constructors/accessors、`BPF_MEMSX`。
4. 收窄 daemon `pub` 为 `pub(crate)`/private，并清掉 stale doc comments。
5. 清理 daemon response legacy fields；如果 runner 仍依赖旧字段，先写 compatibility 注释并限制字段范围。
6. 拆 `daemon/src/commands.rs`、`bpfopt/src/main.rs`、`pass.rs`。这是结构债，不建议和 critical bug 混在同一个 patch。
