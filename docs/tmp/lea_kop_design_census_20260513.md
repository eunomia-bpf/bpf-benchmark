# LEA kop design and testbin census
...
2026-05-13 follow-up implemented an x86-only `bpf_lea{32,64}` kop experiment
...
Current kop passes use a packed sidecar ABI, not normal C call arguments.
`emit_packed_kop_call_with_off()` emits a sidecar pseudo-insn immediately
before the kop call, and the result register is encoded in the payload
...
- `rotate` declares `bpf_rotate64` and `bpf_rotate32` in `KOP_TARGETS`
  ...
  (`bpfopt/crates/bpfopt/src/passes/rotate.rs:64`), then emits the packed kop
...
- The pass registry records kop targets and whether verifier states are
  required (`bpfopt/crates/bpfopt/src/passes/mod.rs:30`), and deliberately does
  not do kop preflight arch gating (`bpfopt/crates/bpfopt/src/passes/mod.rs:35`).
...
- Payload decode helpers live in the module, using shared helpers such as
  `kop_payload_reg()`, `kop_payload_u8()`, and `kop_payload_s16()`
  (`module/include/kop_common.h:17`).
- x86 register mapping helpers are already shared
  (`module/include/kop_common.h:46`), and the module registration macro is
  `DEFINE_KOP_V2_MODULE()` (`module/include/kop_common.h:124`).
- Each kop descriptor supplies `instantiate_insn` and an arch emitter, e.g.
...
- Existing pass YAML uses `log_level: 1`, a `koperation:` list with `name` and
...
- Native x86 LEA count is real and much larger than BPF foldable count, but it
  is mostly not evidence for a BPF-level LEA kop. The dominant native bucket is
...
- Therefore Katran's native LEA richness is almost entirely x86 address-mode
  ...
  `bpf_lea` kop would not capture the Katran gap that motivated this check.
...
instructions. However, a kop-only implementation is still useful as a narrow
x86 experiment: it avoids core kernel-JIT peepholes, exposes full x86 LEA
semantics through the kop payload, and lets the userspace pass decide which
...
displacement forms are supported by the kop ABI but not yet matched by the
userspace pass.
...
pass_entry!("lea", lea::LeaPass, lea::KOP_TARGETS, false)
```

The pass is not verifier-state-gated. KOperation semantics are defined by the module
instantiation, and site selection stays in userspace without pointer/scalar
policy guards.

KOperation descriptors:

```rust
pub(super) const KOP_TARGETS: &[KopDescriptor] = &[
    KopDescriptor {
        name: "bpf_lea64",
...
fn lea_register_uses(payload: u64) -> RegSet {
    ...
    if lea_payload_has_base(payload) {
        regs.insert(kop_payload_reg(payload, 4));
    }
    if ((payload >> 14) & 1) != 0 {
        regs.insert(kop_payload_reg(payload, 8));
    }
    regs
}
...
let (btf_id, kfunc_off) = prog.kop_call(target)?;
...
supports a BPF-level `bpf_lea` kop proposal. Across `testccode`, clang emits
...
Expected effect if implemented as the kop design above:
...
- The implementation would require a new kop module, a verifier-state-gated
  bpfopt pass, pass YAML, and tests to get only a one-native-instruction
  lowering for the dominant 2-slot bytecode pattern.

Recommendation: keep LEA as an x86-only kop experiment, not as evidence for a
...
Revised verdict: implemented x86-only kop experiment; no core kernel-JIT
