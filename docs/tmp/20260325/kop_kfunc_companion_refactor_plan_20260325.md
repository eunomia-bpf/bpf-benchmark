# kop -> kfunc companion metadata 重构方案

日期: 2026-03-25

## 目标

在不改变现有 `kop` 调用 ABI 和证明语义的前提下，删除当前 verifier 中
`FUNC name -> "<name>_desc" -> kallsyms` 的隐式查找路径，把
`func_id -> kop descriptor` 变成显式 companion metadata。
...
替换的部分:
- `fetch_kop_desc_meta()` 中的 `_desc` 命名约定
...
把 `kop` descriptor 挂到现有 kfunc companion metadata 上。

不是把 `kop` 变成普通 kfunc 语义，而是：
...
struct bpf_kfunc_desc {
	...
	const struct bpf_kop *kop; /* optional */
};
```
...
- 全局 descriptor，例如 `const struct bpf_kop bpf_rotate64_desc = {...}`
...
```c
struct bpf_kop_kfunc_pair {
	const char *name;
	const struct bpf_kop *desc;
};

static const struct bpf_kop_kfunc_pair bpf_rotate_pairs[] = {
...
static const struct bpf_kop *bpf_rotate_descs[] = {
	[0] = &bpf_rotate64_desc,
};
```
...
btf_kfunc_kop_desc(struct btf *btf, u32 func_id, struct bpf_prog *prog);
```

语义:
- 输入 `btf + func_id + prog`
- 输出这个 kfunc 是否带 `kop` companion descriptor
...
改成:
- `add_kop_call()` 仍保留
- 但 `fetch_kop_desc_meta()` 简化成:
  - `btf = find_kfunc_desc_btf(env, offset)`
  - 验证 `func_id` 是 `BTF_KIND_FUNC`
  - `kop = btf_kfunc_kop_desc(btf, func_id, env->prog)`
- 不再拼 `_desc`
- 不再查 `kallsyms`
...
- `add_kfunc_call()` 可在创建 `bpf_kfunc_desc` 时顺手填 `desc->kop`
- `add_kop_call()` 可进一步简化成“为 kop 视图做一个轻量缓存”
- 后续 `bpf_jit_find_kop_desc()` 可以从统一表里查

第一阶段不建议硬并 `kfunc_tab` 和 `kop_tab`，先把 lookup 显式化即可。
...
继续保留:
- x86 `emit_kop_desc_call()`
- arm64 `emit_kop_desc_call_arm64()`

变化只在 descriptor 来源:
- 现在 `bpf_jit_get_kop_payload()` 最终依赖 `kop_tab`
- companion metadata 方案下，仍可保留 `kop_tab` 作为轻量缓存
...
- verifier 新增 `btf_kfunc_kop_desc()`
- `fetch_kop_desc_meta()` 改成显式查 companion metadata
...
改动:
- 给 `bpf_kfunc_desc` 直接挂 `const struct bpf_kop *`
- 让 `bpf_jit_find_kop_desc()` 从统一表查
- 删除独立 `kop_tab`
- 删除 `find_kop_desc()` / `kop_desc_cmp_by_imm_off()` / 一部分 `add_kop_call()` 重复逻辑
...
再讨论是否:
- 把 `PSEUDO_KOP_CALL` 变成 `PSEUDO_KFUNC_CALL + special flag`
...
这些是 `kop` 语义本体，不是 registry/lookup 重复。
