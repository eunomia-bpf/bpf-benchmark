# kop 伪指令编码统一到 kfunc CALL 路径调研报告
...
1. 当前代码里的 kop 不是“只差一个 pseudo tag 的 kfunc”。它是一条独立的 `BTF_KIND_VAR -> struct bpf_kop -> verifier proof lowering -> JIT native emit` 管线。
2. 当前树里不存在任务背景里提到的 `bpf_kop_ops` / `bpf_kop_effect` / `bpf_kop_call` / `model_call -> bpf_kop_effect` 实现；`HEAD` 的真实实现是 `struct bpf_kop { instantiate_insn, emit_x86, emit_arm64 }`，并通过 `lower_kop_proof_regions()` 在主 verifier 前做 proof lowering。
3. 如果“字面上只把 `BPF_PSEUDO_KOP_CALL` 改成 `BPF_PSEUDO_KFUNC_CALL`，其余逻辑不动”，当前树不能工作：
   ...
   - `fixup_kfunc_call()` 会改写 `imm`，现有 kop JIT lookup keyed by `(imm, off)` 会失效。
4. 可行的统一方式不是“让 kop 直接吃进标准 `check_kfunc_call()` 语义”，而是：
   - CALL 编码统一成 `BPF_PSEUDO_KFUNC_CALL`
   - 真实目标统一成 kfunc `FUNC` + `register_btf_kfunc_id_set()`
   - 但 verifier 仍对“带 sidecar 的 `KF_KOP` kfunc”走现有 proof-lowering 旁路，避免落入 generic kfunc arg/clobber 语义
   - JIT 在现有 kfunc CALL 分支里按 `KF_KOP` 再分派到现有 `emit_x86/emit_arm64`
5. 从 reviewer 视角，这种改法是“删除并行 kop CALL/VAR registry，保留 sidecar + native emit 的 kfunc 化”，上游接受性明显好于继续维护一套独立 pseudo/UAPI/verifier/JIT path。

## 0. 当前代码现实与任务背景的偏差

这一步必须先讲清楚，否则后面的“可行性”会被错误前提带偏。

- 当前 UAPI 确实同时存在 `BPF_PSEUDO_KFUNC_CALL=2`、`BPF_PSEUDO_KOP_SIDECAR=3`、`BPF_PSEUDO_KOP_CALL=4`，见 `vendor/linux-framework/include/uapi/linux/bpf.h:1378-1394`。
- 当前内核里真实存在的是 `struct bpf_kop`、`struct bpf_kop_id`、`struct bpf_kop_set`，见 `vendor/linux-framework/include/linux/bpf.h:968-1003`。
- 当前 verifier/JIT 依赖的是：
  - `instantiate_insn()` 生成 proof sequence
  - `emit_x86()` / `emit_arm64()` 做 native emit
  - `lower_kop_proof_regions()` / `restore_kop_proof_regions()` 完成 verifier lowering/restore
...
这正是后面“为何不能让 packed kop 直接吃进 generic kfunc verifier 语义”的根因。
...
daemon startup / cmd_apply
  -> discover_kops()                          daemon/src/kfunc_discovery.rs:323
     ...
     -> fill KopRegistry(target -> descriptor VAR id, module BTF fd)

pass rewrite
  -> ensure_btf_fd_slot()                       daemon/src/passes/utils.rs:239-247
  -> emit_packed_kop_call_with_off()          daemon/src/passes/utils.rs:228-237
     -> sidecar MOV pseudo
...
- `daemon/src/insn.rs:56-60`：定义 `BPF_PSEUDO_KFUNC_CALL=2`、`BPF_PSEUDO_KOP_SIDECAR=3`、`BPF_PSEUDO_KOP_CALL=4`
- `daemon/src/insn.rs:229-240`：`call_kop_with_off()`
- `daemon/src/insn.rs:269-275`：`kop_sidecar(payload)`
- `daemon/src/kfunc_discovery.rs:21-33`：`KNOWN_KOPS` 当前是 `(registry_key, descriptor_var_name, module_name)`
...
```text
userspace emits
  -> sidecar MOV (src_reg=PSEUDO_KOP_SIDECAR)
  -> CALL(src_reg=PSEUDO_KOP_CALL, imm=descriptor VAR id, off=fd_array slot)

bpf_check()                                     verifier.c:26518
  -> add_subprog_and_kfunc()                    verifier.c:4149
     -> env->kop_call_cnt++                   verifier.c:4177-4179
     -> add_kop_call()                        verifier.c:3643
        -> fetch_kop_desc_meta()              verifier.c:3607
           -> find_kfunc_desc_btf()             verifier.c:3618
           -> btf_try_get_kop_desc()          btf.c:9175
        -> stash into prog->aux->kop_tab

  -> lower_kop_proof_regions()                verifier.c:3958 / 26607
     -> find preceding sidecar
     -> bpf_jit_find_kop_desc()               verifier.c:3796
     -> kop->instantiate_insn(payload)
     -> validate_kop_proof_seq()              verifier.c:3842
     -> replace [sidecar, call] with proof seq

  -> do_check_main()/do_check_subprogs()
     -> verifier sees proof seq, not kop call
     -> if kop call leaked through:
          do_check_insn() hard-fails            verifier.c:21695-21696

  -> restore_kop_proof_regions()              verifier.c:4042 / 26679
     -> restore original [sidecar, call]

  -> do_misc_fixups()                           verifier.c:23931 / 26711
     -> if sidecar + kop call:
          build_kop_inst_seq()                verifier.c:4080
  ...
  -> fixup_call_args()                          verifier.c:23637 / 26723
     -> reject non-JIT kop programs           verifier.c:23661-23663

JIT x86/arm64
  -> sidecar MOV is skipped                     x86:1906-1911 / arm64:1254-1255
  -> if src_reg == PSEUDO_KOP_CALL:
       bpf_jit_get_kop_payload()              verifier.c:4111
         -> bpf_jit_find_kop_desc()
         -> find sidecar payload
       kop->emit_x86()/emit_arm64()
```
...
| 文件:函数:行号 | 作用 |
| --- | --- |
| `vendor/linux-framework/kernel/bpf/verifier.c:add_kop_call():3643-3678` | 建立 `(imm=descriptor VAR id, off) -> struct bpf_kop*` 映射 |
| `vendor/linux-framework/kernel/bpf/verifier.c:fetch_kop_desc_meta():3607-3640` | 用 descriptor VAR id + 模块 BTF 找 `struct bpf_kop` |
| `vendor/linux-framework/kernel/bpf/btf.c:register_bpf_kop_set():9029-9090` | kop 独立注册表；把 `VAR id -> desc ptr` 放进 `btf->kop_tab` |
| `vendor/linux-framework/kernel/bpf/btf.c:btf_try_get_kop_desc():9175-9190` | 从 `btf->kop_tab` 取 descriptor |
| `vendor/linux-framework/kernel/bpf/verifier.c:lower_kop_proof_regions():3958-4035` | 主 verifier 前把 sidecar+call 替换成 proof sequence |
| `vendor/linux-framework/kernel/bpf/verifier.c:restore_kop_proof_regions():4042-4066` | 主 verifier 后恢复原始 sidecar+call |
| `vendor/linux-framework/kernel/bpf/verifier.c:validate_kop_proof_seq():3842-3893` | proof sequence 合法性检查 |
| `vendor/linux-framework/kernel/bpf/verifier.c:build_kop_inst_seq():4080-4108` | `do_misc_fixups()` 里重新实例化 proof sequence |
| `vendor/linux-framework/kernel/bpf/verifier.c:bpf_jit_get_kop_payload():4111-4131` | JIT 从原始程序里取 descriptor + sidecar payload |
| `vendor/linux-framework/kernel/bpf/verifier.c:check_kop_sidecar_insn():21576-21593` | sidecar 只能跟 `PSEUDO_KOP_CALL`；否则报错 |
| `vendor/linux-framework/kernel/bpf/verifier.c:do_check_insn():21691-21696` | 若 `PSEUDO_KOP_CALL` 没被 lowering 掉，直接报 internal error |
| `vendor/linux-framework/arch/x86/net/bpf_jit_comp.c:579-606` | x86 原生 kop emit 回调 |
| `vendor/linux-framework/arch/x86/net/bpf_jit_comp.c:2487-2493` | x86 独立 `PSEUDO_KOP_CALL` JIT dispatch |
| `vendor/linux-framework/arch/arm64/net/bpf_jit_comp.c:1195-1219` | arm64 原生 kop emit 回调 |
| `vendor/linux-framework/arch/arm64/net/bpf_jit_comp.c:1627-1633` | arm64 独立 `PSEUDO_KOP_CALL` JIT dispatch |

### 2.4 当前 kop 语义的关键事实

最重要的一点：**主 verifier 实际上不会验证 `PSEUDO_KOP_CALL` 本身**。

它验证的是 lowering 后的 proof sequence。因此当前 kop 的 verifier 语义来源不是 kfunc call 规则，而是 `instantiate_insn()` 生成的那段普通 BPF 指令序列。

这意味着：

- 现有 kop 可以保留任意寄存器，只要 proof sequence 没 clobber。
- 现有 kop 的返回值可以直接体现在 payload 指定的目标寄存器上，而不必遵守“结果总在 R0”。
- 这与 generic kfunc call 的 `R0-R5` clobber + `R0` return 语义并不相同。

## 3. 合并可行性分析

### 3.1 如果直接把 kop CALL 改成 `BPF_PSEUDO_KFUNC_CALL`，哪些步骤会自动适用
...
- 当前模块只导出 descriptor 变量，例如：
  ...
  - arm64 对应文件同理
- 当前模块注册宏 `DEFINE_KOP_V2_MODULE()` 只调用 `register_bpf_kop_set()`，见 `module/include/kop_common.h:126-152`。
...
- 当前 packed kop ABI 的真正操作数放在 sidecar payload，不是放在 `R1-R5`。
...
- 当前 kop 语义来自 proof sequence，不存在统一的 `R1-R5` clobber 规则。
- `tests/unittest/rejit_kop.c:875+` 里甚至有 `test_rejit_barrier_preserves_r5()`，这至少说明当前语义和实现都依赖“不是普通 kfunc call clobber”。

结论：只要 kop 进入 `check_kfunc_call()`，就已经偏离当前 ABI/语义。
...
- daemon `kop_sidecar()` 的注释也已经写成“immediately following kfunc call”，见 `daemon/src/insn.rs:263-268`

但当前代码里真正消费 sidecar 的 verifier/JIT 路径仍强绑定 `PSEUDO_KOP_CALL`：

- `check_kop_sidecar_insn()` 要求下一条必须是 `PSEUDO_KOP_CALL`，见 `vendor/linux-framework/kernel/bpf/verifier.c:21576-21593`
- `lower_kop_proof_regions()` / `do_misc_fixups()` / x86/arm64 JIT 都只识别 `PSEUDO_KOP_CALL`

所以 sidecar 本身不是问题，问题是它的消费者需要统一改成识别“带 sidecar 的 kfunc 且目标被标记为 kop”。

### 3.4 `fixup_kfunc_call()` 会不会错误改写 kop CALL
...
而当前 x86/arm64 kop native emit lookup 是按“原始 `(imm, off)`”查 descriptor/payload：

- `bpf_jit_find_kop_desc()`：`vendor/linux-framework/kernel/bpf/verifier.c:3796-3803`
- `bpf_jit_get_kop_payload()`：`vendor/linux-framework/kernel/bpf/verifier.c:4111-4131`

如果让 kop 直接走 `fixup_kfunc_call()`，现有 keyed-by-`(imm, off)` 的 JIT lookup 就会错位。

结论：`KF_KOP` call 必须在 `fixup_kfunc_call()` 里走单独分支，最干净的是 **直接跳过 generic kfunc fixup**。

### 3.5 JIT 中 kfunc CALL emit 路径在哪，怎么加 `KF_KOP` 分支最干净

当前 x86/arm64 都有两条路：

- generic kfunc/helper CALL 路
- 独立 `PSEUDO_KOP_CALL` 路

现状代码点：

- x86 generic CALL：`vendor/linux-framework/arch/x86/net/bpf_jit_comp.c:2483-2510`
- x86 独立 kop branch：`vendor/linux-framework/arch/x86/net/bpf_jit_comp.c:2488-2493`
- arm64 generic CALL：`vendor/linux-framework/arch/arm64/net/bpf_jit_comp.c:1627-1639`
- arm64 独立 kop branch：`vendor/linux-framework/arch/arm64/net/bpf_jit_comp.c:1627-1633`

最干净的改法不是保留第三条 pseudo，而是：

1. CALL 一律编码为 `PSEUDO_KFUNC_CALL`
2. 在 `PSEUDO_KFUNC_CALL` 分支内先查“该 desc 是否带 `KF_KOP` / 是否有关联 `struct bpf_kop`”
3. 若是，则直接调用现有 `emit_kop_desc_call()` / `emit_kop_desc_call_arm64()`
4. 否则走原本 generic kfunc CALL

注意：

- `bpf_jit_find_kfunc_model()` 不是 x86_64 / arm64 program-call emit 的合适切点；它在这两个路径里 today 并没有承担“选择 native kop emit”的责任。
- 更合适的是给 x86/arm64 增一个新 helper，例如 `bpf_jit_get_kop_payload_from_kfunc()` 或在 `bpf_kfunc_desc` 上直接附 `const struct bpf_kop *kop`。
...
3. **引入内部 `KF_KOP` 标记**
4. **让 verifier 继续对 `sidecar + KF_KOP kfunc` 做 proof lowering**
   - 这样主 verifier 仍验证 proof sequence，而不是 generic kfunc call
5. **让 `do_misc_fixups()` 和 JIT 继续复用现有 native emit**
6. **删除独立 `BPF_PSEUDO_KOP_CALL` / descriptor VAR registry / `kop_tab`**
...
如果这层不保留，现有 kop 语义就会退化成“普通 kfunc 调用”，那已经不是当前 feature 了。
...
- `module/arm64/bpf_barrier.c:52-62`
  ...
  - 动作：为 stub symbol 增 `BTF_ID_FLAGS(func, ..., KF_KOP)` 之类的注册项
  - 动作：把 stub symbol 与现有 `struct bpf_kop` 描述符关联起来

### 4.2 daemon 侧

- `daemon/src/kfunc_discovery.rs:21-33`
  - 动作：`KNOWN_KOPS` 从 `(registry_key, descriptor_var_name, module_name)` 改为 `(registry_key, kfunc_func_name, module_name)`
...
- `daemon/src/kfunc_discovery.rs:343-389`
  - 动作：更新注释与日志，不再提“descriptor VAR id accepted by `PSEUDO_KOP_CALL`”
  - 动作：发现结果改成 `BTF_KIND_FUNC` id

- `daemon/src/insn.rs:229-240`
  - 动作：废弃 `call_kop_with_off()`

- `daemon/src/passes/utils.rs:228-237`
  - 动作：`emit_packed_kop_call_with_off()` 的第二条指令改成 `call_kfunc_with_off()`
  - 建议：函数名也同步改，避免继续误导为 “kop call”
...
- `daemon/src/passes/spectre.rs:110/114/230/339/349`
  - 动作：测试和断言从 `PSEUDO_KOP_CALL` 改到 `PSEUDO_KFUNC_CALL`
...
- `tests/unittest/rejit_kop.c:188-196`
  - 动作：`BPF_CALL_KOP()` 宏改成 kfunc CALL 宏

- `tests/unittest/rejit_kop.c:532-603`
  - 动作：`find_var_btf_id()` 改成 `find_func_btf_id()`

- `tests/unittest/rejit_kop.c:672-715`
  - 动作：discovery 逻辑改成发现 `FUNC id`

- `tests/unittest/rejit_kop.c:730-754`
  - 动作：patch helper 从 `PSEUDO_KOP_CALL` 改成 `PSEUDO_KFUNC_CALL`

- `tests/unittest/rejit_kop.c:846-872`
...
- 内核 8 文件净新增大概率会 **下降**，因为：
  - `BPF_PSEUDO_KOP_CALL`
  - `kop_tab`
  - `register_bpf_kop_set()/unregister/try_get`
  - `add_kop_call()/bpf_prog_has_kop_call()/独立 JIT dispatch`
  这批“并行基础设施”会被删除/折叠
...
| 风险项 | 概率 | 影响 | 依据 | 缓解 |
| --- | --- | --- | --- | --- |
| 333 daemon tests 直接回归 | 高 | 高 | 多个 pass/unit test 直接断言 `PSEUDO_KOP_CALL` 和 descriptor VAR id | 必须同步改 emit/discovery/tests |
| vm-selftest 70/70 回归 | 高 | 高 | `tests/unittest/rejit_kop.c` 明确依赖 descriptor VAR id、`PSEUDO_KOP_CALL` | 同步改 unittest 和 VM selftest 预期 |
| sidecar 被 verifier/JIT 误判或丢弃 | 中高 | 高 | today 只有 `PSEUDO_KOP_CALL` 消费 sidecar | 统一改 sidecar consumer；对 `KF_KOP` 做单独 helper |
| generic `check_kfunc_call()` 语义泄漏到 kop | 高 | 高 | 参数检查 + `R0-R5` clobber 与 packed ABI 冲突 | 继续走 proof lowering 旁路，不让它进入 generic kfunc verifier |
| `fixup_kfunc_call()` 改写 `imm` 破坏 native emit lookup | 高 | 高 | today JIT payload lookup keyed by `(imm, off)` | 对 `KF_KOP` 跳过 generic kfunc fixup |
...
| module 生命周期回归 | 低中 | 中 | today 依赖 `kfunc_btf_tab` pin module BTF，`btf_try_get_kop_desc()` 只要求 owner 一致 | 统一到标准 kfunc 后，生命周期通常更简单，不应更差 |
...
- `daemon/src/passes/*` 多处断言/筛选 `src_reg == BPF_PSEUDO_KOP_CALL`
- `daemon/src/kfunc_discovery.rs` today 明确找的是 descriptor `VAR`
- `daemon/src/passes/utils.rs:228-237` today 就是 emit sidecar + `call_kop_with_off()`

如果 kernel/daemon/tests 一起改，非 kop 相关 daemon tests 大概率不受影响；但直接相关的 kop pass/unit tests 需要整体更新。

#### 2. 会不会破坏 vm-selftest（70/70）

会，如果不同时更新 `tests/unittest/rejit_kop.c`。

依据：

- `tests/unittest/rejit_kop.c:188-196` 宏直接编码 `PSEUDO_KOP_CALL`
- `tests/unittest/rejit_kop.c:532-715` discovery 直接找 descriptor `VAR`
- `tests/unittest/rejit_kop.c:730-754` patch helper 直接改 `PSEUDO_KOP_CALL`
...
- current sidecar consumer 只认 `PSEUDO_KOP_CALL`
- generic kfunc fixup/JIT 路不会自动保留 sidecar 语义

控制方法：

- sidecar 保持独立 pseudo，不进入 generic ALU 语义
- `lower_kop_proof_regions()` / `do_misc_fixups()` / `bpf_jit_get_kop_payload()` 全部改成 “sidecar + `KF_KOP` kfunc” 识别
- `fixup_kfunc_call()` 对 `KF_KOP` 直接跳过
...
- `btf_try_get_kop_desc()` 自己并不额外 pin 第二份 module ref，它依赖“descriptor owner 与 module BTF owner 一致”，见 `vendor/linux-framework/kernel/bpf/btf.c:9170-9173`
...
- 新 UAPI pseudo：`BPF_PSEUDO_KOP_CALL`
- 新内核并行注册表：`register_bpf_kop_set()`
...
3. sidecar 作为 kop packed ABI 附件保留
4. verifier 只有一个额外特例：`KF_KOP` kfunc 在主 verifier 前做 proof lowering
5. x86/arm64 只在已有 kfunc CALL 路里多一个 `KF_KOP` 分支
...
也就是说，统一方案不是“完全回到 upstream 零 diff”，而是把 **“独立 kop CALL/VAR registry 的 diff”** 收缩成 **“kfunc 框架上的一个内部扩展 diff”**。
...
- 当前 JIT lookup 依赖 `PSEUDO_KOP_CALL` 的 `(imm, off)` 约定
...
1. **先做模块/daemon/BTF id 统一**
   ...
   - daemon emit 从 `PSEUDO_KOP_CALL` 切到 `PSEUDO_KFUNC_CALL`

2. **再做 verifier/JIT 旁路**
   - 引入 `KF_KOP`
   - proof lowering / fixup / native emit 都改成识别 `KF_KOP kfunc`
   - 不让 `KF_KOP` 落入 generic `check_kfunc_call()` / `fixup_kfunc_call()`

3. **最后删除旧并行基础设施**
   - `BPF_PSEUDO_KOP_CALL`
   - `register_bpf_kop_set()` / `btf_try_get_kop_desc()` old VAR registry
   - `kop_tab`
   - arch 独立 `PSEUDO_KOP_CALL` dispatch

### 一句话判断

“统一到 kfunc CALL 路径”这件事本身是可行且值得做的；但 **必须统一的是“注册/查找/地址解析/编码入口”**，而不是把当前 packed kop 直接塞进 generic kfunc verifier 语义。

如果目标是保留现有 sidecar ABI、proof-seq verifier 语义和 native emit 回调，那么正确实现是 **kfunc 化 transport，保留 kop 旁路语义**。
