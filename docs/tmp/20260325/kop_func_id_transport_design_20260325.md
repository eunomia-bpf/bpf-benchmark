...
1. 当前 `vendor/linux-framework/rejit-v2` 相对 kernel 上游 `origin/master` 的 diff 里，哪些是 REJIT 核心，哪些是 `kop transport` 带来的额外表面积。
2. 在不改变 `kop` 当前寄存器约定、`proof lowering` 语义、native emit 语义的前提下，怎样把 `kop` 从今天的 `VAR-id + 独立注册表` 收敛成 `FUNC-id transport`。

本文档的推荐方案不是“把 `kop` 变成普通 kfunc”，而是：
...
`kop` 继续保持自己的 call ABI 和 verifier 语义，只把“我是谁”这件事从 `VAR id` 改成 `FUNC id`。
...
| `kernel/bpf/verifier.c` | +649/-32 | REJIT + kop verifier + desc cache |
| `kernel/bpf/btf.c` | +239/-1 | kop `VAR` 注册/lookup |
| `kernel/bpf/bpf_struct_ops.c` | +108/-0 | struct_ops refresh |
| `include/linux/bpf.h` | +110/-0 | REJIT + kop 核心声明 |
...
| `include/uapi/linux/bpf.h` | +24/-0 | `BPF_PROG_REJIT` + `PSEUDO_KOP_*` |

这些变化里，`REJIT` 核心和 `kop transport` 不是同一件事。
...
- `BTF_KIND_VAR` 作为 kop identity
- `register_bpf_kop_set()` / `unregister_bpf_kop_set()`
- `btf->kop_tab`
- `prog->aux->kop_tab`
...
- `CALL.imm = module BTF 中某个 `struct bpf_kop` descriptor 的 `BTF_KIND_VAR` id`
- `CALL.off = module BTF fd slot`
- `sidecar` 承载 packed payload

module 侧通过 `register_bpf_kop_set()` 注册：

- `descriptor var name -> const struct bpf_kop *`

verifier load 时通过：

- `offset -> module BTF`
- `(btf, var_id) -> struct bpf_kop *`
...
- BTF 里单独挂 `btf->kop_tab`
- 单独的 `register_bpf_kop_set()`
- 单独的 `btf_try_get_kop_desc()`
- verifier 里单独的 `add_kop_call()` / `find_kop_desc()`
- prog aux 里单独的 `kop_tab`
...
4. upstream review 会看到一个“平行 kop 子系统”
...
所以，若直接改成 `PSEUDO_KFUNC_CALL`，就必须在普通 kfunc 路径里到处加 `if (is_kop)` 守卫。这会把 generic kfunc 代码弄脏。

## 3. 设计目标

必须满足：

1. 不改变 `kop` 当前 call ABI  
2. 不改变寄存器约定  
3. 不改变 sidecar payload 编码  
4. 不改变 proof lowering / restore 模型  
5. 不改变 native emit 模型  
6. 删除 `BTF_KIND_VAR + global_var` 这条 transport 依赖  
7. 尽量删掉独立的 kop 注册/lookup/cache 代码  
8. 不把普通 kfunc verifier/fixup 路径污染成“双语义”

明确不做：

1. 不把 `kop` 变成普通 kfunc 参数传递语义  
...
- `PSEUDO_KOP_CALL + VAR id`
- 独立 `register_bpf_kop_set()`
- 独立 `btf->kop_tab`
...
### 4.2 方案 B：切到 `PSEUDO_KFUNC_CALL + FUNC id + KF_KOP`

定义：

- transport 直接并入 `PSEUDO_KFUNC_CALL`
- 引入内部 `KF_KOP`
- 普通 kfunc 路径里识别并旁路到 `kop` 语义
...
- generic `check_kfunc_call()` / `fixup_kfunc_call()` / JIT call 分支都要知道 `KF_KOP`
...
### 4.3 方案 C：保留 `PSEUDO_KOP_CALL`，切成 `FUNC id transport`

定义：

- `CALL.src_reg` 继续是 `BPF_PSEUDO_KOP_CALL`
- `CALL.imm` 从 `VAR id` 改成 `FUNC id`
- `CALL.off` 继续表示 module BTF fd slot
- verifier 继续在 `proof lowering` 前解析 descriptor
- 普通 `check_kfunc_call()` / `fixup_kfunc_call()` 不知道 `kop`
...
- 可以删掉 `global_var` 依赖
- 代码叙事更清楚：`kop` 不是 kfunc，只是借用 kfunc identity

缺点：

- 仍然保留单独 opcode
- 仍然需要一层 `func_id -> struct bpf_kop *` 元数据
- 不是“完全并入 kfunc”
...
- `CALL.imm` 不再是 `struct bpf_kop` descriptor 的 `BTF_KIND_VAR` id
- `CALL.imm` 改成 kop stub symbol 的 `BTF_KIND_FUNC` id
...
每个 kop 继续保留自己的 descriptor：

```c
static const struct bpf_kop bpf_rotate64_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = ...,
	.max_emit_bytes = ...,
	.instantiate_insn = ...,
...
1. 真实 kfunc ID set  
2. kop companion metadata set

推荐 companion 结构：

```c
struct bpf_kop_func_id {
	const char *func_name;
	const struct bpf_kop *desc;
};

struct bpf_kop_func_set {
	struct module *owner;
	u32 cnt;
	const struct bpf_kop_func_id *ids;
};
```
...
- 它只是把 `FUNC name / FUNC id` 绑定到 `struct bpf_kop *`
...
- `register_bpf_kop_set()`
- `unregister_bpf_kop_set()`
- `btf->kop_tab`
- `btf_try_get_kop_desc()`

替换为：

- `btf_try_get_kop_desc_by_func_id()` 或等价 helper
- 输入为 `(btf, func_id)`
- 输出为 `const struct bpf_kop *`
...
- `PSEUDO_KOP_CALL` 只允许在 `proof lowering` 之前被消费
- `do_check_insn()` 若仍看到 `PSEUDO_KOP_CALL`，继续视为 internal error
- `lower_kop_proof_regions()` / `restore_kop_proof_regions()` 主模型不变

改变：

- `fetch_kop_desc_meta()` 的输入从 `(var_id, offset)` 变成 `(func_id, offset)`
- 先通过 `find_kfunc_desc_btf()` 拿 module BTF
- 再通过 `func_id -> desc` companion metadata 拿 `struct bpf_kop *`

重要的是：

- 不进入 `check_kfunc_call()`
- 不进入 `fixup_kfunc_call()`
- 不让普通 kfunc 的寄存器 clobber 规则污染 `kop`

## 5.5 JIT 行为

JIT 语义保持不变：

- x86 / arm64 仍按 `PSEUDO_KOP_CALL` 识别
- 仍通过 `bpf_jit_get_kop_payload()` 和 desc 获取 payload + emit hook
- 仍直接发 native emit

变化只在 desc lookup：

- 不再查 `prog->aux->kop_tab` 里以 `VAR id` 为 key 的项
- 改成查以 `FUNC id` 为 key 的 kop desc cache

这层可以有两种实现：

1. 保留一个单独的 per-prog `kop_tab`，但 key 从 `VAR id` 改成 `FUNC id`
2. 进一步把 per-prog `kfunc_tab` / `kop_tab` 并成通用 `call_desc_tab`
...
当前的 `btf_kop_tab` / register / unregister / `btf_try_get_kop_desc()` 主要服务于：

- `VAR id -> desc`

切成 `FUNC id` 后：

- 旧 `VAR` 解析和 `btf_type_is_kop_desc()` 可以删除
- register/unregister 逻辑会缩成一层更薄的 `func_id -> desc` companion metadata

所以 `btf.c` 依然可能有少量 kop 代码，但会从“独立 BTF 子系统”降到“薄 metadata 绑定层”。

#### 6.2.3 `include/linux/btf.h`

应该删除或收缩：

- `btf_try_get_kop_desc()` 的 `VAR` 语义声明
...
- 注释从“`imm == BTF_KIND_VAR of struct bpf_kop`”改成“`imm == BTF_KIND_FUNC of registered kop stub`”
...
- `btf_try_get_kop_desc(... var_id ...)`

新增：

- `btf_try_get_kop_desc_by_func_id(... func_id ...)`

### `kernel/bpf/btf.c`

删除：

- `btf_type_is_kop_desc()`
- `btf_resolve_kop_desc_id()`
- `register_bpf_kop_set()`
- `unregister_bpf_kop_set()`
- `btf->kop_tab` 的 `VAR-id` 版本
...
- `fetch_kop_desc_meta()` 改成按 `func_id` 查
- `add_kop_call()` 的 key 从 `VAR id` 改成 `FUNC id`
- `find_kop_desc()` / JIT payload query 的 key 同步切换

保留：

- `lower_kop_proof_regions()`
- `restore_kop_proof_regions()`
- `PSEUDO_KOP_CALL` 在 `do_check_insn()` 里仍然不能落入正常 verifier 主路径

### `kernel/bpf/core.c` / `kernel/bpf/syscall.c`

如果继续保留单独 per-prog `kop_tab`：
...
当前 `DEFINE_KOP_V2_MODULE(...)` 只会注册 descriptor set。

改为：

- 为每个 kop 声明 stub kfunc
- 定义 `BTF_KFUNCS_START(...)`
...
- `KNOWN_KOPS` 里存的是 descriptor variable name
- discovery 查 `BTF_KIND_VAR`

改为：

- `KNOWN_KOPS` 里存 stub function name
...
- module 注册接口不再是 `register_bpf_kop_set()`
- daemon 的 discovery 目标从 descriptor var name 变成 stub function name

## 8.3 为什么这仍然是可接受的 ABI 变化

因为这里变的是 transport identity，不是 `kop` 的行为 ABI。

从用户和 JIT 语义层面看：

- 程序看到的 sidecar 和 `kop` 语义没变
- verifier 证明模型没变
- JIT 发出的机器码没变
...
- loader 如何找到目标
- kernel 如何把一个 BTF id 解析成 `struct bpf_kop *`
...
删掉这些之后，`PSEUDO_KOP_CALL` 更像“自定义 transport opcode”，不是完整平行系统。

### 风险 2：是否还需要单独 per-prog `kop_tab`
...
再决定第二阶段是否把 `kfunc_tab` / `kop_tab` 并表。
...
- companion metadata 只负责把 `FUNC id` 绑定到 `struct bpf_kop *`
...
- 树上不再有任何 kop transport 对 `BTF_KIND_VAR` 的依赖
...
当前最好的方案不是维持 `VAR-id`，也不是立即把 `kop` 塞进 `PSEUDO_KFUNC_CALL`。
...
- 删除 `global_var` 依赖和大部分独立 kop 注册子系统
