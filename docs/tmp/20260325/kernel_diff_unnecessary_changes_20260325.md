...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +1192 | `emit_kop_desc_call_arm64()` 函数 | **核心必要** | kop ARM64 原生 emit 路径 |
| @@ +1251 | `bpf_kop_is_sidecar_insn` 跳过 | **核心必要** | JIT 跳过 sidecar 伪指令 |
| @@ +1624 | `BPF_PSEUDO_KOP_CALL` case | **核心必要** | kop CALL 指令的 ARM64 JIT 入口 |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +576 | `emit_kop_desc_call()` 函数 | **核心必要** | kop x86 原生 emit 路径 |
...
| @@ +1905 | `bpf_kop_is_sidecar_insn` 跳过 | **核心必要** | JIT 跳过 sidecar 伪指令 |
| @@ +2485 | `BPF_PSEUDO_KOP_CALL` case | **核心必要** | kop CALL 指令的 x86 JIT 入口 |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +965 | `struct bpf_kop` / `bpf_kop_id` / `bpf_kop_set` | **核心必要** | kop 描述符核心数据结构 |
| @@ +965 | `bpf_kop_is_sidecar_insn()` / `bpf_kop_sidecar_payload()` | **核心必要** | sidecar 指令判断和 payload 解码 |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +742 | `struct bpf_kop_region` | **核心必要** | verifier kop proof region 结构 |
| @@ +857 | `kop_regions` / `kop_call_cnt` / `kop_region_cnt/cap` in env | **核心必要** | verifier 状态扩展 |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +115 | `struct bpf_kop` forward decl | **核心必要** | |
| @@ +584 | `btf_try_get_kop_desc()` 声明 | **核心必要** | |
| @@ +653 | non-BTF stub for `btf_try_get_kop_desc` | **可疑(微小)** | stub 内的 `(void)btf; (void)var_id; (void)desc;` 是不寻常的 cast-to-void 风格，大部分内核 stub 不这样写。不影响正确性但风格不一致。 |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +484 | `BPF_CALL_KOP` 宏 | **核心必要** | kop CALL 指令构造宏 |
...
| @@ +1383 | `BPF_PSEUDO_KOP_SIDECAR` / `BPF_PSEUDO_KOP_CALL` | **核心必要** | kop 伪指令编码 |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +240 | `struct btf_kop_desc` / `struct btf_kop_tab` | **核心必要** | BTF 端的 kop 描述符表 |
| @@ +277 | `kop_tab` in `struct btf` | **核心必要** | |
| @@ +1815 | `btf_free_kop_tab()` | **核心必要** | |
| @@ +1869 | 在 `btf_free()` 中调用 `btf_free_kop_tab` | **核心必要** | |
| @@ +7837 | EXT prog 的 `dst_prog` NULL check 添加 | **核心必要** | 防止 EXT 程序在 `dst_prog` 被清除后崩溃（REJIT 场景中 EXT 的验证路径需要） |
| @@ +8975 | `btf_kop_desc_cmp` / `btf_type_is_kop_desc` / `btf_resolve_kop_desc_id` | **核心必要** | kop BTF 解析逻辑 |
| @@ +8975 | `register_bpf_kop_set()` / `unregister_bpf_kop_set()` | **核心必要** | kop 注册/注销 |
| @@ +8975 | `btf_try_get_kop_desc()` | **核心必要** | kop 描述符查找 |
...
| @@ +199 | `bpf_free_kfunc_desc_tab()` 替代 `kfree()` + `bpf_free_kop_desc_tab` | **核心必要** | kfunc_tab 动态分配后需要两步释放(内部 descs + 外部 tab) |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +36 | `BPF_PSEUDO_KOP_CALL` case in `__func_get_name` | **核心必要** | disasm 输出 "kop-descriptor" |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +2380 | `__bpf_prog_put_noref` 中 `kvfree(orig_insns)` + kfunc/kop tab 释放 | **核心必要** | prog 释放时清理新增字段 |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +272 | `bpf_pseudo_kop_call()` helper | **核心必要** | |
| @@ +3173 | 删除 `MAX_KFUNC_DESCS` / `MAX_KFUNC_BTFS` 宏 | **核心必要** | 改为动态分配后不再需要 |
| @@ +3193 | `bpf_kfunc_desc_tab` / `bpf_kfunc_btf_tab` 改为动态数组 | **核心必要** | 支持更大数量的 kfunc/kop 描述符 |
| @@ +3193 | `bpf_kop_desc` / `bpf_kop_desc_tab` 新结构 | **核心必要** | |
| @@ +3193 | `ensure_desc_capacity()` 通用动态数组增长函数 | **核心必要** | |
| @@ +3193 | 前向声明 `kop_desc_cmp_by_imm_off` / `find_kop_desc` / `bpf_patch_insn_data` / `verifier_remove_insns` | **核心必要** | 解决前向引用 |
| @@ +3309 | `__find_kfunc_desc_btf` 中 lazy alloc `kfunc_btf_tab` + 删除 `MAX_KFUNC_BTFS` 限制 + 动态增长 | **核心必要（但有附带影响）** | 这是把 kfunc_btf_tab 从固定大小数组改为动态分配。虽然对 kop 也需要相同的 BTF 查找路径，但这个改动同时影响了纯 kfunc 路径。从功能角度仍然是必要的。 |
| @@ +3380 | `bpf_free_kfunc_btf_tab` 增加 `kvfree(tab->descs)` | **核心必要** | 动态数组释放 |
| @@ 新增 | `bpf_free_kfunc_desc_tab()` / `bpf_free_kop_desc_tab()` | **核心必要** | |
| @@ +3569 | `add_kfunc_call` 删除 MAX check，改用 `ensure_desc_capacity` | **核心必要** | |
| @@ 新增 | `fetch_kop_desc_meta()` / `add_kop_call()` | **核心必要** | kop 描述符获取和注册 |
| @@ 新增 | `kop_desc_cmp_by_imm_off()` / `find_kop_desc()` | **核心必要** | |
| @@ +3762 | `bpf_prog_has_kop_call()` | **核心必要** | |
| @@ +3793 | `bpf_jit_find_kop_desc()` / 一系列 kop verifier 函数 | **核心必要** | |
| @@ +3793 | `validate_kop_proof_seq()` | **核心必要** | 验证 kop proof sequence 安全性 |
| @@ +3793 | `scrub_restored_kop_aux()` | **核心必要** | |
| @@ +3793 | `alloc_kop_proof_regions()` / `lower_kop_proof_regions()` / `restore_kop_proof_regions()` | **核心必要** | kop proof region 替换/恢复机制 |
| @@ +3793 | `bpf_kop_has_native_emit()` / `build_kop_inst_seq()` / `bpf_jit_get_kop_payload()` | **核心必要** | |
| @@ +4153 | `add_subprog_and_kfunc` 扩展支持 kop | **核心必要** | |
| @@ +4700 | `disasm_kfunc_name` → `disasm_call_name` 重命名 + 扩展支持 kop | **核心必要（但包含重命名）** | 重命名 `disasm_kfunc_name` 为 `disasm_call_name` 属于 cleanup 但与功能变更一起做是合理的 |
| @@ +4700 | 变量 `func` → `t` 重命名 | **可疑/不必要** | 在 `disasm_call_name` 中把 `const struct btf_type *func` 改名为 `*t`，纯风格变更 |
| @@ +4700 | 添加 NULL check `t ? btf_name_by_offset(...) : "<invalid>"` | **可疑** | 新增了一个 NULL 检查。可能是因为 kop 的 BTF type 可能返回 NULL？如果是的话应该是必要的安全检查，但也可能是防御性代码。 |
| @@ +4950 | `backtrack_insn` 中 sidecar 跳过 | **核心必要** | |
| @@ +21573 | `check_kop_sidecar_insn()` | **核心必要** | verifier 对 sidecar 指令的检查 |
| @@ +21573 | `do_check_insn` 中 sidecar 入口 | **核心必要** | |
| @@ +21665 | `do_check_insn` CALL 指令路径扩展支持 kop | **核心必要** | |
| @@ +23455 | `jit_subprogs` 传播 `kop_tab` | **核心必要** | |
| @@ +23640 | `fixup_call_args` 中 kop non-JIT 检查 | **核心必要** | |
| @@ +23964 | `do_misc_fixups` 中 sidecar+kop 展开逻辑 | **核心必要** | |
| @@ +24301 | `do_misc_fixups` CALL fixup 中跳过 kop | **核心必要** | |
| @@ +26596 | `bpf_check` 中提前调用 `check_btf_info_early` + `add_subprog_and_kfunc` + `lower_kop_proof_regions` | **核心必要** | kop proof lowering 必须在 verifier 主循环之前 |
| @@ +26596 | 删除原位置的 `check_btf_info_early` / `add_subprog_and_kfunc` 调用 | **核心必要** | 对应上面的提前调用 |
| @@ +26596 | `env->explored_states = NULL` | **可疑** | 在 `kvfree(env->explored_states)` 后置 NULL。可能是防御性代码，防止 `restore_kop_proof_regions` 路径出错时 double free。合理但属于安全加固。 |
| @@ +26596 | `restore_kop_proof_regions` 调用 | **核心必要** | |
| @@ +26810 | `kvfree(env->kop_regions)` | **核心必要** | 清理 |
...
| Hunk | 内容 | 判定 | 说明 |
|------|------|------|------|
| @@ +2 | `extmod-btf-global-var-y` / `extmod-btf-global-var-$(if $(KBUILD_EXTMOD),y)` | **核心必要** | 外部模块编译时需要 `global_var` BTF feature 才能让 pahole 生成 kop 描述符的 BTF_KIND_VAR |
| @@ +16 | `$(extmod-btf-global-var-y)` append 到 `--btf_features` | **核心必要** | |

**结论**: 必要。kop 通过 BTF_KIND_VAR 注册描述符，需要此 pahole flag。
...
| 7 | `include/linux/btf.h` | `btf_try_get_kop_desc` stub 中的 `(void)` casts | 非常规内核风格，大部分 stub 使用 `__always_unused` 或直接忽略参数 |
...
### Patch 2: kop 框架 — BTF 注册 + verifier 支持
- `include/uapi/linux/bpf.h`: `BPF_PSEUDO_KOP_SIDECAR` / `BPF_PSEUDO_KOP_CALL`
- `tools/include/uapi/linux/bpf.h`: 同上
- `include/linux/bpf.h`: `struct bpf_kop` / `bpf_kop_set` / 注册 API / kop_tab
- `include/linux/bpf_verifier.h`: `bpf_kop_region` / verifier env 扩展
- `include/linux/btf.h`: `btf_try_get_kop_desc`
- `include/linux/filter.h`: `BPF_CALL_KOP`
- `kernel/bpf/btf.c`: kop BTF 注册/查找全部
- `kernel/bpf/verifier.c`: kop 相关全部（动态数组改造、proof lowering/restoring、fixup 等）
- `kernel/bpf/disasm.c`: kop disasm
- `kernel/bpf/core.c`: kfunc_tab/kop_tab 释放
- `scripts/Makefile.btf`: `global_var` for extmod

### Patch 3: kop JIT 后端 — x86 + ARM64
- `arch/x86/net/bpf_jit_comp.c`: `emit_kop_desc_call` / sidecar skip / kop call case
- `arch/arm64/net/bpf_jit_comp.c`: `emit_kop_desc_call_arm64` / sidecar skip / kop call case
...
2. **kop 机制** — 新的 kernel instruction 描述符框架（BTF 注册 + verifier proof lowering + JIT 原生 emit）
