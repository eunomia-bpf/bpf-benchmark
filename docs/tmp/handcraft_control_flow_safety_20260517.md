...
- verifier proof kind;
- verifier proof branch delta after kop materialization;
...
rejected by the verifier or by kop descriptor validation, but they must not be
able to crash the kernel or make verifier proof differ from final native
execution.

## Branch KOperation Boundary

The branch kop module may remain available as a machine-instruction selector,
...
kop materialization depends on kernel proof expansion and later kernel BPF
...
was a stale or incorrect user-space model of kop proof lengths.

## Fixed-3 Bundle Candidate

One possible kernel-side design is to make every kop a fixed 3-insn bundle:

```text
0: sidecar payload
1: carrier jump
2: call BPF_PSEUDO_KOP_CALL
```
...
- the third instruction remains the kop call, so the existing kfunc/BTF
  descriptor lookup still finds the kop module descriptor;
...
The expected kernel work is small if all koperation are fixed length:

- `struct bpf_kop_region.orig[2]` becomes `orig[3]`;
...
- branch kop JIT uses the normal JIT address table (`addrs[]` on x86,
...
It does not emit branch kop payloads and therefore does not emit
...
over the shadow flag stack written by `cmp/test` koperation. That fallback is for
...
layout. It does not model kop proof expansion length, because that is a
...
If native parity requires branch koperation, use the fixed-3 carrier design rather
