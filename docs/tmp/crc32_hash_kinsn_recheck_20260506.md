# CRC32 + Hash kinsn Recheck for Production Corpus
...
Primary question: did the 2026-03-29 CRC32 study miss enough CRC32, PCLMULQDQ, or hash-acceleration sites in the production corpus to justify a kinsn pass?
...
14. A Jenkins semantic kinsn would be difficult because it is a three-word state transform, not a single-register instruction replacement.
...
3. Required prior CRC document found at actual path: `docs/tmp/20260329/crc32_kinsn_research_20260329.md`.
4. Required prior bit-ops document found at actual path: `docs/tmp/20260329/bit_ops_kinsn_research_20260329.md`.
5. Required prior ANDN document read: `docs/tmp/andn_kinsn_research_20260430.md`.
...
9. A secondary note reports Jenkins mix+final macro bodies because a semantic Jenkins kinsn may need to cover both.
...
15. User-space hash/permutation code is not counted as a BPF kinsn opportunity.
...
9. A scalar CRC32C kinsn pass should not be added for these apps.
10. If future corpus evidence changes, a scalar CRC32C pass should remain restricted to exact semantic CRC32C patterns.
11. Estimated kinsn module delta if revived: x86 120-180 LoC.
12. Estimated kinsn module delta if revived: arm64 100-160 LoC.
...
9. A generic buffer kinsn would need memory range semantics beyond a scalar peephole.
10. A generic buffer kfunc would require verifier and helper ABI design outside this low-risk kinsn class.
...
14. The existing kinsn model is more naturally suited to single-result scalar instructions.
15. A single Jenkins-mix kinsn would need to return or update three values.
16. Packing three 32-bit words into one 64-bit result is not enough.
17. Multiple kinsns for one macro would reduce the advantage.
...
13. Estimated x86 module delta for a Murmur round kinsn: 120-180 LoC.
14. Estimated arm64 module delta for a Murmur round kinsn: 120-180 LoC.
...
9. No BPF kinsn pass should target Maglev permutation in this corpus.
...
9. A generic string compare kfunc would be substantially larger than a scalar kinsn.
10. Current verdict: do not implement.

## D. Cross-check vs Prior Research

### Versus `crc32_kinsn_research_20260329.md`
...
### Versus `bit_ops_kinsn_research_20260329.md`
...
### Versus `andn_kinsn_research_20260430.md`
...
7. Action: leave CRC32 kinsn out of the near-term implementation queue.
...
### 3. Keep Maglev and string compare out of kinsn scope
...
9. Action: do not build Maglev or string-equality kinsns from current evidence.
...
16. OTel userspace Go FNV/CRC references are not BPF kinsn targets.
...
10. No string kinsn is justified by this evidence.
...
3. Counting Jenkins/Murmur now still does not produce a strong hash-kinsn case.
