# Formal Model of Current kop Instances

Date: 2026-03-26  
Status: current in-tree formalization  
Scope: current in-tree kop descriptors and the site conditions required by the current rewrite pipeline

## 0. Why a separate formal model

[`docs/kop-design.md`](./kop-design.md) explains the implementation boundary.
This file makes the current semantics explicit enough to reason about:

- what a kop "is" in the current tree
...
The verifier-visible denotation of a kop site is:
...
A kop site `s` in a BPF program occupies a contiguous, single-entry
...
- `Admissible_K^a(s, p)`: extra conditions the daemon rewrite pipeline enforces
  before emitting a kop site (not checked by the kernel verifier)
...
```text
P_src       original program with sidecar + BPF_PSEUDO_KOP_CALL pairs
    │  lower_kop_proof_regions()
    ▼
...
P_low_ok    verified: all proof sequences are safe
    ...
    ▼
P_restored  original sidecar + kop_call pairs restored
    │  do_misc_fixups()
    ▼
P_fix       sites with native emit retained; sites without → lowered to BPF
    │  arch JIT
    ▼
P_jit       native machine code; kop sites compiled via emit callbacks
```

**Scope.** This formalization proves that `P_jit` refines `P_low` — that is,
native emit at each kop site produces the same observable result as the
proof sequence the verifier analyzed. It does **not** prove that the daemon's
rewrite from an original BPF idiom to a kop site is semantics-preserving;
...
to kop sites `s_1, …, s_n` in `P_src`. Let `P_jit` be the JIT-compiled
...
loops, so kop sites can execute more than once).
...
Between kop sites, `P_jit` and `P_low` execute identical BPF instructions,
so their states remain synchronized. Since kop sites are non-overlapping and
single-entry single-exit (no backward jumps within a proof sequence, enforced
by `validate_kop_proof_seq()`), every inter-site segment sees identical
...
|---------|-------|-------------|
| `instantiate_insn` returns error | `lower_kop_proof_regions()` | Verifier rejects the program |
...
**Important:** the BPF interpreter does not understand `BPF_PSEUDO_KOP_CALL`.
After `restore_kop_proof_regions()`, the program contains raw kop encodings
that only the arch JIT can execute. There is no late interpreter fallback for
kop programs. If JIT compilation fails for a program with retained kop
sites, the program cannot run — it is rejected at load time. This is enforced
by requiring `jit_requested` for programs containing kop sites.
...
cc = KOP_SELECT_COND_NEZ (= 0) ; only "not-equal-to-zero" is currently implemented
```

The `cc` field is a condition-code discriminator, not a reserved constant. The
current implementation defines only `KOP_SELECT_COND_NEZ = 0` (select on
...
kop. The kop only computes the chain predicate.
...
- the daemon `endian_fusion` pass compensates by materializing `base + offset`
  into a register and then emitting a packed kop call with offset `0` when
  needed
...
| Layer | Claim | Status |
|-------|-------|--------|
| (a) Rewrite soundness | Daemon rewrite from BPF idiom to kop site preserves program semantics | Engineering argument in daemon passes; not formally modeled here |
...
kop emit behaves identically to the original BPF program on all observable
...
|------------|-------------|---------|
| Proof-sequence structural safety | Kernel (`validate_kop_proof_seq`) | Yes |
...
The daemon is in the TCB. If a daemon rewrite pass emits a kop site where
...
However, a daemon bug cannot cause a kernel crash through kop alone: native
