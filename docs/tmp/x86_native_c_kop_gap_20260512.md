# x86 native C instruction gap vs current KOperation passes
...
what still differs from the current BPF bytecode -> KOperation -> x86 path?
...
1. Katran jhash is a `rotate32` problem, not a `jhash` KOperation problem. Native
...
3. `cmov`/`setcc` are very real in direct native output: 661 `cmov*` and 475
   ...
   compare-select KOperation could.
4. `lea` is massive in native output, but the count alone does not justify a
   `bpf_lea` KOperation. Much of it is x86 address-mode selection or flagless add
   ...
   than a bpfopt KOperation problem.
5. `bextr`, `lzcnt`, `tzcnt`, and `prefetch*` are not emitted by direct native
   ...
   KOperations. `popcnt` exists, but only 7 sites in Tracee, so it needs a bytecode
...
- Endian: source/native can fuse a memory load and byte swap into `movbe`. The
  current KOperation recovers "endian load" semantics but emits load+`bswap`/`rol`.
- LEA/addressing: native x86 can fold arithmetic into memory operands or
  ...
  sites may not correspond to any bytecode-level KOperation opportunity.
- SIMD: native x86 can use vector registers for fixed copies/reductions. BPF
  verifier state has no vector register model, so a general SIMD KOperation is not a
...
2. Emit the exact native instruction family when the KOperation target guarantees it.
...
The closest native-like KOperation plan is:
...
Do not build a `jhash` KOperation. It would be less reusable, would couple the pass
...
Add a separate compare-select KOperation rather than stretching `bpf_select64`:
...
KOperation should come later, after proving boolean values are consumed as booleans
...
- Direct native also has 4 `bswap` and 6 `rol` sites, but `movbe` is the bigger
  real-app signal.
- This census counts all `movbe` mnemonics, including stores. The current KOperation
...
KOperation target is exposed for a MOVBE-capable x86 target. If the module cannot
...
discovery logs, but old artifacts predate some current KOperation target shape, so a
...
kernel-JIT peephole/addressing improvement, not a bpfopt KOperation. If there are many
...
These are real native x86 instructions, but they are not automatically KOperation
gaps. BPF already has variable shifts, and the kernel JIT can lower explicit BPF
variable shifts to BMI2 where supported. Add a KOperation only if JIT dumps show the
current kernel path is failing to select BMI2 for an existing BPF variable-shift
operation.

Recommendation:

No KOperation work now. Verify with JIT dumps before spending code here.
...
BPF has no scalar popcount instruction. A KOperation could only help if the BPF
...
This is real native output but not a good general KOperation target. BPF verifier
state has scalar registers, not vector registers. A generic vector KOperation would
require vector-state ABI decisions and save/restore rules that are much larger
than the current KOperation model.

Recommendation:

Do not build generic SIMD KOperations. For copy-like cases, continue with scalar
...
No KOperation priority from this evidence.
...
1. Keep `bpf_rotate32` as the Katran jhash solution. Do not add a `jhash`
   KOperation. Use `rotate,dce` for final bytecode cleanup validation.
...
3. Design compare-select (`cmp/test + cmovcc`) as a new KOperation ABI. Current
