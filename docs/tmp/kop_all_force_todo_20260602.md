# kop all-force TODO, 2026-06-02

Goal: make both LLVM backend and `bpfopt` default to force/apply-all kop
selection, then validate the full x86 kop corpus path app by app. YAML is only
...
- Use the full kop pass set for x86 smoke:
...
- [x] `bpfopt --pass <kop>` injects `-bpf-kop-mode=all=force` unless an
  explicit `--kop-mode`/`-bpf-kop-mode=` override is present.
...
- [x] `make host-kop-x86` passed after the LEA module change.
- [x] `cmake --build bpfopt/llvm/build-kop -j2` passed.
...
- [ ] Run x86 KVM `tracee/monitor` with the full kop pass set.
- [ ] Run x86 KVM `tetragon/observer` with the full kop pass set.
- [ ] Run x86 KVM `otelcol-ebpf-profiler/profiling` with the full kop pass set.
- [ ] Run x86 KVM `bcc/set` with the full kop pass set.
- [ ] Run x86 KVM `katran` with the full kop pass set.
- [ ] Run x86 KVM `cilium/agent` with the full kop pass set.
- [ ] Run combined x86 KVM corpus with the full kop pass set after all apps
  pass individually.
- [ ] Record per-app policy table from raw loadtime reports.
- [ ] Generate `docs/eval_kop.md` and figures from raw results.
