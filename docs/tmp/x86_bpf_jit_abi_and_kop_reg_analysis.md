# x86 BPF JIT ABI and kop Register Analysis
...
means for handcraft/native-to-kop conversion.
...
## Current kop Execution Model

Current kop descriptors already have a native form:
...
For x86, the JIT calls `emit_x86()` from `emit_kop_desc_call()` and splices
...
If a kop has no native emit callback, the verifier/fixup path can replace the
...
"proof-only macro" path, not a machine kop path. For instructions already
covered by ordinary BPF, the cleaner policy is even simpler: user space should
emit ordinary BPF directly and not emit a kop at all.

So there are three possible mechanisms:

| Desired behavior | Mechanism | Where it belongs |
|---|---|---|
| Use existing BPF JIT lowering | Emit ordinary BPF, no kop | Normal bpfopt pass output |
| Keep verifier proof but no native machine instruction | `instantiate_insn()` only; kernel patches to BPF proof sequence | Rare macro/proof-only cases, not normal machine kop policy |
| Emit a specific native instruction not expressible by BPF | `instantiate_insn()` plus `emit_x86()`/`emit_arm64()` | kop module |

For normal bpfopt passes, the BPF-to-x86 equivalence table should therefore
live in user-space selection logic, not in the kop module:
...
machine instruction is missing. It should emit one machine-level kop per
...
- The converter is mechanical: one parsed native instruction becomes one x86
  kop selector plus payload. Unsupported instructions are emitted as explicit
...
- Verifier scratch registers and shadow-register save/restore are module
  responsibilities. If a kop needs scratch state for its proof, the
...
- One kop native emitter emits exactly one x86 instruction. There is no
...
- `cmp/test` and `jcc/cmov/setcc` are separate native instructions. The
  ...
  execution.
- Program-level control flow is user-space-owned. For future `jcc/jmp` koperation,
...
- Ordinary BPF-register `movabs` may stay as `BPF_LD_IMM64` when dump
  ...
  `r9/r10/r11/r12/rbp/rsp`, because those destinations need a kop payload and
...
- Native stack/frame instructions such as `push`, `pop`, and `mov rbp,rsp` are
  ...
  machine-level koperation, `rsp` is a first-class payload register (`15`), and
...
- Unit tests should check encoded ABI/layout or real selector behavior. Tests
  ...
  `dst == condition` belong in the main machine-kop suite as real
  instruction-sequence tests, not in standalone compatibility binaries.

## Hidden Register Users

| Native register/state | Kernel use | Consequence for kop |
|---|---|---|
| `r10` / `BPF_REG_AX` | Constant blinding, verifier rewrites, pointer-sanitize rewrites, probe-memory checks, atomic lowering | Safe as a native kop register only inside a region where ordinary BPF/JIT rewrites cannot clobber it live. Otherwise use stack shadow and reload around boundaries. |
...
which ones need kop coverage if we want native-asm parity.
...
This means a machine-level kop can be 1:1 with a final x86 instruction only if
...
- If existing BPF bytecode already makes the x86 JIT emit the same single x86
  ...
  understands it, register provenance is preserved, and there is no kop call
  transport risk.
- A kop is justified when BPF cannot express the instruction shape: flags
  producer/consumer split, `cmov/setcc`, SIB indexed addressing, `lea`,
  `popcnt`, BMI, `prefetch`, `movbe`, `shld/shrd`, etc.
- A kop can still be useful in strict handcraft/native-parity mode even if a
  semantic BPF equivalent exists, but that should not drive normal bpfopt pass
  rewrites.

Current x86 koperation that are redundant for normal pass output:

| Current kop | Existing BPF equivalent | Recommendation |
|---|---|---|
...
| `bpf_x86_xorl with reg-reg payload` | `BPF_ALU XOR X` emits `xor dst, src` | Redundant unless strict flag/native-reg simulation needs a kop. |
...
Current x86 koperation that are conditionally useful, but should not be used merely
because they exist:

| Current kop | Why conditional |
|---|---|
...
| `bpf_x86_movzbl`, `bpf_x86_movzwl` with rr payload | Same-reg zero-extension has BPF endian/ALU alternatives; cross-reg low-byte/low-word extraction may still need explicit kop if exact `movzx` matters. |
...
Current x86 koperation that are genuinely needed for native-shape coverage:

| Current kop | Why BPF is insufficient |
|---|---|
...
- `cond_select`: keep `test/cmov` koperation; the surrounding `movq rr-payload` koperation are
  unnecessary because ordinary BPF `MOV64_REG` emits `mov` and does not clobber
  flags.
- `extract` on x86: current `shrq_imm + andl_imm32` kop lowering is likely
...
- `endian_fusion` on x86: `mov*_mem + bswap*` mostly duplicates `LDX + BPF_END`
  lowering. It should only use kop if it proves a real x86 shape not reachable
...
## Preventing Duplicate koperation

The risk is that future changes re-add machine koperation for x86 instructions that
...
|---|---|---|---|
| Ordinary-BPF exact form | One existing BPF instruction already expresses the operation, and the x86 JIT emits the same instruction shape | Emit ordinary BPF directly | Do not add a kop |
| Semantic BPF expansion | BPF can prove the semantics but needs multiple BPF instructions or emits a different x86 instruction | Use ordinary BPF when native shape does not matter | kop only for strict handcraft/native parity |
| Machine-only | BPF cannot express the native instruction shape or hidden state | Emit kop | Must have verifier proof plus native emit |

Examples of ordinary-BPF exact forms:

| Candidate x86 kop | Canonical BPF |
|---|---|
...
These forms are not koperation. Normal passes and the handcraft converter should
...
- bpfopt passes use it to decide "ordinary BPF or kop";
- the native-asm-to-handcraft converter uses it to decide whether an x86
  instruction can be represented by canonical BPF;
- tests can assert that redundant koperation are not emitted by normal passes.
...
The current out-of-tree mechanism registers each kop as an empty kfunc and
uses BTF name/id discovery as the transport. That is acceptable for a small
number of experimental koperation. It does not scale well if every x86 instruction
...
- descriptors, reg use/def metadata, tests, and discovery paths grow with every
  redundant kop that should have been plain BPF.

If the project ever wants a full machine-kop IR, the better long-term core
shape is a numeric opcode/descriptor registry rather than BTF-per-kfunc as the
primary identity:

- stable opcodes such as `BPF_KOP_X86_CMOVNEQ`;
...
machine-only. For handcraft/native parity, a full machine-kop IR can be useful
...
a real machine kop with explicit payload schema and proof code.
...
- `cmp/test/add/sub/...` koperation update module-owned stack slots representing
...
- User space should not synthesize a separate "adjacent cmp/test proof" payload
  ...
  consumer koperation; the module owns the verifier-facing flag state.

But that only proves the BPF simulation. The final x86 path still emits one
machine instruction per kop and reads real CPU flags. Therefore:

- A flag-producing kop and a flag-consuming kop must remain in one physical
  flag-live region.
...
2. Use known generated lengths (`1` for ordinary BPF, `2` for sidecar+kop
...
payload for one kop. For control-flow koperation, the payload is allowed to carry
...
This means kop proof sequences do not need a special "jump stays inside the
local proof region" rule. That rule was useful for an isolated peephole-kop
...
reject constructs that would corrupt the kop transport ABI itself, such as
...
- user space records the native branch target and relocated BPF target in the
  payload;
- the kop decoder validates that the raw native branch bytes encode the same
  target relation;
- branch targets must land on generated instruction boundaries, never on the
  `BPF_PSEUDO_KOP_CALL` half of a sidecar/call bundle;
- the full verifier proves the proof-lowered BPF CFG.

Until branch koperation cover every needed form, verifier-visible BPF branches
...
This is the "more absolute" kop design:

- User space emits a kop stream mechanically from native x86 asm.
- Each kop has one final x86 instruction.
...
- Hard to make safe in mixed BPF/kop programs.
...
- "One kop = one final x86 instruction" is achievable only after introducing
  region-level constraints and bridge code. The bridge code is not part of the
  kop, but it is still extra machinery.
...
- avoid cross-instruction physical flags unless represented by adjacent koperation
...
- koperation can focus on instruction-selection gaps: `lea`, `cmov`, `setcc`,
...
| Question | Full 16-reg kop simulation | LLVM restricted to 10 BPF data regs |
|---|---|---|
...
| Kernel module complexity | High: many machine koperation plus region rules and ABI bridges | Moderate: targeted koperation for missing x86 instructions |
...
| Production suitability | Risky unless restricted to kop-only regions | Better fit |
...
but it should be treated as a kop-only region experiment:
