# Katran ARM64 kop matched-count gap - 2026-05-06
...
| Run | App | Pass invocations contributing to kop counts | prefetch | cond_select | endian_fusion | bulk_memory | extract | rotate | wide_mem | ccmp |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| ARM64 `aws_arm64_corpus_20260506_193211_995298` | otel | all 12 programs reached the kop pass pipeline | 372/372 | 31/0 | 0/0 | 0/0 | 0/0 | 0/0 | 137/132 | 0/0 |
...
| ARM64 | `balancer_ingres` | error at initial noop ReJIT | noop only | no kop matcher ran |
...
### Target kop names expected from the pass plan
...
From HEAD pass metadata and enabled-pass configs, the target-dependent kop names are:

| Run | Enabled target-dependent passes | Target kop names expected in `target.json` |
| --- | --- | --- |
...
`wide_mem` has `META_NONE` in HEAD and does not request `${TARGET}`. The ARM64 `bpf_ldp` module is loaded by the module loader, but no HEAD pass metadata in this plan uses it as a target kop.

### Evidence the ARM64 target was not sparse

- ARM64 katran `xdp_root` and `healthcheck_enc` both successfully invoked all target-dependent bpfopt passes, including rotate, cond_select, prefetch, extract, endian_fusion, bulk_memory, and ccmp. Since `bpfopt` validates required koperation before running each single pass, these successes imply the relevant target names were present for those per-program target files.
...
The daemon does not build one startup target.json. The runner sends one `kop_probes` list per execute-plan request, built as the union of koperation used by the selected passes. The daemon then re-probes kernel BTF and writes a target file per program workdir if `${TARGET}` is referenced. The names should be stable within a run unless kernel BTF/module state changes; per-program `call_offset` values can differ because the daemon shifts module call offsets after the program's map-fd prefix.

## 3. Code citations

### Target and probe construction

- `runner/libs/rejit_plan.py:102-130` builds `kop_probes` from the union of selected pass metadata.
- `runner/libs/rejit_plan.py:151-157` puts the same step list and `kop_probes` list into the daemon `execute_plan` payload.
- `daemon/src/server.rs:207-262` parses the request's `kop_probes`.
- `daemon/src/commands.rs:456-475` creates `target.json` inside the per-program workdir when any step references `${TARGET}`.
- `daemon/src/commands.rs:460-475` probes target koperation before the pass loop and stores the probed koperation for the ReJIT fd array.
...
- `daemon/src/bpf.rs:84-98` builds the target JSON from detected arch, features, and probed koperation.
- `daemon/src/bpf.rs:166-214` enumerates kernel BTF objects to find kop functions.
...
- `bpfopt/crates/bpfopt/src/passes/mod.rs:90-96` declares target koperation for rotate, cond_select, ccmp, extract, endian_fusion, bulk_memory, and prefetch.
...
- `bpfopt/crates/bpfopt/src/main.rs:340-350` runs one pass by building a context, validating required koperation, then executing the pass pipeline.
- `bpfopt/crates/bpfopt/src/main.rs:451-470` validates required koperation for target-dependent passes before matcher execution.
- `bpfopt/crates/bpfopt/src/main.rs:647-665` reads `target.json`, applies target features, and builds the kop registry.
- `bpfopt/crates/bpfopt/src/main.rs:749-780` resets feature booleans from target features and maps target kop entries into registry IDs/call offsets.
...
Dominant cause: ARM64 katran's large `balancer_ingres` program fails the initial `noop` ReJIT, so the kop matchers never run on the program that produces almost all x86 katran matches. Confidence: high.

This is a pass-coverage gap, not primarily a matcher-enumeration or kop-registration gap. The x86 aggregate includes `balancer_ingres` matcher reports before a later prefetch ReJIT failure. The ARM64 aggregate excludes all target-dependent reports for `balancer_ingres` because it stops at `noop` with `EOPNOTSUPP`.
...
4. Hypothesis 2, target.json kop list differs per app/prog: weak. Target files are not persisted, but successful target-dependent pass validation on ARM64 katran's reached programs and all ARM64 otel programs implies the target was not globally sparse.
...
- Target/probe stability axis: compare module BTF discovery, target kop names, and feature lists across EC2 instances and apps when investigating true registration drift.
