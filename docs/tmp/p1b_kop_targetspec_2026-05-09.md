# P1-B kop TargetSpec consolidation

Date: 2026-05-09

## Summary

`bpfopt/crates/bpfopt/src/kop.rs` now owns the single kop target table:

```rust
pub struct TargetSpec {
    pub canonical_name: &'static str,
    pub registry_slot: KopSlot,
...
- `KopRegistry`: stores `btf_ids: [i32; KopSlot::COUNT]` and `call_offsets: [i16; KopSlot::COUNT]`; all name lookups go through `TargetSpec.registry_slot`.
- `main.rs`: target JSON and `--koperation` parsing use `target_spec_by_name()` for canonicalization; required koperation are read from pass metadata instead of a second pass-name match.
- `PASS_REGISTRY`: `KopRef` stores only the canonical target name and resolves `json_name` / `probe_aliases` through `TargetSpec` serialization.
- `utils.rs`: proof-region dispatch iterates `KOP_TARGETS`, matches by slot BTF ID and slot call offset, then calls the proof decoder selected by `TargetSpec.proof_layout`.

## Daemon Decision

No daemon mirror table was added.

Reason: `daemon/src/server.rs` parses runner-provided `kop_probes[]`, and `daemon/src/bpf.rs` probes only that request list. There is no daemon-side hardcoded target metadata to consolidate. This keeps the daemon independent from `bpfopt` and avoids a path dependency.
...
| `bpfopt/crates/bpfopt/src/kop.rs` | 0 | 46 | +46 |
...
That command reports net `-436` over tracked files. The table above includes the new untracked `kop.rs` file, giving the P1-B core-file total `-100`.
...
- No hardcoded BTF IDs; IDs still come from target JSON or `--koperation`.
...
`PASS_REGISTRY` still carries one small canonical-name slice per kop pass. A later cleanup could derive all kop pass metadata directly from `KOP_TARGETS.required_by_pass`, while preserving explicit pass ordering and non-kop metadata in `PASS_REGISTRY`.
