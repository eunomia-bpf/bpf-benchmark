# KOperation-only Corpus Smoke Summary (2026-05-04)

## Setup
- **Kernel HEAD**: `81cb8848b` (BUG_ON → -EBUSY return; no global REJIT swap mutex)
- **Main repo HEAD**: `d5d93292`
- **Passes**: `rotate,cond_select,extract,endian_fusion,bulk_memory,prefetch` (6 kop x86)
...
### Comparison: kop-only vs default-11-pass (tetragon)
| Pass set | Applied | Geomean | Wins/Losses |
|---|---|---|---|
| kop 6-pass | 241/287 (84%) | **0.873** | 24 / 8 |
| default 11-pass (no mutex) | 227/287 (79%) | 1.210 | 7 / 25 |

→ Default-11 introduces extra passes (const_prop, dce, wide_mem, bounds_check_merge, skb_load_bytes, map_inline) that hurt some progs in tetragon scenario. **kop-only is markedly better** for tetragon.
...
3. **Default 11-pass perf regression in tetragon**: investigate which non-kop pass causes regression (likely const_prop / dce on 60-200ns retkprobe hot path).
