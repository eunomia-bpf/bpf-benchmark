...
- `ADC/SBB`：语义上是合格的 x86 kop 候选，但当前 corpus 几乎没有需求信号。短期不值得进入默认 pipeline；更适合作为 future work 或专项 microbenchmark。
...
2. 核对当前 in-tree kop 集合、daemon registry/discovery、x86 kop module 实现。
...
当前 formal model 里的 in-tree kop 集合只有：
...
这很重要，因为当前树里能成立的 kop 候选都同时满足两件事：
...
`RDTSC` 不是好的默认 kop 候选，也不是好的透明 REJIT pass 候选。
...
### 5. 从 kop 设计角度看，ADC/SBB 其实是“可以做”的

和 `RDTSC` 不同，`ADC/SBB` 在抽象层面是能放进当前 kop 模型里的。
...
从这个角度说，它是一个“语义边界正确”的 x86 kop 候选。
...
`ADC/SBB` 在语义上是合格的 kop 候选，但在当前树里没有足够的 workload 支撑。
