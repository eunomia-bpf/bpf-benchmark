# 2026-03-29 bit-ops kop research
...
- 在当前 corpus 里，`POPCNT`、`CLZ/CTZ`、`PEXT/PDEP` 都没有扫描到可支撑投入的模式站点，主结论是：**现在不值得做新的 bit-op kop。**
...
1. `18:32` 那次 session 已经确认：
   - 现有 kop plumbing 的主路径在 `daemon/src/pass.rs`、`daemon/src/kfunc_discovery.rs`、`daemon/src/passes/*.rs`、`module/x86/*.c`、`module/arm64/*.c`。
   - 现有 x86/arm64 模块和 pass 的代码体量，可以作为新 kop 复杂度的 baseline。
...
但这组站点**不构成新增 kop 机会**。当前 upstream x86 JIT 已经在 `vendor/linux-framework/arch/x86/net/bpf_jit_comp.c:2022-2046` 中，对 `BPF_LSH|X / BPF_RSH|X / BPF_ARSH|X` 直接走：
...
ARM64 侧也是同样逻辑：变量移位本来就能直接映射到普通 ISA 指令 `LSLV/LSRV/ASRV`，不需要为了这类语义再加一层 kop。

## 实现复杂度

### 需要几个 kop

- 若硬要覆盖这批新语义，最少是 `5` 个 kop：
  - `bpf_popcnt64`
  - `bpf_clz64`
  - `bpf_ctz64`
  - `bpf_pdep64`
  - `bpf_pext64`
- `SHRX/SHLX` 不需要新 kop，因为原生 BPF ISA 已经有变量移位 opcode，x86 JIT 也已经直接发 BMI2 指令。
...
这些 baseline 已经说明：哪怕是“语义相对简单”的 kop，也不是几十行的小补丁。
...
新增任何一个 kop，至少都要改下面这些固定路径：

- `daemon/src/pass.rs`
  - 给 `KopRegistry` 增加新 `*_btf_id`
...
- `SHRX/SHLX` 的问题则相反：site 很多，但已经被上游 JIT 吃掉了，再做 kop 只会重复已有优化。
...
- **`SHRX/SHLX/SARX`: 不值得做 kop。**
  - 理由不是“site 少”，而是“site 多但上游 x86 JIT 已经做了”。
- **`PDEP/PEXT`: 不值得做。**
  - 当前 corpus 严格候选为 `0`。
  - 只有 `loxilb` 里有 `24` 个单-bit 插入样式，远达不到为 `PDEP/PEXT` 建一整套 kop 的门槛。
...
对当前 `bpf-benchmark` 项目来说，这组 bit-op 里**没有一个值得进入近期 kop 路线图**。
