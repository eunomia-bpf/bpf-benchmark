...
## Coverage
- Mandatory files with findings: `corpus/orchestrator.py`, `e2e/run.py`, `e2e/cases/bcc/case.py`, `e2e/cases/tracee/case.py`, `e2e/cases/tetragon/case.py`, `e2e/cases/bpftrace/case.py`, `e2e/cases/scx/case.py`, `runner/libs/rejit.py`, `runner/libs/results.py`, `runner/libs/bpf_stats.py`, `runner/libs/workload.py`, `runner/libs/agent.py`, `runner/libs/run_artifacts.py`, `runner/src/kernel_runner.cpp`, `runner/src/common.cpp`, `runner/Makefile`, `tests/python/test_micro_driver.py`, `tests/python/test_runner_results.py`, `tests/python/test_case_common.py`, `tests/python/test_corpus_orchestrator.py`.
- Mandatory files checked with no requested issues found: `corpus/config/macro_apps.yaml`, `corpus/config/benchmark_config.yaml`, `runner/libs/app_runners/__init__.py`, `runner/libs/app_runners/bcc.py`, `runner/libs/app_runners/tracee.py`, `runner/libs/app_runners/tetragon.py`, `runner/libs/app_runners/bpftrace.py`, `runner/libs/app_runners/scx.py`, `runner/libs/app_runners/process_support.py`, `runner/libs/app_runners/kubearmor.py`, `runner/libs/app_runners/libbpf_bootstrap.py`, `runner/libs/app_runners/systemd.py`, `runner/libs/app_runners/tubular.py`, `runner/libs/app_runners/xdp_tools.py`, `runner/libs/app_runners/xdp_tutorial.py`, `runner/libs/app_runners/coroot_node_agent.py`, `runner/libs/app_runners/datadog_agent.py`, `runner/libs/vm.py`, `runner/libs/__init__.py`, `runner/CMakeLists.txt`, `Makefile`, `tests/python/test_bcc_case.py`, `tests/python/test_bcc_runner.py`, `tests/python/test_bpf_stats.py`, `tests/python/test_e2e_run.py`, `tests/python/test_rejit.py`, `tests/python/test_run_artifacts.py`, `tests/python/test_temp_paths.py`, `tests/python/test_tetragon_case.py`, `tests/python/test_tracee_case.py`, `tests/python/test_vm.py`.
...
- `tests/python/test_micro_driver.py:15-45` still locks in `program_object=...demo.bpf.o` and `micro_exec` command construction. This tests deleted/irrelevant object-path behavior rather than the app-runner architecture. Violates: `P2`, `P3`.
- `tests/python/test_runner_results.py:19-88` still asserts `canonical_object_name`, `objects_attempted`, and the old message text `"app_native reported exec_ns=0 in baseline and REJIT"`. That locks in removed object/app-native taxonomy. Violates: `P2`, `P3`, `P5`.
- `tests/python/test_case_common.py:271-301` accepts partial kinsn module-load success (`loaded_modules == ["bpf_endian"]`) instead of requiring a hard failure when expected modules are missing. It codifies the degraded behavior in shared lifecycle code. Violates: `P6`.
- `tests/python/test_corpus_orchestrator.py:55-57,200-203` only checks that `repeat` is echoed into the payload, not that the app actually runs `repeat` times. It locks in the current silent wrongness. Violates: `P6`.
...
- `tests/python/test_runner_results.py:67-80` the term `app_native` survives only in assertion text. Low severity, but it shows the rename away from the old `app_native/test_run` taxonomy is still incomplete. Violates: `P5`.
