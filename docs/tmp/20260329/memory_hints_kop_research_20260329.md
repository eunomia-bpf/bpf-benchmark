...
- 从机制上说，这两个方向都不是“因为需要插入新指令所以做不了”。当前 daemon pass 已经能插入 `kop_sidecar + call_kop`，并重建指令流、修 branch target、携带 BTF FD。
...
- 如果要把 memory hints 作为 BpfReJIT 的一个研究方向继续推进，应该优先保留 `Prefetch`，并明确把它定义成一个 runtime-guided 方向；`NT store` 不值得作为当前阶段的优先 kop。
...
2. 复核仓库内已有 PGO / kop 调研文档，特别是 `docs/tmp/20260320/pgo_policy_design_20260320.md`。
3. 复核 daemon 当前 pass / kop plumbing，确认“插入新指令”在现有框架里是否可行。
...
## 4. 当前 daemon / kop 框架是否支持“插入新指令”

答案是支持，困难不在 plumbing。

已经确认的机制证据：

- `daemon/src/passes/utils.rs:494-517` 已经有 `emit_packed_kop_call_with_off()` 和 `ensure_btf_fd_slot()`，可以发出 `kop_sidecar + call_kop`，并把目标 BTF FD 填进 `fd_array`。
- `daemon/src/passes/rotate.rs:140-185` 已经在实际 pass 里重建整段 instruction stream，并在替换后做 `fixup_all_branches()`。
- `daemon/src/pass.rs:457-488` 的 `PassContext` / `KopRegistry` 已经提供了 kop target、BTF FD 和平台能力入口。
...
- **helper lowering / specialized kop**
  - 如果想碰到 hash/LRU-hash 真正高 miss 的内部访问，最终更像是把 helper 语义下沉，或者新增一个在 kernel/JIT 空间里自行推导目标地址的 prefetch kop，而不是在 BPF-side 盲目 prefetch。
...
就当前 corpus 而言，第 1 条路的 direct site 太少，第 2 条路又已经不再是“小 kop”问题，而是 helper specialization 问题。
