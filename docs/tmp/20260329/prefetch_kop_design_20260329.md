# Prefetch kop 完整实现设计（2026-03-29）
...
- 新增一个单目标 kop：`bpf_prefetch(ptr)`。
- packed transport 继续沿用当前树的 `sidecar + BPF_PSEUDO_KOP_CALL`。
- x86 JIT emit 为 `PREFETCHT0 [ptr]`；ARM64 JIT emit 为 `PRFM PLDL1KEEP, [ptr]`。
- verifier 语义定义为纯 hint：
  - 不修改任何寄存器状态。
  - 不暴露任何内存访问。
  - 不产生返回值。
- 在当前已实现的 kop v2 tree 上，推荐把 canonical proof sequence 实现成单条 `JA +0` no-op。这样无需先落地完整 `bpf_kop_ops::model_call()` 基础设施，也能满足“verifier 看起来什么都没发生”。
...
1. 定义一个与现有 packed kop 风格一致的 `bpf_prefetch(ptr)`。
...
这正是 `Prefetch` 能体现 kop 价值的地方：固定内核 JIT 不知道哪些点值得 hint，daemon 可以按 runtime profile 选择性地插。
...
pc+1:   call kop            ; imm = BTF ID of bpf_prefetch
```
...
let insns = emit_packed_kop_call_with_off(payload, prefetch_btf_id, btf_slot);
```
...
这和 `docs/kernel-jit-optimization-plan.md §4.4`、`docs/tmp/20260323/kop_ops_design_20260323.md` 里定义的 `pure hint / no visible side effect` kop 是一致的。

#### 2.3.2 当前 tree 的落地方式

当前仓库里真实实现的 kop v2 还不是 `model_call()` 路径，而是：

- module 提供 `instantiate_insn(payload, insn_buf)`
- verifier 对 kop proof region 做 lowering
...
- 理想设计：`bpf_kop_ops::model_call()` 返回 empty effect。
...
const struct bpf_kop bpf_prefetch_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = 1,
	.max_emit_bytes = 6,
...
payload decode 与 x86 相同，寄存器映射复用 `kop_arm64_reg()`。
...
- `daemon/src/pass.rs`: `KopRegistry` 新增 `prefetch_btf_id`
...
distance 不是 kop payload 的一部分，而是 pass 放置位置策略。
...
fn run(program, analyses, ctx) -> PassResult {
    if ctx.kop_registry.prefetch_btf_id < 0 {
        return skip("bpf_prefetch kfunc not available");
    }
    if !ctx.kop_registry.packed_supported_for_target_name("bpf_prefetch") {
    ...
    let btf_slot = ensure_btf_fd_slot(program, ctx.kop_registry.btf_fd_for_target_name("bpf_prefetch")?);
        ...
        if let Some(prefetches) = by_insert_pc.get(&pc) {
            for candidate in prefetches {
                ...
                new_insns.extend_from_slice(
                    &emit_packed_kop_call_with_off(payload, ctx.kop_registry.prefetch_btf_id, btf_slot)
                );
            }
        }
...
- 真要冲 hash/LRU_HASH 核心 miss，后续应走 helper-lowering 或 specialized lookup kop。
...
### P0: kop + plumbing（2-3 天）

- `module/x86/bpf_prefetch.c`
- `module/arm64/bpf_prefetch.c`
- `module/*/Makefile`
- `daemon/src/kfunc_discovery.rs`
- `daemon/src/pass.rs` 的 `KopRegistry`
