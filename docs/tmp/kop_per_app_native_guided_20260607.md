# kop per-app native-guided tuning 2026-06-07

This note records two per-app configuration trials for Cilium and Katran. Both trials only edited
`runner/config/passes/kop/{cilium,katran}.yaml`; no LLVM selector, bpfopt gate, or benchmark
policy was changed. Runs used x86 KVM, `BPFREJIT_CORPUS_APPS="cilium/agent,katran"`,
`BPFREJIT_BENCH_PASSES=kop`, `SAMPLES=3`, and `WORKLOAD_DURATION=30`.
...
| `shlx`/`shrx` | native/codegen and micro bytecode hits | add x86 BMI2 variable-shift koperation |
...
existing `bpf_x86_popcntq` kop target. This was not a new opcode or a global
...
I then reran the same matched micro with `bitops=disable` added to the kop
mode:
...
The follow-up added a new x86 kop module and selector for register-count
...
all-29 geomean `1.208x` and kop-bearing geomean `1.222x` over 27 cases.
...
`kop_policy.pass_args=[]` and only `prefetch=false`, meaning the existing
...
now a kop target with micro and Cilium app coverage. For arm64, `bfi`/`bfxil`
