...
This review fixed the LOC regression without restoring `passes/utils.rs`, without adding compatibility wrappers, and without changing kinsn emit bytes.
...
1. **P1-C LOC gate failure**
   ...
   - Fix: removed duplicated logic and repeated fixtures instead of reverting P1-C:
     ...
     - `bpfopt/crates/bpfopt/src/main.rs:1190` adds a compact kinsn-target test fixture helper.
...
3. **Redundant `KinsnRegistry` helper wrappers**
   - Problem: the atomic call setter had three private one-call helpers, including a duplicated collision check.
   - Fix: `set_kinsn_call_for_descriptor` now performs final-key collision checking, old-key removal, and insertion directly at `bpfopt/crates/bpfopt/src/pass.rs:1311`.
...
4. **Repeated target JSON test fixtures**
   ...
   - Fix: added `kinsn_target()` and `registered_call_name()` helpers at `bpfopt/crates/bpfopt/src/main.rs:1190` and rewrote the duplicate-BTF tests to use them.
...
- Removed wrappers remain absent: `resolve_kinsn_call_off_for_target`, `resolve_kinsn_call_off_for_pass`, `target_name_for_pass`.
- `KinsnRegistry::is_target_available` exists at `bpfopt/crates/bpfopt/src/pass.rs:1252` and callers use it for availability checks.
- `btf_id_for_target_name` returns `anyhow::Result<i32>` at `bpfopt/crates/bpfopt/src/pass.rs:1236`.
- Kinsn proof dispatch still reads both `btf_id` and `call_off` from bytecode at `bpfopt/crates/bpfopt/src/pass.rs:410` and resolves through `lookup_by_kinsn_call` at `bpfopt/crates/bpfopt/src/pass.rs:442`.
- `remap_btf_metadata` / `remap_kinsn_btf_metadata` live in `pass.rs`; branch/map rewrite helpers live in `passes/rewrite.rs`; DCE helpers live in `passes/dce.rs`; instruction emit helpers live in `insn.rs`.
...
```text
rg -n "resolve_kinsn_call_off_for_target|resolve_kinsn_call_off_for_pass|target_name_for_pass|kinsn_registered_for_target_name|lookup_by_btf_id|set_btf_id_for_target_name|set_call_off_for_target_name" bpfopt/crates/bpfopt/src daemon/src
...
- `emit_packed_kinsn_call_with_off` still emits exactly `BpfInsn::kinsn_sidecar(payload)` followed by `BpfInsn::call_kinsn_with_off(kinsn_btf_id, kinsn_off)` at `bpfopt/crates/bpfopt/src/insn.rs:534`.
- `BpfInsn::kinsn_sidecar` and `BpfInsn::call_kinsn_with_off` were not changed by this review.
- The seven kinsn pass emit call sites were not changed by this review; they still obtain the same runtime BTF ID and call offset from `KinsnRegistry`.
- The registry cleanup only removed internal helper wrappers around the same final `(btf_id, call_off)` index update.
- The DCE cleanup reuses the same branch fixup implementation already used by `RewritePlan::commit`; it changes no instruction construction for kinsn lowering.
