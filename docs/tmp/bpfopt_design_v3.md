...
- `bpfrejit-daemon`：runner socket + JSON 边界，负责 live discovery、map invalidation、自动 side-input 准备、fd-array 构造、最终 `BPF_PROG_REJIT`。
- `bpfget`：daemon-owned library，只做 live program snapshot。
- `kernel-sys`：唯一 BPF syscall 边界。

`bpfverify` 和 `bpfrejit` crate 已删除。daemon 主路径不做 final `BPF_PROG_LOAD` dry-run；candidate 由内核在 `BPF_PROG_REJIT` 内走标准 verifier 路径。默认 optimizer policy 恢复 v2 行为：12 个默认 pass 全启用，daemon 自动为需要 side-input 的 pass 准备数据。
...
side-input 自动准备 path：

```bash
printf '{"cmd":"optimize","prog_id":123,"enabled_passes":["const_prop","dce"]}\n' | socat - /var/run/bpfrejit.sock
printf '{"cmd":"optimize","prog_id":123,"enabled_passes":["map_inline"]}\n' | socat - /var/run/bpfrejit.sock
```
...
主 `BPF_PROG_REJIT` 是同步 syscall，daemon 不加 timeout 或 subprocess fallback；如果 kernel verifier hang，daemon 会被阻塞。这是当前接受的限制，目的是避免用复杂 watchdog 隐藏真实 kernel-side defect。
...
3. **默认 12-pass + 自动 side-input**：default `bpfopt optimize` 包含 `const_prop`，但不包含 Paper B `branch_flip` 或非默认 `ccmp`。daemon 根据 enabled pass 自动生成 side-input；需要 verifier states 的 pass（当前 `map_inline`、`const_prop`）会触发 thin dry-run，调用方不需要 opt-in。
...
- default `optimize` pass list 是 12 pass，包含 `const_prop`，不含 Paper B `branch_flip` 和非默认 `ccmp`。
- `const_prop` 需要真实 verifier-state side-input；daemon 会自动准备。离线 CLI 调用仍必须显式提供 `--verifier-states`。`branch_flip` 必须显式 opt-in 并提供真实 PMU profile。
...
6. 对需要 verifier states 的请求自动执行 thin dry-run -> verifier states side-input -> `bpfopt` CLI。
...
### 3.5 Thin Dry-Run for Verifier-State Side Inputs

thin dry-run 是 `map_inline` / `const_prop` 的 verifier-state side-input 生成机制，不是 ReJIT 接受条件。
...
- 不写空 states，不继续跑需要 verifier states 的 pass，不降级为无 states 的 pass。
...
daemon/
  ...
  src/dry_run.rs         # verifier-state thin dry-run for side-input consumers
...
- daemon final ReJIT acceptance path does not call `BPF_PROG_LOAD`; only verifier-state side-input capture uses thin dry-run.
- `const_prop` is present in default optimize pass lists and benchmark config.
