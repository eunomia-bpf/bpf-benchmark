...
- bpfopt-suite/crates/bpfget/src/kfunc_discovery.rs:1 — copied kop discovery code is not reachable from the stub `bpfget` main. Safe to delete? Y unless `bpfget` is implemented before camera-ready.
...
- runner/libs/workspace_layout.py:75 — `kop_module_dir()` chooses `/artifacts/kop` whenever that directory exists, even outside the runtime image and regardless of `target_arch`. It should only use the runtime path inside the runtime image, or require an explicit override.
