# Region kop research: high-register-pressure regions

Status: ⏸ 后续 phase（等 kop v3 / region ABI 收敛）

Scope: research-only. No code, vendor, or module edits. No tests were run.

## 1. 当前 kop ABI 局限

当前 kop 已经不是普通 helper call，而是 `sidecar + BPF_PSEUDO_KOP_CALL`
...
1. userspace pass emits `kop_sidecar(payload)` + `call_kop_with_off()`
2. `lower_kop_proof_regions()` replaces that pair with `instantiate_insn(payload)`
3. verifier analyzes the proof sequence
4. `restore_kop_proof_regions()` restores the sidecar/call pair
5. arch JIT calls `emit_x86` / `emit_arm64`

This is enough for current single-operation koperation:
...
What region kop needs but current ABI does not define explicitly:
...
multi-insn BPF idioms with a 2-insn kop call form. The missing part is a general, reviewable contract
...
- Temporary BPF registers are allowed only when they are not live-out, matching the existing formal projection
  model in `docs/kop-formal-semantics.md`.
- Keep the first implementation stack-free. `kop_v2_instantiate_design` already identifies stack-free proof
...
- optionally `R12` only if the emitter saves/restores it or the JIT ABI explicitly reserves it for kop
...
- optionally `x13..x17` if the arch JIT contract reserves them for kop emit
- avoid `x18`
...
- Cilium has direct Jenkins hash users:
  ...
  - `runner/repos/cilium/bpf/lib/ghash.h`: `hash_32()` is a small multiply/shift hash and does not need region kop.
...
- Tetragon has many raw clusters because generic tracing templates are duplicated across kernel-version variants.
  ...
  It is not a strong first-wave region-kop driver unless profiling proves those decoders dominate runtime.

## 5. First implementation scope

Do not build a generic arbitrary-N region kop first.
...
Classification: ⏸ 后续 phase（等 kop v3 / region ABI 收敛）

Region kop is not "do not do"; Cilium and Calico show legitimate hash/mix targets, and the object census shows many
high-pressure scalar-ish blocks. But the mechanism is bigger than a normal single-operation kop. It needs a region ABI,
