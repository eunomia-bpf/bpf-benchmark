# 2026-05-05 KVM x86 5-kinsn Pass Error Analysis
...
- `cond_select`: no dead register for immediate materialization, interior branch targets, and many tail-call-helper protected kinsn sites.
- `extract`: mostly tail-call-helper protected kinsn sites.
...
- `bulk_memory`: aliasing not provably safe and tail-call-helper protected kinsn sites.
...
1. Fix the ENOSPC/log path first. This single issue accounts for 220 of 233 errors. The daemon currently makes verbose verifier-state collection mandatory for every per-pass ReJIT. For kinsn-only pass lists that do not need later verifier-state side inputs, either avoid `log_level=2` by design or add a kernel-side/log-streaming mechanism that does not turn log overflow into a pass failure. Do not silently treat ENOSPC as success; preserve fail-fast semantics and make the design decision explicit.
