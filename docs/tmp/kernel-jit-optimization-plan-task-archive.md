...
| 617 | kinsn module 证据 | ✅ | `metadata.kinsn_modules` 写入 deployment suite artifact。 |
...
| **622** | deployment suite 性能瓶颈分析 | ✅ | lkm_seeker EINVAL 是最大阻塞（已修 #616）。Katran 19 insns 是上限不是 bug。kinsn modules 缺观测证据（已修 #617）。报告：`deployment-suite_performance_blockers_20260329.md`。 |
...
| **643** | **Pass split + kinsn 开关实测** | ✅ | 修复 `--no-kinsn` 假关闭（现已真实生效），完整跑三组：`full+kinsn`、`map_inline,const_prop,dce+kinsn`、`map_inline,const_prop,dce+no-kinsn`。**结论**：`kinsn` 不是通用默认项，关掉后多数 live deployment suite 变好，但 corpus 从 **0.984x → 0.949x** 变差；`map_inline` 也不是无效，只是尚未做 isolated ablation，不能单独归因。artifact 元数据同步补齐 launcher identity / stale recovery / per-tool pass provenance。报告：`docs/tmp/20260331/pass_split_analysis.md`。 |
...
| **650** | **Runner deployment-suite/test suite 独立入口（2026-04-11）** | ✅ | Phase 3+4 完成：新增 `runner.suites.deployment-suite` 与 `runner.suites.test` CLI，封装旧 deployment-suite/test 分支的 runtime env、container、bpf_stats、SCX/Katran artifact 校验、kinsn module 加载、unittest/negative/full/fuzz 模式和 `deployment-suite/driver.py` 调用逻辑；KVM micro/corpus/deployment-suite/test 四个 suite 均直接生成 `python3 -m runner.suites.* ...`，不再经旧统一入口。验证：`python3 -m py_compile runner/suites/deployment-suite.py runner/suites/test.py runner/suites/micro.py runner/suites/corpus.py runner/libs/kvm_executor.py runner/libs/run_contract.py runner/libs/workspace_layout.py`、`python3 -m runner.suites.deployment-suite --help`、`python3 -m runner.suites.test --help`、`make check`。 |
