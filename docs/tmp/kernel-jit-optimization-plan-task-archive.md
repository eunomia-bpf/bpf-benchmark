...
| 304 | **v2 架构探索+收敛（2026-03-21）** | ✅ | v1 native-level rewrite → v2 bytecode-layer transformation + kernel verifier 验证。**收敛方案**：(A) GET_ORIGINAL+REJIT 程序变换；(B) kop ISA 扩展（KF_INLINE_EMIT）。REJIT 接受完整新 bytecode（非 patch）。**+200 行 kernel**。10 份调研报告在 `docs/tmp/20260320-21/`。POC 三 Phase 完成（`rejit-v2` 分支）。主仓库 commits: `e50365a`/`ba3c76e`/`f5eb9f1`。关键发现：xlated 不可逆需保存原始 bytecode；kop 优于新增独立 opcode；re-verify ~ms 级。 |
...
| 307 | module/x86/ 真实 kop 模块 | ✅ | `bpf_rotate.c`(ROL)、`bpf_select.c`(CMOV)、`bpf_extract.c`(BEXTR)。 |
...
| 323 | ARM64 kop 模块 | ✅ | `module/arm64/` 3 模块。+613 行。交叉编译通过。 |
| 324 | deployment-suite v2 daemon 集成 | ✅ | 5 case 接入 baseline→apply-all→post_rejit。 |
...
| 334 | func_cnt 限制调研 | ✅ | swap 路径不完整。报告：`docs/tmp/20260322/func_cnt_and_deployment-suite_compat_20260322.md`。 |
...
| 350 | kop module 重编 | ✅ | 3/3 modules 加载成功。 |
| 351 | micro v2 with kop | ✅ | RotatePass **412 sites**。rotate64_hash **20.4% 加速**。报告：`docs/tmp/20260322/micro_analysis_v2_20260322.md`。 |
| 352 | corpus attach_trigger | ✅ | 新增 `runner/libs/attach.py`。 |
| 353 | applied 数下降修复 | ✅ | WideMemPass width=3/5/6/7 skip。121 tests。 |
| 354 | deployment-suite 准备 | ✅ | 5/5 case 可跑。dev: Katran 1.108x, Tracee +8.1%。 |
...
| 358 | v2 全量评估 | 🔄 → #637 | micro 1.153x（62/62, 49 applied）。corpus/deployment-suite 需在新架构下重跑（#637）。 |
...
| 368 | Corpus measured_pairs 修复 | ✅ | vng --rwdir + kop auto-load。0→94 measured。Commit `7a5009e`。 |
...
| 373 | kop module 重编 | ✅ | 3/3 x86 modules。Commit `9e6eb3d`。 |
...
| 382 | 真实 Spectre barrier | ✅ | x86 LFENCE + arm64 DSB+ISB kop。**224 tests**。 |
| 383 | kop common header | ✅ | `DEFINE_KOP_MODULE` 宏，10 modules 简化 ~300 行。 |
| 384 | PGO 闭环实现 | ✅ | profiler→ProfilingData→BranchFlipPass 完整数据流。**224 tests**。 |
| 385 | deployment-suite case_common 提取 | ✅ | `deployment-suite/case_common.py`，5 case 去重。 |
...
| 389 | Makefile 全自动 pipeline | ✅ | kernel defconfig + kop-modules 依赖链。 |
...
| 404 | deployment suite 3 bug 修复 | ✅ | persist_results + scx bpftool + katran CONFIG_NET_IPIP。 |
| 407 | Transform trace observability | ✅ | 结构化 `OptimizeOneResult`。runner JSON 嵌入。**246 tests**。 |
| 408 | kop BTF 常量修复 | ✅ | `BPF_BTF_GET_NEXT_ID` 22→23。`CONFIG_DEBUG_INFO_BTF_MODULES=y`。 |
...
| **417** | **⚠️ kop operand-encoded ALU 设计（2026-03-23）** | ✅ 设计完成 | **核心 novelty**：kop 不是函数调用，是自定义 ALU 指令。dst_reg 编码操作数，off 编码常量，verifier 只 mark dst_reg modified + narrow_result 回调恢复精度。每个 kop 替换都是 N→1 指令，零开销。**关键**: 这是论文 kop 机制的核心贡献 — 如何让 verifier 正确建模平台特定内联指令。设计报告：`docs/tmp/20260323/kop_register_design_20260323.md`。 |
| 418 | kfunc verifier 机制调研 | ✅ | 无现成 clobber 控制。报告：`docs/tmp/20260323/kfunc_verifier_mechanisms_20260323.md`。 |
| **419** | **⚠️ bpf_kop_ops 完整设计（2026-03-23）** | ✅ | codex 调研。独立 sidecar `struct bpf_kop_ops`（不扩展 btf_kfunc_id_set）。`model_call()` 返回声明式 `bpf_kop_effect`（input_mask + clobber_mask + result range/tnum + mem_accesses），不暴露 bpf_reg_state。packed ABI 用 sidecar pseudo-insn（不复用 off 字段）。endian_load 改 const void*。KF_KOP 替代 KF_INLINE_EMIT。设计报告：`docs/tmp/20260323/kop_ops_design_20260323.md`。 |
| **420** | **kop_ops 完整实现** | ✅ | **53/62 applied, 1612 sites**（rotate 701, extract 524, endian 256）。vm-selftest **70/70 PASS**。Commit `8c1dd22`。Review：`docs/tmp/20260323/kop_implementation_review_20260323.md`。 |
| 421 | SIMD kop module | ❌ 不做 | x86 SIMD 不做（FPU 开销），ARM64 NEON 条件性 Phase 2。见 #505。 |
...
| 425 | vm-micro 正确性验证 | ✅ | **53/62 applied, 0 mismatch**。kop pass 生效。 |
| 426 | vm-corpus 正确性验证 | ✅ | 152 measured, **56 applied**（14x 提升）。0 mismatch。 |
| 427 | vm-deployment-suite 正确性验证 | ✅ | 4/5 PASS（katran timeout）。 |
...
| 435 | deployment suite 真实应用模式 | ✅ | katran xdpgeneric + retry。**5/5 PASS**。 |
| 436 | 删除 deployment suite manual fallback | ✅ | 删 `manual.py`（-244 行）。Commit `174bfe5`。 |
...
| 446 | kop 设计文档 | ✅ | `docs/kop-design.md`（1097 行）。Commit `5e10be2`。 |
...
| 450 | Tetragon deployment-suite 修复 | ✅ | kprobe→tcp_connect。 |
| 451 | Tracee deployment-suite 修复 | ✅ | setup.sh 重写。 |
...
| 456 | ARM64 QEMU 测试 | ✅ 见 #593 | ARM64 kop 6/6 + rejit_poc 6/6。 |
...
| 462 | BCC deployment-suite case | ✅ | 8 个 tool，真实 binary。 |
| 463 | kop v2 instantiate 设计 | ✅ | 报告：`docs/tmp/20260323/kop_v2_instantiate_design_20260323.md`。 |
...
| 472 | **未实现优化深度调研** | ✅ | 8 份设计/调研报告全部完成，均在 `docs/tmp/20260324/`：(a) `dynamic_map_inlining_design`、(b) `verifier_constprop_dce_design`、(c) `lfence_nospec_elimination_design`、(d) `bounds_check_elimination_research`、(e) `128bit_wide_loadstore_research`、(f) `addr_calc_lea_research`（**14 sites，ROI 低**）、(g) `simd_kop_design`、(h) `helper_call_inlining_research`（`skb_load_bytes` 最可行）。 |
...
| **475** | **kop 统一到 kfunc 路径** | 📝 设计完成 | 消除平行子系统，统一到 `KF_KOP` + `PSEUDO_KFUNC_CALL`。净减 ~150-260 行 kernel。报告：`docs/tmp/20260325/kop_kfunc_unification_research_20260325.md`、`docs/tmp/20260325/kop_kfunc_unification_codex_research_20260325.md`。**待做**：(1) 10 module 补 kfunc stub + daemon 切 FUNC discovery；(2) verifier/JIT `KF_KOP` 旁路 + normalize shim；(3) 删旧 `kop_tab`/`register_bpf_kop_set()`/独立 JIT dispatch。 |
...
| 481 | QEMU 8.2.2 TCG crash + 9.2.2 升级 | ✅ | QEMU bug（非内核）。源码编译 9.2.2。200 轮零 segfault。报告：`docs/tmp/20260325/tcg_min_repro_and_deployment-suite_investigation_20260325.md`。 |
...
| 500 | map_inline 真实 VM 零命中修复 | ✅ | 真实 clang bytecode pattern 更复杂。Katran `xdp_pktcntr` 命中。**454 tests**。Commit `e658335`。报告：`map_inline_final_fix_report_20260326.md`。deployment suite capture Tracee+Tetragon **20 fixtures**。 |
...
| 502 | Corpus 覆盖 + map 初始化调研 | ✅ | **568 obj / 2073 prog，覆盖 97.7%**。推荐 deployment suite capture fixture 方案。报告：`corpus_coverage_and_map_init_investigation_20260326.md`。 |
...
| 505 | SIMD/FPU kop 深度调研 | ✅ | x86 SIMD 不做（FPU ~200-800 cycles）。ARM64 NEON 条件性 Phase 2。报告：`simd_fpu_kop_deep_research_20260326.md`。 |
...
| 507 | deployment suite map state capture | ✅ | v2：从 daemon response 提取 `inlined_map_entries`。Tracee 13 + Tetragon 7 = **20 fixtures**。Commit `bdc11ca`。 |
...
| 515 | Daemon 简化 + deployment suite pass plumbing | ✅ | 删 --pgo/-passes（**-335 行**）。deployment suite 统一 `benchmark_performance_passes()`。报告：`map_inline_debug_status_20260327.md`。 |
...
| 520 | Katran live deployment-suite 真值 | ✅ | BPF **229→225ns (1.016x)**。wide_mem+endian 6 sites。map_inline 0 hit。 |
...
| 523 | deployment suite/Corpus request-level pass 收口 | ✅ | daemon serve request-level `enabled_passes`。6 deployment suite case 统一。**540 tests**。归入 #541。 |
...
| 541 | deployment suite pass 控制面统一 | ✅ | `rejit.py` 从 YAML 读 active passes，所有 case 统一。 |
| 542 | Daemon 质量修复 | ✅ | 静默吞错→传播、rollback 归因→per-pass diff、pass 别名→报错。**503 tests**。报告：`daemon_quality_fixes_20260328.md`。 |
| 543 | deployment suite 首次全量结果 | ✅ | **6/6 PASS**。Katran BPF **-4.5%**/throughput **+1.8%**。BCC **0.983x**。Tracee **+11.5%**。报告：`deployment-suite_full_run_results_20260328.md`。 |
...
| 548 | deployment suite post-fix 重跑 | ✅ | **6/6 ok**。Tetragon 不再 Permission denied。 |
...
| 552 | deployment suite + corpus 代码 review | ✅ | CRITICAL: 失败伪装 skipped。HIGH: enabled_passes=[] 语义。报告：`deployment-suite_corpus_code_review_20260328.md`。 |
...
| 561 | 静默失败 + 安全 pass 清理 | ✅ | 安全 pass .rs 删。enabled_passes=[] 统一。deployment suite skip→error。 |
...
| 571 | deployment suite + Katran 验证（旧 binary） | ✅ | Katran map_inline=**6**。BCC **1.023x**。报告：`deployment-suite_and_katran_post_all_fixes_20260328.md`。 |
...
| 574 | deployment suite case scaffold 去重 | ✅ | `run_case_lifecycle()` 统一 6 case。 |
...
| 583 | deployment suite 最终重跑 | ✅ | **5/6 PASS**。Katran veth symbol 不兼容（#591 修）。 |
...
| 590 | kop vs kfunc novelty 文档 | ✅ | 当前真实 novelty：proof-visible instruction form + packed sidecar + native emit + graceful degradation。5/11 pass 依赖 kop。报告：`kop_vs_kfunc_novelty_20260328.md`。 |
...
| 593 | ARM64 交叉编译 + QEMU test | ✅ | ARM64 kop **6/6**，`rejit_poc` **6/6**。报告：`arm64_test_20260328.md`。 |
...
| 601 | map_inline 放宽 review | ✅ | 3 放宽但 deployment suite 效果有限。报告：`map_inline_relaxation_20260328.md`。 |
| 602 | Pass 覆盖率调查 | ✅ | **595/1241 = 47.9%**。报告：`pass_coverage_investigation_20260328.md`。 |
| 603 | deployment suite 6/6 正面数据 | ✅ | Tracee **+3.09%**，Katran **+1.78%/p99 -1.57%**，BCC **1.015x**，bpftrace **1.140x**，scx **+54.6%**。 |
...
| 614 | Daemon 生命周期优化 | ✅ | session 级 daemon。deployment suite scan+apply 复用。 |
...
| 617 | kop module 证据 | ✅ | `metadata.kop_modules` 写入 deployment suite artifact。 |
...
| **622** | deployment suite 性能瓶颈分析 | ✅ | lkm_seeker EINVAL 是最大阻塞（已修 #616）。Katran 19 insns 是上限不是 bug。kop modules 缺观测证据（已修 #617）。报告：`deployment-suite_performance_blockers_20260329.md`。 |
...
| **625** | **LLVM IR map_inline 可行性调研** | ✅ | **结论：可行，推荐 hybrid 方案**。保留 kop pass 在 BPF 层，map_inline+const_prop+DCE 走 LLVM IR 子路径。`llvmbpf` lifter 覆盖大部分 BPF 指令，已有 array-map inline 实现可证明路线正确。**最大风险**：verifier 兼容性（lifted IR 缺 PTR_TO_CTX/loop-bound/pointer provenance）。不需要改 LLVM BPF backend（第一阶段）。**时间线**：1 周窄 POC，2 周接入 daemon，1 个月工程化。827 行报告：`docs/tmp/20260329/llvm_ir_map_inline_feasibility_20260329.md`。 |
...
| **628** | deployment suite/corpus 质量 review + 清理 | ✅ | 11 个问题修复（skip→error、daemon crash fail-fast、silent fallback 等）。**78 pytest pass**。报告：`deployment-suite_corpus_quality_review_20260329.md`。 |
...
| **631** | 大规模代码清理（2026-03-29） | ✅ | **C++ -4033 行**（删 batch_runner/daemon_client/json_parser/attach_trigger/prepared state，保留 test-run+llvmbpf）。**Python -5994 行**（删 batch plan/guest-host 双路径/trigger 命令/compile_only）。**Daemon -11 行**（已干净）。deployment suite 清理（旧 schema/兼容路径）。macro_corpus.yaml 统一 measurement: app_native/test_run。报告：`cpp_cleanup_report`/`python_cleanup_report`/`python_cleanup_review`/`deployment-suite_review_and_cleanup`/`daemon_rust_cleanup`（均在 `docs/tmp/20260329/`）。 |
...
| **637** | **新架构 corpus + deployment suite 端到端验证** | ✅ | corpus suite driver 重写完成（loader-instance lifecycle）。大规模清理 **-21462 行**（`579c081`）。import cycle 修复 5/5（`2dd43c8`）。新增 9 个 app runner。残留清理+Tracee重构（`3101592`）。**2026-04-02 本地 VM 重跑完成**：`make vm-corpus` **20/20 app ok**、applied-only / all-comparable geomean **1.033x**、applied sample **61**；`make vm-deployment-suite` **6/6 ok**。新 YAML schema 已落到 `macro_apps.yaml`，object-centric 规划不再是主线。报告：`corpus_app_native_coverage_and_deployment-suite_dedup`、`benchmark_framework_redesign`、`benchmark_yaml_redesign`、`corpus_deployment-suite_deep_review`（均在 `docs/tmp/20260329/`）。 |
| 638 | 新 kop 调研（6 组） | ✅ | 全部完成。结论：(1) POPCNT/CLZ/PDEP/PEXT/SHRX — **❌ 不做**（corpus 无 site）；(2) ARM64 CCMP — **✅ 值得做**（74 site），MADD/UBFX 低优先级；(3) Prefetch — **✅ 值得做**（17391 map_lookup site，需 PGO），NT store — ❌ 不做；(4) CRC32 — ⏸ 不做默认 pass（loxilb 2 site）；(5) RDTSC + ADC — 待评估；(6) PAUSE/YIELD — ❌ 不做（kernel 已有）。报告均在 `docs/tmp/20260329/`：`bit_ops_kop_research`、`arm64_kop_research`、`memory_hints_kop_research`、`crc32_kop_research`、`rdtsc_adc_kop_research`、`pause_yield_kop_research`。 |
...
| **643** | **Pass split + kop 开关实测** | ✅ | 修复 `--no-kop` 假关闭（现已真实生效），完整跑三组：`full+kop`、`map_inline,const_prop,dce+kop`、`map_inline,const_prop,dce+no-kop`。**结论**：`kop` 不是通用默认项，关掉后多数 live deployment suite 变好，但 corpus 从 **0.984x → 0.949x** 变差；`map_inline` 也不是无效，只是尚未做 isolated ablation，不能单独归因。artifact 元数据同步补齐 launcher identity / stale recovery / per-tool pass provenance。报告：`docs/tmp/20260331/pass_split_analysis.md`。 |
| **644** | **Apply-only 口径收敛 + full-pass 重跑** | ✅ | benchmark 默认 pass 集恢复为当前全部 in-scope performance passes（不含 `branch_flip` / 安全 pass）；BCC `execsnoop` 的单独 `rejit_passes` 缩窄配置已删除。deployment suite / corpus 报告统一改为 **“apply 多少报告多少”**：site totals 只来自 daemon 实际 apply 结果，不再把 scan candidate counts 当最终数字。**重跑结果**：bpftrace **5** 个脚本有 applied sites、总计 **33**；BCC **9** 个工具、总计 **961**；SCX **13** 个程序、总计 **359**；Tetragon `execve_rate=7`、`event_execve=8`。验证：`make check`、`make vm-selftest`、`make vm-test`、`make vm-negative-test`、`make vm-corpus`、`make vm-deployment-suite`、`make vm-micro-smoke`、`make vm-micro` 全通过。**备注**：本轮 `vm-micro` guest 仍提示 governor unknown / turbo on / no CPU affinity，适合回归确认，不应当作为最终噪声受控绝对性能数字。 |
| **645** | **Private-stack 覆盖迁移 + 2026-04-03 全量验证** | ✅ | 将 brittle 的 upstream `verifier_private_stack` 覆盖保留在默认 deny-list 外，同时把关键行为迁移到 repo-owned tests：新增 `tests/unittest/rejit_private_stack.c`，并在 `tests/unittest/rejit_kop.c` 增加 kop 临时寄存器保存回归；`upstream-selftests-build` 固定 LLVM 20 工具链，避免 host 默认 LLVM 版本漂移。内核侧仅保留必要的 x86 JIT 修复（`vendor/linux-framework` commit `a1b8bade1`），主仓库提交 `e3a9de1`。**2026-04-03 验证**：`make all`、`make check`、`make vm-test` 全通过。 |
| **647** | **工程化 review + 清理（2026-04-07）** | ✅ | **两轮清理合计 -2435 行**。第一轮（测试）：删除 BCC per-tool `rejit_passes`、重复 BPF stats（合并到 `bpf_stats.py`）、死 integration 脚本（`vm_comprehensive.sh`/`vm_daemon_live.sh`/`hold_bpf_prog`）、重复 SCX repro、`rejit_daemon_deployment-suite.c`、纯 contract 测试；恢复并改写 `scx_bad_rejit_replay.c` 为自包含 negative test（自建 map + good prog → 提交 bad bytecode → 验证 verifier 拒绝）；简化重复 mock 测试和 verifier rejection 覆盖；对齐 Makefile 与 §6.4（`vm-selftest`/`vm-negative-test`/`vm-micro-smoke`）。第二轮（生产代码）：删 11 个未使用 app runner（calico/coroot/datadog/kubearmor/loxilb/suricata/tubular/xdp-tools/xdp-tutorial/libbpf-bootstrap/systemd）+ adapter 函数、去重 `geometric_mean`、删死 Makefile corpus-build targets。**验证**：python 104 pass / daemon 535 pass / negative 编译通过。**VM 重构设计**：`docs/tmp/vm_management_refactor_design_20260407.md`（4 phase：合并重复 → manifest 迁 Python → Executor 抽象 → 统一 Python 编排；当前不动，论文提交后执行）。Commits：`d253445d`、`a849f182`。 |
| **646** | **bpfopt 独立优化工具链（2026-04-07, 2026-04-28 废弃）** | ✅ | 2026-04-07：从 daemon 提取 pass 代码到独立 workspace。2026-04-26：workspace 收缩并重命名为 `bpfopt/`，只保留 `bpfopt` library + CLI；删除 `kernel-sys`、`bpfget`、`bpfverify`、`bpfprof`、`bpfrejit`、`bpfrejit-daemon` stub crate。2026-04-28 后续 refactor 确认 daemon 是唯一运行时入口且 pass/IR/analysis 已与 `bpfopt` 分叉，删除 `bpfopt/`，单一 pass 实现回到 `daemon/src/`。如未来恢复离线工具链，必须从 daemon 单一实现抽取，不能复制维护第二份 pass。设计文档：`docs/tmp/bpfopt_design_v3.md`。Benchmark review：`docs/tmp/benchmark_status_review_20260403.md`。Commits：`ea92e14`、`8b66471`。 |
| **648** | **Runner micro suite 独立入口（2026-04-11）** | ✅ | Phase 1 完成：新增 `runner.suites.micro` CLI，KVM micro 分支直接生成 `python3 -m runner.suites.micro ...`，不再经旧统一入口调用 micro driver；旧入口临时保留给 test/corpus/deployment-suite。复用现有 `micro/driver.py` 与 `runner/libs` artifact/path helper。验证：`python3 -m runner.suites.micro --help`、`python3 -m runner.suites.micro --arch x86 --samples 1 --warmups 0 --inner-repeat 1 --list`、`make check`。 |
...
| **650** | **Runner deployment-suite/test suite 独立入口（2026-04-11）** | ✅ | Phase 3+4 完成：新增 `runner.suites.deployment-suite` 与 `runner.suites.test` CLI，封装旧 deployment-suite/test 分支的 runtime env、container、bpf_stats、SCX/Katran artifact 校验、kop module 加载、unittest/negative/full/fuzz 模式和 `deployment-suite/driver.py` 调用逻辑；KVM micro/corpus/deployment-suite/test 四个 suite 均直接生成 `python3 -m runner.suites.* ...`，不再经旧统一入口。验证：`python3 -m py_compile runner/suites/deployment-suite.py runner/suites/test.py runner/suites/micro.py runner/suites/corpus.py runner/libs/kvm_executor.py runner/libs/run_contract.py runner/libs/workspace_layout.py`、`python3 -m runner.suites.deployment-suite --help`、`python3 -m runner.suites.test --help`、`make check`。 |
| **651** | **Runner 旧 suite 框架删除（2026-04-11）** | ✅ | Phase 5 完成：AWS 远端 `run-workspace` 改为直接分发到 `runner.suites.micro/corpus/deployment-suite/test`；删除旧统一入口、guest prereq 层和 prereq contract 层；KVM 删除旧 fallback；`runtime_path_value` 收敛到 `workspace_layout.py`；清理 `case_common.py` 未使用 helper 和残留文档引用。验证：见本轮 `py_compile` 与 `make check`。 |
...
| **653** | **AWS x86 runtime image deployment-suite + corpus 验证（2026-04-18）** | ✅ | 复用 `.cache/container-images/x86_64-runner-runtime.image.tar`，未重建 image。`make aws-deployment-suite RUN_TARGET_ARCH=x86_64` **6/6 PASS**；`make aws-corpus RUN_TARGET_ARCH=x86_64` **20/20 app ok**，corpus status `ok`。AWS `t3.large` / `us-east-1`，kernel `7.0.0-rc2`。报告：`docs/tmp/aws-x86-deployment-suite-20260418/summary.md`。 |
| **654** | **Wave 1 silent fallback removal（2026-04-21）** | ✅ | 删除 `rejit.py` / daemon / AWS executor / corpus+deployment-suite driver 中的 silent fallback，改为 strict schema/error propagation；KVM 复验：`make vm-deployment-suite` **ALL PASSED**，`make vm-corpus` status `ok`。KVM x86 corpus `applied_sample_count` **10→12**，`sample_count` **10→12**，`no_programs_changed_in_loader` **38→36**。报告：`docs/tmp/wave1-fallbacks-20260421/summary.md`。 |
| **655** | **Wave 1.5 AWS x86 复验（2026-04-21）** | ✅ | 基于 `0df38b7f` 复跑 AWS x86：`make aws-deployment-suite RUN_TARGET_ARCH=x86_64` **ALL PASSED**；`make aws-corpus RUN_TARGET_ARCH=x86_64` status `ok`。无代码修复；复用 `.cache/container-images/x86_64-runner-runtime.image.tar`。AWS x86 corpus 相对 pre-wave1 baseline `aws_x86_corpus_20260421_200048_542429`：`applied_sample_count` **8→10**，`sample_count` **8→10**，`no_programs_changed_in_loader` **39→36**，方向与 KVM Wave 1 复验一致（applied ↑，excluded ↓）。报告：`docs/tmp/wave1.5-aws-x86-verify-20260421/summary.md`。 |
| **656** | **Unused contract surface verification（2026-04-21）** | ✅ | 复核 review §5 后，删除 corpus wrapper 的死 `--warmups` surface（`runner.suites.corpus` parser、`runner.libs.suite_args` emission、root `VM_CORPUS_SUITE_ARGS`），删除 root `Makefile` 中未使用的 `PROFILE` / `KALLSYMS_EXTRA_PASS`，并确认 `aws-deployment-suite` / `aws-corpus` 仍是 live AWS 单 suite 入口，**保留**。验证：`make -n vm-deployment-suite`、`make -n aws-deployment-suite RUN_TARGET_ARCH=x86_64`、`python3 -m py_compile runner/suites/corpus.py runner/suites/deployment-suite.py runner/libs/suite_args.py`。报告：`docs/tmp/unused-surface-20260421/summary.md`。 |
| **657** | **AWS x86 runtime image 打包修复（2026-04-18）** | ✅ | `668ddae9` 补 Makefile `aws-deployment-suite` / `aws-corpus` targets 与 `aws_executor.py` 命令实现；`6099e04e` / `1c8da845` 把 tracee musl runtime 库（loader path）打进 runtime image；`94da7880` bcc kheaders cache 跨 run 共享、per-run scratch 干净；`b3cc3bd8` 记录 AWS x86 runtime 验证补遗。配合 #653，AWS x86 image 不再缺 musl / kheaders。 |
| **658** | **arm64 kop "Invalid module format" 修复（2026-04-18）** | ✅ | `0133184c` 每次都从当前 image tar 重新提取 arm64 host kernel artifact，避免 host 侧残留与 image 内 kernel 不匹配导致 insmod EINVAL。配套 5 份 docs/tmp summary 一并入 git。 |
| **659** | **Dead code cleanup：corpus/deployment-suite driver 不可达路径（2026-04-21）** | ✅ | 删 `--rejit-passes` CLI flag（`c6e9815c`）、`deployment-suite/driver.py` 不可达 empty-rejit-passes fallback（`a1f2fbc2`），保留 reachable `requested_rejit_passes` fallback 分支（`bf29e010`，codex 自我 review 防止误删）。总结：`3519116f`。 |
