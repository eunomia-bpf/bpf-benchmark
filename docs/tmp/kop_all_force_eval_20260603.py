#!/usr/bin/env python3
"""Post-hoc all-force kop corpus evaluation for 2026-06-03.
...
RUN_DIR = artifact_path(
    "KOP_EVAL_RUN_DIR", "corpus/results/x86_kvm_corpus_20260603_175429_964295"
)
SMOKE_DIR = artifact_path(
    "KOP_EVAL_SMOKE_DIR", "corpus/results/x86_kvm_corpus_20260603_185015_116803"
)
OUT_MD = ROOT / "docs" / "tmp" / "kop_all_force_eval_20260603_summary.md"
FIG_OUT = ROOT / "docs" / "figures" / "eval-kop-all-force-corpus-20260603.png"
PASS_NAME = os.environ.get("KOP_EVAL_PASS_NAME", "")
BPF_CALL = 0x85
BPF_PSEUDO_KOP_CALL = 4
KOP_FAMILIES = [
    "lea",
    "cond_select",
    "rotate",
...
def load_base() -> Any:
    path = ROOT / "docs" / "tmp" / "kop_eval_20260531.py"
    spec = importlib.util.spec_from_file_location("kop_eval_20260531_base", path)
...
def load_kop_targets(workdir: pathlib.Path) -> dict[tuple[int, int], str]:
    target = json.loads((workdir / "target.json").read_text())
    pair_to_name: dict[tuple[int, int], str] = {}
    for name, entry in target.get("koperation", {}).items():
        pair = (int(entry["call_offset"]), int(entry["btf_func_id"]))
        previous = pair_to_name.get(pair)
        if previous is not None:
            raise RuntimeError(f"{workdir}: duplicate kop target {pair}: {previous}, {name}")
        pair_to_name[pair] = name
    return pair_to_name


def decode_applied_kops(workdir: pathlib.Path) -> Counter[str]:
    data = (workdir / "input.bin").read_bytes()
    if len(data) % 8:
        raise RuntimeError(f"{workdir}: input.bin size is not a BPF instruction multiple")
    pair_to_name = load_kop_targets(workdir)
    names: Counter[str] = Counter()
    for pc in range(0, len(data), 8):
        code, regs, off, imm = struct.unpack_from("<BBhi", data, pc)
        src_reg = regs >> 4
        if code != BPF_CALL or src_reg != BPF_PSEUDO_KOP_CALL:
            continue
        pair = (int(off), int(imm))
        name = pair_to_name.get(pair)
        if name is None:
            raise RuntimeError(f"{workdir}: kop call {pair} has no target.json entry")
        names[name] += 1
    return names


def applied_kop_summary() -> tuple[dict[str, Counter[str]], Counter[str], dict[str, Counter[str]], Counter[str]]:
...
def plot_corpus(
    ...
    ax.set_title("Applied kop families")
    ...
    fig.suptitle("x86 KVM all-force kop corpus evaluation", fontsize=16)
...
def write_summary(
    ...
    apply: dict[str, dict[str, Counter[str]]],
    applied_names: dict[str, Counter[str]],
    kop_names: dict[str, Counter[str]],
    total_kop_names: Counter[str],
    kop_families: dict[str, Counter[str]],
    total_kop_families: Counter[str],
    fig: pathlib.Path,
) -> None:
    ...
    lines: list[str] = []
    lines.append("# All-Force KOperation Corpus Evaluation, 2026-06-03")
    lines.append("")
    lines.append("Generated by `docs/tmp/kop_all_force_eval_20260603.py` from raw artifacts.")
    lines.append("")
    ...
    lines.append("- Policy: backend default is all-force kop; no YAML workload/app disable was used.")
    ...
    family_summary = ", ".join(
        f"{family}={total_kop_families.get(family, 0)}" for family in KOP_FAMILIES
    )
    lines.append(f"- Applied kop families decoded from retained bytecode: `{family_summary}`.")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Applied KOperation Families")
    lines.append("")
    lines.append(
    ...
    lines.append(
        f"| `total` | "
        f"{total_kop_families.get('lea', 0)} | {total_kop_families.get('cond_select', 0)} | "
        f"{total_kop_families.get('rotate', 0)} | {total_kop_families.get('extract', 0)} | "
        f"{total_kop_families.get('endian_fusion', 0)} | {total_kop_families.get('bulk_memory', 0)} | "
        f"{total_kop_families.get('prefetch', 0)} | {total_kop_families.get('other', 0)} | "
        f"{sum(total_kop_families.values())} |"
    )

    lines.append("")
    lines.append("## Applied KOperation Names")
    lines.append("")
    lines.append("| App | Applied kop names by count |")
    lines.append("| --- | --- |")
    for key, _, _ in APPS:
        rendered = ", ".join(f"`{name}`={count}" for name, count in kop_names[key].most_common())
        lines.append(f"| `{key}` | {rendered or 'none'} |")
    total_rendered = ", ".join(f"`{name}`={count}" for name, count in total_kop_names.most_common())
...
def main() -> None:
    ...
    kop_names, total_kop_names, kop_families, total_kop_families = applied_kop_summary()
    bpf = base.bpf_rows(applied_names)
    fig = plot_corpus(workloads, bpf, apply, kop_families)
    write_summary(
        workloads,
        bpf,
        apply,
