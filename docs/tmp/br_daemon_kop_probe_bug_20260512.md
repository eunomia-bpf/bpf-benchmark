...
```rust
pub(crate) fn probe_target_json(targets: &[KopProbeTarget]) -> Result<TargetJson> {
    if targets.is_empty() {
        bail!("target probing requested with no kop targets");
    }
```
...
if referenced.contains(VAR_TARGET) {
    let probed = bpf::probe_target_json(kop_probes)
        .with_context(|| format!("probe target koperation for prog {prog_id}"))?;
```
...
means `run_program_steps()` decided a target file was needed, but the daemon socket request carried an empty `kop_probes` list.
...
- `status: "error"`
- `passes: []`
- `error_message: "probe target koperation for prog ...: target probing requested with no kop targets"`
...
1. `runner/libs/rejit_plan.py:88-97` builds `kop_probes` as the union of every enabled pass YAML `koperation:` entry.
2. The BR pass set is `noop,wide_mem,const_prop,dce,bounds_check_merge,skb_load_bytes_spec`.
3. Those pass YAML files declare no `koperation:` entries, so `build_kop_probes()` returns `[]`.
...
6. Because DCE references `${TARGET}`, `run_program_steps()` calls `bpf::probe_target_json(kop_probes)`.
7. `kop_probes` is empty, so `daemon/src/bpf.rs:88-89` bails.
...
- no enabled pass YAML contributing `koperation:`
...
The isolated kop runs and `prefetch` do reference `${TARGET}`, but they also declare `koperation:` in their pass YAML. For example:

- `prefetch` declares `bpf_prefetch`
- `rotate` declares `bpf_rotate64` and `bpf_rotate32`
- the 5-pass kop set contributes rotate/select/extract/endian/bulk-memory kop probes

Those runs call the same daemon target-probing path, but with a non-empty `kop_probes` list, so `probe_target_json()` proceeds to kernel BTF probing.

The BR pass set is the broken corner because it has no kop-class pass but, through DCE's YAML command, still requests `${TARGET}`.

Note: `docs/evaluation-05-07-2026.md:253-260` also reports BR at 0.0% success, but its documented dominant failure is `bpfopt_failed[const_prop]`, not this daemon-side empty-kop probe failure. The May 12 `bcc__set.json` failure is earlier: pass count is zero for every program.
...
a91d5753 2026-05-05 Probe all endian load koperation
cff84bff 2026-05-01 fix: restore call_offset and BTF module fds in target probing and fd_array
```

The most relevant daemon-side change is `56549b8a`: it removed the daemon's hardcoded `KOP_PROBE_TARGETS` and made `probe_target_json()` consume caller-supplied `KopProbeTarget` metadata. That commit also added the empty-target bail. The later execute-plan/YAML path now lets a command request `${TARGET}` while supplying no kop probes.
...
The BR set contains bytecode-only passes and should be able to run without kop target probing. Current `bpfopt` metadata also says DCE does not need target metadata: `bpfopt/crates/bpfopt/src/passes/mod.rs:125` registers `dce` with `META_NONE`, and `bpfopt/crates/bpfopt/src/passes/dce.rs:15-17` ignores `PassContext`.

The immediate trigger is therefore the stale or incorrect DCE YAML `--target ${TARGET}`. The daemon-side defect is that target-file preparation is coupled to "probe koperation" instead of to the actual target JSON content needed by the step. An empty kop list is a valid situation for non-kop bytecode passes if they only need arch/features, and it should not abort the whole program before pass 0.
...
- In `daemon/src/bpf.rs`, add a target-construction helper that treats empty probe requests as a platform-only target, not as kop probing:

```rust
pub(crate) fn target_json_for_probes(targets: &[KopProbeTarget]) -> Result<TargetJson> {
    if targets.is_empty() {
        return Ok(TargetJson {
            arch: detect_arch(),
            features: detect_features(),
            koperation: BTreeMap::new(),
        });
    }
...
```rust
let probed = bpf::target_json_for_probes(kop_probes)
    .with_context(|| format!("prepare target.json for prog {prog_id}"))?;
```

- Preserve bpfopt-side kop validation. If a real kop pass has a broken YAML entry with missing `koperation:`, bpfopt should still fail with its required-kop check after reading an empty target, instead of daemon failing before any pass detail exists.

Plan/config validation:

- Add a runner config validation check: if a pass command references `${TARGET}` but its YAML has no `koperation:`, require an explicit marker such as `target_kind: platform` or reject the config. This catches accidental kop YAML drift while still allowing intentional arch/features-only target consumers.
- Add a focused regression test for the empty-probe target helper: empty `kop_probes` should produce `TargetJson { arch, features, koperation: {} }` without kernel BTF probing. A separate config validation test should cover `dce/default.yaml` so BR cannot regress into a daemon pre-pass failure.
