...
- R2-9 `rep_admit_kop_site_window` returning `(BlockId, Range<usize>)`: fixed. It now returns `Option<KopAdmissionWindow>` at `bpfopt/crates/bpfopt/src/analysis/bbprogram.rs:1684-1717`; callers use `start_site()` / `end_site()`, for example `bpfopt/crates/bpfopt/src/passes/rotate.rs:87-94`.
