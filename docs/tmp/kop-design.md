# kop v2 Design and Implementation

Date: 2026-03-26  
Status: current design as implemented in this tree  
Theme: first-class kop instruction encoding + proof lowering + shared kfunc/BTF metadata plumbing
...
- a standalone `register_bpf_kop_set()` API
- a standalone `btf->kop_tab`
- a standalone `prog->aux->kop_tab`
- `BPF_PSEUDO_KOP_CALL` sites whose `imm` names a `BTF_KIND_VAR` descriptor object

What the current tree **does** have is:

- dedicated kop instruction encoding in BPF bytecode
- dedicated verifier lowering and restore logic for kop proof sequences
- dedicated native JIT dispatch for kop sites
...
- Modules, daemon, and tests:
  - [`module/include/kop_common.h`](../module/include/kop_common.h)
  ...
  - [`tests/unittest/rejit_kop.c`](../tests/unittest/rejit_kop.c)
- Formal semantics:
  - [`docs/kop-formal-semantics.md`](./kop-formal-semantics.md)

Useful higher-level design references:

- [`docs/tmp/20260323/kop_v2_instantiate_design_20260323.md`](./tmp/20260323/kop_v2_instantiate_design_20260323.md)
- [`docs/tmp/20260325/kop_kfunc_unification_research_20260325.md`](./tmp/20260325/kop_kfunc_unification_research_20260325.md)
- [`docs/tmp/20260325/kop_func_id_transport_design_20260325.md`](./tmp/20260325/kop_func_id_transport_design_20260325.md)

## 1. One-sentence definition

`kop` is a first-class BPF instruction form whose canonical semantics come from
...
The current UAPI has dedicated kop opcodes:
...
[`vendor/linux-framework/include/uapi/linux/bpf.h`](../vendor/linux-framework/include/uapi/linux/bpf.h)
explicitly says that `BPF_PSEUDO_KOP_CALL` carries the BTF ID of a
`BTF_KIND_FUNC` kop stub.

This means the bytecode-level representation is not a plain kfunc call.
Verifier and JIT both recognize `BPF_PSEUDO_KOP_CALL` directly.

### 2.2 Canonical semantics still come from `struct bpf_kop`
...
There is no active `model_call()` or `bpf_kop_effect` path in the current tree.

### 2.3 Current transport identity

The transport for one kop site is:

- one sidecar pseudo-insn carrying the packed payload
- one `CALL` whose `src_reg = BPF_PSEUDO_KOP_CALL`

The sidecar payload format is implemented by
`bpf_kop_sidecar_payload()` in
[`vendor/linux-framework/include/linux/bpf.h`](../vendor/linux-framework/include/linux/bpf.h)
and mirrored by `BpfInsn::kop_sidecar()` in
[`daemon/src/insn.rs`](../daemon/src/insn.rs):
...
- `CALL.imm` is the BTF ID of a `BTF_KIND_FUNC` kop stub
...
in module BTF as variables, but that is not what `BPF_PSEUDO_KOP_CALL`
addresses.
...
3. provide a parallel `kop_descs[]` array holding the actual
   `const struct bpf_kop *` descriptors
...
So the current implementation is not a fully separate "kop registry"
...
- [`vendor/linux-framework/kernel/bpf/btf.c`](../vendor/linux-framework/kernel/bpf/btf.c)
  stores the kfunc set and the parallel kop descriptor mapping in
  `btf->kfunc_set_tab`
- verifier uses `btf_kfunc_kop_desc()` to recover the `struct bpf_kop *`
- verifier caches per-program metadata in `prog->aux->kfunc_tab` and
  `prog->aux->kfunc_btf_tab`
- JIT later resolves payload and descriptor through `bpf_jit_get_kop_payload()`

There is no current `register_bpf_kop_set()`, `btf->kop_tab`, or
`prog->aux->kop_tab`.
...
[`vendor/linux-framework/kernel/bpf/verifier.c`](../vendor/linux-framework/kernel/bpf/verifier.c)
is where current kop sites first enter verifier bookkeeping.

For each `BPF_PSEUDO_KOP_CALL` site, the verifier:

- counts the site in `env->kop_call_cnt`
- routes metadata creation through `add_kfunc_desc(..., true)`
- resolves the backing `struct bpf_kop *` through `fetch_kfunc_meta()` and
  `btf_kfunc_kop_desc()`
- stores the resulting entry in `prog->aux->kfunc_tab`

So current kop calls are "special" in instruction encoding, but not in the
...
two-insn kop encoding, but the program is restored back to the original
`sidecar + kop_call` form before later rewrite/JIT stages.

### 3.3 Lowering and restore mechanics

Current lowering is intentionally minimal:

- `lower_kop_proof_regions()` walks the program from back to front
- for each kop site it requires a preceding sidecar
- it resolves `(kop, payload)` via `bpf_jit_get_kop_payload()`
...
- it records the original two instructions in `env->kop_regions[]`

Restore is the inverse:

- `restore_kop_proof_regions()` patches back the original two instructions
- it removes the remaining proof instructions
- it cleans affected jump-table aux state

There is no separate `build_kop_inst_seq()` helper in the current tree.

### 3.4 What `validate_kop_proof_seq()` actually enforces
...
- nested kop sidecar pseudo-insns
- all calls and exits
...
Current x86 and arm64 JITs both have dedicated kop dispatch:
...
name again. Instead it calls `bpf_jit_get_kop_payload()` and resolves the
descriptor from the already prepared `prog->aux->kfunc_tab` entry.

So the current statement is:

- kop has dedicated JIT dispatch
...
- if `prog->jit_requested` is true **and** the descriptor has a native emit
  callback for the running architecture, the original kop site stays intact
...
- native emit available -> keep original kop representation for JIT
...
- discover module BTF-backed kop stubs from `/sys/kernel/btf/<module>`
...
- emit `sidecar + BPF_PSEUDO_KOP_CALL` sequences
...
- kop discovery through module BTF and BTF FDs
...
- first-class kop instruction encoding
...
So the precise current statement is **not** "kop is just a kfunc", but it is
also **not** "kop has a completely separate descriptor transport and table
stack".
