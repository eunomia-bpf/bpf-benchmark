# Feasibility study for kop skip-reason relaxation
...
### A. `kop site in program with tail-call helper`

#### 1. Current code path

Every kop pass uses the same shared guard:

- `bpfopt/crates/bpfopt/src/passes/cond_select.rs:188` calls `kop_replacement_subprog_skip_reason`.
- `bpfopt/crates/bpfopt/src/passes/bulk_memory.rs:176` calls `kop_replacement_subprog_skip_reason`.
- `bpfopt/crates/bpfopt/src/passes/extract.rs:145` calls `kop_replacement_subprog_skip_reason`.
- `bpfopt/crates/bpfopt/src/passes/endian.rs:459` calls `kop_replacement_subprog_skip_reason`.
...
- `kop_replacement_subprog_skip_reason` at `utils.rs:719` checks subprogram boundaries, replacement boundaries, and subprogram entry overlap.
- It then calls `kop_replacement_tail_call_skip_reason` at `utils.rs:769`.
- `kop_replacement_tail_call_skip_reason` at `utils.rs:773` scans every instruction in the program and returns the first tail-call helper it finds.
- The exact emitted skip is at `utils.rs:779`: `kop site in program with tail-call helper (tail call pc {pc}, site pc {start_pc})`.
...
The guard does not compare the tail-call PC with the candidate site, does not inspect CFG reachability, and does not inspect whether the replacement changes instruction count before a tail-call site. It is a blanket per-program disable for kop replacements.

The exact assumption is: in a program containing any tail-call helper, any kop replacement may invalidate kernel tail-call poke descriptors or text-poke state, so every site in that program is skipped.

Related source: `tail_call_protected_prefix_end` at `utils.rs:787` returns the end of the last tail-call instruction. Non-kop passes use this more granular concept. For example, `const_prop` has tail-safe replacement filtering, while `dce` disables instruction-deleting cleanup when tail calls are present.
...
skip("kop site in program with tail-call helper ...")
```
...
- The replacement must remain inside one subprogram and must not cover a subprogram entry. This is already enforced by `kop_replacement_subprog_skip_reason`.
- Branch targets and BTF metadata must remap consistently when the replacement changes length. Existing helpers `fixup_all_branches` and `remap_kop_btf_metadata` already provide this for normal sites.
...
- No new kop registration required.
...
Current kop sites in programs without tail calls should not change if the guard is relaxed only for tail-call programs. The regression risk is in newly applied sites:
...
if tmp exists:
    ...
    use victim as the kop operand
    suffix += LDXDW victim, [r10 + slot]
```

This requires `CondSelectLowering` to support a suffix after the kop call, not only a prefix before it. The current implementation builds a prefix, sidecar load, and call replacement. It does not currently restore a spilled live register after the call.
...
- The kop call must not leave the victim live-clobbered after restore. A suffix restore is needed because helper/kfunc calls normally clobber caller-saved registers.
...
The exact assumption is: when source and destination base registers differ, the pass can only prove non-aliasing if one base is likely stack-derived and the other is not. If both look stack-like or both look non-stack-like, the copy may overlap and must not be replaced by the bulk memcpy kop.
...
An overlap-safe variant would be a different feature: a new `memmove_bulk` or directional-copy kop. That is not the minimum relaxation because it requires new module registrations and new JIT emit code.
...
- Overlapping copies must preserve original load/store ordering. With the existing memcpy kop, overlapping different-base copies should remain skipped unless a new overlap-safe kop exists.
...
- Overlap-safe kop route: about 250-500 LOC across module x86/arm64 emit code, daemon probing, bpfopt registry, pass logic, and tests.
- No daemon-side change is needed for the conservative provenance route.
- No new kop registration is needed for the conservative provenance route.
...
The regression risk is semantic: an unsound alias proof can change overlapping copy behavior. This is higher risk than a missed optimization because the original sequence has precise load/store order and the bulk kop may not.
...
A broader split rewrite would clone or preserve entry-specific original code and replace only the normal-entry path, but that is no longer a local kop replacement. It is higher risk and should not be the first relaxation.
...
- Branch targets must not be remapped into the middle of a sidecar load or kop call pair. This is a hard verifier/code-generation wall.
...
- `module/x86/bpf_endian.c` defines and registers all three endian load koperation.
- `module/arm64/bpf_endian.c` also contains all three definitions by source inspection.
- Git history contains `a91d5753 Probe all endian load koperation`, whose message says the modules already provided all three and the daemon probe list was the missing piece.
...
- The daemon must probe all three public kop symbols and pass their BTF ids to bpfopt. Current source appears to satisfy this.
- bpfopt must map each name to the matching registry field. `set_kop_btf_id` handles `bpf_endian_load16`, `bpf_endian_load32`, and `bpf_endian_load64`.
- The x86 and arm64 kop modules must register and emit correct code for all three sizes. Source inspection confirms all three are present; runtime ReJIT validation still needs the rerun.
- Per-size call offsets must remain correct when multiple endian koperation share a module. Existing endian tests cover per-size call offsets.
...
- No new kop module registration appears required.
...
`grep -RN "tail.call\|tail_call\|tail-call" bpfopt/` shows all kop tail-call rejection flows through `passes/utils.rs`, with additional tail-call-specific behavior in non-kop passes such as `const_prop` and `dce`.

For kop replacements, the mechanism is truly program-wide:

```text
for pc in 0..program.insns.len():
    if is_tail_call_insn(program.insns[pc]):
        return SkipReason("kop site in program with tail-call helper ...")
```
...
4. C, `bulk_memory` different-base alias. The upper bound is 99 sites. It is worth doing only as a conservative pointer provenance analysis; a general alias system or new memmove kop is not justified by the current count.
...
Replace the shared program-wide tail-call kop skip with a tail-call-layout-aware guard that allows fixed-length replacements after the last tail-call instruction and preserves the current skip for every size-changing replacement before or across a tail-call site. That single shared change targets the largest blanket skip family, keeps the correctness invariant simple, and avoids touching daemon/kernel behavior.
