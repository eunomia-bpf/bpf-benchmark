...
- **authoritative 拓扑必须按 Katran README/EXAMPLE 的假设来做：DSR、L3 routed、single-interface load balancer on a stick、real 侧 IPIP decap。**
- **attach 模式建议用 Katran 官方文档更推荐的 shared mode**：先挂 `xdp_root`，再让 `katran_server_grpc` 通过 pinned `prog_array` 注册 `balancer_ingress`。
- **流量建议用“官方 gRPC client 做控制面 + 外部 HTTP/TCP 负载生成器做数据面”**。控制面仍然走 Katran 自己的 Go gRPC client；数据面用 `wrk` 或等价的连接风暴工具。
- **编译建议在 host 做，运行和测量在 VM 做。** 原因是 Katran userspace 构建依赖重，放进 guest 会显著拉长 setup 和调试时间，而且不影响 datapath benchmark 的构造有效性。
- **当前 framework kernel 有一个硬 blocker：`CONFIG_NET_IPIP` 没开。** 这会直接阻止 README/EXAMPLE 里要求的 real-side `ipip0` decap 拓扑，因此在真正实现 Katran authoritative case 前，必须先补 kernel config。
...
- router 为 VIP 做静态路由，client 才能把 VIP 流量送到 Katran；
- real 上要有 `ipip0` decap 和 `VIP/32 on lo`；
- 回包必须绕过 Katran，直接从 real 返回 client。
...
因此 Katran authoritative case 应当优先按 **namespace + veth / virtio loop NIC** 的方式做，而不是强行容器化。
...
**建议在 host 编译 Katran，VM 只负责运行和测量。**
...
按 Katran README/EXAMPLE，topology 需要同时满足：
...
authoritative VM 内建议用 5 个 namespace：
...
```text
ns_client -- link1 -- ns_router -- link2 -- ns_lb
                        |
                        +-- link3 -- ns_real1
                        +-- link4 -- ns_real2
                        +-- link5 -- ns_real3
...
- 完全是 L3 routed
- `ns_lb` 只有 **一个数据接口**
- client->VIP 流量经 router 进 `ns_lb`
- `ns_lb` 把封装包再发回同一个 router
- router 再把 outer dst=real_ip 的包送到 real
- real 直接把回复从 VIP 发回 client，绕过 lb

#### 推荐地址规划

可直接沿用一组稳定的私网地址：

- client<->router：`172.16.0.0/30`
- lb<->router：`172.16.1.0/30`
- real1<->router：`172.16.10.0/30`
- real2<->router：`172.16.20.0/30`
- real3<->router：`172.16.30.0/30`
- VIP：`10.200.200.1/32`

示例：

- `ns_client`: `172.16.0.2/30`, gw `172.16.0.1`
- `ns_router` on client link: `172.16.0.1/30`
- `ns_lb`: `172.16.1.2/30`, gw `172.16.1.1`
- `ns_router` on lb link: `172.16.1.1/30`
- `ns_real1`: `172.16.10.2/30`, gw `172.16.10.1`
- `ns_real2`: `172.16.20.2/30`, gw `172.16.20.1`
- `ns_real3`: `172.16.30.2/30`, gw `172.16.30.1`

router 里再额外配一条 VIP 静态路由：

- `10.200.200.1/32 via 172.16.1.2 dev <router-lb-if>`

这样 client 发往 VIP 的包就会被 router 定向送到 Katran。
...
- `NET_IPIP` 没开：**official Katran real-side decap 直接被阻塞**
...
#### 没有 `veth` 时如何在 VM 内搭拓扑

这不是 blocker，因为现有 `xdp_forwarding` case 已经证明可以复用 `vng --network loop` 注入预连接的 virtio loop NIC。

对于 Katran authoritative case，推荐用 **preexisting NIC pairs**，不是 guest-created veth：

- `--network loop` x 5
- 进入 guest 后把每对网卡分别移动到不同 namespace
- 在 router namespace 里保留每条链路的一端

这比 bridge/veth 拓扑更原始，但完全满足 Katran README 的 L3 路由要求。
...
- `default_mac` 取 router 在 LB 这条链路上的 MAC
...
real 建议 3 台，权重按官方建议尽量让总和接近 ring size `65537`：
...
katran_goclient -server 127.0.0.1:50051 -a -t 10.200.200.1:80 -r 172.16.10.2 -w 21845
katran_goclient -server 127.0.0.1:50051 -a -t 10.200.200.1:80 -r 172.16.20.2 -w 21846
katran_goclient -server 127.0.0.1:50051 -a -t 10.200.200.1:80 -r 172.16.30.2 -w 21846
katran_goclient -server 127.0.0.1:50051 -l
```

#### real 侧

每个 real namespace 都要按官方 DSR/IPIP 要求做：
...
sysctl -w net.ipv4.conf.<real_if>.rp_filter=0
```
...
所以 authoritative case 最好再补一个“短连接偏多”的 workload。可选两个实现：

- `wrk` 加 `Connection: close`
- 自己在 `e2e/common/workload.py` 补一个 HTTP short-connect storm

我更建议后者，因为它更容易拿到：

- 完成请求数
- 连接错误数
- 可复用现有 `connect_storm` 设计思路
...
1. bring-up topology
2. 起 `xdproot`
3. 起 `katran_server_grpc`
4. 用官方 gRPC client 配 VIP/reals
5. warmup 网络路径
6. baseline workload
7. `scan_programs`
8. `apply_recompile`
9. 如果 applied > 0，则 post-ReJIT workload
10. 输出 JSON/Markdown
...
- `xdp_forwarding` 的 VM NIC preexisting 模式
...
| 搭 authoritative namespace/router/IPIP/DSR 拓扑 | `2.0-3.0` 天 |
| 接 Katran server / goclient / health / stats collector | `1.5-2.0` 天 |
| 接 scanner enumerate / policy remap / recompile / before-after measurement | `1.0-1.5` 天 |
| 稳定化、清理、VM 重跑、结果格式化 | `1.5-2.0` 天 |
| **合计** | **`8-11.5` engineer-days** |
...
如果不打开 `CONFIG_NET_IPIP`，real 侧无法按官方方式创建：

```bash
ip link add name ipip0 type ipip external
```

那 authoritative Katran DSR/IPIP testbed 就根本搭不起来。
...
### 4.4 interface naming 不稳定

`vng --network loop` 注入的 virtio NIC 名字在 guest 里未必稳定。Katran 启动参数要求明确的 `-intf=<name>`，所以 authoritative case 必须在 bring-up 里做：

- stable rename
- stable netns move
- stable MAC/IP snapshot
...
5. **复用 `xdp_forwarding` 的 VM preexisting NIC 模式**
   - authoritative VM run 用 `vng --network loop` 多次注入 NIC pair。
   - guest 内 rename 后移动到 `ns_client/ns_router/ns_lb/ns_real*`。

6. **实现 Katran authoritative topology bring-up**
   - router IP forwarding
   - client default route
   - router VIP route via lb
   - real default route
   - real `ipip0` + `VIP/32 on lo` + `rp_filter=0`

7. **实现 Katran server lifecycle**
   - mount bpffs
   - 起 `xdproot`
   - 起 `katran_server_grpc`
   - 起官方 Go client 配 VIP/reals
   - 做 health check：gRPC port、`katran_goclient -l`、`find_bpf_programs(server_pid)`

8. **接数据面 workload**
   - authoritative 主 workload 用 `wrk`
   - 增加一个 short-connect workload
   - 同时后台起 `katran_goclient -s -lru` 或 `-s -sum` 收控制面 counters
...
- **用官方 gRPC client 做 VIP/real/stats 配置**
- **复用现有 `e2e/common` 的 agent/scanner/recompile/metrics/vm 基础设施**
