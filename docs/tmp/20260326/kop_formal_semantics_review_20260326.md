# kop Formal Semantics Review (2026-03-26)
...
`docs/kop-formal-semantics.md:121-123` 写的是：

> Let `P` be a BPF program accepted by the verifier after proof lowering, containing kop sites `s_1, …, s_n`.

这句话本身就不成立。`docs/kop-design.md:205-217` 已经明确说了 verifier 在 `lower_kop_proof_regions()` 后分析的是 **instantiated proof sequence**，而不是原始 `sidecar + kop_call`；也就是说，“after proof lowering”的程序不再“contain kop sites”。
...
现在 theorem 直接比较 `P_jit` 和 `P_low`（`docs/kop-formal-semantics.md:129-136`），但没有定义从哪个程序经过哪一步得到它们。按照真实实现，至少应该区分：

- `P_src`: 含 `sidecar + kop_call`
- `P_low`: `lower_kop_proof_regions()` 之后的 proof-sequence 程序（`vendor/linux-framework/kernel/bpf/verifier.c:3723-3835`）
- `P_restored`: `restore_kop_proof_regions()` 之后恢复原 site 的程序（`vendor/linux-framework/kernel/bpf/verifier.c:3837-3880`）
...
proof sketch 里更严重的问题是 `docs/kop-formal-semantics.md:138-143` 的 “structural induction on the program's topological order”。`validate_kop_proof_seq()` 只保证 **site 内部** 没有 back-edge（`vendor/linux-framework/kernel/bpf/verifier.c:3688-3710`），它完全不保证**整个程序**无环。BPF 程序本身当然可以有 loops；kop site 也可以位于 loop 内。这种情况下不存在你文中所说的“program's topological order”。
...
这也直接影响用户问到的“相邻 kop sites 共享寄存器时 structural induction 是否成立”。当前写法下，答案是**不成立**；只有在你把 `LiveOut` 精确定义为 lowered CFG 上的全局 liveness，并改写为 continuation/simulation proof 后，寄存器共享才有机会被正确吸收进去。

### P0: 状态模型不足以支撑 `endian_load*` 的 refinement 结论

`docs/kop-formal-semantics.md:28-49` 把状态定义成 `State = (R, M)`，只保留寄存器和内存字节；`docs/kop-formal-semantics.md:103-111` 的 local refinement 只要求 `Valid_K(p)` 和 `Admissible_K^a(s,p)`。

这对纯寄存器 kop（`rotate64`/`select64`/`extract64`）还勉强够用，但对 `endian_load*` 不够。因为 `docs/kop-formal-semantics.md:366-377` 里的
...
- `docs/kop-formal-semantics.md:136` 里的 “verifier-accepted domain” 也没有形式化定义，无法知道到底量化的是哪些初始状态。

如果你不想把 verifier abstract state 全部形式化，至少要显式引入一个额外前置条件，例如 `SafeLoad_n(base, offset, σ)` 或者 “the current verifier state proves this access is legal”，否则 theorem 里关于 memory-reading kop 的量词范围是不闭合的。

### P0: failure-mode 分析和真实代码路径不一致，且遗漏 late JIT failure / two-pass 语义

`docs/kop-formal-semantics.md:150-157` 和 `docs/kop-design.md:56-59` 都在暗示：
...
3. 若保留 native kop site，后续 arch JIT 直接调用 module emit callback

x86 路径：

- `emit_kop_desc_call()` 在 JIT 时调用 `kop->emit_x86()`（`vendor/linux-framework/arch/x86/net/bpf_jit_comp.c:579-606`）
...
- `emit_kop_desc_call_arm64()` 调用 `kop->emit_arm64()`（`vendor/linux-framework/arch/arm64/net/bpf_jit_comp.c:1201-1235`）
- callback 返回负值会直接让 `build_body()` 失败（`vendor/linux-framework/arch/arm64/net/bpf_jit_comp.c:1643-1647`, `2209-2233`）

问题在于：这时程序里保留的仍然是 `sidecar + BPF_PSEUDO_KOP_CALL`，不是 lowered proof sequence。文档里的 “falls back to the interpreter on the lowered program” 并没有发生。

更糟的是，interpreter 对 `BPF_JMP | BPF_CALL` 的处理是直接跳到 `(__bpf_call_base + imm)`（`vendor/linux-framework/kernel/bpf/core.c:2037-2044`），并不理解 `BPF_PSEUDO_KOP_CALL`。verifier 也明确禁止 non-JITed kop program（`vendor/linux-framework/kernel/bpf/verifier.c:23450-23452`）。所以这里不是一个小措辞问题，而是**你文档声称存在的安全 fallback 路径，在代码里并不存在**。
...
顺带一提，`bpf_prog_select_runtime()` 只把 `kfunc` 视为 `jit_needed`（`vendor/linux-framework/kernel/bpf/core.c:2531-2554`），没有把 kop 单独纳入这个条件。至少从这段代码看，late JIT failure for kop 的最终行为是**不清楚且未文档化的**。我不会直接在 review 里把它定性成实现 bug，但你现在文档里的 fallback 叙述显然没有被代码支持。

### P1: “daemon enforced admissibility” 是 theorem 条件，不是 kernel-enforced invariant；文中把两者说混了

`docs/kop-formal-semantics.md:457-479` 把正确性 obligation 分成 kernel verifier 和 daemon 两部分，这本身是诚实的；但 `:477-479` 紧接着说：
...
`docs/kop-formal-semantics.md:97-104` 把 native semantics 写成 `Emit_K^a(p)`，但真实 callback API 是：

- `emit_x86(..., u64 payload, const struct bpf_prog *prog)`
- `emit_arm64(..., u64 payload, const struct bpf_prog *prog)`

见 `docs/kop-design.md:94-99` 和 `vendor/linux-framework/include/linux/bpf.h:973-978`。
...
`docs/kop-formal-semantics.md:323-324` 和 `:342-343` 把 x86 native path 描述成 `BEXTR`。但 `module/x86/bpf_extract.c` 的真实实现是：
...
`docs/kop-formal-semantics.md:229-233` 把 `rotate64` 的 admissibility 写成无条件的 `tmp ∉ LiveOut(site)`。但真实实现允许 `shift = 0`：
...
`docs/kop-formal-semantics.md:71-74` 说 `Exec_BPF` taken as given from the kernel BPF specification。对系统 note 可以接受；对正式 theorem 不够。
...
- `docs/kop-formal-semantics.md:112-117` 说 “three formal pieces”，但同一文档 `:441-442` 又把对象写成 6-tuple。这里明显不一致。
- `π_s` 用的是 site-level projection，而 theorem 结论又用 `P_jit(s_0)|_{r0,M}` 这种新的投影写法（`docs/kop-formal-semantics.md:133`），没有先定义。
...
1. 原始 BPF idiom 被 rewrite 成 kop site 是否语义保持
2. kop site 被 lower 成 proof sequence 是否 verifier-safe
3. native emit 是否 refinement of proof sequence

现在 formal section 主要在讲 (2)+(3)，但又在 `Admissible` 那里不断借用 daemon pass 的 rewrite logic（`docs/kop-formal-semantics.md:116-117`, `457-479`）。这会让 reader 以为你证明了 (1)，但其实没有。
...
你现在已经承认模型不编码 microarchitectural speculation state（`docs/kop-formal-semantics.md:47-49`, `421-432`, `481-498`），这是好的。但论文里如果想把 barrier 当成 security contribution，必须把边界说得更直白：
...
- appendix：每个 kop 实例的 algebraic semantics，外加 artifact cross-reference
