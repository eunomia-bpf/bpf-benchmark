...
- 当前仓库并没有活跃的 `model_call / bpf_kop_effect` 路径；`docs/kop-design.md:109` 已明确 verifier 语义仍以 `instantiate_insn()` 为准。因此本报告的“Verifier Modeling”审查，实际按“`instantiate_insn()` 与 native emit 是否一致”来做。
...
- `decode_memcpy_payload()` 把 `len` 解成 `kop_payload_u8(payload, 40) + 1`，但没有像 ARM64 版本那样显式限制 `len <= 128`。
- `decode_memset_payload()` 同样没有限制 `len <= 128`。
- 但 descriptor 却声明：
  - `bpf_memcpy_bulk_desc.max_insn_cnt = 128 * 2 = 256`
  - `bpf_memset_bulk_desc.max_insn_cnt = 128`
- verifier 会先按 `max_insn_cnt` 分配 `proof_buf`，再调用 `instantiate_insn()` 写入；只有写完以后才通过 `validate_kop_proof_seq()` 检查 `cnt > max_insn_cnt`。
...
- 在两个 decode 函数中统一加上 `len == 0 || len > KOP_BULK_MAX_BYTES` 的拒绝。
...
- 如果未来想把这个 kop 明确定义成“对所有 proof-legal payload 都无条件等价”，这里还不够 fail-closed。
...
- 但当前 daemon registry 里没有这两个 target：
  - `KNOWN_KOPS` 没有对应项
  - `KopRegistry` 也没有对应 BTF ID 字段

影响：

- 就当前仓库状态而言，这两个 kop 还不能通过现有 daemon 的自动发现/发射路径端到端使用。
...
- 当前 payload 实际只消费到：
  ...
  - `base` bits `[11:8]`
  - `offset` bits `[27:12]`
- `decode_pair_payload()` 只用 `kop_payload_reg(payload, 28)` 检查了 `[31:28]` 这 4 bit。
...
- 三个模块都通过 `DEFINE_KOP_V2_MODULE(...)` 注册，风格与现有模块一致。
