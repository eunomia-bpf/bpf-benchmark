# ARM64 `LDP/STP` 128-bit kop 设计文档

日期：2026-03-26  
状态：design-only  
范围：为 BpfReJIT 设计 ARM64 `LDP/STP` pair load/store kop，不修改 kernel/daemon 代码
...
4. **Verifier 建模应完全走当前仓库的 v2 `instantiate_insn()` proof-lowering 模型。** 不要为这个设计回退到已经过时的 `model_call()` / `bpf_kop_effect` DSL。
...
1. `kop` 已经是 **v2 first-class instruction**，核心接口是：
   - `instantiate_insn(payload, insn_buf)`
   - `emit_arm64(...)`
2. verifier 的主线是：
   - 识别 `sidecar + BPF_PSEUDO_KOP_CALL`
   - `instantiate_insn()` 生成 proof sequence
   - 验证 proof sequence
   - 主分析时临时 lower 成普通 BPF
   - 分析结束后 restore 成原始 kop
3. ARM64 JIT 当前对普通 `BPF_LDX|STX|BPF_MEM|BPF_DW` 仍然逐条发 `LDR/STR`，没有现成 pair lowering。
4. 当前 ARM64 JIT 普通 scalar memory path 会为每条可能 fault 的内存指令接 `add_exception_handler()`。而 kop native emit 路径没有自动的 pair-memory extable/fixup 机制。
...
1. `STP` kop 仍然只编码两个源寄存器，不直接编码立即数。
...
## 3. kop API 与 Packed ABI 设计

## 3.1 API 形状

建议新增两个 ARM64 kop target：
...
- `CALL.imm` = 导出的 `struct bpf_kop` descriptor 的 BTF VAR id
...
3. 避免 daemon/liveness 对多输出 kop 的额外复杂度。
...
用户问题里提到的 `bpf_kop_effect` / `kop_ops model_call`，对应的是旧思路。  
**当前仓库真实主线不是这个模型。**

当前实现见 `docs/kop-design.md`：

- kernel 中的 descriptor 是 `struct bpf_kop`
...
虽然 kernel 不再消费 `bpf_kop_effect`，但概念上可以把语义写成：
...
而 `emit_kop_desc_call_arm64()` 只负责让 module emit 原生指令，本身不补 pair-memory extable/fixup。
...
- 当前 verifier fixup 逻辑只按 “target 有无 native emit” 决定是否保留原始 kop；
- 一旦某个 kop target 有 native emit，ARM64 JIT 就会尝试发射；
...
未来如果需要让后续 pass 跨过 `ldp/stp` 继续做分析，再补一层 daemon-side `KopSemantics` 表即可；不要为这个去修改 kernel verifier ABI。
...
对单个 pair-mem kop，**应选 signed offset**：
...
- 可以避免为一个新 kop 去扩大通用 ARM64 helper 面
...
因为当前 kop native emit 路径没有 pair-memory extable glue。  
...
- rewrite 后 `sidecar + kop call`：仍然是 2 条 BPF 指令
...
- `LDP/STP pair kop` 是一个 **residual optimizer**
...
- 长 copy/zero run 用 block-mem kop 更划算
...
- 如后续 pass 需要跨 kop 做 dataflow，再补 daemon-side `KopSemantics`
...
1. **把它定义成 pair-memory kop，不是 128-bit BPF ISA 扩展。**
