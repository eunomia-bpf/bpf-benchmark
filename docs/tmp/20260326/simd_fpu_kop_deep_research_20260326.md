# SIMD/FPU kop 深度研究（BpfReJIT，面向 OSDI'26）
...
- **v1 不应该做 true SIMD/FPU kop**。应该坚持现有设计：  
...
- **论文故事**可以讲，但不该讲成“我们在 kernel BPF 里广泛用 AVX2/NEON 很赚”。更强的故事其实是：
  - **BpfReJIT 的 semantic kop 能恢复宽机器语义；**
...
- **仓库内已有设计文档**：
  ...
  - `docs/tmp/20260324/simd_kop_design_20260324.md` [SIMD-DESIGN]
...
> **我们的贡献不是“强行给 BPF 加 SIMD”，而是识别出 semantic kop 的真正 profitable frontier。**
...
- 不必把 `bpf_crc32(c)` 自动等价成 “必须引入 FPU kop”
...
- **“所以应该给 BPF JIT 做 SIMD hash kop” 不成立**
...
1. **每个 SIMD kop site 自己 begin/end**
...
> **如果以后真的做 SIMD/FPU kop，它应该是少数几个高度语义化的 operation，而不是一个“通用向量模式”。**

---

## 8. 实现复杂度评估

## 8.1 需要多少新 kop/module？
...
> **保持 semantic kop 不变，把 no-FPU / scalar / SIMD 作为 backend selection。**
...
- 只要 kop 仍然能被 instantiate 成等价的普通 BPF 证明序列，就可以让 native emitter 在内部偷偷用 SIMD reg
...
建议是 **operation family**，不是“一个 generic SIMD kop”。
...
> **保留按语义划分的 kop family，让每个 family 内部自己做 no-FPU / scalar / SIMD backend dispatch。**
...
- **不要**把 “SIMD kop” 在 x86 上实现成“默认广泛用 FPU”
...
1. **semantic kop 让 JIT 能恢复更宽机器语义**
2. **这个 frontier 分两层**
...
> **OSDI'26 主线：YES to wide-machine kop，NO to x86 true-SIMD mainline。**
...
- [SIMD-DESIGN] `docs/tmp/20260324/simd_kop_design_20260324.md`
