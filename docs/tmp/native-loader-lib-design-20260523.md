...
2026-05-28 update: the companion-object open/load path is no longer part of
libnativeloader. The micro runner owns `bpf_object__open_file` /
`bpf_object__load`, keeps the companion object alive for the test, copies the
source instructions, and calls the same fd-based loader path used by the shim.
The loader now accepts an already-loaded `original_prog_fd` plus either source
instructions from the runner or source bytecode/fd-array facts from the shim.
...
### 2026-05-28 short-term native-loader cleanup plan

This pass is intentionally limited to the performance-critical native-loader /
native-link boundary. It does not attempt the larger kernel-module redesign.

- [x] Remove runtime map-retention instructions from the stub. The loader now
  appends retained map fds to `BPF_PROG_LOAD` `fd_array`, sets
  `fd_array_cnt`, and keeps the post-load `prog_info.map_ids` fail-fast
  validation. This replaces the old dummy
  `r1 = 1; if r1 == 1 jump-over; ldimm64 map_fd...` sequence with a
  verifier/load-time map reference mechanism, so the final JIT has no
  map-retention branch.
- [x] Keep helper calls as direct `call rel32` only. The x86 linker and native
  lab module now reject helper slots that cannot be patched as rel32; the old
  PLT32/GOT indirect-call fallback is not allowed to reach runtime.
- [x] Verify representative stage2 lookup lowering. `map_hash_lookup`,
  `map_percpu_array`, and `map_percpu_hash_counter` native final JIT dumps
  use direct `e8 rel32` calls or inline array/percpu lowering, with no
  `call *%rax`/`call *%rip` fallback.
- [x] Finish the typed native-link plan rollout. The loader now writes a
  versioned JSON `--link-plan` for helpers, maps, lookup sites, update sites,
  ABI facts, and map facts. The old positional `--helper` / `--map` /
  `--lookup-site` / `--lookup-map` / `--update-site` parser has been removed
  from native-link, and the standalone native-lab attach harness also emits a
  link-plan.
- [ ] Thin `libnativeloader` further. The target shape is still:
  `original_prog_fd + native manifest -> query prog/map info -> choose native
  object -> resolve runtime addresses -> call native-link -> load native blob
  -> verify used_maps -> return native fd`. App-specific map-name and
  bytecode-shape workarounds should move into generated manifest facts before
  they are removed from generic loader code.

Expanded thin-loader TODO:

- [x] Move micro companion `.bpf.o` opening/loading out of `libnativeloader`.
  The micro runner should own `bpf_object__open_file` / `bpf_object__load`,
  keep the companion object alive for the test, and pass the resulting loaded
  program fd plus source/fd-array facts into the same fd-based loader path used
  by the shim. After this lands, delete `LoadOptions::bpf_object_path` and
  `load_from_companion_object()`.
- [ ] Replace loader-side manifest matching predicates that inspect source
  bytecode (`source_map_prefix`, `source_has_helper`,
  `source_lacks_helper`) with generated manifest keys that are stable at the
  app/build boundary: program name, prog type, source hash, section/symbol
  name, and explicit map/callsite facts. Until that manifest exists, do not
  delete the current predicates because they choose between real corpus objects.
- [x] Move the remaining app-specific map semantics out of generic loader
  code. `cilium_calls` prog-array discovery is now expressed as a generated
  manifest `native_symbol` rule, and `policy_filter_` inner-map shape
  reconstruction is expressed as generated manifest `inner_shape` facts.
  `libnativeloader` no longer contains corpus app names.
- [ ] Split `native_loader.cpp` into header-only support modules without
  adding another C++ translation unit: manifest parsing/matching, source
  bytecode facts, link-plan JSON emission, fd-array/stub load, and runtime
  address resolution. The `.cpp` should shrink toward orchestration only.
  2026-05-28: link-plan JSON emission is now split into
  `native_loader_link_plan.hpp`. The new module is header-only and only writes
  the typed JSON plan consumed by `native-link`.
- [ ] After each split/removal, rerun at minimum:
  `make -C native-sim/libnativeloader`,
  `cargo test --release --manifest-path native-sim/x86/native_lab/native_link/Cargo.toml`,
  a focused stage2 micro helper/map smoke, and a focused bcc/set native-loader
  corpus smoke. After all corpus apps pass, rerun the formal paper-grade
  micro/corpus matrix before updating `docs/eval_native.md`.

Validation so far:

- `make -C native-sim/libnativeloader` passed after the `fd_array_cnt`
  retained-map change.
- `cargo test --release --manifest-path native-sim/x86/native_lab/native_link/Cargo.toml`
  passed after the rel32 fail-fast changes.
- Stage2 map-retention smoke:
  `TIMEOUT=3600 SUITE=micro/config/micro_stage2.yaml BENCH=map_array_lookup RUNTIMES="native_kernel kernel" SAMPLES=1 WARMUPS=0 INNER_REPEAT=10 make micro`
  passed in `micro/results/x86_kvm_micro_20260528_022737_947738`.
  `native_kernel` measured 50 ns and `kernel` measured 92 ns in that focused
  run; the native final JIT contained no `ff d0`, no `ff 15`, and no
  retained-map `mov r15d,1` artifact.
- Focused bcc/set native-loader smoke after the map-retention change:
  `BPFREJIT_SHIM_NATIVE_LOADER=post SKIP_REJIT=norejit BPFREJIT_CORPUS_APPS="bcc/set" SAMPLES=1 WORKLOAD_DURATION=10 TIMEOUT=3600 make corpus`
  passed in `corpus/results/x86_kvm_corpus_20260528_023235_095867`.
- Stage2 helper-call smoke after deleting the PLT32/GOT fallback:
  `TIMEOUT=3600 SUITE=micro/config/micro_stage2.yaml BENCH=helper_only_uid_gid RUNTIMES="native_kernel kernel" SAMPLES=1 WARMUPS=0 INNER_REPEAT=10 make micro`
  passed in `micro/results/x86_kvm_micro_20260528_024227_303766`.
  The native final JIT used one direct `e8 rel32` helper call and no
  indirect-call fallback.
- Focused bcc/set native-loader smoke after the PLT32 fix passed in
  `corpus/results/x86_kvm_corpus_20260528_025555_549713`. The shim logs
  showed no native-loader failure, no PLT32 helper failure, no GOT-relative
  indirect fallback, no `ff d0`, no `ff 15`, and no legacy nop padding.
- Stage2 lookup smoke:
  `TIMEOUT=3600 SUITE=micro/config/micro_stage2.yaml BENCH="map_hash_lookup map_percpu_array map_percpu_hash_counter" RUNTIMES="native_kernel kernel" SAMPLES=1 WARMUPS=0 INNER_REPEAT=10 make micro`
  passed in `micro/results/x86_kvm_micro_20260528_030218_099131`.
  Focused samples were: hash lookup 201 ns native vs 211 ns kernel,
  percpu-array 35 ns native vs 115 ns kernel, and percpu-hash 219 ns native
  vs 203 ns kernel. The percpu-hash result needs a longer run before drawing a
  performance conclusion, but the disassembly shows direct/equivalent lowering
  rather than an indirect-call fallback.
- Stage2 helper-call smoke after switching the loader/native-link interface to
  `--link-plan` passed in `micro/results/x86_kvm_micro_20260528_032108_175250`.
  `native_kernel` measured 83 ns and `kernel` measured 103 ns in that focused
  run. The final native JIT contains the expected direct `e8 rel32` helper
  call and no matching failure signatures in the artifact logs.
- Build/unit validation after deleting the positional parser:
  `make -C native-sim/libnativeloader` passed,
  `make -C native-sim/test-others all` passed, and
  `cargo test --release --manifest-path native-sim/x86/native_lab/native_link/Cargo.toml`
  passed.
- Final focused stage2 smoke after all short-term changes:
  `TIMEOUT=3600 SUITE=micro/config/micro_stage2.yaml BENCH="helper_only_uid_gid map_hash_lookup map_percpu_array map_percpu_hash_counter" RUNTIMES="native_kernel kernel" SAMPLES=1 WARMUPS=0 INNER_REPEAT=10 make micro`
  passed in `micro/results/x86_kvm_micro_20260528_033432_889450`.
  This is still a short smoke with CPU-governor warnings, not a paper-grade
  performance run. The samples were helper 75 ns native vs 54 ns kernel,
  hash lookup 271 ns native vs 205 ns kernel, percpu-array 48 ns native vs
  121 ns kernel, and percpu-hash 136 ns native vs 194 ns kernel. The important
  correctness/property check for this pass is that the final native JIT dumps
  contain direct `e8 rel32` calls or inline lowering only, with no `ff d0`,
  `ff 15`, `ff 25`, legacy nop padding, or retained-map `mov r15d,1`.
- Final focused bcc/set native-loader corpus smoke after all short-term
  changes:
  `BPFREJIT_SHIM_NATIVE_LOADER=post SKIP_REJIT=norejit BPFREJIT_CORPUS_APPS="bcc/set" SAMPLES=1 WORKLOAD_DURATION=10 TIMEOUT=3600 make corpus`
  passed in `corpus/results/x86_kvm_corpus_20260528_033938_293932`.
  App status was `ok`; post replaced 25 programs; the shim logs contain no
  native-loader failure, link-plan mismatch, PLT32 helper failure,
  GOT-relative indirect failure, `ff d0`, `ff 15`, legacy nop padding, or
  reloc-bounds error. Raw stress-ng smoke counters were syscall 514 -> 7554
  bogo ops, cap 3037545 -> 3044438, set 233035 -> 233532, and sockfd
  2235560 -> 2203864. These are functional smoke counters only; the formal
  workload-number comparison still requires the 180s/sample paper setup.
- Build/unit validation after moving companion `.bpf.o` loading out of
  libnativeloader and into the micro runner:
  `make -C native-sim/libnativeloader` passed, and
  `cargo test --release --manifest-path native-sim/x86/native_lab/native_link/Cargo.toml`
  passed. The focused stage2 helper/map smoke
  `TIMEOUT=3600 SUITE=micro/config/micro_stage2.yaml BENCH="helper_only_uid_gid map_hash_lookup map_percpu_array map_percpu_hash_counter" RUNTIMES="native_kernel kernel" SAMPLES=1 WARMUPS=0 INNER_REPEAT=10 make micro`
  passed in `micro/results/x86_kvm_micro_20260528_041946_716351`.
  The matching bcc/set corpus smoke passed in
  `corpus/results/x86_kvm_corpus_20260528_042511_272401`; app status was
  `ok`, post replaced 25 programs, and the shim logs contained no
  native-loader failure, link-plan mismatch, indirect-call fallback,
  legacy-nop padding, retained-map `mov r15d,1`, or reloc-bounds error.
- Validation after splitting link-plan JSON emission into
  `native_loader_link_plan.hpp`: `make -C native-sim/libnativeloader` passed,
  `cargo test --release --manifest-path native-sim/x86/native_lab/native_link/Cargo.toml`
  passed, and the focused stage2 helper/map smoke passed in
  `micro/results/x86_kvm_micro_20260528_043307_011123`. The matching bcc/set
  corpus smoke passed in
  `corpus/results/x86_kvm_corpus_20260528_043809_086869`; app status was
  `ok`, post replaced 25 programs, and the same shim/native failure-signature
  scan was empty. Raw stress-ng counters in this smoke were syscall
  514 -> 514, cap 3022011 -> 3046348, set 235738 -> 236326, and sockfd
  2244102 -> 2212473. These remain smoke counters only.
- Validation after deleting the cross-process kallsyms cache:
  `make -C native-sim/libnativeloader` passed,
  `cargo test --release --manifest-path native-sim/x86/native_lab/native_link/Cargo.toml`
  passed, the focused stage2 helper/map smoke passed in
  `micro/results/x86_kvm_micro_20260528_053819_279334`, and the bcc/set
  native-loader corpus smoke passed in
  `corpus/results/x86_kvm_corpus_20260528_054311_978575`. The bcc smoke
  replaced 25 programs and had no native-loader failure, reloc-bounds error,
  indirect-call fallback, legacy-nop padding, or retained-map `mov r15d,1`
  signature.
- Validation after replacing the last generic-loader Cilium map alias with a
  manifest `native_symbol` rule and fixing the map-rule timing bug:
  the focused stage2 helper/map smoke passed in
  `micro/results/x86_kvm_micro_20260528_055351_773463`; Cilium passed in
  `corpus/results/x86_kvm_corpus_20260528_055846_891463` with 113 native
  replacements; bcc/set passed again in
  `corpus/results/x86_kvm_corpus_20260528_060459_351420` with 25 native
  replacements; and Tetragon passed in
  `corpus/results/x86_kvm_corpus_20260528_061124_489289` with 288 native
  replacements. The failure-signature scan was empty for these smokes.
- Validation after removing dead timing/state fields:
  `make -C native-sim/libnativeloader` passed,
  `cargo test --release --manifest-path native-sim/x86/native_lab/native_link/Cargo.toml`
  passed, the focused stage2 helper/map smoke passed in
  `micro/results/x86_kvm_micro_20260528_062402_731848`, and bcc/set passed in
  `corpus/results/x86_kvm_corpus_20260528_062858_936625`. The smoke had
  correct results for all four focused stage2 programs; bcc/set app status was
  `ok`, post replaced 25 programs, and the shim failure-signature scan was
  empty. The low-`INNER_REPEAT` stage2 smoke showed mixed timing for
  `map_percpu_hash_counter`, so it is not used as a performance conclusion.

Current code-size delta for this cleanup batch: tracked changes in
`native_loader.hpp`, `native_loader.cpp`, `native_loader_bytecode.hpp`,
`native_loader_link_schema.hpp`, `native_loader_manifest.hpp`,
`native_lab_runner.cpp`, `native-link` x86 support, and the vendor manifest
generator are 377 insertions and 717 deletions. `native_loader.cpp` is now
3442 lines, and the new header-only `native_loader_link_plan.hpp` is 150
lines. The next useful reductions are source-bytecode manifest predicates,
fd-array/stub load, and runtime address resolution.
