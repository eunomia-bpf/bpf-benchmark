...
- [ ] Move micro companion `.bpf.o` opening/loading out of `libnativeloader`.
...
- [ ] Move the remaining app-specific map semantics out of generic loader
  code. Current known holdouts are `cilium_calls` prog-array discovery and
  `policy_filter_` inner-map shape reconstruction; both should become typed
  manifest facts generated by the vendor/native build.
