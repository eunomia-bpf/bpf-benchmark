# P89H kop verifier root causes

## Scope

Latest saved failure corpus used for host-side replay:

`corpus/results/x86_kvm_p89_kop_corpus_20260430_131403_981524`
...
Root cause: the x86 lowering path for the endian-fusion kop call materialized
frame-pointer stack offsets into a temporary register before the kop sidecar
...
Fix: encode stack offsets directly in the kop sidecar payload when the operand
...
path rejecting the transformed program. The current kop ReJIT metadata
...
Fix: `extract` now requires single-subprogram func-info before emitting kop
...
emit kop candidates into programs whose func-info layout was not proven safe
for the current ReJIT replacement boundary.

Fix: `bulk_memory` now applies the same single-subprogram func-info proof guard
before emitting kop candidates. Multi-subprogram inputs are left byte-for-byte
