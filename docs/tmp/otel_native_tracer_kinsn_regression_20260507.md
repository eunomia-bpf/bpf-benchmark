# OTEL `native_tracer_entry` kinsn-only Regression Investigation
...
ratio of `2.486` under the kinsn-only pass set, making post-ReJIT about 2.5x slower
...
Latest local artifact matching the dedicated kinsn-only list and containing the reported
`2.486` ratio:
...
kinsn-only artifact that reproduces the `2.486` ratio does not. For this program,
...
## kinsn Pass Application on `native_tracer_entry`
...
Current `kinsn_replacement_subprog_skip_reason()` only checks replacement/subprogram
...
- Each prefetch insertion emits a packed kinsn call sequence, so 12 sites add 24 BPF
  instructions.
...
### 4. Medium-low probability: register pressure / spills from combined kinsn calls

Evidence:

- Active transformations introduce packed kinsn call sequences:
...
- Skip bulk-memory replacement when the scalarized stores target stack/per-CPU scratch
  state in a perf-event entry path and the replacement would introduce packed kinsn calls.
