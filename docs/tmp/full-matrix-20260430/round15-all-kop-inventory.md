...
- 检查每个模块注册的 `bpf_kop` descriptor（`DEFINE_KOP_V2_MODULE`）
- 检查 `bpfopt/crates/bpfopt/src/pass.rs` 中 `KopRegistry` 的 BTF ID 字段
- 检查 `bpfopt/crates/bpfopt/src/passes/mod.rs` 和各 pass 文件确认哪些 pass 消费哪个 kop
- 检查 `daemon/crates/bpfget/src/lib.rs` 的 `KOP_PROBE_TARGETS` 列表
- 检查 `corpus/config/benchmark_config.yaml` 的 12-pass 默认策略
- 检查 `docs/kernel-jit-optimization-plan.md` 中规划/研究过的 kop
- 参考 `docs/tmp/setcc_cset_kop_research_20260430.md`、`andn_kop_research_20260430.md`、`bls_kop_research_20260430.md`、`region_kop_research_20260430.md`

---

## 完整 kop inventory 表

### 已实现的 kop（module 层有 `.c` 文件）

| kop 名 | kernel 端实现文件 | x86 JIT emit | ARM64 JIT emit | bpfopt pass 消费 | daemon bpfget 探针 | 12-pass 默认策略 | 状态 |
|---|---|---|---|---|---|---|---|
...
### 仅在设计/调研文档中讨论、尚未实现的 kop 候选

| kop 候选名 | 设计状态 | x86 原语 | ARM64 原语 | 调研文档 | 推荐 |
|---|---|---|---|---|---|
| `bpf_setcc` | 📝 调研完成，待实现 | `cmp/test; mov 0; setcc` | `cmp/tst; cset` | `setcc_cset_kop_research_20260430.md` | **Round 16/17 候选** — 9417 sites |
| `bpf_andn` | ❌ 不做第一波 | BMI1 ANDN | ARM64 BIC | `andn_kop_research_20260430.md` | 暂缓 — 全部需 liveness proof |
| `bpf_blsi/blsr` | ⏸ 后续 phase | BMI1 BLSI/BLSR | 无直接对应 | `bls_kop_research_20260430.md` | 暂缓 — 支持 app 0 sites |
| region kop | ⏸ 后续 phase | 通用多寄存器区域 | 同 | `region_kop_research_20260430.md` | 等 kop v3 ABI 收敛 |

---

## bpfget 探针与 pass 关联

`daemon/crates/bpfget/src/lib.rs` 中 `KOP_PROBE_TARGETS` 列表共探针 **8 个逻辑 kop**（其中 2 个有旧名兼容）：
...
`wide_mem` pass 不依赖任何 kop（纯 bytecode 替换，不发 kop call）。

---

## 架构不对称性汇总

| kop | x86 实现 | ARM64 实现 | 说明 |
|---|---|---|---|
...
## 可以在 round 16/17 新增 pass 激活的 kop
...
- **缺什么**：
  1. bpfget KOP_PROBE_TARGETS 未添加 probe
  2. daemon missing_target_kops 未加 ldp/stp case
  3. bpfopt KopRegistry 无 ldp128/stp128 BTF ID 字段
  4. 无对应 bpfopt pass（扫描相邻 LDX_MEM/STX_MEM 对，emit kop call）
...
- **状态**：`docs/tmp/setcc_cset_kop_research_20260430.md` 调研完成，结论 ✅ 推荐第一波实现
...
项目 kernel module 层共实现 **13 个 kop**（不计 setcc 等未实现候选）：
- `bpf_rotate64`、`bpf_select64`、`bpf_extract64`、`bpf_endian_load{16,32,64}`（各 3 kop 共 5 variant）、`bpf_memcpy_bulk`、`bpf_memset_bulk`、`bpf_prefetch`、`bpf_ccmp64`、`bpf_ldp128`、`bpf_stp128`、`bpf_speculation_barrier`。
