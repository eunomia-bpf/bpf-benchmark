# Round 15：kop passes 启用状态 + dead code 审计
...
| 6 | `wide_mem` | `wide_mem.rs` | 否（pure bytecode）| 无（注释明确："No kop support needed -- pure BPF replacement"） |
...
**kop pass 定义**：运行时会发出 kfunc `CALL` 指令（BPF_CALL kfunc），需要 BTF module fd_array 才能通过 ReJIT verifier。
...
- **daemon 代码存根**：`pass_needs_target()` line 1489 和 `missing_target_kops()` line 1505 都有 "ccmp" 的 match arm，但由于 `effective_pass_list()` 阻止 ccmp 进入 pass_list，这些 arm **永远不会被执行**（见下方 dead code 分析）
...
- **类型**：pure bytecode pass（不依赖 kop/kfunc）
...
#### D2：`pass_needs_target` 和 `missing_target_kops` 中的 "ccmp" match arm

**位置**：
- `daemon/src/commands.rs:1489`（`needs_target()` 函数）
- `daemon/src/commands.rs:1505`（`missing_target_kops()` 函数）
...
| `rotate` | **kop** | ✅ | 早期 | bpf_rotate64，ROL/ROR |
| `cond_select` | **kop** | ✅ | 早期 | bpf_select64，CMOV/CSEL |
| `extract` | **kop** | ✅ | 早期 | bpf_extract64，BEXTR |
| `endian_fusion` | **kop** | ✅ | 早期 | bpf_endian_load32/16/64，MOVBE |
...
| `bulk_memory` | **kop** | ✅ | 早期 | bpf_memcpy_bulk + bpf_memset_bulk |
| `prefetch` | **kop** | ✅ | 2026-04-30（9b9bd28e） | bpf_prefetch；最新加入 |
| `ccmp` | **kop** | ❌ **未启用** | 2026-04-30（b80a1c19） | ARM64 only，bpf_ccmp64 |
| `branch_flip` | pure bytecode（PGO） | ❌ **未启用** | Paper B scaffolding | 需要 per-site PMU profile |

**默认列表中 kop passes 数量：7 个**（rotate, cond_select, extract, endian_fusion, bulk_memory, prefetch，以及 ccmp 虽有 daemon 代码存根但不在默认列表）

实际数：`rotate + cond_select + extract + endian_fusion + bulk_memory + prefetch = 6 个 kop pass 在默认列表中`
...
| D2 | `daemon/src/commands.rs:1489,1505` | `needs_target/missing_target_kops` 中的 "ccmp" arm | 低 | 加入 ccmp 到默认 list 时自然激活；现在是死 match arm |
...
### 3.3 Round 16 kop-only pass list 建议

目标：只跑 kop 路径，隔离 BTF module fd_array 和 per-pass ReJIT 的正确性测试。

**推荐 kop-only pass list（按 DEFAULT 顺序取子集）**：
...
**不包含**：
- `wide_mem`（pure bytecode，不走 kop 路径）
- `map_inline`（需要 map 值 snapshot，不是 kop）
- `const_prop`（需要 verifier states，pure bytecode）
- `dce`（pure bytecode）
- `bounds_check_merge`（pure bytecode）
- `skb_load_bytes_spec`（pure bytecode）
- `ccmp`（kop 但未在 daemon 默认列表；daemon 拒绝非默认 list）

**注意**：当前 `effective_pass_list()` 拒绝任何非默认列表，所以这个 kop-only list **无法直接传给 daemon**。
选项：
1. 临时修改 `DEFAULT_DAEMON_PASS_LIST` → 改为 kop-only 子集
2. 或者放宽 `effective_pass_list()` 允许默认 list 的子集（非空且每个元素在默认 list 内）

**round 16 若只需验证 kop 路径**，推荐方案 1（临时替换 DEFAULT_DAEMON_PASS_LIST），运行完恢复。
