...
Round 14 （commit `298211a0` "Refactor daemon to per-pass ReJIT"）删除了 `call_offset` 字段和 BTF 模块 fd 的加载逻辑，导致所有使用 kop 的程序 REJIT 失败。本报告记录根因分析和修复。

## Bug A：bpfopt extract pass 产生错误 BTF ID

### 症状

`bpfopt --pass extract` 输出的字节码中，kop CALL 指令的 `off=0`（vmlinux BTF），
但 `btf_func_id`（如 129876）属于内核模块 BTF 对象，不是 vmlinux。
内核 verifier 报错：`kernel btf_id 129876 is not a function`。

### 根因

`bpfget::TargetKopJson` 在 commit 298211a0 中被删除了 `call_offset` 字段。
`probe_kops_in_btf` 探测出 kop 函数后，不再记录该函数所在的 fd_array slot。
`bpfopt main.rs::kop_registry_from_target` 读取 `target.json` 时 `call_offset=None`，
于是 `KopRegistry::call_off_for_target_name` 返回 0，所有 kop CALL 的 `off` 都是 0（vmlinux）。
但 kop 实现位于内核模块 BTF 对象中，verifier 需要 `off=N`（fd_array[N-1] 是该模块 fd）才能找到函数类型。

### 修复（bpfget/src/lib.rs）

1. `TargetKopJson` 增加 `call_offset: u32` 字段（`#[serde(default, skip_serializing_if="is_zero_u32")]`，向后兼容）。
2. `probe_target_kops` 维护 `module_slot_map: BTreeMap<u32, u32>`，按 BTF 对象枚举顺序为每个新发现的模块 BTF ID 分配 1 起的 slot。
   - 能独立加载（`load_from_kernel_by_id` 成功）的 BTF 对象为 vmlinux，`call_offset=0`。
   - 只能 split-BTF 加载的 BTF 对象为内核模块，分配递增 slot。
3. `probe_kops_in_btf` 新增 `is_module: bool` 和 `module_slot_map` 参数，将 `call_offset` 写入返回的 `TargetKopJson`。
...
同 Bug A：`call_offset` 被删除导致 `kop_call_matches` 无法区分来自不同模块但具有相同
`btf_func_id` 的不同 kop。具体来说，`bpf_extract64`（btf_id=4, call_offset=5）和
`bpf_rotate64`（btf_id=5, call_offset=3）在各自模块内的 type id 相同（均为 129876）。
在 `remap_kop_btf_metadata` → `collect_kop_proof_regions` → `kop_proof_len` 路径中，
`kop_call_matches` 看到 btf_id 匹配 extract64 于是调用 `extract_proof_len`，
但实际 payload 是 prefetch 格式（register number in bits[3:0]），`bit_len=0`，触发 bail。

### 修复

Bug B 与 Bug A 共享同一修复：恢复 `call_offset` 后，`kop_call_matches` 能通过
...
1. `TargetKopJson`（commands.rs 本地定义）增加 `call_offset: u32` 字段。
2. `try_apply_one_with_map_access` 将 probed koperation 保存在内存中，传给 `build_rejit_fd_array`。
3. `build_rejit_fd_array` 新增 `koperation: &HashMap<String, TargetKopJson>` 参数：
