...
| kop modules | 18 | 50 | +2,233 / -204 | `b3a5f650` prefetch modules; `d02ac35e` ccmp module; `ce956452` BTF metadata remap |
...
4/30 focused on image and pass/kop: `a27b8100` added the GHCR push/pull mechanism, but the report shows the GHCR token lacked package scope and push did not succeed; `bd423eef` narrowed Katran artifact cache inputs to avoid small daemon/kernel changes triggering full folly/Katran rebuilds. P89 prefetch finished module, pass, and runtime wiring, and structural prefetch was enabled by default by `9b9bd28e`. In the same period verifier rejects such as wide_mem misaligned load, tail-call-sensitive kop, and BTF remap were fixed.
...
| verifier reject optimized bytecode | ~13% | misaligned wide load, kop subprog/tail-call boundary, candidate BTF metadata inconsistency |
