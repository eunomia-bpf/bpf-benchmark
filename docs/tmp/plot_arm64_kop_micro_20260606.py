#!/usr/bin/env python3
"""Post-hoc arm64 kop microbenchmark analysis for the 2026-06-06 artifacts."""
...
def sample_kop_counts(sample: dict) -> tuple[int, int, Counter[str]]:
    matched = 0
    applied = 0
...
def load_rows(path: Path) -> tuple[dict, list[dict]]:
    ...
    for bench in data["benchmarks"]:
        ...
        for sample in rejit_samples:
            matched, applied, sample_names = sample_kop_counts(sample)
...
def summarize(data: dict, rows: list[dict]) -> dict:
    ...
    kop_speedups = [row["speedup"] for row in rows if row["applied_median"]]
    ...
    return {
        ...
        "kop_geomean": geomean(kop_speedups) if kop_speedups else None,
        "kop_benchmarks": len(kop_speedups),
...
def write_summary(pure_data: dict, pure_rows: list[dict], helper_data: dict, helper_rows: list[dict]) -> dict:
    ...
    kop_geomean = (
        f"{summary['kop_geomean']:.3f}x over {summary['kop_benchmarks']} benchmarks"
        if summary["kop_geomean"] is not None
        else "N/A"
    )
    helper_kop_geomean = (
        f"{helper_summary['kop_geomean']:.3f}x over {helper_summary['kop_benchmarks']} benchmarks"
        if helper_summary["kop_geomean"] is not None
        else "N/A"
    )
    lines = [
        ...
        f"- KOperation-bearing geomean: {kop_geomean}",
        ...
        f"- KOperation calls by name, all samples: {format_counter(summary['names'])}",
        "",
        "## Helpers/maps rerun",
        "",
        ...
        f"- KOperation-bearing geomean: {helper_kop_geomean}",
        ...
        f"- KOperation calls by name, all samples: {format_counter(helper_summary['names'])}",
        "",
        "## Per-case results",
        "",
...
def draw(rows: list[dict], summary: dict) -> None:
    ...
    ax.set_title(
        "arm64 AWS kop ReJIT full micro (pure bytecode 29, SAMPLES=3)",
        fontsize=16,
        pad=10,
    )
    ...
    fig.tight_layout()
    fig.savefig(FIG / "kop-micro-arm64-rejit-20260606.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    print(f"wrote {FIG / 'kop-micro-arm64-rejit-20260606.png'}")
