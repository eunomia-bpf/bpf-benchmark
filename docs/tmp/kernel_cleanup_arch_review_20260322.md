...
### B.3 inline kfunc (KF_INLINE_EMIT) 路径
...
**JIT 分发**：
- x86：`emit_inline_kfunc_call()` → `ops->emit_x86()` → 检查 `ret == off && ret <= max_emit_bytes`
- ARM64：`emit_inline_kfunc_call_arm64()` → `ops->emit_arm64()` → 检查 `ctx->idx - saved_idx == n_insns` + `n_insns * 4 <= max_emit_bytes`
