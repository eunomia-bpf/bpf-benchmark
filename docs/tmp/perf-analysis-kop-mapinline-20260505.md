# Performance analysis: kop, wide_mem, map_inline (2026-05-05)
...
- Pass set + samples: `['kop']`; samples `30`; workload_seconds `1.0`
...
This kop run is not numerically usable as a kop-only authoritative result: the directory contains only `details/apps/calico__felix.json`, that app failed before baseline/post counters were collected, and `details/result.json` has an empty top-level `per_program` list. All Method B values are therefore `N/A` rather than zero-speedup results.
...
|---|---|---:|---|---:|---:|---:|
| kop-only authoritative | `['kop']` | 30 | error | 0 | N/A | N/A |
...
- KOperation vs map_inline: the specified kop-only directory has no comparable program measurements, so the kop-vs-map_inline geomean comparison is not computable from the requested on-disk data. Among the analyzable runs here, map_inline is better than wide_mem on the retained/applied Method B metric: map_inline `0.980383` vs wide_mem `1.079309`.

### Recommendation

No pass deserves a strong paper headline number from these three directories alone. Map_inline is the best candidate among the analyzable data because it is the only pass set with an applied-only Method B ratio below 1.0, but the retained applied population is only 3 programs. Wide_mem alone regresses on both applied-only and all-retained Method B, and the named kop-only run failed before measurements.

### Open questions / data gaps

- Need a valid kop-only corpus run on the current supported app set; the specified kop run stopped at removed/unsupported `calico/felix`.
- Need a 12-pass cumulative run to measure pass interaction instead of isolated wide_mem/map_inline behavior.
- Need ARM64 data for map_inline/kop comparison.
