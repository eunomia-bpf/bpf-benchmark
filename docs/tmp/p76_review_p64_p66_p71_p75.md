...
| `cli_pipeline.rs` 83/162 LOC removal did not remove e2e smoke coverage | ✅ | `bpfopt/crates/bpfopt/tests/cli_pipeline.rs:191`, `:237`, `:315`, `:348`, `:429` | Removed tests were duplicate CLI report shape or explicit-pass missing-side-input variants. Retained tests still cover default pipeline fail-fast, full 12-pass report, invalid bytecode, map-inline missing/null snapshots, and missing kop target failure. This is not the benchmark e2e suite. |
...
| Runtime artifact paths are compatible | ✅ | `runner/containers/runner-runtime.Dockerfile:261`, `:262`, `:263`, `:264` | Runtime copies `/artifacts/kernel`, `/artifacts/modules`, and manifest. Headers are copied to `/usr/src/linux-headers-fork` for the runtime artifact stage; the kop stage reads `/artifacts/headers` directly from the kernel-fork image. |
| KOperation module builds from exported headers, not kernel source | ✅ | `runner/containers/runner-runtime.Dockerfile:371`, `:380`, `:422` | `runner-runtime-kop-artifacts` is `FROM runner-runtime-kernel-base` and runs `make -C /artifacts/headers ... M=...`, avoiding vendor kernel rebuild. |
...
| BuildKit cache mounts remain | ✅ | `runner/containers/kernel-fork.Dockerfile:52`, `runner/containers/runner-runtime.Dockerfile:380`, `:438`, `:464` | Kernel-fork, kop, daemon Cargo, and bpfopt Cargo stages all retain cache mounts. |
