...
The user-provided path `docs/tmp/bit_ops_kop_research_20260329.md` does not exist in this tree. The actual file is `docs/tmp/20260329/bit_ops_kop_research_20260329.md`.
...
This should be one architecture-neutral semantic kop family, not separate x86-only and ARM64-only semantics. To cover Cilium, the implementation must also handle 32-bit ALU semantics (`w` registers zero-extend on BPF ALU32). Either use separate `bpf_andn32`/`bpf_andn64` targets or encode width in the packed payload. A 64-bit-only kop would miss all `14` Cilium sites.

Non-BMI1 x86 should skip the pass or not advertise the target. Emitting a hidden `not; and` native fallback would erase the ROI and hide a missing capability. The verifier/proof `instantiate_insn` path can still expand the kop to the original BPF sequence, as existing kop modules do.

Implementation work:

- Add kop registry / target JSON / CLI plumbing for `bpf_andn`.
...
- x86 BMI1 + ARM64 BIC make the semantic kop clean, but the measured opportunity is too small compared with the plumbing and test burden.
