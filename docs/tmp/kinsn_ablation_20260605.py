#!/usr/bin/env python3
"""Post-hoc kinsn ablation plots for 2026-06-05.
...
import kinsn_eval_20260604 as kinsn_eval

RESULTS = ROOT / "corpus" / "results"
FIG_OUT = ROOT / "docs" / "figures" / "eval-kinsn-ablation-20260605.png"
SUMMARY_OUT = ROOT / "docs" / "tmp" / "kinsn_ablation_20260605_summary.md"
...
X86_VARIANTS = [
    ("x86_full", "Full", ("kinsn_all_prefetch",), "x86_kvm_corpus"),
    ("x86_no_prefetch", "No prefetch", ("kinsn_all_no_prefetch",), "x86_kvm_corpus"),
    (
        "x86_no_bulk_prefetch",
        "No bulk",
        ("kinsn_all_no_bulk_prefetch",),
        "x86_kvm_corpus",
    ),
    (
        "x86_no_bulk_no_prefetch",
        "No bulk/pref",
        ("kinsn_all_no_bulk_no_prefetch",),
        "x86_kvm_corpus",
    ),
]

ARM64_VARIANTS = [
    ("arm64_conservative", "Conservative", ("kinsn",), "aws_arm64_corpus"),
    (
        "arm64_all",
        "All selectors",
    ...
    ("arm64_no_prefetch", "No prefetch", ("kinsn_all_no_prefetch",), "aws_arm64_corpus"),
    (
...
def loadtime_counters(artifact: pathlib.Path) -> tuple[Counter[str], Counter[str], Counter[str]]:
    ...
    for path in sorted(report_dir.glob("*.jsonl")):
        for line in path.read_text().splitlines():
            ...
            by_name.update(report.get("kinsn_calls_by_name") or {})
            by_family.update(report.get("kinsn_calls_by_family") or {})
    return by_app, by_name, by_family


def workload_ratio(app: dict[str, Any]) -> float:
    baseline = [sample[0] for sample in kinsn_eval.phase_samples(app, "baseline")]
    post = [sample[0] for sample in kinsn_eval.phase_samples(app, "post_rejit")]
...
def bpf_ratio(app: dict[str, Any]) -> tuple[float, int, int, int, int]:
    ...
    pairs = kinsn_eval.paired_bpf_rows(app)
...
def plot(x86_runs: list[RunMetric], arm64_runs: list[RunMetric]) -> pathlib.Path:
    ...
    fig.suptitle("kinsn ablation follow-up (SAMPLES=3, 30s)", fontsize=14)
...
def write_summary(x86_runs: list[RunMetric], arm64_runs: list[RunMetric], fig: pathlib.Path) -> pathlib.Path:
    lines: list[str] = []
    lines.append("# kinsn ablation follow-up 2026-06-05")
    lines.append("")
