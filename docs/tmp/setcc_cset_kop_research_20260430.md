...
`SETcc/CSET` 值得做，但应作为独立的 boolean materialization kop，而不是并入现有 `cond_select`。当前 supported runtime corpus 中有 **9417** 个强匹配 boolean-set site，集中在 Tetragon/Cilium/BCC/Calico；请求指定的 `corpus/bcf` + `runner/repos` broad census 中有 **28653** 个 raw object-level site。大多数是 `MOV 1; Jcc +1; MOV 0`，正好对应 `cmp/test; setcc` 或 `cmp/test; cset`。
...
- `docs/tmp/20260329/arm64_kop_research_20260329.md`
- `docs/tmp/20260329/memory_hints_kop_research_20260329.md`
...
- `arm64_kop_research_20260329.md` 没有 SETcc/CSET corpus census；只把 `cond_select` 作为复杂度参照。
- `memory_hints_kop_research_20260329.md` 与 CSET 无关。
...
BPF 没有原生 SETcc/CSET instruction，因此需要新 kop，而不是纯 bytecode rewrite：
...
- 仍需要 branch target/interior target 检查、branch fixup、BTF/line info remap、subprog boundary check，复用现有 kop replacement utilities。
