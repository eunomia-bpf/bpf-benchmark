#!/usr/bin/env python3
"""Post-hoc arm64 kinsn microbenchmark analysis for 2026-06-05 artifacts."""
...
SUMMARY = ROOT / "docs" / "tmp" / "arm64_kinsn_micro_20260605_summary.md"
...
def sample_kinsn_counts(sample: dict) -> tuple[int, int, Counter[str], Counter[str]]:
    matched = 0
    applied = 0
    ...
    for program in (sample.get("rejit_result") or {}).get("per_program", {}).values():
        for pass_result in program.get("passes", []):
            ...
            families.update(summary.get("kinsn_calls_by_family") or {})
...
def suite_rows(data: dict) -> list[dict]:
    ...
    for bench in data["benchmarks"]:
        ...
        for sample in rejit_samples:
            matched, applied, families, names = sample_kinsn_counts(sample)
...
def draw(summaries: list[dict]) -> None:
    ...
    for ax, summary in zip(axes, summaries):
        ...
        ax.set_title(
            f"arm64 AWS kinsn ReJIT: {summary['label']} "
    ...
    fig.suptitle("Matched arm64 kinsn microbenchmark follow-up (AWS t4g.small, SAMPLES=3)", fontsize=18)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.savefig(FIG / "kinsn-micro-arm64-rejit-20260605.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    print(f"wrote {FIG / 'kinsn-micro-arm64-rejit-20260605.png'}")
