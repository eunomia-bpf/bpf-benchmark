#!/usr/bin/env python3
"""Generate kinsn microbenchmark figures from local raw result.json artifacts."""
...
BEST_RAW_KINSN = ROOT / "micro/results/x86_kvm_micro_20260519_114214_364050/details/result.json"
...
def best_raw_rows() -> list[tuple[float, str, float, float]]:
    kinsn = median_exec_ns(load(BEST_RAW_KINSN))
    stock = median_exec_ns(load(LATEST_STOCK_KERNEL))
    rows = []
    for name in sorted(set(kinsn) & set(stock)):
        rows.append((stock[name] / kinsn[name], name, stock[name], kinsn[name]))
...
def draw_upper_bound_summary() -> None:
    ...
    ax.set_title("LLVM-kinsn upper-bound microbenchmark effect", fontsize=16, pad=10)
    ...
    ax.text(
        0.5,
        -0.18,
        "Best local raw LLVM-kinsn candidate vs latest stock-kernel run. Median exec_ns over 3 samples; INNER_REPEAT=100000.",
    ...
    fig.savefig(FIG / "kinsn-micro-upper-bound-summary-20260527.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def draw_best_raw_full() -> None:
    ...
    ax.set_title("Best local raw LLVM-kinsn full-suite candidate", fontsize=17, pad=10)
    ...
    fig.savefig(FIG / "kinsn-micro-best-raw-full-20260527.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def main() -> None:
    for path in [BEST_RAW_KINSN, LATEST_STOCK_KERNEL]:
        if not path.exists():
            raise SystemExit(f"missing artifact: {path}")
    if correctness_mismatches(load(BEST_RAW_KINSN)):
        raise SystemExit("best raw kinsn artifact has correctness mismatches")
