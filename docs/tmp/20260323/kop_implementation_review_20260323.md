...
codex 的 kop_ops 实现**总体质量很高**，核心架构与设计文档高度一致。但存在 **2 个需要修复的问题**和 **1 个已知限制**需要讨论。
...
- daemon 仍用 `emit_kfunc_call_with_off()` 生成 `MOV r1, src; MOV r2, shift; CALL; MOV dst, r0` 等序列。这些 MOV 是标准 kfunc ABI 要求的，不是额外开销。kernel JIT 对 legacy kop 仍 inline emit，但包含 `mov rax,rdi; mov rcx,rsi; rol rax,cl` 等固定寄存器间搬运。这是 legacy ABI 的固有代价，packed ABI 已消除。
...
- 所有 5 个 kop 模块都实现了 `BPF_KOP_ENC_LEGACY_KFUNC` 支持
- `supported_encodings` 都包含 `BPF_KOP_ENC_LEGACY_KFUNC`
- verifier `check_kop_call()` 对 legacy 路径调用 `check_kfunc_args()` 然后走 model_call
- 如果 legacy encoding 不在 `supported_encodings` 中，返回 `-EAGAIN` 回退到普通 kfunc 路径

#### Packed ABI（sidecar pseudo-insn + operand encoding）

- 所有 5 个 kop 模块都实现了 `BPF_KOP_ENC_PACKED_CALL` 支持
- sidecar insn 编码 `(BPF_ALU64 | BPF_MOV | BPF_K, dst_lo4, BPF_PSEUDO_KOP_SIDECAR, payload_hi)`
- verifier `check_kop_sidecar_insn()` 验证 sidecar 后必须跟 kfunc CALL
- verifier `do_check_insn()` 遇到 sidecar 直接 skip（无 ALU 处理），insn_idx++ 让下条 CALL 读取
- `bpf_prepare_kop_call()` 在 verifier 和 JIT 双方统一从 sidecar 提取 payload

### 3. Verifier modeling 正确性 — PASS

#### KF_KOP 不 clobber r1-r5（只 clobber clobber_mask 声明的）

**正确。** `check_kop_call()` 调用 `bpf_kop_apply_clobber_mask()` 而非普通 kfunc 的全量 r0-r5 clobber。clobber_mask 由 `model_call()` 动态返回：

| kop | legacy clobber_mask | packed clobber_mask |
|-------|-----|-----|
...
- extract: `umax = (1 << bit_len) - 1`，tnum 通过 `kop_tnum_low_bits()` 提供
...
**正确。** `bpf_kop_apply_subreg_def()` 根据 `effect->result_size` 设置：
- `sizeof(u32)` -> `insn_idx + 1`（32-bit subreg definition）
- 其他 -> `DEF_NOT_SUBREG`（64-bit）

#### 不暴露 bpf_reg_state

**正确。** module 的 `model_call` 接收 `const struct bpf_kop_scalar_state *`（独立的 shadow 结构），返回 `struct bpf_kop_effect`。Core verifier 负责所有 `bpf_reg_state` 读写。

### 4. 所有 kop pass 能 apply — PARTIAL PASS
...
- `decode_call`: 从 raw insn/payload 解码为标准化 `bpf_kop_call` — 所有模块都实现了
- `validate_call`: 编码语义校验（不触碰 verifier state）— 所有模块都实现了
- `model_call`: 核心 callback，返回完整 `bpf_kop_effect` — 所有模块都实现了
- 不暴露 `bpf_reg_state` — 正确
- effect 是声明式（`bpf_kop_effect`），由 core 解释执行 — 正确

注册 API 也按设计：
- `bpf_register_kop_ops(func_name, ops)` — 正确
- `bpf_kop_forbidden_flags()` 阻止与 KF_ACQUIRE/RELEASE/RET_NULL 等叠加 — 正确
- 注册时检查 `ops->owner`, `ops->model_call`, 至少一个 `emit_*` — 正确

`kop_common.h` 提供了 `DEFINE_KOP_MODULE` 和 `DEFINE_KOP_MODULE_MULTI` 宏，大幅减少样板代码。
...
同时 `model_call` 声明了 `nr_mem_accesses=1`，`mem_accesses[0].base_reg` 指向指针寄存器，core verifier 通过 `bpf_kop_check_mem_accesses()` 做真正的内存边界检查。

### 7. 测试 — PASS

#### rejit_kop.c（legacy ABI 测试）

10 个测试：
1. `kop_discovery` — 验证 5 个模块 BTF 发现和 7 个 kfunc 注册
2. `barrier_preserves_r5` — barrier kop 不 clobber r5
...
#### rejit_kop_packed.c（packed ABI 测试）

7 个测试：
1. `packed_kop_discovery` — 模块发现
...
使用 `BPF_KOP_SIDECAR(payload)` 宏和 `KOP_*_PAYLOAD()` 编码宏，直接构造 packed BPF insn 序列。
...
1. **设计文档到实现的一致性极高**：`bpf_kop_ops` 的 7 个字段、3 个 callback、注册 API、forbidden flags 检查都精确匹配设计文档。

2. **verifier 安全模型正确**：
   - 不暴露 `bpf_reg_state` 给 module
   - effect 声明式 + core 校验 (`bpf_validate_kop_effect`)
...
4. **kop_common.h 宏设计优雅**：单 kfunc 模块和多 kfunc 模块都有简洁的宏，包含完整的 init/exit/error-unwind 逻辑。
...
| 所有 kop pass 能 apply | **PARTIAL** | rotate/extract/endian OK, cond_select 0 sites (pattern限制), branch_flip 0 sites (PGO prerequisite) |
| bpf_kop_ops 按设计 | **PASS** | 完全匹配设计文档 |
