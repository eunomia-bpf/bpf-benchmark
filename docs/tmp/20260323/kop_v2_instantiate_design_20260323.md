...
日期：2026-03-24  
状态：target spec  
主题：first-class kop IR + proof lowering + pure transport

## 0. 文档定位

这份文档定义的是 `kop v2` 的长期目标架构。
...
当前仓库里已经落地的 `v1`，继续以 `docs/kop-design.md` 和代码现状为准。  
本文档只定义终局目标：`kop` 长期应该是什么抽象，kernel、JIT、generic fallback、daemon 各自应围绕什么边界演进。

## 1. 目标

`kop v2` 的目标不是单纯把 `model_call()` 换成另一套 verifier helper，也不是只换一个 call 编码。

长期上，`kop` 应满足：

1. `kop` 是 first-class IR / first-class instruction，而不是 “is-a kfunc”
...
4. 执行和 native JIT 仍以原始 `kop` 为主表示，而不是把最终 IR 永久降成普通 BPF
5. generic fallback 在需要时，按同一 `instantiate_insn()` 重建执行序列
6. transport 最终也应纯化，不再借 `kfunc` 身份、`kfunc` 名字和 `PSEUDO_KFUNC_CALL`
7. 纯 microarchitectural hints / non-BPF-visible semantics 也必须能被 `kop` 承载

一句话概括：

**`kop` 的本体是 first-class instruction；普通 BPF 是它的 proof object 和 fallback object，不是它的长期身份。**

## 2. 核心语义模型

## 2.1 first-class kop IR

原始程序中的 `kop` 不是“更聪明的 kfunc call”，也不是“等待被永久宏展开的语法糖”。
...
- verifier 不再以 `bpf_kop_effect` 作为主要语义源
...
```text
原始程序:
  P = 含 first-class kop 的程序

proof object:
  L(P) = 把每个 kop site 用 instantiate_insn(payload) 展开后的普通 BPF 程序
```
...
- verifier 证明 `L(P)` 安全
- 证明结果回到原始程序 `P`
- 这个桥接成立，是因为每个原始 `kop` site 的 BPF-visible semantics 由 `instantiate_insn()` 定义
...
1. `kop` 永久 lower 成普通 BPF，native JIT 再对 lowered region 做 fusion
2. `kop` 保持 first-class IR，只在 verifier / fallback 中进行 proof lowering

本文档选择第 2 条，原因不是“代码量更少”，而是抽象更对。

## 3.1 保留 first-class instruction 身份

如果把最终执行 IR 永久降成普通 BPF，那么 `kop` 的长期身份会退化成：

- “一段宏展开”
- “一段带 provenance 的 lowered region”

这不符合本文档的目标。  
本文档要求 `kop` 本体长期存在，而不是只剩 metadata。
...
如果 ordinary BPF 是最终 IR，那么 verifier/post-verifier rewrites 对 lowered BPF 做的所有变换，都可能反过来塑造 `kop` 的长期执行形态。

proof lowering 路线下：

- verifier 可以自由对 proof object 做它需要的分析和临时变换
- 这些变换不要求成为原始 `kop` 的长期身份

也就是说，verifier 使用普通 BPF 的优势可以保留，而 first-class `kop` 的长期抽象也不会被吞掉。
...
- 但原始 `kop` 作为长期主 IR 时，这些临时寄存器不必出现在最终执行表示里
...
这不是附带的小优化，而是 first-class `kop` 相对“永久展开成普通 BPF”很实际的一点优势。
...
proof object 不是“把 kop 包装成一个伪 subprog call”。
...
1. verifier 主分析前先把 `kop` site 展开成普通 BPF
2. verifier 在展开后的 proof object 上做分析
3. 分析完成后，再把对应 region replace 回原始 `kop`
...
这不是大工程。  
它就是一个小的 `scrub_restored_kop_aux()` helper。
...
- `remove_fastcall_spills_fills()`、`optimize_bpf_loop()`、`do_misc_fixups()` 等后续 rewrite 应该看到最终恢复后的原始 `kop`
...
1. 接收原始 `kop` site 的 payload 和 target
...
- 当 JIT 不支持某个 `kop` 时，系统退回的是 instantiated 指令序列，而不是退回 `kfunc` 语义

## 6. native JIT 模型

长期 native JIT 应直接面对原始 `kop`，而不是被迫从最终 ordinary BPF 里做 pattern fusion。

因此长期接口语义应是：

- JIT 识别一个 first-class `kop` site
- 读取 payload
...
## 7.2 microarchitectural hint / non-BPF-visible kop
...
1. 不允许 helper/kfunc/kop/subprog/tailcall
2. 不允许 back-edge
3. region 必须局部封闭
...
长期 API 应围绕 first-class `kop` 和 proof lowering，而不是围绕 `effect DSL`。
...
这意味着 pure transport 下，daemon 必须把 “发现 kfunc” 改成 “发现 kop descriptor / target”。
...
如果 transport 纯化，这一层必须改成以 `kop target` 为中心，而不是以 `kfunc name` 为中心。
...
如果 transport 纯化，这些 helper 必须换成 dedicated `kop` emission helper。
...
- 各 pass 的单元测试中对 `is_kop_sidecar()` 和 `src_reg() == BPF_PSEUDO_KFUNC_CALL` 的断言
...
- 引入 first-class `struct bpf_kop`
- 逐步退休 `struct bpf_kop_ops`
- 逐步退休 `struct bpf_kop_call`
- 逐步退休 `struct bpf_kop_effect`
...
也就是说，`include/linux/bpf.h` 现在把 `kop` 定义成：
...
- 移除 `model_call()` / `effect` 为核心的 kop verifier 路径
...
1. **注册与 lookup**
   - `struct bpf_kop_ops_desc`
   - `bpf_get_kop_ops_desc()`
   - `bpf_find_kop_ops()`
   - `register_bpf_kop_ops()`
   - `desc->kop_ops = ... (*kfunc.flags & KF_KOP) ? ...`

2. **call 解析与 effect 主链**
   - `bpf_build_legacy_kop_call()`
   - `bpf_prepare_kop_call()`
   - `bpf_collect_kop_scalar_inputs()`
   - `bpf_validate_kop_effect()`
   - `bpf_kop_check_mem_accesses()`
   - `bpf_kop_apply_clobber_mask()`
   - `bpf_kop_apply_result()`
   - `bpf_kop_apply_subreg_def()`
   - `check_kop_call()`

3. **sidecar / kfunc 入口耦合**
   - `bpf_verifier_find_kop_sidecar()`
   - `check_kop_sidecar_insn()`
   - `check_kfunc_call()` 中 `(meta.kfunc_flags & KF_KOP)` 分支
...
同时把第 1 块里的 target 身份，从 `KF_KOP` 绑到 `kfunc` 的模式拆出来。

### 12.1.3 `vendor/linux-framework/arch/x86/net/bpf_jit_comp.c`

长期上需要：

- 让 native path 直接面对 first-class `kop`
...
- `emit_kop_call()`
- `bpf_jit_find_kop_ops()`
- `bpf_jit_get_kop_call()`
- `case BPF_JMP | BPF_CALL` 里：
  - `if (src_reg == BPF_PSEUDO_KFUNC_CALL && bpf_jit_find_kop_ops(...))`

这说明现在的 x86 native path 本质上还是：

- “在 kfunc call dispatch 里，顺手识别一个 KF_KOP 目标”
...
### 12.1.5 `module/include/kop_common.h` 与 `module/{x86,arm64}/*.c`
...
- `module/x86/*.c` 和 `module/arm64/*.c`
  ...
  - 每个 target 都有 `static const struct bpf_kop_ops ...`
...
需要 dedicated `kop` transport 定义，例如：

- dedicated pseudo tag
- 或 dedicated target encoding

总之不能继续把 `kop` 定义成 `PSEUDO_KFUNC_CALL`。
...
拆成两套明确语义，而不是继续让 `kop` 通过 `kfunc` 通道搭车。
...
- 会让 `kop` 长期退化成 lowered region
- 会把 native emit 正确性转化成 fusion 正确性
- 会让 verifier/post-verifier 对 ordinary BPF 的变换，反过来塑造 `kop` 的长期主表示
...
- `docs/tmp/20260323/kop_v2_instantiate_design_20260323.md`
  - 描述 `v2 target`
  - 以 first-class `kop` + proof lowering + pure transport 为准

## 15. 最终结论

`kop v2` 的长期目标应定为：

- `kop` 是 first-class IR
- `instantiate_insn()` 定义 canonical BPF-visible semantics
- verifier 通过临时 lowered proof object 证明它
- 原始 `kop` 仍是执行与 native JIT 的主表示
- generic fallback 在需要时，按同一 `instantiate_insn()` 重建执行序列
- 纯 microarchitectural hints 也纳入 `kop` 范围，但其等价定义只覆盖 BPF-visible semantics
...
**`kop v2` 的正确终局不是“更聪明的 kfunc”，也不是“最终只剩 ordinary BPF + fusion”；而是“first-class kop + proof lowering + pure transport”。**
