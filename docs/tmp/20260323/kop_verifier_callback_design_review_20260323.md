...
4. `narrow_result(dst, insn)` 只看 `insn->off` 不够。对 `select` 这类结果依赖输入寄存器状态的 kop，还需要读源寄存器状态；对 memory kop，还需要 base reg/offset/access size 语义。
...
6. memory kop 不能只靠 `narrow_result`。像 `endian_load` 这种必须复用 core verifier 的 `check_mem_access()` 族逻辑，否则就是把“真实内存访问”藏进了 opaque kfunc。
...
| x86 JIT 的 inline 路径是“找到 `inline_ops` 就调用 emit；否则返回错误并回退到普通 CALL” | `vendor/linux-framework/arch/x86/net/bpf_jit_comp.c` L579-L599, L2464-L2470 | 一旦 verifier 对 kop 采用非 CALL 语义，这个 fallback 就不再安全 |
...
- 如果保留这个接口，最少也要要求“宁可 over-approximate，绝不能 under-approximate”，并为每个 kop 加寄存器保留自测。
...
- debug-only 自测：为每个 kop 构造“所有 caller-saved BPF regs 都 live”的测试程序，JIT 后执行并核对未声明 clobber 的寄存器是否保持。
- build-time/disasm test：对固定 byte sequence 的 in-tree kop 做反汇编白名单比对。
...
- 对 memory kop，`off` 只能告诉你一部分位置信息，不能表达 base reg 当前是什么类型、可访问范围多大、是否是 packet/stack/map value/ctx pointer。
...
### 3.4 多操作数 kop（如 `select`）里，`off` 16 位够不够？
...
### 3.5 对内存型 kop（如 `endian_load`），verifier 需不需要额外验证内存访问？
...
- memory kop 不能只走“kfunc scalar return refine”路径。
- 应该新增“memory operand descriptor”，由 core verifier 调用现有 `check_mem_access()`/`check_packet_access()` 逻辑。
- 如果不想做这层，建议第一版明确禁止 memory kop，只支持 pure scalar kop 和 barrier。
...
- 只允许 pure scalar/no-ref/no-sleep/no-callback/no-exception/no-memory-write 的 inline kop。
...
- 向模块暴露一个更小的、专门给 inline kop 用的 result descriptor。
- 例如只允许表达：
  - scalar 或 void；
  - 可选 `tnum`；
  - 可选 64/32-bit bounds；
...
- 你们要做的是“kop 不再遵循普通 CALL 的寄存器语义”，这比 fastcall 更激进。
...
- 对 `KF_INLINE_EMIT` 单独走一个 `check_kop_operands()`。
...
- 你们如果改成“inline kop 写 `dst_reg` 而不是 `r0`”，就不能再只处理 `BPF_REG_0` 的 `subreg_def`。
- 例如某个 inline kop 语义上只定义 32-bit 结果并零扩展到 64-bit，那么 verifier 应该给 `dst_reg` 正确的 subreg/zext 元数据；否则后续精度和 JIT zext 优化都会失真。
...
- 纯 1-slot inline kop：subprog 不会是主问题。
...
2. 旧/新编码区分不充分。
   文档里用“`dst_reg != 0` 表示新编码”区分 old/new call，但 `bpf_speculation_barrier()` 这种无操作数 kop 在旧编码和新编码下完全同形。
...
6. memory faulting model。
   若某 memory kop 可能 fault，它不是普通 `LDX` 语义，也不是普通 kfunc 语义，需要明确是 raw access 还是 probe-style access。

7. 架构差异。
   x86 和 arm64 同名 kop 的真实 scratch set 已经不同；如果未来一个 registration 同时提供 `emit_x86` 和 `emit_arm64`，单一 `clobber_mask`/result contract 不一定够。
...
- 第一阶段只支持 pure scalar inline kop 和 barrier。
...
- 为当前固定 ABI kop 增加 kernel-owned per-kfunc clobber policy。
