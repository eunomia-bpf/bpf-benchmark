...
- 新增 `BPF_PSEUDO_KOP_SIDECAR` (= 3) 伪源寄存器
...
- 新增 kop 类型系统：`bpf_kop_encoding`, `bpf_kop_result_type`, `bpf_kop_operand_kind`, `bpf_kop_mem_flags`, `bpf_kop_effect_flags` 枚举
- 新增 kop 数据结构：`bpf_kop_scalar_state`, `bpf_kop_operand`, `bpf_kop_call`, `bpf_kop_mem_access`, `bpf_kop_effect`, `bpf_kop_ops`
- 新增 sidecar 内联函数：`bpf_kop_is_sidecar_insn()`, `bpf_kop_sidecar_payload()`
- 新增 `bpf_tramp_user` 结构体（trampoline 反向索引）
- `bpf_prog_aux` 新增字段：`rejit_mutex`, `trampoline_users`, `orig_insns`, `orig_prog_len`
- 新增函数声明：`bpf_dispatcher_refresh_prog`, `bpf_trampoline_refresh_prog`, `bpf_struct_ops_refresh_prog`, `bpf_register_kop_ops`, `bpf_unregister_kop_ops`, `bpf_free_kfunc_desc_tab`, `bpf_jit_find_kop_ops`, `bpf_jit_get_kop_call`
- 完整的 `!CONFIG_BPF_SYSCALL` / `!CONFIG_BPF_JIT` 静态 stub

### 4. `include/linux/bpf_verifier.h` (+3)
- `bpf_insn_aux_data` 新增 `kop_clobber_mask` (u32) 和 `kop_call` (bool)

### 5. `include/linux/btf.h` (+2)
- 新增 `KF_KOP = (1 << 17)` 和别名 `KF_INLINE_EMIT`
...
核心组件：
- **kop 注册系统**：`bpf_kop_ops_desc` 链表 + mutex，`bpf_register_kop_ops()` / `bpf_unregister_kop_ops()`
- **`bpf_kop_lookup()`**：查找 + `try_module_get`
- **`bpf_free_kfunc_desc_tab()`**：释放时 `module_put` kop_ops
- **`add_kfunc_call()` 扩展**：为 KF_KOP kfunc 关联 kop_ops 指针
- **sidecar 支持**：`bpf_verifier_find_kop_sidecar()`, `bpf_prog_find_kop_sidecar()`, `check_kop_sidecar_insn()`
- **kop call 准备**：`bpf_build_legacy_kop_call()`, `bpf_prepare_kop_call()`, `bpf_jit_get_kop_call()`
- **backtracking 扩展**：`bt_clear_reg_mask()`, sidecar/kop_call 特殊处理
- **kop 验证主流程 `check_kop_call()`**：decode -> validate -> check args/operands -> collect scalar -> model_call -> validate effect -> check mem -> apply clobber -> apply result -> apply subreg_def
- **effect 验证 `bpf_validate_kop_effect()`**：~95 行，检查 mask/reg/mem 合法性
- **内存访问检查 `bpf_kop_check_mem_accesses()`**：调用 `check_mem_access` 复用现有验证
- **结果应用**：`bpf_kop_apply_clobber_mask()`, `bpf_kop_apply_result()`, `bpf_kop_apply_subreg_def()`
- **`check_kfunc_call()` 扩展**：KF_KOP 分支进入 `check_kop_call`
- **`do_check_insn()` 扩展**：sidecar 指令拦截
- **`is_kop_unsized_mem_arg()`**：允许 kop kfunc 接受 `void *` 参数
...
- **`emit_kop_call()`**：查找 kop_ops，调用 `ops->emit_x86`，验证输出大小
- `do_jit` BPF_MOV_K case 新增 sidecar 跳过
- `do_jit` BPF_JMP CALL case 新增 kop 内联分支

### 14. `arch/arm64/net/bpf_jit_comp.c` (+47)
- **`emit_kop_call_arm64()`**：调用 `ops->emit_arm64`，验证 insn 计数和字节大小
- `build_insn` 新增 sidecar 跳过
- `build_insn` CALL case 新增 kop 内联分支
...
| 5 | 无 try_module_get for kop_ops | HIGH | **已修** | `bpf_kop_lookup()` 中 `if (desc && try_module_get(desc->ops->owner)) ops = desc->ops;`。`bpf_free_kfunc_desc_tab()` 中 `if (ops) module_put(ops->owner);`。完整的 get/put 配对。 |
...
| # | 位置 | 描述 |
|---|------|------|
| L1 | `include/linux/bpf.h` | `BPF_KOP_SIDECAR_PAYLOAD_BITS = 52` 定义但未使用。纯文档常量。 |
| L2 | `include/linux/btf.h` | `KF_INLINE_EMIT` 别名定义但未在任何内核代码中使用。仅 module 侧可能用。 |
| L3 | `include/linux/bpf.h:bpf_kop_call` | `reserved` 字节对内核内部结构无意义。 |
| L4 | `include/linux/bpf.h:bpf_kop_operand` | `imm16` (offset 4) 和 `imm32` (offset 8) 之间有 2 字节 padding hole（对齐导致）。 |
| L5 | `kernel/bpf/verifier.c:bpf_kop_forbidden_flags()` | 黑名单方式：新增 KF_ flag 时需手动更新。白名单更安全。 |
...
| # | 位置 | 类型 | 说明 |
|---|------|------|------|
| D1 | `include/linux/bpf.h` | 未使用常量 | `BPF_KOP_SIDECAR_PAYLOAD_BITS = 52`，无任何代码引用 |
| D2 | `include/linux/btf.h` | 未使用别名 | `KF_INLINE_EMIT = KF_KOP`，内核代码全部使用 `KF_KOP` |
| D3 | `include/linux/bpf.h` | 未使用枚举值 | `BPF_KOP_OPERAND_NONE = 0`，内核代码未引用（module 侧可能用） |
| D4 | `include/linux/bpf.h` | 未使用字段 | `bpf_kop_call.reserved`，无代码读写 |
| D5 | `kernel/bpf/verifier.c` | 可简化函数 | `bpf_build_legacy_kop_call()` 返回值永远是 0（`memset` + 赋值不会失败），调用处检查 `if (err)` 无意义 |
...
| **kop 验证器建模** | ~656 | verifier.c |
| **kop 头文件/类型** | ~170 | bpf.h, btf.h, bpf_verifier.h, filter.h, uapi/bpf.h |
...
- 注释用 `/* */` 风格
- `#include` 位置合理
- 函数命名一致：`bpf_kop_*`（内部）、`bpf_prog_rejit_*`（syscall）、`bpf_*_refresh_prog`（refresh）
- EXPORT_SYMBOL_GPL 用于 `bpf_register_kop_ops` / `bpf_unregister_kop_ops`
...
3. **D1+D2: 删除未使用定义** — `BPF_KOP_SIDECAR_PAYLOAD_BITS` 和 `KF_INLINE_EMIT`。如果外部 module 需要 `KF_INLINE_EMIT`，保留但加注释。

4. **D5: 简化 bpf_build_legacy_kop_call** — 返回 void 而非 int，删除调用处的错误检查。
...
6. **L5: forbidden_flags 改白名单** — 改为允许 `KF_KOP | KF_TRUSTED_ARGS`，拒绝其他所有。
...
### 亮点
1. kop_ops 模块接口设计干净：decode → validate → model → emit 四阶段，验证器完整建模 effect
...
- 新增 kop_ops 验证器建模（~470 LOC）是最大新增功能
