# kop Register and Instruction Encoding Design
...
The current kop implementation uses standard kfunc calling convention: arguments in r1-r5, return in r0. This creates two critical problems:
...
## 1. Recommended Solution: Operand-Encoded kop (ALU-style CALL)

### 1.1 Core Idea

Repurpose the BPF CALL instruction's unused fields for kop calls marked by `KF_INLINE_EMIT` to encode operand information directly, eliminating argument-setup MOVs. The verifier treats `KF_INLINE_EMIT` calls as single-instruction ALU-like operations rather than function calls.
...
- `dst_reg`: Normal kfunc calls never use dst_reg (result always goes to r0). For kop, we encode the operand register here.
- `off`: When btf_fd_idx=0 (the common case for all our kop modules, which register into BPF_PROG_TYPE_UNSPEC), off is available for constant operands.

**btf_fd_idx compatibility**: The `off` field currently encodes the BTF FD index for cross-module kfunc references. For `KF_INLINE_EMIT` kfuncs, we restrict: `off` carries operand data, and the kfunc must be resolvable via btf_id alone (btf_fd_idx=0). This is already the case for all BpfReJIT kop modules since they register globally via `BPF_PROG_TYPE_UNSPEC`. If a future kop needs btf_fd_idx != 0, the daemon can fall back to the old calling convention for that case.

### 1.3 Per-kop Encoding Details
...
if (is_kfunc_inline_emit(&meta)) {
    ...
    err = narrow_kop_result(env, &meta, insn, regs);
    if (err)
        return err;
} else {
...
```c
static int check_kop_operands(struct bpf_verifier_env *env,
...
After the kop, the destination register becomes `SCALAR_VALUE` with unknown range (conservative). However, we can do better with per-kfunc range narrowing:

| kop | Destination range | Reasoning |
|-------|------------------|-----------|
...
**Conclusion**: With proper `narrow_result` callbacks, there is **no meaningful precision loss** for any of the 5 kop types. The extract and endian cases can be made exactly as precise as the originals. The rotate case was already imprecise in the original. The select case may have minor theoretical imprecision but no practical impact.
...
When the kop module is not loaded, `bpf_jit_find_kfunc_inline_ops()` returns NULL. The current fallback path emits a standard CALL instruction. With operand encoding:
...
if (is_kop && !kop_ops) {
    ...
    err = emit_kop_fallback(&prog, bpf_prog, insn);
    if (!err)
        break;
...
### Approach 3: Per-kop clobber_mask
...
- **JIT change**: Updated emit callbacks (~50 lines per arch per kop).
...
- Define a new `BPF_KOP` instruction class (e.g., code = 0x0F).
...
- Leverage the existing `KF_FASTCALL` / `bpf_fastcall` mechanism to have clang emit spill/fill around kop calls, then let the verifier's fastcall optimization remove them.
...
| 3. Per-kop mask | Still bloated | ~0 | ~10 | 0 | Low |
...
**Recommendation**: Approach 4 (operand-encoded kop) delivers the strongest performance result with manageable complexity.
...
**Changes**:
1. In `check_kfunc_call()`: detect `KF_INLINE_EMIT`, branch to the `check_kop_call()` handler
2. `check_kop_call()`: validate `dst_reg`, skip r1-r5 clobber, apply `mark_reg_unknown` to dst_reg only
...
**Testing**: Existing 6 REJIT functionality tests + 20 safety tests. Add 3 new tests specifically for kop register preservation.
...
2. **Kernel JIT (x86)**: Update `emit_kop_call()` and each x86 emit callback to read operands from `insn->dst_reg`/`insn->off` and generate register-flexible native code.
...
6. **Kernel verifier**: Add operand validation (`check_kop_operands`).
7. **Kernel fallback**: Add `emit_kop_fallback()` for when module is not loaded.
...
**Deliverable**: All 5 kop types use operand encoding. Extract: 2 -> 1 insn. Rotate: 4 -> 1 insn. Select: 4 -> 1 insn. Endian: 2 -> 1 insn. Barrier: 1 insn (unchanged).
...
**Deliverable**: Verifier precision matches the original code for all kop types. No programs rejected due to range loss.
...
| 1 | +95 | -200 | Zero-overhead kop encoding |
...
5. **Reduces kernel code size**. By enabling userspace to drive optimizations with single-instruction kop calls, the kernel avoids needing to implement complex peephole optimizations internally.

### 6.2 Arguments Against and Mitigations

1. **"Overloading instruction fields is confusing"**
   - Mitigation: Clear documentation. Add `BPF_INSN_KOP()` macro for encoding. Verifier rejects invalid encodings.
...
4. **"What about future kop types that need more operands?"**
   - Response: The encoding has 4 + 16 = 20 bits for operands (dst_reg + off). This covers 1 register + 16 bits of constants, or 4 register IDs + 4 bits of flags. For hypothetical kop types needing more, the 2-slot encoding (like LDIMM64) provides 4 + 16 + 4 + 16 + 32 = 72 additional bits.
...
2. **Patch 2/4**: Verifier: Operand validation for kop dst_reg/off (Phase 1 verifier part)
   - Adds `check_kop_operands()` and `narrow_result` infrastructure

3. **Patch 3/4**: x86 JIT: Operand-aware emit dispatch for KF_INLINE_EMIT (Phase 1 JIT part)
   - Updates `emit_kop_call()` to pass operand info to callbacks
   - Adds fallback path

4. **Patch 4/4**: Range narrowing callbacks (Phase 2)
   - Adds `narrow_result` to existing kop modules
...
### 7.2 Per-kop Encoding Table

| kop | dst_reg | off encoding | Example |
|-------|---------|-------------|---------|
...
1. **btf_fd_idx conflict**: If a future kop module registers on a specific prog_type (requiring btf_fd_idx != 0 in `off`), the operand encoding in `off` conflicts. Mitigation: BpfReJIT kop modules always register as `BPF_PROG_TYPE_UNSPEC` (btf_fd_idx=0). Document this as a requirement for kop modules that want operand encoding.

2. **Verifier logging**: The verifier log should clearly show the decoded operands for kop calls (e.g., `call bpf_extract64(R3, shift=8, width=8)` instead of just `call bpf_extract64`). This aids debugging.
...
The operand-encoded kop design solves both problems:

- **Instruction inflation**: Every kop replacement is 1 BPF instruction, always fewer than the original pattern.
- **Caller-saved conflict**: The verifier no longer clobbers r1-r5 for kop, eliminating all 780+ skipped sites.
