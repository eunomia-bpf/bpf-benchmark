...
1. **寄存器 clobber**: 当前无条件 clobber r0-r5，但 kop 只修改 r0（或 r0+某些临时寄存器）
2. **返回值 range**: 当前标量返回值为 unbounded，但 kop 的输出有明确范围（如 extract 的输出 ∈ [0, 2^len-1]）
...
2. kop 实际上只写入 r0，不修改 r1-rN（因为它直接 emit 机器码操作 x86 寄存器）
...
- 参数寄存器 r1-rN 是输入，emit 回调不应该修改它们（这是 kop 的契约）
- 如果某个 kop 违反这个契约（修改了 r1-r5），那是 module 的 bug，不是 verifier 的问题
...
但这需要在 `check_kfunc_call` 中对每个具体 kop 做特殊处理，不够通用。
...
1. **契约违反**：如果 kop 的 emit 回调偷改了 r1-r5 对应的寄存器，verifier 不会检测到 → 可能导致 BPF 程序行为不正确。但这与 helper inline（`verifier_inlines_helper_call`）是同样的信任模型。
