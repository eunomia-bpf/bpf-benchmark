...
推荐把 kop 定义为一个独立的 sidecar 行为对象：`struct bpf_kop_ops`。

- kop 仍然首先是一个 kfunc：可见性、allowlist、BTF 原型、prog_type 绑定仍由 `btf_kfunc_id_set` 管。
- kop 额外再绑定一份 `bpf_kop_ops`：JIT emit 和 verifier modeling 都挂在这里。
- verifier 和 JIT 都只看 `bpf_kfunc_desc->kop_ops`，不再出现旧命名。
- 不推荐把 JIT/verifier 回调直接塞进 `btf_kfunc_id_set`；也不推荐伪造一个“base kfunc struct”再做嵌入，因为内核里根本没有一等公民的 `struct bpf_kfunc` 可供继承。

### 0.2 对当前提议的关键修正

1. 不能把 `insn->off` 直接拿来给 module kop 编操作数。
...
结论：`dst_reg/off` 直接编码操作数的方案，不适合作为当前 module kop 的推荐主方案。

2. “KF_INLINE_EMIT 只 clobber r0”是错的。

- 当前 `module/x86/bpf_rotate.c` 的 emit 会写 `RCX`，对应 BPF 的 `r4`。
- 所以 verifier 不能只做 “preserve r1-r5”；必须允许 per-kop、甚至 per-call 的 clobber 建模。

3. `clobber_mask + narrow_result()` 这两个零散字段不够。

- `clobber_mask` 对 packed/in-place kop 往往是动态的。
...
- 不把 packed ABI 和核心 `kop_ops` 绑死

这样可以先解决 780+ skip 和 verifier precision 问题，再单独推进“1 条 BPF insn 编码一个 kop”的性能 ABI。

## 1. 现状约束

### 1.1 当前内核路径

当前内核里已经有一个最小的 “kop” 机制：
...
- x86 JIT 在 `emit_kop_call()` 中按 `insn` 查 `kop_ops` 并调用 emit，见 `vendor/linux-framework/arch/x86/net/bpf_jit_comp.c:579-599`
...
结论：对现有 BpfReJIT module kop，`off` 不是空闲字段。
...
- “Phase 0 直接改成只 clobber r0”会把当前 rotate kop 变成 verifier unsound
- per-kop clobber 是必需项，不是可选优化项

## 2. 设计目标

1. kop 在语义上是 kfunc 的子集，而不是一条全新 BPF ISA 指令。
2. 公开命名使用 `kop`，去掉旧命名。
...
7. memory kop 不能绕过现有内存安全检查。
8. API 需要支持未来 arm64/x86 的 emit 差异，但 verifier 模型应保持平台内聚。

## 3. 结构体与注册设计

### 3.1 推荐模式：独立 `bpf_kop_ops` sidecar

推荐模式：

- `btf_kfunc_id_set` 继续负责 “这个 kfunc 是否允许被某类 BPF prog 调用”
- `bpf_kop_ops` 负责 “如果它还是个 kop，那么 verifier 和 JIT 应如何理解它”
- 内部缓存点仍放在 `struct bpf_kfunc_desc` 上：
  - 当前字段 `inline_ops`
  - 改为 `const struct bpf_kop_ops *kop_ops`
...
不推荐设计一个“`bpf_kop_ops` 里嵌一个 `struct bpf_kfunc *`”：
...
```c
enum bpf_kop_encoding {
	BPF_KOP_ENC_LEGACY_KFUNC = 1 << 0,
	BPF_KOP_ENC_PACKED_CALL  = 1 << 1,
};

enum bpf_kop_result_type {
	BPF_KOP_RES_VOID,
	BPF_KOP_RES_SCALAR,
};

struct bpf_kop_scalar_state {
	struct tnum var_off;
	u64 umin_value;
	u64 umax_value;
	s64 smin_value;
...
struct bpf_kop_call {
	...
	u8 dst_reg;
	u8 reserved;
	struct bpf_kop_operand operands[4];
	u64 payload;
};

struct bpf_kop_mem_access {
	u8 base_reg;
	u8 size;
	u8 access_type;
	u8 flags;
...
struct bpf_kop_effect {
	u32 input_mask;
	u32 clobber_mask;
	enum bpf_kop_result_type result_type;
	u8 result_reg;
	u8 result_size;
	u8 nr_mem_accesses;
	u8 flags;
	...
	s64 smin_value;
	s64 smax_value;
	struct bpf_kop_mem_access mem_accesses[2];
};
...
struct bpf_kop_ops {
	...
	int (*decode_call)(const struct bpf_insn *insn,
			   struct bpf_kop_call *call);
	int (*validate_call)(const struct bpf_kop_call *call,
			     struct bpf_verifier_log *log);
	int (*model_call)(const struct bpf_kop_call *call,
			  const struct bpf_kop_scalar_state *scalar_regs,
			  struct bpf_kop_effect *effect);

	int (*emit_x86)(u8 *image, u32 *off, bool emit,
			const struct bpf_kop_call *call,
...
- 把 raw `bpf_insn` 解码成标准化 `bpf_kop_call`
...
- 和 verifier 共享同一个 `bpf_kop_call`
- 避免 verifier/JIT 各自重新解释 packed payload 而出现不一致

### 3.7 flag 命名

建议新增 `KF_KOP`，语义上替代 `KF_INLINE_EMIT`。

- 对内核内部，所有 helper/变量名改成 `kop`
- 过渡期可以保留：

```c
#define KF_KOP       (1 << N)
#define KF_INLINE_EMIT KF_KOP
```

这样现有代码和文档能平滑迁移，但正式概念统一成 kop。
...
```c
int bpf_register_kop_ops(const char *func_name,
			   const struct bpf_kop_ops *ops);
void bpf_unregister_kop_ops(const char *func_name);
```
...
- `kop_common.h` 提供新的批量 helper，把 kfunc set 注册和 kop ops 注册打包
...
int bpf_register_kop_descs(const struct bpf_kop_desc *descs, u32 cnt);
void bpf_unregister_kop_descs(const struct bpf_kop_desc *descs, u32 cnt);
```
...
结论：`KF_KOP` 最好只服务于“纯计算/纯 load/store-like”语义。
...
- module 只返回一个声明式 `bpf_kop_effect`
- core verifier 负责：
  - 检查输入寄存器是否已初始化
  - 检查寄存器类型是否符合约束
  - 检查内存访问是否合法
...
check_kfunc_call(env, insn, insn_idx_p):
    ...
    if !meta.has(KF_KOP) or !desc->kop_ops:
    ...
    if !(ops->supported_encodings & BIT(call.encoding)):
        if call.encoding == BPF_KOP_ENC_LEGACY_KFUNC:
            return check_plain_kfunc_call_existing_path(...)
        reject
    ...
    if call.encoding == BPF_KOP_ENC_LEGACY_KFUNC:
        ...
        err = check_kop_encoded_operands(env, &call)
        if err:
            reject
...
- in-place kop 的 clobber 可能取决于 `dst_reg`
...
3. 结果写到哪个寄存器
4. 对 memory kop，还要看到 core 已确认过的 memory access 语义
...
- `bpf_kop_effect` 支持可选 `result_tnum`
...
结论：kop 应允许出现在普通 subprog 中，不需要额外禁止。
...
- 普通 subprog：允许
- callback subprog：沿用现有 kfunc 的特殊限制；kop 框架本身不新增全局禁止

### 4.12 memory kop 需要什么额外验证
...
- `dst_reg` 可用，但 `off` 不可作为 module kop 的主承载字段
...
- packed ABI：不再复用 `check_kfunc_args()`，而是走新的 `check_kop_encoded_operands()`
...
- 继续发标准 kfunc ABI
- 如果 `kop_ops` 根本不存在，或存在但不声明 `BPF_KOP_ENC_LEGACY_KFUNC`，则保持 plain kfunc 路径
- 若验证时没有选中 kop path（例如没有 `desc->kop_ops`），则仍按 plain kfunc 语义工作
- 若验证时选中了 kop path，则按 `model_call()` 建模
- 一旦按 kop 语义建模，JIT 就必须 inline；不能在编译末端再偷偷退回普通 kfunc call

2. 新 daemon packed ABI

- 只有当 kernel 宣告支持 packed ABI 且对应 `kop_ops->supported_encodings` 包含它时才启用
- 不满足时，daemon 回落到 legacy kfunc ABI

3. 所有已选中的 kop path 都不能依赖“emit 失败后 fallback 普通 call”
...
- “plain kfunc call” 和 “kop path” 是验证阶段就要选定的两条不同语义路径
- 只有在验证阶段没有进入 kop path 时，JIT 才能生成普通 kfunc call
- 一旦进入 kop path，emit 失败应当是硬失败，而不是 fallback
...
- 不扩 `btf_kfunc_id_set`
- 定义独立的 `struct bpf_kop_ops`
- 内部在 `struct bpf_kfunc_desc` 上缓存 `kop_ops`
- 通过注册 API 把 kop ops 和 kfunc 关联
...
- 输入标量寄存器的 range/tnum
- 结果寄存器位置
- 对 memory kop，还需要 load/memory access 上下文
...
legacy ABI 走现有路径；packed ABI 走新的 kop operand checker。
...
- verifier decode 完把 `bpf_kop_call` 缓存在 `insn_aux_data`
- JIT 直接消费同一份 decode 结果

### 7.3 disasm / verifier log 需要认识 kop
...
不要让 kop 背 refcount、iterator、sleepable、throw、callback 注册之类语义。
...
1. 每个 kop 的 clobber selftest
2. verifier modeled result range selftest
3. legacy ABI 和 packed ABI 的等价性 selftest
4. memory kop 的 invalid access reject selftest

## 8. 推荐实施顺序

### Phase A: rename + sidecar ops

- `bpf_kfunc_inline_ops` -> `bpf_kop_ops`
- `inline_ops` -> `kop_ops`
- `KF_INLINE_EMIT` -> `KF_KOP` alias
...
### Phase C: memory kop core helper

- 引入 memory access descriptor
- 引入 load-like kop 的 scalar-state builder
...
不含各具体 kop module 的算法改写，只算框架层：

1. `include/linux/bpf.h`

- `bpf_kop_ops` 和相关辅助结构/原型
- 预计 `+90 ~ +130 LOC`

2. `include/linux/btf.h`

- `KF_KOP` flag alias/注释
- 预计 `+5 ~ +12 LOC`
...
- JIT emit helper -> `emit_kop_call`
- 消费标准化 `bpf_kop_call`
- kop path must-inline 规则
- 预计 `+40 ~ +70 LOC`

5. `module/include/kop_common.h`
...
1. 把 kop 正式定义为 `bpf_kop_ops`，不再沿用旧命名。
2. 采用独立 sidecar registry，而不是扩 `btf_kfunc_id_set`。
3. verifier 侧不要做 “只 clobber r0” 特判；必须做 per-call effect model。
4. 不暴露 `bpf_reg_state` 给 module；module 只返回声明式 `bpf_kop_effect`。
5. `clobber_mask + narrow_result()` 不够，统一为 `model_call()`。
6. memory kop 必须走 core memory validation，且最好复用 load result 建模。
7. `off` 不能作为 module kop 的主操作数字段；packed ABI 要单独设计 sidecar。
8. `KF_KOP` 与 `KF_FASTCALL` 互斥。
9. 旧 daemon 的标准 kfunc call 必须保持可用；packed ABI 作为后续可选增强。

这套方案能同时解决：

- 当前 verifier 无条件 clobber 导致的 780+ skip
- 现有 `rotate` 等 kop 的真实 scratch/clobber 建模
