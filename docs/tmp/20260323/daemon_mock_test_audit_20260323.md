...
| `test_discover_kfuncs_no_modules_loaded` | Nothing (real filesystem) | **LOW** | Runs `discover_kfuncs()` against the real `/sys/kernel/btf/`. On a host without kop modules, validates the graceful fallback. | Keep. |
