#!/usr/bin/env python3
"""Post-hoc kop corpus evaluation tables and figures for 2026-06-04.
...
OUT_MD = ROOT / "docs" / "tmp" / "kop_eval_20260604_summary.md"
FIG_OUT = FIG_DIR / "eval-kop-corpus-20260604.png"
...
def loadtime_rows() -> tuple[
    ...
    for key, info in CORPUS.items():
        ...
        for line in loadtime_path(run_dir, info["stem"]).read_text().splitlines():
            ...
            names = Counter({str(name): int(count) for name, count in (report.get("kop_calls_by_name") or {}).items()})
            families = Counter(
                {str(name): int(count) for name, count in (report.get("kop_calls_by_family") or {}).items()}
            )
            if sites and sum(names.values()) != sites:
                raise RuntimeError(
                    f"{run_dir}: {record.get('prog_name')} has sites_applied={sites}, "
                    f"but kop_calls_by_name totals {sum(names.values())}"
                )
            if sites and sum(families.values()) != sites:
                raise RuntimeError(
                    f"{run_dir}: {record.get('prog_name')} has sites_applied={sites}, "
                    f"but kop_calls_by_family totals {sum(families.values())}"
                )
            app_names.update(names)
            app_families.update(families)
        summaries[key] = summary
        applied_programs[key] = app_programs
        kop_names[key] = app_names
        kop_families[key] = app_families
    return summaries, applied_programs, kop_names, kop_families
...
def plot_corpus(
    ...
    fig.suptitle("KOperation ReJIT on x86 KVM corpus apps", fontsize=16)
...
def write_summary(
    ...
    total_names = total_counter(kop_names)
    total_families = total_counter(kop_families)

    lines: list[str] = []
    lines.append("# KOperation Corpus Evaluation Summary, 2026-06-04")
    lines.append("")
    lines.append("Generated by `docs/tmp/kop_eval_20260604.py` from raw artifacts.")
    lines.append("")
...
def main() -> None:
    ...
    apply, applied_programs, kop_names, kop_families = loadtime_rows()
    bpf = bpf_rows(applied_programs)
    fig = plot_corpus(workloads, bpf, kop_families)
    write_summary(workloads, bpf, apply, applied_programs, kop_names, kop_families, fig)
