# P1-B pass-owned KopDescriptor review
...
1. **Pass-owned descriptors: pass**
   - `KopDescriptor` lives in `bpfopt/crates/bpfopt/src/pass.rs:25`.
   - Each kop pass owns `KOP_TARGETS`:
     - `rotate.rs:14`
     - `cond_select.rs:16`
     - `ccmp.rs:14`
     - `extract.rs:14`
     ...
     - `bulk_memory.rs:22`
     - `prefetch.rs:35`
   - `PassMetadata.kop_targets` is the aggregation field at `passes/mod.rs:88`, wired from pass-local constants at `passes/mod.rs:107-113`.
...
2. **Registry discovers all targets from `PASS_REGISTRY`: pass**
   - `KopRegistry::new()` iterates `PASS_REGISTRY` and `metadata.kop_targets` at `pass.rs:794-804`.
...
3. **Compound key maintenance: fail for one edge**
   - Normal synchronization is present: `set_btf_id_for_descriptor()` removes old `(btf_id, call_off)` and inserts the new key at `pass.rs:910-928`; `set_call_off_for_descriptor()` does the same at `pass.rs:931-949`; duplicate detection uses `KopCallKey { btf_id, call_off }` at `pass.rs:958-976`.
   - P1 edge: `kop_registry_from_target()` sets `btf_id` before `call_offset` for each entry at `main.rs:912-918`. Because new entries start with `call_off=0`, this falsely rejects a valid pair when another descriptor already owns `(same_btf_id, 0)`.
   - Reproduction:
     ```text
     ...
     output:  error: kop btf_id 128703 call_off 0 is already registered for bpf_endian_load16
     ```
...
4. **CLI alias canonicalization and JSON output: pass**
   ...
   - `list-passes --json` preserves the `kops_used` field via `#[serde(rename = "kops_used")]` at `main.rs:180` and emits pass metadata from `entry.metadata.kop_targets` at `main.rs:269-284`.
   - Observed output includes all kop passes. Example: `bulk-memory` emits `json_name: bpf_bulk_memcpy` with probe alias `bpf_memcpy_bulk`; rotate/endian/prefetch emit their expected `bpf_*` probe names.
...
6. **`remap_kop_btf_metadata` dispatch: pass**
   - `collect_kop_proof_regions()` reads both `imm` and `off` from the kop call at `passes/utils.rs:263-266`.
   - `kop_proof_len()` dispatches through `registry.lookup_by_kop_call(btf_id, call_off)` and then calls `desc.decode_proof` at `passes/utils.rs:289-302`.
...
3. **No fallback / silenced errors in reviewed change: pass-with-note**
   ...
   - Existing pass-level "kfunc unavailable" skips remain outside this change and are guarded by CLI required-kop validation for normal CLI use.
...
2. **Emit paths: pass**
   ...
   - The shared emitter remains `emit_packed_kop_call_with_off()` at `passes/utils.rs:830-838`.
...
3. **Panic-fix byte-identical evidence: pass**
   ...
   - The panic-fix report's SHA comparison covers one rotate lowering case. Static diff evidence is stronger here: no kop pass emit path changed in the current diff.
...
2. **`KopSlot`: pass**
   - `rg -n 'KopSlot' bpfopt/crates/bpfopt/src daemon/src` returned zero matches.

3. **`ProofLayout`: pass**
   - `rg -n 'ProofLayout' bpfopt/crates/bpfopt/src daemon/src` returned zero matches.

4. **Legacy target table names: pass**
   - `crate::kop`, `mod kop`, `pub mod kop`, `TargetSpec`, and `KopRef`: zero matches in `bpfopt/crates/bpfopt/src daemon/src`.
   - `find bpfopt/crates/bpfopt/src daemon/src -name 'kop.rs' -print` returned zero files.

5. **Dead helper / wrapper scan: pass-with-note**
   - New registry helpers have real callers: `lookup_by_kop_call` is used by proof remap and tests; `target_name_for_pass` is used by `resolve_kop_call_off_for_pass`; payload helpers are used by pass-local decoders.
...
2. **KOperation pass tests cover key paths: pass**
