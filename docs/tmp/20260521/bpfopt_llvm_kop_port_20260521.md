# bpfopt/llvm 移植到仓库内 kop LLVM-23 (2026-05-21)
...
## 目标
让 `bpfopt/llvm`(eBPF→LLVM IR→O3→eBPF 回环工具)依赖仓库里的 kop LLVM
(`llvm-backend/build-bpf-kop`, **LLVM 23.0.0git**, 内含 kop pass),而不是系统的
LLVM-18。这样回环工具就能用到 kop 后端。

## 结果:成功
`bpfopt/llvm/build-kop/bpfopt` 已能针对 kop LLVM-23 **静态链接**编译通过
...
- 丢弃 `llvm-config --libs` shell-out:kop LLVM build 没有可用的 `llvm-config`
...
- host 后端(`native x86 aarch64`)按 `LLVM_TARGETS_TO_BUILD` 条件加入——kop LLVM
  只 build 了 BPF target,系统 LLVM-18 才有 x86/aarch64。
- 按 `LLVM_ENABLE_RTTI` 决定 `-fno-rtti`:kop LLVM 是 RTTI-OFF,我们的 TU 用
  RTTI-ON 编译会留下 `ErrorInfoBase`/`format_adapter` 的 typeinfo 未解析符号。
- `build-kop` 必须用 `-DCMAKE_BUILD_TYPE=Release`(`NDEBUG`),否则 LLVM 头里的
  inline assert 触发(见下)。

### 2. kop LLVM 缺失的库
kop LLVM 是个最小 codegen build,缺 JIT/反汇编库。在 `build-bpf-kop` 里补 build:
...
- kop-23 在 LLVM-18 失败的 35 个 "stack limit" 大程序上反而 **0 失败**。
- `map_inline` 单 pass 在两个 build 上都是 0 成功——因为 `--pass map_inline` 缺
  `--inline-hint`/`--map-values` 侧输入;两版行为一致,非 kop 回归。真正的
  map_inline 端到端(带 hint)由 loader 在 katran 上 TEST_RUN 驱动验证(此前已测,
  ~20% 提速),需 root。

## 内核加载 + TEST_RUN 性能(kop-built bpfopt, 本机 stock 6.15 kernel)
用 `bpfopt-loader` 把全部 35 个 testobject 逐个 libbpf 加载进内核、跑 pass、重验、
`BPF_PROG_TEST_RUN`(`--bpfopt` 指向 kop build)。
...
map_inline 比 noop 基线快 ~2%,**落在测量噪声带内**(两组区间重叠)。即 kop-built
...
### map_inline 真实结果(kop bpfopt, katran balancer_ingress, 5 hint)
...
**`BPF_PROG_LOAD` + verify 全部成功(exit 0)**——kop bpfopt 的 O3 roundtrip 产物
