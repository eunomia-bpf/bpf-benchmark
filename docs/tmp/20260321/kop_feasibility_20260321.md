# KOperation Feasibility on `vendor/linux-framework/` (7.0-rc2)

Scope: analyze whether a new kop mechanism can reuse the existing kfunc verifier/interpreter model and only extend JIT `BPF_CALL` lowering on x86 and arm64.
...
For the base kop design, this verifier logic does not need to change.
...
Two helper inline precedents matter because the proposed kop dispatch lives in the same JIT `CALL` lowering slot:
...
**Precise insertion point for kop dispatch on x86**
...
**Precise insertion point for kop dispatch on arm64**
...
That placement preserves existing helper special cases and lets kop bypass the normal call/return-register shuffle.
...
This means kop native emission can fit into the existing framework if:
...
The rough initial estimate still uses 64 bytes per BPF insn (`5318-5325`), with the comment "each BPF instruction is translated to less than 64 bytes". Existing JIT code relies on that. So for a generic kop facility there are two safe choices:
...
The existing call path already demonstrates size changes across passes (`221-259`, `2699-2780`), so a straight-line kop emitter fits cleanly.
...
kop can follow the same pattern with a new helper such as:
...
- base kop needs no constant-propagation plumbing and can use register-form native instructions;
...
That is outside the minimal JIT-only kop mechanism.
...
| | Dedicated kop opcode | KF_INLINE_EMIT-backed kop (`v1`) | Current BpfReJIT |
| --- | --- | --- | --- |
...
The strongest upstream argument for kop is not "we need a custom opcode", but rather:
...
`kop` is a credible and relatively low-intrusion way to add instruction-like BPF extensions on top of the existing kfunc ecosystem in this tree.
