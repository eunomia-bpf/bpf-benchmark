# kop 最小实现可行性分析（基于 `vendor/linux-framework/` 内核源码）
...
- `kop-v1` 只扩展 `BPF_ALU64`
- 语义限定为“标量寄存器变换”
- 不允许扩展指令自己访问内存、创建分支、修改 verifier CFG
- 三操作数形态 `dst = f(src, imm)` 通过已有 `MOV` + `kop` 两条指令表达；`kop` 本身仍保持 eBPF 既有的两操作数风格
...
这正是我推荐 `kop` 用“保留一个主 opcode，再把 `off` 当 ext-id”的直接先例。
...
### 1.8 最终结论：到底有多少空闲 opcode 可给 kop
...
3. “我建议 kop-v1 实际保留的 dispatcher byte”口径：2 个，即 `BPF_ALU64|0xe0|BPF_K` 和 `BPF_ALU64|0xe0|BPF_X`。`0xf0` 整个 nibble 留作将来二期扩展。
...
所以，如果把 `kop` 放进 `ALU64` 的保留 op nibble，就必须在 `check_alu_op()` 之前截获；否则它会被现有 `opcode > BPF_END` 路径直接拒绝。
...
这些都来自 `do_jit()` 的多 pass 框架：`vendor/linux-framework/arch/x86/net/bpf_jit_comp.c:5318-5392`。这也是我建议 `kop-v1` 禁止扩展指令自己发控制流的原因。
...
所以，如果 `kop` 要给 module 开一个“足够写 ROTATE”的 x86 emit API，最少要有：
...
因此，和 x86 一样，`kop-v1` 最好限制成不发控制流的直线代码。
...
这和前面 opcode 分析完全一致：`kop-v1` 最适合做成

- 固定主 opcode：`BPF_ALU64 | BPF_KOP | {K/X}`
- 二级 id：`insn->off`

然后 interpreter 只新增一个静态 label，例如 `ALU64_KOP`。
...
### 5.3.4 kfunc 对 `kop` 的直接启发

`kop` 应该模仿的不是 BTF 本身，而是这套“程序级 owner pinning”：
...
`kop` 这里我建议比 kfunc 多做一步：

- 提供 `bpf_unregister_insn_extension()`

因为 `kop` 用的是纯 runtime registry，不依赖 BTF set 拼接，做 unregister 很容易。
...
为了把改动压到最小，我建议 `kop-v1` 的 ABI 明确写死这几个约束：

1. 只扩展 `BPF_ALU64`
2. 固定 opcode：`BPF_ALU64 | BPF_KOP | {BPF_K, BPF_X}`
3. `insn->off` 作为 `u16 ext_id`
4. 只允许“纯寄存器、无控制流、无内存副作用”的标量变换
5. `kop` 自身采用与现有 ALU 相同的两操作数语义：`dst = op(dst, imm/src)`  
...
#define BPF_KOP 0xe0 /* ALU64 extension dispatch; insn->off stores ext_id */
```
...
#define BPF_KOP_CODE_K (BPF_ALU64 | BPF_KOP | BPF_K)
#define BPF_KOP_CODE_X (BPF_ALU64 | BPF_KOP | BPF_X)

#define BPF_KOP_IMM(dst, ext_id, imm) \
	BPF_RAW_INSN(BPF_KOP_CODE_K, dst, BPF_REG_0, (ext_id), (imm))

#define BPF_KOP_REG(dst, src, ext_id) \
	BPF_RAW_INSN(BPF_KOP_CODE_X, dst, src, (ext_id), 0)

enum bpf_insn_ext_flags {
	BPF_INSN_EXT_F_IMM            = BIT(0), /* supports BPF_KOP_CODE_K */
	BPF_INSN_EXT_F_REG            = BIT(1), /* supports BPF_KOP_CODE_X */
...
```c
/* verifier.c */
static int check_kop_alu64(struct bpf_verifier_env *env, struct bpf_insn *insn)
{
	const struct bpf_insn_extension *ext;
	...
	if (BPF_CLASS(insn->code) != BPF_ALU64 || BPF_OP(insn->code) != BPF_KOP) {
		verbose(env, "invalid kop opcode %02x\n", insn->code);
		return -EINVAL;
	}

	if (!ext_id) {
		verbose(env, "kop ext_id 0 is reserved\n");
		return -EINVAL;
	}
	...
	if (is_reg) {
		if (!(ext->flags & BPF_INSN_EXT_F_REG)) {
			verbose(env, "kop %s does not support reg form\n", ext->name);
			return -EINVAL;
		}
	} else {
		if (!(ext->flags & BPF_INSN_EXT_F_IMM)) {
			verbose(env, "kop %s does not support imm form\n", ext->name);
			return -EINVAL;
		}
		if (env->prog->blinding_requested &&
		    !(ext->flags & BPF_INSN_EXT_F_BLINDABLE_IMM)) {
			verbose(env, "kop %s imm form is not supported under constant blinding\n",
				ext->name);
			return -EOPNOTSUPP;
		}
	}
	...
	if (!ext->verify) {
		verbose(env, "kop %s has no verifier callback\n", ext->name);
		return -EINVAL;
	}
...
{
	...
	if (class == BPF_ALU64 && BPF_OP(insn->code) == BPF_KOP) {
		err = check_kop_alu64(env, insn);
		if (err)
			return err;
		return 0;
	}
...
```c
/* x86 JIT */
static int emit_kop_x86(struct bpf_prog *prog, const struct bpf_insn *insn,
...
case BPF_KOP_CODE_K:
case BPF_KOP_CODE_X:
	err = emit_kop_x86(bpf_prog, insn, i - 1, &prog,
...
```c
static int emit_kop_arm64(struct jit_ctx *ctx, const struct bpf_insn *insn,
			    int insn_idx)
{
	const struct bpf_insn_extension *ext;
...
```c
[BPF_KOP_CODE_K] = &&ALU64_KOP_K,
[BPF_KOP_CODE_X] = &&ALU64_KOP_X,
```
...
见 `vendor/linux-framework/include/linux/bpf.h:1741-1743`。`kop` 直接照着加一个：
...
- 主 opcode 只保留一次：`BPF_ALU64 | BPF_KOP | {K/X}`
...
而 `kop` 的 verify callback 若放到 module，就变成：
...
1. `kop-v1` 只允许 ALU64 scalar transform
...
所以如果 `K` 形式 `kop` 直接放行到 JIT blinding：

- immediate 会以明文保留在指令里

这是安全问题。`kop-v1` 的最小解决方案应当是：
...
- 在 `bpf_jit_blind_insn()` 里新增 `BPF_KOP_CODE_K` → `BPF_KOP_CODE_X` 的 blind rewrite
...
如果只加功能不加 disasm 支持，那么 `BPF_KOP` 在 verifier log / 内核 disasm 里要么显示成一个泛化字符串，要么显示不友好。
...
`kop` 要想有可读名字，也需要一条类似的中心化 disasm hook。
...
| 文件 | 估计新增/修改行数 | 说明 |
| --- | ---: | --- |
| `vendor/linux-framework/include/uapi/linux/bpf.h` | 4 | 新增 `#define BPF_KOP 0xe0` 注释和宏 |
...
| `vendor/linux-framework/kernel/bpf/verifier.c` | 68 | `check_kop_alu64()`、`do_check_insn()` hook、少量日志/包装 |
| `vendor/linux-framework/kernel/bpf/core.c` | 64 | opcode whitelist、interpreter jump table/label、free path |
| `vendor/linux-framework/arch/x86/net/bpf_jit_comp.c` | 54 | `emit_kop_x86()`、两个 switch case、x86 ops wrapper |
| `vendor/linux-framework/arch/arm64/net/bpf_jit_comp.c` | 46 | `emit_kop_arm64()`、两个 switch case、arm64 ops wrapper |
| **合计** | **661** | 功能最小闭环 |
...
| 额外文件 | 额外行数 | 说明 |
| --- | ---: | --- |
| `vendor/linux-framework/kernel/bpf/disasm.c` | 18 | verifier log / 内核 disasm 显示 `kop` 名称 |
...
- **可行**：如果把目标限定为“`BPF_ALU64` 标量寄存器变换类扩展”，并接受一次性在 verifier/JIT/interpreter/core 里打一个统一的 `kop` hook。
