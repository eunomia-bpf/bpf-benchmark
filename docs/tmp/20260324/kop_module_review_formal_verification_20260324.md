# kop Module 代码审阅与形式化验证可行性分析

日期：2026-03-24
审阅范围：module/x86/（5 files）+ module/arm64/（5 files）+ module/include/kop_common.h
审阅基于 kop v2 设计（first-class IR，instantiate_insn + native emit）

---

## Part 1：逐文件代码审阅

### 1.0 公共头文件：module/include/kop_common.h（154 LOC）
...
| BPF Reg | kernel reg2hex | kop_x86_reg_code | Match | kernel is_ereg | kop_x86_reg_ext | Match |
|---------|:-:|:-:|:-:|:-:|:-:|:-:|
...
| BPF Reg | kernel bpf2a64 | kop_arm64_reg | Match |
|---------|:-:|:-:|:-:|
| REG_0 | A64_R(7) | 7 | OK |
| REG_1 | A64_R(0) | 0 | OK |
| REG_2 | A64_R(1) | 1 | OK |
...
`bpf_kop_sidecar_payload()` 在 kernel 中定义为：
```
(u64)(dst_reg & 0xf) | ((u64)(u16)off << 4) | ((u64)(u32)imm << 20)
```

各 helper 的对应关系：
- `kop_payload_reg(payload, 0)` → 低 4 bit = dst_reg 字段 → 正确
- `kop_payload_reg(payload, 4)` → bit[7:4] = off 的低 4 bit → 正确（对应 off 字段第一个 nibble）
- `kop_payload_u8(payload, 8)` → bit[15:8] = off 的高 12 bit 中 bit[11:4] → 正确
- `kop_payload_s16(payload, 8)` → bit[23:8] → 这跨越了 off 和 imm 的边界

**问题 (Minor)**：`kop_payload_s16(payload, 8)` 提取 bit[23:8]，而 sidecar 布局是 `[3:0]=dst_reg, [19:4]=off, [51:20]=imm`。bit[23:8] 跨越了 off 字段的 bit[15:4] 和 imm 字段的 bit[3:0]。实际上各 module 使用的 payload 编码与 sidecar 布局需要配合 daemon 端的打包逻辑来理解——payload 是一个自由格式的 52-bit 字段，各 kop 自行定义其内部布局。sidecar 只是传输载体。因此这不是 bug，而是设计意图。

#### DEFINE_KOP_V2_MODULE 宏

**正确**：正确使用 `register_bpf_kop_set()` / `unregister_bpf_kop_set()`，设置 `module_init` / `module_exit`，声明 `MODULE_LICENSE("GPL")`。
...
- `emit_shr_imm`：REX.W + 0xC1 + ModRM(0xE8 | dst) + imm8 → SHR r/m64, imm8。ModRM.reg = 5 (`0xE8 = 0xC0 | (5<<3)`)。
  - **等等**：0xE8 = 11_101_000，即 mod=11, reg=5, rm=000。对于 SHR，opcode extension 是 `/5`，所以 reg=5 是正确的。加上 dst 的低 3 bit：`0xE8 | kop_x86_reg_code(dst_reg)`。**正确**。
...
**Major 发现**：`emit_and_imm32` 中 `0xE0 | kop_x86_reg_code(dst_reg)`。0xE0 = 11_100_000，即 mod=11, reg=4, rm=000。AND 的 opcode extension 是 `/4`，reg=4。**正确**。
...
**严重程度评估**：由于 kop 替换的是"extract bitfield"语义，`(dst >> 0) & 0xFFFFFFFF` 的正确结果应该只保留低 32 bit。所以 native emit 是正确的，proof sequence 在 `bit_len == 32, start == 0` 的 case 下有 bug——它不清零高位。
...
| 级别 | 问题 |
|------|------|
| Minor | `emit_rex` 在 `emit_load16_zx` 和 `emit_load32` 中不设 REX.W（is64=false），这是正确的（32-bit/16-bit 操作不需要 REX.W），但如果 dst 或 base 是 extended reg（R8-R15），仍然需要 REX prefix。代码正确处理了这种情况（`kop_x86_reg_ext` 检查）。 |
...
**module/x86/Makefile**（20 LOC）和 **module/arm64/Makefile**（20 LOC）：标准 out-of-tree module 构建。`KDIR` 指向 `vendor/linux-framework`。`ccflags-y += -I$(src)/../include` 确保找到 `kop_common.h`。**正确**。
...
| 文件 | LOC |
|------|-----|
| module/include/kop_common.h | 154 |
...
#### 性质 A：单个 kop 的 emit 等价性

> native emit 产生的机器码是否与 instantiate_insn 产生的 BPF proof sequence 在 bit-exact 意义上等价？

**可行性评估**：

- **搜索空间**：每个 kop 的 native emit 产生 1-7 条 x86 指令（3-16 bytes）或 1-3 条 ARM64 指令（4-12 bytes）。这是极小的代码片段。
...
| kop | native insn count | payload params | 枚举/symbolic |
|-------|:-:|------|------|
...
**结论**：每个 kop 的 native emit 都是 3-16 字节的确定性代码片段，可以用 SMT solver 做 bit-exact equivalence checking。
...
**困难点**：
- endian kop 涉及内存 load，需要建模内存语义
- barrier kop 的语义不是 functional（副作用是 pipeline flush），无法做功能等价
...
> original BPF → daemon rewrite → kop substitution → re-verify → JIT emit 整条链的语义保持
...
- **每个 kop 一个 verification condition**：对 5 个 kop family（rotate, select, extract, endian, barrier），x86 和 ARM64 各一个 VC，共约 10 个。
- **预估工作量**：
...
- 需要证明：对每个 kop，instantiate 产生的 proof sequence 被 verifier 分析后，得到的抽象状态 sound 地覆盖了 native emit 的实际语义
- **比目标 B 多 50% 工作量**，主要在 verifier 抽象模型上

**目标 D（full pipeline）**

- 这基本是 "verified BPF JIT compiler" 级别的工作（Jitk/Jitterbug/Serval 类）
- BpfReJIT 的特殊之处在于 verifier + daemon rewrite + kop substitution + JIT 四层交互
- **ROI 极低**：论文目标不是 verified compilation，而是 extensible compilation framework

### 2.3 最小可行验证方案

#### 推荐方案：5 个 kop 的 emit/instantiate 等价性（目标 B）

**范围**：
- 5 个 kop × 2 架构 = 10 个 verification conditions（但 endian 有 3 个 size variant，实际 7 × 2 = 14 个 VC）
...
5. 对每个 kop，构造 `verify (∀ regs, payload: native_semantics(regs, payload) == bpf_semantics(regs, payload))`
...
在最小方案基础上加入：
- proof soundness（目标 C）的部分验证：只验证 extract 和 select 两个最有 subtlety 的 kop
- 额外 3-5 人天
...
理由：
1. **验证范围太小**：只有 5 个 kop（~10 条指令），不是完整编译器。这更接近 "sanity check" 而非 "verified compiler"。
2. **与 Jitterbug/Jitk 定位不同**：
   - Jitterbug 验证了整个 BPF x86 JIT compiler（数千行代码）
   - Jitk 在 Coq 中证明了 BPF 到 x86 的完整编译正确性
   - BpfReJIT 的验证只覆盖 kop 这一层，不覆盖 daemon rewrite 或 verifier 交互
...
> "每个 kop 的 native emit 只有 3-16 字节，搜索空间极小。我们使用 Rosette/Z3 对所有 5 个 kop（x86 和 ARM64 共 14 个 variant）进行了 bit-exact 等价性验证，确认 native emit 与 BPF proof sequence 的语义一致性。这种 micro-level verification 的可行性正是 kop 设计的关键优势：与验证整个 JIT compiler（如 Jitterbug 需要数人月）不同，每个新 kop 的验证可以在数小时内完成，使得扩展新指令的安全成本极低。"
...
1. "how do you know the kop emit is correct?" → 回答：(a) proof lowering 让 verifier 验证 BPF 等价物，(b) 我们用 Z3 验证了 emit/proof 等价性，(c) kop 极小（3-16 字节）所以 verification 可行
2. "what if a module has a bug?" → 回答：safety 由 verifier 保证（分析的是 proof sequence），correctness 风险局限在单个 kop 的 emit，不扩散
3. "比较 Jitterbug/Jitk?" → 回答：他们验证整个 JIT；我们的 kop 是 modular 的，每个 kop 可以独立验证，这是微内核式设计的验证优势
...
| **BpfReJIT kop** | 单个 kop emit（3-16 bytes）| Z3/Rosette | **数小时/per kop** | **新 kop 独立验证，不影响其他** |
...
2. **提取公共 emit helper**：将 `emit_u8`, `emit_rex_rr`, `emit_mov_rr` 等提取到 `kop_common.h` 的 `#ifdef CONFIG_X86_64` 区域。
