...
Committed HEAD: `cb06f7908` (bpf: Refactor kop handling and improve verifier region management)
...
|---|----------|-------------|-----------|----------|
| **M1** | `kallsyms_lookup_name()` in `fetch_kop_desc_meta()` | **Major (blocker)** | **FIXED** | Replaced with `btf_try_get_kop_desc()` registration-based API in new `kernel/bpf/btf.c`. `kallsyms_lookup_name` no longer appears in working tree diff. |
| **M2** | `bpf_prog_has_kop_call()` does O(N) scan | Major | **FIXED** | Uncommitted change: `return !!prog->aux->kop_tab;` (mirrors `bpf_prog_has_kfunc_call`). |
| **M3** | `count_kop_calls()` duplicates work | Major | **FIXED** | Uncommitted change: function deleted, replaced by `env->kop_call_cnt` tracked during `add_subprog_and_kfunc()`. |
| **M4** | `kfunc_desc_tab` dynamic allocation mixed with kop feature | **Major (blocker)** | **NOT FIXED** | Still in the same commit(s). Must be split to a separate preparatory patch for upstream. |
| m1 | `bpf_kop_sidecar_payload()` truncation without BUILD_BUG_ON | Minor | **PARTIALLY FIXED** | `BPF_KOP_SIDECAR_PAYLOAD_BITS` constant removed entirely. No validation added. Acceptable since the encoding is self-documenting in the inline function. |
| m2 | `bpf_prog_rejit_swap()` fragile field-by-field copy | Minor | Not addressed | Same as v3. Non-blocking. |
| m5 | `BPF_KOP_SIDECAR_PAYLOAD_BITS` unused constant | Minor | **FIXED** | Removed from working tree `include/linux/bpf.h`. |
...
| n5 | `btf.h` blank line removal | Nit | **FIXED** | Uncommitted change restores the blank line. Working tree btf.h diff only has functional kop additions. |
...
This is the kop registration API that replaces `kallsyms_lookup_name`. It follows the `register_btf_kfunc_id_set()` pattern.

### Architecture

- `struct btf_kop_desc { var_id, desc }` -- maps BTF VAR id to `struct bpf_kop *`
- `struct btf_kop_tab { cnt, descs[] }` -- flex array, sorted by var_id, stored in `struct btf`
- `register_bpf_kop_set()` -- called by kop modules at init, resolves BTF names to var_ids, stores in btf->kop_tab
- `unregister_bpf_kop_set()` -- cleanup on module exit
- `btf_try_get_kop_desc()` -- verifier-side lookup by var_id with `try_module_get`

### [Major] M-NEW-1: No concurrency protection for `btf->kop_tab` access

- **File**: `kernel/bpf/btf.c`, lines 9024-9085 and 9087-9163
- **Issue**: `register_bpf_kop_set()` and `unregister_bpf_kop_set()` read and write `btf->kop_tab` (including `kfree(old_tab)` + `btf->kop_tab = new_tab`) without any locking. If two modules call `register_bpf_kop_set` concurrently for the same BTF (e.g., vmlinux BTF), there is a use-after-free/double-free race on `old_tab`.

  The existing `register_btf_kfunc_id_set` has the same lack of locking but documents "This function must be invoked only from initcalls/module init functions" -- which are serialized by the module loader's `module_mutex`. The new kop registration has **no such documentation**.

  Additionally, `btf_try_get_kop_desc()` reads `btf->kop_tab` without barriers. If called concurrently with `register_bpf_kop_set()` (verifier running while a module is loading), the pointer read could see a stale `kop_tab`. In practice, module init completes before verification uses the module's kop, but this is not enforced.
...
### [Major] M-NEW-2: `btf_type_is_kop_desc()` duplicated in btf.c and verifier.c

- **File**: `kernel/bpf/btf.c` line 8983 and `kernel/bpf/verifier.c` line 3616
- **Issue**: Two separate `static` definitions of `btf_type_is_kop_desc()` with slightly different signatures:
  - btf.c: `static bool btf_type_is_kop_desc(const struct btf *btf, u32 type_id)`
  - verifier.c: `static bool btf_type_is_kop_desc(struct btf *btf, u32 type_id)` -- missing `const`, has extra redundant `btf_type_by_id` check
...
- **Fix**: Move `btf_type_is_kop_desc()` to a shared header or export it from btf.c. Use the `const struct btf *` signature. Remove the redundant `btf_type_by_id` call from the verifier.c copy.

### [Minor] m-NEW-1: `(void)param;` casts in `btf_try_get_kop_desc` stub
...
### [Minor] m-NEW-2: `unregister_bpf_kop_set()` silently fails on allocation failure

- **File**: `kernel/bpf/btf.c`, lines 9136-9138
- **Issue**: If `kmalloc` for `new_tab` fails during unregistration, the function silently returns without removing any descriptors. The old_tab retains stale entries pointing to soon-to-be-unloaded module memory. When the module is freed, `btf_try_get_kop_desc()` could return a dangling `desc` pointer (the `try_module_get` would fail since the module is being unloaded, so this is safe in practice -- `try_module_get` returns false for modules in `MODULE_STATE_GOING`).

  However, the `var_ids` allocation at line 9105 also fails silently, and in that case the function returns without doing anything. If the module then fully unloads, a subsequent `btf_try_get_kop_desc()` call would:
  1. `bsearch` finds the stale entry
  2. `res->desc` points to freed module memory
  3. `kop->owner` is read from freed memory (use-after-free)
  4. `try_module_get(kop->owner)` -- UAF on the `struct module`

  This is a real UAF vulnerability on the `kop->owner` dereference in `btf_try_get_kop_desc` line 9185.
...
- **Fix**: Either:
  ...
  3. Or at minimum, set `old_tab->descs[i].desc = NULL` for each entry being removed, so the `!res->desc` check in `btf_try_get_kop_desc` catches it.

### [Minor] m-NEW-3: `unregister_bpf_kop_set` has O(N*M) complexity
...
### [Nit] n-NEW-1: `btf_free_kop_tab` does not release module refs

- **File**: `kernel/bpf/btf.c`, lines 1815-1819
- **Issue**: `btf_free_kop_tab()` only does `kfree(btf->kop_tab)`. It does NOT call `module_put` on the kop descriptors' owners. Compare with `bpf_free_kop_desc_tab()` in verifier.c which DOES call `module_put`. The btf-side tab stores `const struct bpf_kop *desc` but `register_bpf_kop_set` does NOT call `try_module_get` during registration (the module is guaranteed alive during init). And `btf_try_get_kop_desc` does its own `try_module_get`. So `btf_free_kop_tab` doesn't need to `module_put`. This is correct.
- **Verdict**: Correct, but the asymmetry with `bpf_free_kop_desc_tab` (verifier side) is confusing. Add a comment.

---

## 3. New Code Review: `kernel/bpf/verifier.c` changes (+65/-49 uncommitted)

### [Major] M-NEW-3: `kop_call_cnt` can over-count

- **File**: `kernel/bpf/verifier.c`, uncommitted change in `add_subprog_and_kfunc()`
- **Issue**: The code increments `env->kop_call_cnt` before calling `add_kop_call()`:
  ```c
  else {
      env->kop_call_cnt++;
      ret = add_kop_call(env, insn->imm, insn->off);
  }
  ```
  If `add_kop_call()` fails (returns error), the count is already incremented but no descriptor was actually added. The count is then used to allocate `kop_regions`. An over-count leads to a slightly over-sized allocation (harmless waste) but the actual region usage (`kop_region_cnt`) is correctly tracked separately during `lower_kop_proof_regions`.

  More importantly, `add_subprog_and_kfunc` returns the error immediately after the failed `add_kop_call`, so the over-counted value is never used. The allocation happens later in `bpf_check()` only if `add_subprog_and_kfunc` succeeds.
...
### Verified: `fetch_kop_desc_meta()` correctly uses new API

- The function now calls `btf_try_get_kop_desc(btf, var_id, kop)` which does the BTF lookup + `try_module_get`.
...
- The old `!(*kop)->owner` check is correctly removed -- built-in descriptors have `owner == NULL`, which is now explicitly documented in `struct bpf_kop`.
...
- Added `bpf_kop_id` and `bpf_kop_set` structs: clean, minimal.
- Added `struct module *owner` comment "NULL for built-in/vmlinux descriptors": good documentation.
- Added `register_bpf_kop_set`/`unregister_bpf_kop_set` declarations + stubs: correct.
- Removed `BPF_KOP_SIDECAR_PAYLOAD_BITS`: good cleanup.

### `include/linux/btf.h` (+12 uncommitted)

- Added `struct bpf_kop` forward declaration: needed for `btf_try_get_kop_desc`.
- Added `btf_try_get_kop_desc` declaration + stub: correct.
- `(void)` casts in stub: style issue (see m-NEW-1 above).

### `include/linux/bpf_verifier.h` (+1 uncommitted)

- Added `u32 kop_call_cnt`: clean, used correctly.
...
| File | Change | Necessary for REJIT/kop? | Status |
|------|--------|:---:|---|
...
| `scripts/Makefile.btf` | `global_var` for external modules (+2 lines) | YES (for kop module BTF) | Keep |
| `kernel/bpf/verifier.c` | `btf_type_is_kop_desc()` duplicate | NO (should use btf.c version) | **Should deduplicate** |
...
|---|----------|------|-------|------------|
| 1 | Major | verifier.c | `kfunc_desc_tab`/`kfunc_btf_tab` dynamic allocation mixed with kop feature -- must be separate preparatory patch | Easy (git rebase) |
| 2 | Minor | btf.c | `register_bpf_kop_set` missing "must be called from init" documentation | Trivial (add comment) |
| 3 | Minor | btf.c + verifier.c | `btf_type_is_kop_desc()` duplicated with different signatures | Easy (deduplicate) |

### Important Non-blocking Issues

| # | Severity | File | Issue |
|---|----------|------|-------|
| 4 | Minor | btf.c | `unregister_bpf_kop_set` silent failure on OOM can leave dangling descriptors (UAF on `kop->owner`) |
| 5 | Minor | btf.h | `(void)param` casts in stub -- non-standard kernel style |
| 6 | Minor | verifier.c | `kop_call_cnt` incremented before `add_kop_call` success check |
...
1. `validate_kop_proof_seq` JMP32|JA bug (v1 Critical) -- FIXED in committed HEAD
2. `lower_kop_proof_regions` control flow bug (v2 Critical) -- FIXED in committed HEAD
3. `bpf_kop_sidecar_payload` dst_reg masking (v1 Major) -- FIXED in committed HEAD
...
8. `kop_tab` swap symmetry (v1 Critical) -- FIXED in committed HEAD
...
11. `kop_regions` fixed array (v2 Major) -- FIXED in committed HEAD (dynamic allocation)
...
14. `bpf_prog_has_kop_call` O(N) scan (v3 Major) -- **FIXED** in uncommitted verifier.c
15. `count_kop_calls` duplication (v3 Major) -- **FIXED** in uncommitted verifier.c
16. Dead code (KF_KOP, api_version, flags, tnum.h, PAYLOAD_BITS, __maybe_unused) -- all FIXED
...
### Trust Model
- `register_bpf_kop_set()` requires module init context (module loading requires `CAP_SYS_MODULE`).
- `btf_try_get_kop_desc()` is only called from the verifier path which requires `CAP_BPF`.
...
- If `unregister_bpf_kop_set()` fails (OOM on `var_ids` or `new_tab` allocation), stale entries remain in `btf->kop_tab`.
- When the module is fully unloaded, `kop->owner` becomes a dangling pointer.
- `btf_try_get_kop_desc()` reads `kop->owner` before calling `try_module_get`.
...
- `struct_size(new_tab, descs, old_cnt + set->cnt)` in `register_bpf_kop_set`: `set->cnt` is u32, `old_cnt` is u32. Sum could overflow. `struct_size` uses `size_mul`/`size_add` which return `SIZE_MAX` on overflow, causing `kmalloc` to fail. **Safe**.
...
| File | +/- | Purpose |
|------|-----|---------|
| kernel/bpf/verifier.c | +682/-32 | kop verifier, kfunc refactor, proof lowering |
| kernel/bpf/syscall.c | +601/-1 | REJIT syscall, swap, poke_tab |
| kernel/bpf/btf.c | +237/-0 | **NEW**: kop registration API |
...
| arch/x86/net/bpf_jit_comp.c | +51/-2 | x86 kop emit |
| arch/arm64/net/bpf_jit_comp.c | +38/-0 | arm64 kop emit |
| include/uapi/linux/bpf.h | +24/-0 | UAPI: REJIT, kop, prog_info |
| kernel/bpf/dispatcher.c | +20/-3 | dispatcher refresh |
| include/linux/btf.h | +11/-0 | kop BTF declarations |
| include/linux/filter.h | +11/-0 | BPF_CALL_KOP macro |
| kernel/bpf/core.c | +10/-1 | ksym reinit, free tab |
| include/linux/bpf_verifier.h | +10/-0 | kop_region, kop_call_cnt |
| net/core/filter.c | +5/-0 | XDP refresh |
| scripts/Makefile.btf | +3/-1 | external module BTF |
| kernel/bpf/disasm.c | +2/-0 | kop disasm string |
...
The 550-line claim is only achievable by counting the minimal REJIT syscall + kop proof lowering without:
...
2. **[Should fix]** Add init-only comment to `register_bpf_kop_set`
3. **[Should fix]** Deduplicate `btf_type_is_kop_desc()` between btf.c and verifier.c
4. **[Should fix]** Remove `(void)` casts in btf.h stub
5. **[Should fix]** Fix `unregister_bpf_kop_set` silent failure (set `desc = NULL` in-place)
6. **[Should fix]** Move `kop_call_cnt++` after `add_kop_call` success
