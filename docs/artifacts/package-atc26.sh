...
carries fresh, provenance-complete Katran and Cilium \`map_inline\` causality
triplets — for each, one optimized run plus two matched no-pass controls, each
with its own make-console log — whose retained per-step bytecode binds the
derived applied-site rewrite (16 sites for Katran, 3787 for Cilium) to the
measured control-corrected throughput ratio.
