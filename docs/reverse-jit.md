# ReverseJIT / ReverseInterpreter Research Note
...
| 3 | **ReverseJIT / ReverseInterpreter** (this doc) | Run arbitrary x86/arm64 native code safely inside the kernel without requiring the developer to write eBPF or to submit a separate proof. | An x86 or arm64 interpreter (or a JIT that emits eBPF) written in eBPF C. Specialization to a specific target program collapses the interpreter to straight-line eBPF, which the stock verifier checks. The kernel JIT then lowers it to native. | Near zero. |
...
A kernel-ABI variant of "ReverseJIT" (dual-semantics ISA owned by the kernel)
...
ReverseJIT / ReverseInterpreter writes a target-ISA interpreter or JIT in C,
...
target native binary (x86 or arm64)
  -> compile-time specialization of a trusted interpreter or JIT written in C
  -> eBPF program
...
```text
ReverseJIT/ReverseInterpreter brings native code into the eBPF safety model by
treating an eBPF-written interpreter (or JIT) as the trusted lowering and the
...
- **ReverseInterpreter**: a target-ISA interpreter in C, specialized to one
...
- **ReverseJIT-in-eBPF**: a native-to-eBPF JIT written in C, compiled to eBPF,
...
differ in whether specialization is a clang job (interpreter variant) or an
...
proof and runs the native code without runtime checks. ReverseJIT does not
...
ReverseJIT/ReverseInterpreter:
  user native program
    -> trusted lowering (interpreter or JIT, both written in eBPF)
    -> eBPF
    -> stock eBPF verifier
    -> stock JIT
    -> native execution
...
Kinsn (idea #2) and ReverseJIT/ReverseInterpreter (idea #3) attack the same
...
combined with a ReverseInterpreter that emits kinsns where helpful is a natural
ablation point, but neither requires the other.

## Mechanism

This section specifies the ReverseInterpreter variant. The ReverseJIT-in-eBPF
...
[ReverseJIT-In-eBPF Variant](#reversejit-in-ebpf-variant).

### Per-Program Specialization

Write a target-ISA interpreter in C. For each user program:
...
2. The interpreter is specialized to that data at compile time. Because the
...
4. The kernel eBPF JIT executes that program. Despite being called an
   "interpreter", the actual runtime path is straight-line per-handler native
   code with no dispatch overhead, equivalent in shape to a direct translation
   of the target instructions to eBPF.

"Interpreter on the source side, JIT on the runtime side" is the same code path
when specialization is performed at compile time rather than runtime. The
interpreter is the semantic specification; the verifier-visible artifact is its
specialization to one program.

### Why The Verifier Accepts It

A generic eBPF-written interpreter for a Turing-complete ISA cannot pass the
...
interpreter abstraction. If `P` reads past a packet end, the corresponding
...
interpreter does not perform any masking, bounds insertion, or sandbox
...
- the C source of the interpreter `I`;
- clang's compilation of `I` to eBPF.

The interpreter `I` is small, fixed per target ISA, and amenable to formal
verification. Possible sources for the interpreter semantics:

- a hand-written reference interpreter, formally verified once;
- a generated interpreter from a machine-checked ISA specification (for
  example Sail, `github.com/rems-project/sail`);
- ARM publishes an official C reference interpreter that could serve as a
...
### Why It Is Not An Actual Interpreter At Runtime

Naive reading: this is just a software interpreter and will be slow.
...
"interpreter" is the semantic anchor for the specialization, not the runtime
shape.

### Helper, Kfunc, And Boundary ABI

Native programs make calls. The interpreter cannot emit those as opaque
...
31 on arm64 vs. 10 BPF registers). The specialized interpreter places the
...
| Component | LOC | Notes |
|---|---|---|
| Per-arch interpreter (x86) | 1.5k–3k C | dispatch + per-insn handlers |
| Per-arch interpreter (arm64) | 1k–2k C | smaller ISA, simpler |
| Specialization driver | 500–1k Rust/C | reads target binary, emits specialized interpreter source or eBPF directly |
...
interpreter for a 50-instruction program, and verify that (a) clang produces a
...
interpreter variant — the JIT replaces the interpreter as the trusted lowering,
...
When to prefer the interpreter variant:

- the lowering rules are simple and uniform per native instruction;
- semantic verification is anchored on the interpreter rather than the emitter
  (interpreters are easier to formalize against an ISA specification than
  emitters);
...
```text
Any ReverseJIT/ReverseInterpreter program accepted by the existing eBPF
...
- the interpreter or JIT written in C (one per target ISA);
...
```text
native program P
  -> lower(P) via interpreter or JIT I = eBPF program B
  -> verifier accepts B
...
The native code does not become safe because some separate eBPF VM was
...
- specialized interpreter (this doc's main case): the interpreter is the
  trusted lowering;
...
proves the safety of sandboxed x86 VM semantics, not the safety of treating
`rax` as a real kernel address. ReverseInterpreter's lowering is faithful
...
ReverseJIT instruction had both a verifier-visible `instantiate_insn()`
...
execution refinements. The verifier path lowers ReverseJIT instructions to
...
kinsn paper line, not in this one. The interpreter and JIT variants above keep
...
PCC checks a user-provided proof for native code. ReverseJIT/ReverseInterpreter
...
ReverseInterpreter avoids arbitrary native validation by never running
...
preserves eBPF semantics. ReverseInterpreter is dual: it proves that a
...
for arm64 and RISC-V, and ARM's official C reference interpreter. These are
candidate sources for the trusted interpreter `I`.
...
```text
ReverseJIT/ReverseInterpreter brings native code into the kernel under the
...
```text
ReverseJIT: Bringing Native Code into the eBPF Safety Model
...
- complexity of the trusted lowering (interpreter or JIT in eBPF) and its
...
- If clang does not reliably collapse the specialized dispatch, the
  verifier-visible artifact is a generic interpreter and will not pass the
...
- If the helper/kfunc recognition table goes out of sync with the running
  ...
  versions.
- If the trusted interpreter or JIT is large and ad hoc, the TCB story becomes
...
```text
ReverseJIT/ReverseInterpreter does not execute user-supplied native code. It
executes the stock eBPF JIT lowering of an eBPF program produced by a trusted
in-eBPF interpreter or JIT, which makes a one-to-one faithful translation of
...
以下是讨论该 specialized eBPF interpreter variant 的原始对话记录,原文保留,
未做归纳或措辞调整。出于隐私考虑不标注具体发言者,按时间顺序呈现。

> 这里核心的问题是"interpreter 对目标程序的解释执行"通过验证器的检查,并不能
> 说明目标程序真的安全,比如目标 native 程序里的一个 oob,可能不会导致
> interpreter 的 oob,除非这个目标程序自己是 sanitized 的(插了安全检查)。

> 这个感觉很酷哈哈,但是可能不好实现

> 工程来说 这个 interpreter 可能需要用到很多 bpf 的 hack,不过应该是 feasible

> 对,我们不仅仅要让 interpreter 通过验证器的检查,实际上还要形式化验证这个
> interpreter/JIT 是完全等价于 x86 指令的

> 目前看来....我今天试了试,确实不太好做

> 甚至可能 JIT 反而更好做一点

> 假如不能信任 clang 的编译(应该是不能?),那么我们要形式化验证 interpreter/JIT
> 大概率要手写指令集转换的?
...
> 信任 clang 的话我基本上写出来 interpreter 了
...
> 这里是指信任 clang 编译的 interpreter (in bpf bytecode)?我有点 confused

> 是的,信任 clang 编译的 ebpf interpreter 的编译过程正确

> 这样我们形式化验证 c 写的 ebpf interpreter 正确就好了?

> 然后我们再把 c 写的 ebpf interpreter 加上对应的 x86 指令给 ebpf verifier,
> 等于我们让 ebpf verifier 去 verify "正确的 interpreter 执行这几条 x86 指令的
> 过程"

> X86 interpreter 到 ebpf bytecode 编译的等价性可以假设

> 只要 ebpf verifier 确认"正确的 interpreter 执行这几条 x86 指令的过程"是
> 安全的那就安全了
...
> 如果目标程序不插检查它就通不过 ebpf verifier 了,因为这样的话会导致 interpreter
> 解释的过程中出现越界访问?
...
> 不过今天讨论的这个如果是 interpreter 其实变成又和 kinsn 不一样的 idea 了(?

> 确实在一个地儿 我都没注意到

> X86 Interpreter 视角没有 oob,所以我说目标程序的 oob 可能不会体现为
> interpreter 的 oob

> verifier 不能只 verify interpreter(图灵完备的不停机 interpreter 也过不了
> 编译器),verify 的一定是"正确的 interpreter 执行这几条 x86 指令的过程"

> 所以目标程序的 oob 一定会在执行的过程中体现为 interpreter 的 oob

> 就是这个 interpreter 在执行的时候的某一个步骤会去越界访问 packet 之类的,
> 因为它的执行会是和 native insn 完全一致的

> interpreter 本身感知不到 oob 也不会去检查,它就是个单纯的翻译器
...
> 加了 interpreter 这一层以后,不知道会增加多少验证难度
...
> compiler,不如直接验证 interpreter)

> x86 interpreter 也很复杂
...
> interpreter 可以想象成 native 指令对应的 eBPF 指令序列的 map
...
> 最好基于 reference interpreter,适配到能编到 eBPF

> ARM 应该有官方的 C interpreter

> 有什么别人已经形式化验证过了的吗

> https://github.com/rems-project/sail

> 直接从 formal semantics 生成 interpreter
...
> abi(kfunc/helper/pseudo call) 确实是个问题,在 interpreter 层面对应改 pc,
...
3. ReverseJIT / ReverseInterpreter (本 doc)
   定位: x86/arm interpreter or JIT written in eBPF,
        enabling safely running native code in the kernel
        without additional proof from the user
   关键技术: 特化的 eBPF interpreter(主线) 或 native-to-eBPF JIT in eBPF
   内核改动: 接近零
   形式化对象: 单个 C interpreter / JIT 的正确性
