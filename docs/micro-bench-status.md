...
| Local-call fanout | Entry dispatch still calls local functions, but local callees use compact native wide loads/rotates | BPF retains bpf2bpf calls; current LLVM backend can now replace aligned local-subprogram byte ladders with verifier-native `LDD` | Interprocedural inlining/layout plus proof-stack-safe subprogram transforms |
