...
- 方法：从 `105` 个 code-size paired instances 出发，尝试以 `--io-mode packet --input-size 64 --repeat 1000` 在双 runtime 上执行
- 成功：`44/105` instances 双侧运行成功（`46` 个 kernel ok，其中 `44` 个 llvmbpf 也 ok）；按 unique program name 去重后为 `8` 个 unique programs
- 失败分解：`58` 个 `ENOTSUPP`（tracepoint/kprobe/cgroup），`1` 个 `Bad address`（iter/task），`2` 个 llvmbpf error

| 指标 | 值 |
|------|---:|
| Paired exec-time instances | 44 |
| Unique programs | 8 |
| Exec ratio geomean (L/K, all 44) | 0.753x |
| Exec ratio geomean (L/K, 8 unique) | 0.894x |
| llvmbpf wins / kernel wins (unique) | 5 / 3 |

**重要 caveat**：所有 `44` 个 instance 的 kernel `exec_ns` 均低于 `100ns`，属于 sub-resolution 计时区间。这是因为 dummy 全零包触发了真实 TC 程序的快速退出路径（检查包头失败 → 立即返回），测量的是程序最短执行路径而非完整逻辑。因此这些结果仅提供方向性证据：llvmbpf 在真实程序入口路径上也倾向于更快，但不能用于定量推断完整负载下的性能差异。
...
- 真实程序 execution-time 对比受 `BPF_PROG_TEST_RUN` 结构性限制：tracepoint/kprobe/perf_event 程序返回 `ENOTSUPP` (`524`)，fentry/fexit 在 dummy input 下 `exec_ns=0`。TC/classifier 和 socket_filter 程序可以运行（需提供 `--input-size 64` 以满足最小包长），当前已在 `44/105` 个 paired instance（`8` 个 unique program）上获得执行时间数据，但所有程序均为 sub-resolution（kernel exec < 100ns），因为 dummy 全零包触发了程序的快速退出路径。因此真实程序的 execution-time 外部验证目前仅提供方向性证据（geomean `0.894x`，llvmbpf 5/8 wins），不能作为定量结论。
