...
```
micro/config/micro_pure_jit.yaml ──→  pure-JIT / staged-codegen 微基准   (32 case = 30 pure-jit + 2 packet-backed)
micro/config/micro_runtime.yaml  ──→  map/helper runtime 机制基准        (9 case)
corpus/config/macro_corpus.yaml  ──→  真实程序语料库入口 + 外部验证层   (first slice)
```
...
- vendored bpftool（可选）
- 声明式 `micro/config/*.yaml` / `corpus/config/*.yaml`
...
- 脚本（历史 `llvmbpf` 结果对应路径；2026-03-19 起脚本语义已切到 `kernel` vs `kernel-recompile`）：`corpus/run_real_world_exec_time.py`
...
> 数据来源：`micro/results/pure_jit_authoritative.json`，归档分析：`micro/results/archive/stability_analysis.md`
...
> 修改：`runner/src/kernel_runner.cpp` 添加 rdtsc/rdtscp 围栏计时
...
> 归档报告：`micro/results/archive/representativeness_report.md`
...
- [x] **深度性能分析** — 归档报告 `micro/results/archive/performance_deep_analysis.md`，6 维分析 + 报告
...
- [x] suite rename — `micro/config/micro_pure_jit.yaml` 的 `suite_name` 现为 `micro_staged_codegen`
...
- [~] **T3.9 静态特征→优化收益预测模型 (RQ3.3)** — 归档性能分析已完成初步 linear regression：3 个静态特征（insn_count, branch_count, code_ratio）预测 exec_ratio，R²=0.070，说明简单静态特征几乎无法预测运行时性能。详见 `micro/results/archive/performance_deep_analysis.md`。剩余：随机森林 / 更多特征（helper count, loop depth）未尝试。
...
3. 在对应 `micro/config/*.yaml` 中添加 benchmark 条目：`name`、`base_name`、`kernel_input_size`、`input_generator`、`expected_result`、category/family/level/hypothesis/tags
