...
- Macro benchmark 批跑
- 默认 suite 中的 `bpf-to-bpf local call` family（历史上受 llvmbpf ELF loader 限制；现已修复 real-world loader，但该 family 仍未纳入 authoritative micro suite）
...
> 2026-03-07 multi-function loader fix：llvmbpf 的 multi-function ELF loader 已修复，真实程序 compile-only paired set 从 `105` 扩到 `162`（`36` 个 unique program name），llvmbpf compile-only inspect 成功数从 `890` 提升到 `948`。当前 authoritative micro suite 仍未把 local-call family 纳入默认覆盖，因此 representativeness 里的 multi-function gap 仍需单独承认。
...
- 报告：`corpus/results/real_world_exec_time.md`
- 方法：从更新后的 `162` 个 code-size paired instances 出发，尝试以 `--io-mode packet --input-size 64 --repeat 1000` 在双 runtime 上执行
...
| Unique programs | 14 |
| Exec ratio geomean (L/K, all 98) | 0.694x |
| Exec ratio geomean (L/K, 14 unique) | 0.769x |
| llvmbpf wins / kernel wins / tie (unique) | 9 / 4 / 1 |

**重要 caveat**：所有 `98` 个 instance 的 kernel `exec_ns` 仍低于 `100ns`，没有任何程序进入 above-resolution 区间。这是因为 dummy 全零包触发了真实 TC/socket 程序的快速退出路径（检查包头失败 → 立即返回），测量的是程序最短执行路径而非完整逻辑。因此这些结果依然只提供方向性证据：llvmbpf 在真实程序入口路径上整体更快（all-instance geomean `0.694x`，unique geomean `0.769x`），但不能用于定量推断完整负载下的性能差异。
...
- 真实程序 execution-time 对比受 `BPF_PROG_TEST_RUN` 结构性限制：tracepoint/kprobe/perf_event 程序返回 `ENOTSUPP` (`524`)，fentry/fexit 在 dummy input 下 `exec_ns=0`。TC/classifier 和 socket_filter 程序可以运行（需提供 `--input-size 64` 以满足最小包长），当前已在 `98/162` 个 paired instance（`14` 个 unique program）上获得执行时间数据，但所有程序仍为 sub-resolution（kernel exec < 100ns）；expanded paired set 中仍没有任何 above-resolution kernel timing。这些结果因此仍只提供方向性证据（instance geomean `0.694x`，unique geomean `0.769x`，llvmbpf 9/14 wins），不能作为定量结论。
...
- **子程序结构存在结构性空白**：从静态 `bpf2bpf_call_count` 维度看，range coverage 高达 `96.7%`，但这并不代表“多函数程序”被覆盖；报告同时显示 `97.2%` 的真实程序包含多个函数，而当前 suite 中这一比例仍为 `0%`。虽然 llvmbpf 的 multi-function ELF loader 已修复并把外部 paired code-size 集扩到 `162/36`，当前 authoritative micro suite 仍没有 local-call benchmark family，因此该结构性缺口并未被 representativeness 分析自动消除。
...
**External validation**: 162 paired real programs (36 unique) from 4 repos — code-size geomean **0.618x**; llvmbpf is still smaller on **34/36** unique programs. Exec-time: 98/162 packet-runnable programs completed on both runtimes (14 unique), with geomean **0.694x** by instance and **0.769x** by unique program, but all remain sub-resolution (dummy-packet fast-exit; no kernel timing above `100ns`). Feature-space coverage: **0.8%** of 1588-program BCF corpus.

**Publication readiness**: 7.5/10. Strong micro-benchmark rigor and causal analysis; external validity is better than the 105-pair stage but still bounded (0.8% coverage, 36 unique code-size programs, no above-resolution real-program exec-time). Top gaps are above-resolution packet inputs and adding multi-function micro cases, not the old loader blocker itself.

## 下一步优先级（2026-03-07 更新）

> 按 impact/effort 排序的行动计划。当前评分 7.5/10，目标 8.5/10。

### P0 — 解决 sub-resolution 外部验证（7.5 → 8.0 的关键）

- [ ] **N1. 构造真实网络包** — 当前 98 个 exec-time 实例全部 sub-resolution（kernel exec < 100ns），根因是 dummy 全零包触发 TC 程序快速退出。需要构造合法 Ethernet + IP + TCP header（≥54 字节），使程序走完整处理路径。目标：获得第一批 kernel exec > 100ns 的真实程序性能数据。
  - 实现：在 `corpus/run_real_world_exec_time.py` 中生成 valid packet header 替代全零包
  - 验证：至少 10 个 Cilium TC 程序的 kernel exec_ns > 100ns
...
- [ ] **N5. micro suite 增加 local-call benchmark family** — 现在 loader 支持多函数了，可以写含 BPF-to-BPF call 的 benchmark，覆盖当前 0% 的子程序结构空白。
  - 实现：新建 `bpf_to_bpf_call.bpf.c`，含 1-3 个 subprogram call，测 call overhead
...
- [x] **exec-time 重跑** — 98/162 paired（14 unique），geomean 0.694x，仍全部 sub-resolution
...
- 双 runtime exec-time：`98/162` paired instances（`14` 个 unique program name），但仍全部 sub-resolution
