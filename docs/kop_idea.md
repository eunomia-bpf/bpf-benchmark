# KOperation:让 eBPF 更贴近硬件的一个新 OS 抽象

状态:研究方向 · idea #2 的论文线 hub

> **边界更新（2026-07-10）**：KOperation 与 speculative optimization 不再
...
**KOperation**(kernel instruction)是一个新的 OS 抽象,它让内核能向 eBPF 程序暴露硬件
专用的指令原语,而无需扩展核心 eBPF 指令集、也无需为每个特性打 verifier 或 JIT patch。

每条 kop 都是一个**双语义原语**:

- **Verifier 语义**:模块提供 `instantiate_insn(payload, insn_buf)`,把该
  kop 展开成 verifier 已经理解的普通 BPF proof sequence。内核 verifier
...
kop 以现有 kfunc BTF 发现机制承载身份,但不再通过 `KF_KOP` 绑在 kfunc
flag 上。当前实现把 kfunc BTF id 映射到 `struct bpf_kop` descriptor;verifier
用 `instantiate_insn()` 做 proof lowering,JIT 在 CALL-emit 时识别 kop pseudo
call 并分派到 descriptor 的 native emit 回调。

新优化 = 在一个小内核模块里定义一条新 kop descriptor + 一个用户态模式识别器
(在 `bpfopt` 里),把匹配的 BPF 字节码重写成调用该 kop。**每个新优化对核心内核
verifier、JIT 或 BPF ISA 的改动为零。**

## 3. 为什么这是一个新抽象,而不是又一个 peephole

三条性质把 kop 与内核内 peephole 优化区分开:

1. **策略与机制分离**。内核模块提供机制(指令做什么、verifier 该验证哪个 proof object、
   JIT 该如何 emit 它)。用户态 `bpfopt` 提供策略(重写哪些模式、用什么 cost model、何时值得插入
   一条 kop)。机制小而固定;策略丰富而可迭代。

2. **每个优化不打核心内核 patch**。核心内核 JIT 和 verifier 不动。加一条新 kop 是一次
   模块改动加一个外部字节码重写 pass。上游周期是一次性的(针对 kop 框架本身),不是
   每个特性一次。

3. **随 workload 自适应的插入**。一条 kop 是否有益取决于 workload 特征(CMOV 看分支
...
- kfunc BTF id 仍用于发现和重定位,但 kop 身份来自注册时附加到
  `btf_kfunc_id_set` 的 descriptor 数组,而不是一个新的 `KF_KOP` flag。
- 打包编码:一条紧邻在 kop `BPF_CALL` 之前的 sidecar 伪指令
  (`BPF_PSEUDO_KOP_SIDECAR`)携带操作数位。verifier 先把
  `sidecar + BPF_PSEUDO_KOP_CALL` 临时 lower 成 proof sequence 做普通 BPF
  验证,再恢复原始两条指令交给 JIT。
- 模块生命周期:标准 Linux 模块 load/unload。当一个 kop 模块未加载时,verifier 拒绝
...
| 文件 | 职责 |
|------|------|
| `include/linux/bpf.h` | `struct bpf_kop` descriptor、sidecar payload helpers、JIT lookup API |
| `include/linux/bpf_verifier.h` | kop verifier 辅助结构体 |
| `include/linux/btf.h` | `btf_kfunc_id_set.kop_descs` descriptor hook |
| `include/uapi/linux/bpf.h` + `tools/include/uapi/linux/bpf.h` | `BPF_PSEUDO_KOP_SIDECAR` + `BPF_PSEUDO_KOP_CALL` enum 扩展 |
| `kernel/bpf/btf.c` | kop BTF id 解析 |
| `kernel/bpf/verifier.c` | kop proof lowering / restore、descriptor 查找、sidecar 解码 |
| `kernel/bpf/disasm.c` | kop 反汇编支持 |
| `arch/x86/net/bpf_jit_comp.c` | x86 JIT CALL-case kop 内联 dispatch |
| `arch/arm64/net/bpf_jit_comp.c` | arm64 JIT kop 内联 dispatch |

净 diff 体量(仅 kop 分支,叠加在 stock 7.0-rc baseline 之上):
**10 个文件共 +869 / -101 LOC**(零 REJIT 引用,纯 kop surface)。

## 5. 覆盖面与取舍

kop 暴露面是刻意有界的。覆盖决策由 corpus 证据驱动:只有当支持的运行时 corpus 里有
足够多的 site 来证明内核模块暴露面的合理性、且在插入点处有可测量的性能收益时,才加入
一个候选 kop。

**已实现的 kop**(在对应架构上为默认策略):

| KOperation | 架构 | 状态 | Site 证据 | 设计 / 调研引用 |
|---|---|---|---|---|
...
| 候选 | Site 证据 | 理由 / 调研引用 |
|---|---|---|
| POPCNT / CLZ / CTZ | 0 site | clang 已展开 `__builtin_popcount` 为高效位操作序列。`bit_ops_kop_research_20260329.md` |
| CRC32 | loxilb SCTP CRC32C: 2 个 byte-update site | broad corpus 覆盖低;若做,第一版 CRC32C-only no-FPU scalar backend + loxilb-targeted `step8/step64` idiom pass。`crc32_kop_research_20260329.md` |
| PDEP / PEXT | 0 site | corpus 无 site。`bit_ops_kop_research_20260329.md` |
| SHRX / SHLX | 0 incremental gain | OoO CPU 上无增量收益。`bit_ops_kop_research_20260329.md` |
| MADD / MSUB | MADD 47 direct sites;MSUB 0 direct sites | MADD 仅二级优化;MSUB 宽松形式需 liveness/semantic work,不进 first wave。`arm64_kop_research_20260329.md` |
...
它,但该路线被"不改核心 JIT"政策排除。已实现的纯 kop 路线保住了那条边界,并让用户态
...
两个 idea 共用 `bpfopt` 作为用户态重写工具和同一套 runner/corpus,但不
共用 daemon。Speculative 路线由 app 进程内 shim 编排 stock-kernel
`BPF_PROG_LOAD`;KOperation 路线额外依赖 KOperation kernel/module 能力。

- idea #1 的默认 pipeline 只有 BPF-to-BPF 重写(`map_inline`、`const_prop`、`dce`、
  `bounds_check_merge`、`branch_flip` 等)。它要求接近零的内核改动。
- idea #2 的 pipeline 可以包含引入 kop 的 pass(`rotate`、`cond_select`、`extract`、
  `endian_fusion`、`prefetch` 等)。这些需要内核里的 kop 框架 patch 加上各架构 kop
  模块。
...
KOperation 覆盖普通 eBPF 表达不好的少数几种模式。NativeBPF 覆盖目标 ISA 能表达的任何东西,
只要其 lowering 对 verifier 可处理。两者并不互斥:一个支持 kop 的内核,配上一个在有
益处时 emit kop 的 NativeBPF,是一个自然的 ablation 点,但二者互不依赖。
...
```text
KOperation: A Hardware-Aware Instruction Extension Surface for eBPF
```

核心的系统主张:

```text
KOperation introduces a new kernel abstraction that lets eBPF programs use
platform-specific hardware instructions safely. The verifier checks each
kop through a BPF-visible proof sequence supplied by a kernel module; the
...
- 广泛的 kop 覆盖(至少 rotate / cond_select / extract / endian / prefetch /
  pair load-store / bulk memory),而非单个原语;
- 真实 eBPF 程序(Cilium / Katran / Tetragon / Tracee / BCC / bpftrace /
  loxilb),而非只有 microbenchmark;
- 在至少一条策略敏感的 kop 上(CMOV 是典型例子),随 workload 自适应的插入胜过固定的
  内核启发式;
- 小而可理解的内核暴露面(框架 patch);
- 模块级隔离(加载或卸载一个 kop 模块不影响无关程序);
- arm64 结果,而非只有 x86,以验证按架构区分的抽象。

## 8. 相关工作定位

- **kfuncs(现有上游)**:kop 复用 kfunc BTF id 发现和重定位,但把目标身份映射到
  `struct bpf_kop` descriptor。贡献点是 proof-lowering verifier 路径和双语义
  native emit 路径,而非 BTF 本身。
- **JIT peephole**:核心 JIT 里按架构区分的 peephole(例如上游 arm64 LDP fusion)。
  kop 把同样的能力推到一个模块边界之后,使得新增不触碰核心 JIT。
- **JVM intrinsics**:HotSpot intrinsic 把标准库调用替换成由 JIT 挑选的手写 native
  序列。kop 是把同一思路用到 eBPF 上,并让 verifier 通过 proof-lowered BPF
  sequence 检查该 intrinsic 的 BPF-visible 语义。
- **K2 / Merlin / EPSO**:在 load 前对源 `.bpf.o` 操作的 BPF 字节码优化器。它们不扩展
  BPF ISA,也无法 emit 标准 JIT 词汇表之外的 native 指令。kop 通过拓宽 emit 词汇表来
  与它们互补。
- **hXDP(OSDI 2020)/ eBPF Program Warping(ATC 2022)**:与 kop 思路同构的最近邻
  prior work。Program Warping 用 peephole 把一串 eBPF 指令替换成**优化过的硬件实现**,
  hXDP 给 eBPF 扩 ISA 以在 FPGA NIC 上执行。这正是 kop 的核心("识别一段 BPF 模式,
  emit 成更接近硬件的单条原语,拓宽 emit vocabulary"),区别在于它们的 target 是 FPGA
  overlay,而 kop 在 stock host x86/arm64 JIT 上做、走模块边界、verifier 见
  proof-lowered BPF。
- **经验证的 JIT / translation validation(Jitterbug OSDI 2020、Synthesizing JIT
  Compilers for In-Kernel DSLs CAV 2020)**:kop 的 soundness 论证(§9:
  ...
  既有方法论范式。
- **peephole / 超优化的正确性谱系(kop soundness 的直接方法论来源)**:kop 的
  ...
  - kop 与它们的区别:它们离线发现 / 验证一条 host 指令序列;kop 把这种"更接近硬件
...
- **ePASS(in-kernel eBPF 编译框架,2025)**:把 SSA IR 与 verifier 协同放进内核做分析 /
  变换,与 kop 的"内核模块 + verifier 协同"架构可直接对比(也与 idea #1 相关)。
- **BeeBox / MOAT / Hive(USENIX Security 2024)**:给 eBPF 加硬件或 SFI 隔离的对照组
  (分别针对瞬态执行、Intel MPK、AArch64 硬件隔离)。可用于论证 kop 的 TCB 增量相对
  这些"改运行时隔离"路线的取舍。

## 9. 主要风险

- 每条新 kop 都增加一小块内核暴露面(模块 proof generator、JIT emit)。总暴露面随
  kop 数量增长,审计成本随之累积。
...
- 随 workload 自适应的插入需要可靠的 per-site profile 数据。idea #1 的 `bpfprof
  ...
  回退到一个保守默认值,而不是去猜。
- 一些看起来很自然的 kop(FPU SIMD、RDTSC)无法加入,因为要么会改变语义(RDTSC 非
