...
| **646** | **bpfopt 独立优化工具链（2026-04-07, 2026-04-28 修正）** | ✅ | 2026-04-07：从 daemon 提取 pass 代码到独立 workspace。2026-04-26：workspace 收缩并重命名为 `bpfopt/`，只保留 `bpfopt-core`（pass library）和 `bpfopt`（CLI）；删除 `kernel-sys`、`bpfget`、`bpfverify`、`bpfprof`、`bpfrejit`、`bpfrejit-daemon` stub crate。**2026-04-28 修正**：`08f8a7cf` 的“删除 bpfopt、保留 daemon pass”方向已被反向修正；pass/IR/analysis 单一实现位于 `bpfopt-core`，daemon 依赖该 crate 并只保留 serve/syscall/verify/REJIT/profiler/invalidation 等 runtime 逻辑。`MapInfoProvider` / `MapValueProvider` 区分 daemon live raw syscall 与 bpfopt snapshot；`KinsnCallResolver` 区分 daemon fd-array slots 与 bpfopt static call offsets。设计文档：`docs/tmp/bpfopt_design_v3.md`、`docs/tmp/daemon-bpfopt-unification-20260428.md`。Benchmark review：`docs/tmp/benchmark_status_review_20260403.md`。Commits：`ea92e14`、`8b66471`。 |
