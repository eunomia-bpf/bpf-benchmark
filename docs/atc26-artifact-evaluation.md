...
and the optimized run retaining per-step bytecode. `wide_mem` is a pure
BPF-to-BPF rewriting pass rather than a kfunc-lowering pass, and Tetragon is the
densest retained `wide_mem` producer of the six applications (616 applied sites
across its retained report streams, against 299 for Tracee and 65 for BCC), so
the controlled measurement has the most sites behind it. The retained report
stream gives 254 changed load instances totalling 254 applied sites
