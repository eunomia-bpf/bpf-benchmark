...
A thirteenth derivation moves the **fresh provenance-complete** treatment to a
different pass, so the controlled-causality method is not specific to kfunc
lowering: `docs/artifacts/evidence/rq2-tetragon-wide-mem-fresh-causality/`
...
derivation gives median raw 1.0142x, no-pass control median 0.9346x (controls
0.9358/0.9334), control-corrected 1.0852x over 3+3 samples at 60 s. The
workload is the same component-less stress-ng shape as the ninth derivation, so
the derived scalar is the same workload-level `stress-ng: metrc:` bogo-ops
column. The sign of the control-corrected ratio agrees with the May Tetragon
`map_inline` batch's independent `1.0840` (both above 1.0), but that batch is a
different pass and a different generation, so the two are never merged. The
renderer generalizes its single fresh-causality builder on the pass under test,
so this row is gated on `enabled_passes==["wide_mem"]` and on its own evidence
directory and cannot be satisfied by any `map_inline` triplet's files; its
declared constants are frozen from this triplet and any drift degrades the row
to `PARTIAL` rather than silently re-baselining.

A fourteenth derivation is the second **fresh provenance-complete** treatment of
a non-`map_inline` pass and, with the thirteenth, shows the derivation is not
specific to a pass family or a workload rate shape:
`docs/artifacts/evidence/rq2-cilium-wide-mem-fresh-causality/` carries one
`wide_mem` run (`x86_kvm_corpus_20260926_133930_796763`) plus two matched
no-pass controls (`x86_kvm_corpus_20260926_135405_708186`,
`x86_kvm_corpus_20260926_140808_071557`), each with its own make-console log and
the optimized run retaining per-step bytecode. The retained report stream gives
164 changed load instances totalling 164 applied sites (instruction counts
159,896 to 156,508, −3,388), all 164 of whose retained before/after images match
the reported counts and differ; the throughput derivation gives median raw
1.1232x, no-pass control median 1.0234x (controls 1.0187/1.0282),
control-corrected 1.0974x over 3+3 samples at 60 s. Unlike the thirteenth
derivation, whose workload is a single component-less stress-ng run, Cilium's
fresh workload emits two kernel-pktgen components, so the derived scalar is the
sum over both components' pktgen pps; the shared summed extractor already
accepts both shapes, so the same builder serves the triplet unchanged. The
corrected ratio is above 1.0 here, whereas Cilium's `map_inline` triplet lands
below it: the two are different passes measured in separate generations, so the
row claims the controlled measurement rather than a speedup and the two are
never merged. The row is gated on `enabled_passes==["wide_mem"]` and on this
