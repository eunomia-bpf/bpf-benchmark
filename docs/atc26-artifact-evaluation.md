...
are never merged. The renderer's single generic builder now serves four fresh
triplets across two workload shapes, each gated on its own app record and
report path and each carrying its own frozen declared constants, so a Tracee
triplet cannot be satisfied by Katran, Cilium, or Tetragon files.
