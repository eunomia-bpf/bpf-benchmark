# katran icmp-protocol-refresh correctness
...
- Coverage: passed by source inspection; non-ICMP packets keep the protocol
  value returned by `parse_l3_headers()`, and ICMP error packets still refresh
  from `pckt.flow.proto`.
- Source restore: passed; `patch -p1 -R < source.diff` completed and
  `git -C vendor/repos/katran status --short` is empty.

Disk after run:

- `/dev/nvme0n1p2 915G 689G 180G 80% /`
