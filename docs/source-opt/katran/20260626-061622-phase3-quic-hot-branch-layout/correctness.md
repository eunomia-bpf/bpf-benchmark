...
| ABI | Patch only adds branch prediction hints in QUIC/stable-routing parser fallback branches; no map key/value, packet metadata, tail-call, or action ABI changed. | pass |
| Coverage | No parser branch, stats update, map lookup, policy check, or fallback path was removed. | pass |
