...
| Coverage | The bypass only applies to no-selector tracepoint policy entries after `event_find_curr()` succeeds. All other cases fall back to the original filter tail-call path, including selector-enabled entries and `event_find_curr_probe()` fallback. | pass |
