...
| attempt 5 | planned | pending | target a hot BPF path after attempt 4 |

## Completion Gates

- [x] Clean-source baseline result recorded.
- [ ] Five source-only attempts recorded.
- [ ] Every accepted attempt has `source.diff`, command, result path, and correctness notes.
- [ ] App source restored after every attempt.
- [x] Root `docs/source-opt/README.md` performance table updated through attempt 4.
