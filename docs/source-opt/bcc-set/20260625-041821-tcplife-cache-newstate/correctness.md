...
| --- | --- | --- |
| Build | formal `make corpus` rebuilt `tcplife.bpf.o`, regenerated `tcplife.skel.h`, rebuilt `tcplife`, and copied updated BCC binaries into the runtime image | pass |
...
| Coverage | `source.diff` does not remove filters, output, cleanup, or programs | pass |
