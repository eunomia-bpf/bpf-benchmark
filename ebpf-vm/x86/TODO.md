# x86 VM Direct-Native Safety TODO
...
| --- | --- | --- |
| Branch proof assertions | done | `last_cmp_*` and `x86_vm_assert_*` are removed; branches use only x86 flags. |
...
| Runtime fallback return | done for active top-level path | Top-level trap return calls an invalid helper so a reachable trap rejects load instead of returning `XDP_ABORTED`; generated micro objects load, so no top-level trap path is verifier-reachable in the current corpus. |
| Stack slot range checks | done for current micro, proof open | The current generated objects only use modeled slots; any reachable unmodeled slot still rejects load through the fail-closed trap path. Final acceptance must prove stack slot coverage before native execution. |
| Runtime `X86_INTERP_TRAP` reachability | done for current micro, open for proof | Current 29/29 load result means no trap path is verifier-reachable in these objects; final direct-native safety still needs a proof or load-time rejection rule for every accepted artifact. |
...
| Full micro rerun after safety cleanup | done | `results/README-20260518-201933.md` records 29/29 compile/load/test success after adding slot7/slot8 pointer payload support. |
