...
| Build reusable eBPF interpreter/helper bytecode | partial | C-authored template object exists. The loader now inlines helper bodies and patches JSON argument loads to constants. |
| Batch runner uses JSON path as the pass/fail source | partial | It builds the C template object and runs JSON -> loader -> raw BPF load/test_run. |
| All 29 micro programs load and return expected result | blocked | 2/29 pass on the strict JSON-link path. Remaining failures are verifier state explosion, long native-CFG jumps after inlining, native call flow, and ctx/stack memory semantics. |

Current strict JSON-link status:

| Micro program | JSON-link status | Note |
| --- | --- | --- |
| `simple`, `simple_packet` | ok | JSON is generated by Python, C-authored templates are linked by the Rust loader, the kernel verifier accepts the result, and `BPF_PROG_TEST_RUN` returns the expected result. |
| loop-heavy packet programs | verifier-state-limit | Memory helper templates now use offset/tag state and verifier-visible packet bounds, but loop programs such as `bitmap_popcount_scan` hit the 1,000,000 processed-insn verifier limit. |
| larger CFG programs | linker-branch-limit | Full helper inlining can put native branch targets beyond BPF's signed 16-bit branch offset. Needs short-jump islands, CFG chunking, or smaller specialized templates. |
| `bpf_local_call_fanout_dispatch` | linker-missing | Native `call` flow is still rejected by the JSON linker. |
| ctx-output programs | semantic-gap | `tc_packet_checksum_fold`/`cgroup_skb_hash_chain` use `[rdi+0x10]`/`[rdi+0x14]` output stores; ctx memory needs an explicit linked ABI instead of treating all stores as packet stores. |
...
| Define C helper-template ABI | partial | Current ABI is `state*, insn*, data, data_end`; loader inlines helper bodies, patches JSON argument loads, and stores pointer tags as 64-bit scalar state. |
| Implement minimal C-template link for `simple` | done | `simple` and `simple_packet` pass verifier and expected-result checks. |
| Extend template classes one at a time | in progress | Packet load/store/compare/ALU-mem templates are C-authored; ctx-output, native stack, and native call templates still need explicit linked semantics. |
...
```text
JSON step -> fill stack `struct x86_insn`
          -> inline C-authored `x86_tmpl_*` body
          -> patch `insn->op/dst/src/flags/aux` loads to JSON constants
...
operands, C defines helper semantics, and the Rust loader only links and patches
template parameters. The remaining hard blocker is not the old packet-pointer
scalar failure; it is verifier state growth for loops and oversized branch
distances after full helper inlining.

Active JSON-link todo:

1. Keep the Rust loader as a BPF linker only: schema validation, labels, branch
   fixups, `BPF_PROG_LOAD`, and `BPF_PROG_TEST_RUN`.
2. Move remaining x86 semantics into named C/bytecode templates, not loader
   code and not Python BPF emitters.
3. Replace broad stack-backed virtual state with verifier-friendly templates
   where proof permits it. Current blockers are loop state explosion, dead flag
   updates, and packet pointer range propagation.
4. For each micro, require all four checks: JSON generation, kernel verifier
   load, `BPF_PROG_TEST_RUN`, and expected result.
5. Treat any local abort/fuel bound/tail-inline shortcut as a proof obligation;
