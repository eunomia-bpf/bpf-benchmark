#!/usr/bin/env python3
"""Generate helper-selection x86-VM verifier artifacts.
...
def op_helper(encoded: EncodedInsn) -> str:
    ...
    try:
        ...
        raise ValueError(f"missing interpreter helper for {encoded.op}") from err
...
def render_program(name: str, insns: list[NativeInsn],
    ...
    lines.extend([
        '#include "../../x86_vm_bpf_helper_selection.h"',
        "",
    ])
    for symbol, fn_insns in subfunctions.items():
