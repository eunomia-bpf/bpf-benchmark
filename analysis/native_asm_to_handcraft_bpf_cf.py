...
SCRATCH_SAVE_OFF = -376
SCRATCH_R7_SAVE_OFF = -368
...
def div32_reg_bpf(src: tuple[str, int, str]) -> tuple[str, ...] | None:
    src_off = SHADOW_REG_OFF_BY_TOKEN.get(src[0])
    if not is_bpf_reg_name(src[0]) and src_off is None:
        return None
    load_divisor = (
        f"HC_MOV64_REG(BPF_REG_7, {src[0]})"
        if is_bpf_reg_name(src[0])
        else f"HC_LDX(BPF_W, BPF_REG_7, BPF_REG_10, {src_off})"
    )
    return (
        save_branch_scratch(),
        save_branch_scratch_r7(),
        load_divisor,
        f"HC_MOV64_REG(BPF_REG_6, BPF_REG_0)",
        f"HC_RAW(BPF_ALU | BPF_MOD | BPF_X, BPF_REG_6, BPF_REG_7, 0, 0)",
        f"HC_RAW(BPF_ALU | BPF_DIV | BPF_X, BPF_REG_0, BPF_REG_7, 0, 0)",
        "HC_RAW(BPF_ALU | BPF_MOV | BPF_X, BPF_REG_3, BPF_REG_6, 0, 0)",
        restore_branch_scratch_r7(),
        restore_branch_scratch(),
    )
...
def bpf_ja() -> str:
    return f"HC_RAW(BPF_JMP32 | BPF_JA, 0, 0, 0, {BRANCH_OFF})"


def save_branch_scratch() -> str:
    return f"HC_STX(BPF_DW, BPF_REG_10, BPF_REG_6, {SCRATCH_SAVE_OFF})"


def restore_branch_scratch() -> str:
    return f"HC_LDX(BPF_DW, BPF_REG_6, BPF_REG_10, {SCRATCH_SAVE_OFF})"


def save_branch_scratch_r7() -> str:
    return f"HC_STX(BPF_DW, BPF_REG_10, BPF_REG_7, {SCRATCH_R7_SAVE_OFF})"


def restore_branch_scratch_r7() -> str:
    return f"HC_LDX(BPF_DW, BPF_REG_7, BPF_REG_10, {SCRATCH_R7_SAVE_OFF})"
...
def branch_scratch_imm(op: str, lhs: tuple[str, int, str], imm: int) -> tuple[str, ...] | None:
    ...
    return (
        save_branch_scratch(),
        *load,
        f"HC_RAW({cls} | {op} | BPF_K, BPF_REG_6, 0, 2, {c_s32(imm)})",
        restore_branch_scratch(),
...
def branch_scratch_reg(op: str, lhs: tuple[str, int, str], rhs: tuple[str, int, str]) -> tuple[str, ...] | None:
    ...
    return (
        ...
        *lhs_load,
        *rhs_load,
        f"HC_RAW({cls} | {op} | BPF_X, BPF_REG_6, BPF_REG_7, 3, 0)",
        restore_branch_scratch_r7(),
        restore_branch_scratch(),
...
def branch_scratch_imm_width(op: str, lhs: tuple[str, int, str], imm: int, width: int) -> tuple[str, ...] | None:
    ...
    return (
        ...
        *normalize_for_branch("BPF_REG_6", width, signed),
        f"HC_RAW({cls} | {op} | BPF_K, BPF_REG_6, 0, 2, {c_s32(cmp_imm)})",
        restore_branch_scratch(),
        "HC_RAW(BPF_JMP32 | BPF_JA, 0, 0, 0, 2)",
        restore_branch_scratch(),
        bpf_ja(),
...
def branch_cmp_imm(op: str, lhs: tuple[str, int, str], imm: int) -> tuple[str, ...] | None:
    if is_bpf_reg_name(lhs[0]) and lhs[0] != "BPF_REG_10" and lhs[1] in {32, 64}:
        return (bpf_jmp_imm(op, lhs, imm),)
    if lhs[1] in {32, 64}:
        return branch_scratch_imm(op, lhs, imm)
    if lhs[1] in {8, 16}:
        return branch_scratch_imm_width(op, lhs, imm, lhs[1])
    return None
...
def branch_cmp_reg(op: str, lhs: tuple[str, int, str], rhs: tuple[str, int, str]) -> tuple[str, ...] | None:
    if lhs[1] != rhs[1] or lhs[1] not in {8, 16, 32, 64}:
        return None
    if (is_bpf_reg_name(lhs[0]) and lhs[0] != "BPF_REG_10" and
            is_bpf_reg_name(rhs[0]) and rhs[0] != "BPF_REG_10" and
            lhs[1] in {32, 64}):
        return (bpf_jmp_reg(op, lhs, rhs),)
    if lhs[1] in {8, 16}:
        return branch_scratch_reg_width(op, lhs, rhs, lhs[1])
    return branch_scratch_reg(op, lhs, rhs)
    ...
    if want_set:
        return (
            ...
            *mask_to_width("BPF_REG_6", lhs[1]),
            f"HC_RAW(BPF_JMP | BPF_JSET | BPF_K, BPF_REG_6, 0, 2, {c_s32(mask)})",
            restore_branch_scratch(),
            "HC_RAW(BPF_JMP32 | BPF_JA, 0, 0, 0, 2)",
            restore_branch_scratch(),
            bpf_ja(),
...
def branch_test_reg(lhs: tuple[str, int, str], rhs: tuple[str, int, str], want_set: bool) -> tuple[str, ...] | None:
    ...
    return (
        ...
        *mask_to_width("BPF_REG_6", lhs[1]),
        f"HC_RAW(BPF_JMP | {op} | BPF_K, BPF_REG_6, 0, 3, 0)",
        restore_branch_scratch_r7(),
        restore_branch_scratch(),
        "HC_RAW(BPF_JMP32 | BPF_JA, 0, 0, 0, 3)",
        restore_branch_scratch_r7(),
...
def translate(insn: NativeInsn) -> Translation:
    ...
    if len(ops) == 2 and op in {"mov", "movabs"}:
        ...
        if dst_reg and re.match(r"^-?(0x[0-9a-fA-F]+|\d+)$", src):
            ...
            if op == "movabs" or dst_reg[1] == 64 and abs(imm) > 0x7fffffff:
                ...
                return Translation("exact-bpf", code, "movabs via verifier-visible BPF shadow write")
            if dst_reg[1] == 32:
    ...
    if op == "div" and len(ops) == 1:
        src = bpf_reg(ops[0])
        if src and src[1] == 32:
            code = div32_reg_bpf(src)
            if code is None:
                return Translation("warning-unmapped", (), f"div {ops[0]} needs a verifier-visible scratch form")
            return Translation("exact-bpf", code, "div r/m32 via verifier-visible BPF quotient/remainder")
        return Translation("warning-unmapped", (), f"DIV operand form has no verifier-visible equivalent yet: {insn.raw}")
...
def direct_bpf_jcc(branch: NativeInsn, producer: NativeInsn | None) -> tuple[str, ...] | None:
    ...
    if code is None:
        return None
    # Ordinary BPF branches execute after kinsn restore, so they may only read
    # BPF-visible runtime state. Shadow x86 registers live only in verifier
    # proof slots unless a machine branch kinsn consumes the real native flags.
    if any(f", {off})" in item or f", {off}," in item for item in code for off in SHADOW_REG_OFFSETS):
        return None
    return code
    ...
    for insn in insns:
        ...
        if insn.mnemonic in JCC_OP:
            if flag_producer is None:
                ...
                code = direct_bpf_jcc(insn, flag_producer)
                if code is not None:
                    trans = Translation("exact-bpf", code,
                                        f"{insn.mnemonic} ordinary BPF branch from preceding {flag_producer.mnemonic}",
                                        trans.target_addr)
                else:
                    trans = Translation("control-flow-gap", (), f"{insn.mnemonic} after {flag_producer.mnemonic} has no direct BPF branch form", trans.target_addr)
...
def main() -> int:
    ...
    fatal = [
        (insn, trans)
        for insn, trans in zip(insns, translations, strict=True)
        if not trans.code and trans.status != "padding"
    ]
    if fatal:
