...
当前活跃开发区域是 `micro/`，配套的配置文件在 `config/`，研究文档在 `docs/`。
...
run_micro.py (orchestrator)
  ├─ benchmark_catalog.py    读取 config/*.yaml → SuiteSpec dataclass
...
| 变更 | 说明 |
|------|------|
| config 拆分 | `micro/config/suite.yaml` → 根目录 `config/micro_pure_jit.yaml` + `micro_runtime.yaml` + `macro_corpus.yaml` |
