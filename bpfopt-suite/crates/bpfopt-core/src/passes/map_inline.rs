...
fn try_extract_constant_key_from_map_value(
    ...
    let source_meta = program
        .map_metadata
        .get(&source_map_id)
        .ok_or_else(|| format!("no metadata for pseudo-map-value source map {}", source_map_id))?;
    if !source_meta.frozen {
        return Err(format!(
    ...
    let source_key = vec![0u8; source_meta.key_size as usize];
    let source_value = program
        .map_values
        .get(&(source_map_id, source_key.clone()))
        .ok_or_else(|| {
            format!(
                "failed to read pseudo-map-value source map {}: no value in map_values snapshot",
                source_map_id
            )
        })?
        .clone();
...
fn build_site_rewrite(
    ...
    let value = match program.map_values.get(&(info.map_id, encoded_key.clone())) {
        Some(value) => {
            log_map_inline_debug(&format!(
        ...
        None => {
            log_map_inline_debug(&format!(
                "site at PC={}: map_values lookup(map_id={}, key={}) not found in snapshot",
                site.call_pc,
                info.map_id,
                format_bytes_preview(&encoded_key),
            ));
            return Err(anyhow::anyhow!(
                "map value not found in snapshot for map_id={} key={}",
                info.map_id,
                format_bytes_preview(&encoded_key)
            ));
        }
...
fn resolve_frozen_map_value(
    ...
    let resolved = (|| -> anyhow::Result<Option<FrozenMapValue>> {
        ...
        let meta = match program.map_metadata.get(&map_id) {
            Some(m) => m,
            None => return Ok(None),
        };
        if !meta.frozen {
        ...
        let key = vec![0u8; meta.key_size as usize];
        let value = match program.map_values.get(&(map_id, key)) {
            Some(v) => v.clone(),
            None => return Ok(None),
        };
