...
impl Analysis for MapInfoAnalysis {
    ...
    fn run(&self, program: &BpfProgram) -> MapInfoAnalysisResult<MapInfoResult> {
        let metadata = program.map_metadata.clone();
        collect_map_references_with_bindings(
            &program.insns,
            &program.map_ids,
            &program.map_fd_bindings,
            move |map_id| resolve_from_metadata(&metadata, map_id),
        )
    }
}
...
fn resolve_from_metadata(
    metadata: &HashMap<u32, crate::pass::MapMetadata>,
    map_id: u32,
) -> MapInfoAnalysisResult<Option<MapInfo>> {
    match metadata.get(&map_id) {
        Some(m) => Ok(Some(MapInfo {
            map_type: m.map_type,
            key_size: m.key_size,
        ...
        None => Err(format!("resolve live map info for map {map_id}: no metadata provided")),
    }
}
