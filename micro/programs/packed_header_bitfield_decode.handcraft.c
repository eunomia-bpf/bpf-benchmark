 ...
 * - 0x112a: cmp    DWORD PTR [rsi+0xc],0x2 [warning-unmapped: CMP operand form has no current kinsn selector: cmp    DWORD PTR [rsi+0xc],0x2]
 ...
 * - 0x11df: or     r10,rbp [warning-unmapped: ALU source rbp is not supported]
 * - 0x11e5: or     r11,rbp [warning-unmapped: ALU source rbp is not supported]
 * - 0x11e8: lea    ebp,[r12+r13*1] [warning-unmapped: LEA operands are not representable]
 * - 0x120b: or     r10,rbp [warning-unmapped: ALU source rbp is not supported]
...
static const struct bpf_insn program[] = {
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_3, BPF_REG_2, 0, 0, 1, 0, 8, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_3, BPF_REG_2, 0, 0, 1, 0, 528, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_RAW(BPF_JMP | BPF_JNE | BPF_K, BPF_REG_6, 0, -15, 2),
    /* 0x1130: push   rbp [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1131: push   r15 [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1133: push   r14 [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1135: push   r13 [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_1, BPF_REG_2, 0, 0, 1, 0, 31, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    /* 0x1169: movzx  r9d,BYTE PTR [rdi-0xe] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R9, BPF_REG_1, -14, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x116e: mov    r13d,r9d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_7, HC_X86_R9, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x1171: shl    r13d,0x8 [exact-kinsn: shl32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_7, 8, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_0, 16, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x117c: movzx  r12d,BYTE PTR [rdi-0xc] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R12, BPF_REG_1, -12, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x1181: shl    r12d,0x18 [exact-kinsn: shl32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R12, 24, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x1185: or     r12d,eax [exact-kinsn: or32 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R12, BPF_REG_0, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORL),
    /* 0x1188: movzx  r10d,BYTE PTR [rdi-0xb] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R10, BPF_REG_1, -11, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x118d: shl    r10,0x20 [exact-kinsn: shl64 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R10, 32, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1191: movzx  ebp,BYTE PTR [rdi-0xa] [warning-unmapped: destination register ebp is not in the BPF JIT register file] */
    /* 0x1195: shl    rbp,0x28 [warning-unmapped: ALU destination rbp is not in the BPF JIT register file] */
    /* 0x1199: movzx  r11d,BYTE PTR [rdi-0x9] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R11, BPF_REG_1, -9, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x119e: shl    r11,0x30 [exact-kinsn: shl64 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R11, 48, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_0, 24, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_9, 32, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_2, 40, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x11ba: lea    rcx,[r15+rsi*1] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_4, BPF_REG_9, BPF_REG_2, 0, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x11be: or     r15,rax [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_9, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_0, 48, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_3, 56, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x11d0: or     rdx,rax [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_3, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_8, 56, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x11dc: or     r13d,ebx [exact-kinsn: or32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_7, BPF_REG_6, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORL),
    /* 0x11df: or     r10,rbp [warning-unmapped: ALU source rbp is not supported] */
    /* 0x11e2: or     r14,r11 [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, HC_X86_R11, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x11e5: or     r11,rbp [warning-unmapped: ALU source rbp is not supported] */
    /* 0x11e8: lea    ebp,[r12+r13*1] [warning-unmapped: LEA operands are not representable] */
    /* 0x11ec: or     rdx,rsi [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_3, BPF_REG_2, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_0, 63, 0), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x11f4: shr    r13d,0x6 [exact-kinsn: shr32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_7, 6, 0), MICRO_HANDCRAFT_BPF_X86_SHRL),
    /* 0x11f8: and    r13d,0x1f [exact-kinsn: and32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_7, 31, 0), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x11fc: shr    r14,0x36 [exact-kinsn: shr64 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_8, 54, 0), MICRO_HANDCRAFT_BPF_X86_SHRQ),
    /* 0x1200: add    rax,r14 [exact-kinsn: add64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_8, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x1203: add    rax,r13 [exact-kinsn: add64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_7, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    /* 0x120e: shr    ebp,0xb [warning-unmapped: ALU destination ebp is not in the BPF JIT register file] */
    /* 0x1211: and    ebp,0x7ff [warning-unmapped: ALU destination ebp is not in the BPF JIT register file] */
    /* 0x1217: shr    r12d,0x16 [exact-kinsn: shr32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R12, 22, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHRL),
    /* 0x121b: movzx  esi,r12b [exact-kinsn: movzx r32,r8 kinsn] */
    HC_KINSN(HC_REG_REG_PAYLOAD(BPF_REG_2, HC_X86_R12), MICRO_HANDCRAFT_BPF_X86_MOVZBL_RR),
    /* 0x121f: add    rax,rbp [warning-unmapped: ALU source rbp is not supported] */
    /* 0x1222: add    rax,rsi [exact-kinsn: add64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_2, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_2, 8, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x122c: shr    r10,0x1e [exact-kinsn: shr64 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R10, 30, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHRQ),
    /* 0x1230: and    r10d,0x3fff [exact-kinsn: and32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R10, 16383, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x1237: shr    r11,0x2c [exact-kinsn: shr64 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R11, 44, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHRQ),
    /* 0x123b: and    r11d,0x3ff [exact-kinsn: and32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R11, 1023, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x1242: add    r11,r10 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R11, HC_X86_R10, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x1245: add    r11,rax [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_2, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORL),
    /* 0x1251: or     esi,r13d [exact-kinsn: or32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, BPF_REG_7, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORL),
    /* 0x1254: mov    r12d,r13d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(HC_X86_R12, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x1257: and    r12d,0xf [exact-kinsn: and32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R12, 15, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x125b: shr    r15,0x18 [exact-kinsn: shr64 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_9, 24, 0), MICRO_HANDCRAFT_BPF_X86_SHRQ),
    ...
    /* 0x1263: shr    r9d,1 [exact-kinsn: shr32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 1, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHRL),
    /* 0x1266: and    r9d,0x1f [exact-kinsn: and32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 31, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x126a: shr    rcx,0x26 [exact-kinsn: shr64 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 38, 0), MICRO_HANDCRAFT_BPF_X86_SHRQ),
    /* 0x126e: and    ecx,0x7f [exact-kinsn: and32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 127, 0), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x1271: add    r12,r15 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R12, BPF_REG_9, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x1274: add    r12,rdx [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R12, BPF_REG_3, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x1277: shr    eax,0xc [exact-kinsn: shr32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_0, 12, 0), MICRO_HANDCRAFT_BPF_X86_SHRL),
    /* 0x127a: add    rax,rcx [exact-kinsn: add64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x127d: add    rax,r12 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, HC_X86_R12, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x1280: shr    esi,0x4 [exact-kinsn: shr32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_2, 4, 0), MICRO_HANDCRAFT_BPF_X86_SHRL),
    ...
    /* 0x1287: lea    rsi,[r9+r13*1] [exact-kinsn: LEA via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_2, HC_X86_R9, BPF_REG_7, 0, 1, 1, 0, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x128b: mov    r8,QWORD PTR [rsp-0x18] [warning-unmapped: memory base rsp is not in the BPF JIT register file] */
    /* 0x1290: add    rsi,r8 [exact-kinsn: add64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, BPF_REG_5, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    /* 0x1296: lea    rax,[rbp*2+0x0] [warning-unmapped: LEA base/index not in BPF register file: [rbp*2+0x0]] */
    /* 0x129e: add    rax,rbp [warning-unmapped: ALU source rbp is not supported] */
    /* 0x12a1: lea    r12,[r13*8+0x0] [exact-kinsn: LEA via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(HC_X86_R12, 0, BPF_REG_7, 3, 0, 1, 0, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12a9: sub    r12,r13 [exact-kinsn: sub64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_8] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R12, BPF_REG_7, BPF_REG_6, BPF_REG_8), MICRO_HANDCRAFT_BPF_X86_SUBQ),
    /* 0x12ac: lea    r13,[r10+r10*4] [exact-kinsn: LEA via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_7, HC_X86_R10, HC_X86_R10, 2, 1, 1, 0, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12b0: xor    r12,rax [exact-kinsn: xor64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R12, BPF_REG_0, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x12b3: xor    r12,r13 [exact-kinsn: xor64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_8] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R12, BPF_REG_7, BPF_REG_6, BPF_REG_8), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x12b6: lea    rax,[r14+r14*4] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_0, BPF_REG_8, BPF_REG_8, 2, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12ba: lea    rax,[r14+rax*2] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_0, BPF_REG_8, BPF_REG_0, 1, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12be: lea    r13,[r15+r15*2] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_7, BPF_REG_9, BPF_REG_9, 1, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12c2: lea    r15,[r15+r13*4] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_9, BPF_REG_9, BPF_REG_7, 2, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12c6: lea    r13,[rdx+rdx*8] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_7, BPF_REG_3, BPF_REG_3, 3, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12ca: xor    r13,r15 [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_7, BPF_REG_9, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x12cd: mov    r15d,r9d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_9, HC_X86_R9, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x12d0: shl    r15d,0x4 [exact-kinsn: shl32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_9, 4, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x12d4: add    r15,r9 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_9, HC_X86_R9, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x12d7: xor    r15,rax [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_9, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x12da: xor    r15,r13 [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_9, BPF_REG_7, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x12dd: lea    rax,[rcx+rcx*8] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_0, BPF_REG_4, BPF_REG_4, 3, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12e1: lea    rax,[rcx+rax*2] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_0, BPF_REG_4, BPF_REG_0, 1, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x12e5: xor    rax,r15 [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_9, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x12e8: and    bl,0x7 [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_IMM_TMP_PAYLOAD(BPF_REG_6, 7, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ANDB_IMM),
    /* 0x12eb: lea    ecx,[rbx+0x1] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_4, BPF_REG_6, 0, 0, 1, 0, 1, 0), MICRO_HANDCRAFT_BPF_X86_LEAL),
    ...
    /* 0x12fb: add    rax,r12 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, HC_X86_R12, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x12fe: or     rsi,r15 [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, BPF_REG_9, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1301: xor    rsi,r11 [exact-kinsn: xor64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, HC_X86_R11, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x1304: add    rsi,QWORD PTR [rsp-0x8] [warning-unmapped: ALU memory base rsp is not in the BPF JIT register file] */
    /* 0x1309: xor    rsi,rax [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x130c: lea    rbx,[rdx+r9*1] [exact-kinsn: LEA via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_6, BPF_REG_3, HC_X86_R9, 0, 1, 1, 0, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x1310: add    rbx,r10 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_8] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_6, HC_X86_R10, BPF_REG_7, BPF_REG_8), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_3, BPF_REG_8, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x1319: and    dl,0xf [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_4, BPF_REG_3, 0, 0, 1, 0, 1, 0), MICRO_HANDCRAFT_BPF_X86_LEAL),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_6, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_1, 16, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    HC_RAW(BPF_JMP | BPF_JNE | BPF_K, BPF_REG_5, 0, -233, 32),
    /* 0x1348: mov    rdx,QWORD PTR [rsp-0x10] [warning-unmapped: memory base rsp is not in the BPF JIT register file] */
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 24, 0), MICRO_HANDCRAFT_BPF_X86_SHRL),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 32, 0), MICRO_HANDCRAFT_BPF_X86_SHRQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 40, 0), MICRO_HANDCRAFT_BPF_X86_SHRQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 48, 0), MICRO_HANDCRAFT_BPF_X86_SHRQ),
