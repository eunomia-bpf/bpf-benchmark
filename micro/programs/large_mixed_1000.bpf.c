...
#define LARGE_MIXED_STEP(SALT)                                                  \
    do {                                                                        \
        ...
        tmp ^= micro_read_u64_le(                                               \
            data,                                                               \
            8U + ((((i) + (SALT)) & (LARGE_MIXED_1000_COUNT - 1U)) * 8U));     \
...
#define LARGE_MIXED_BLOCK_4(BASE)                                               \
    LARGE_MIXED_BLOCK_2(BASE);                                                  \
    LARGE_MIXED_BLOCK_2((BASE) + 2U)
...
#define LARGE_MIXED_BLOCK_16(BASE)                                              \
    LARGE_MIXED_BLOCK_8(BASE);                                                  \
    LARGE_MIXED_BLOCK_8((BASE) + 8U)

static __always_inline int bench_large_mixed_1000(const u8 *data, u32 len, u64 *out)
{
    ...
    if (count != LARGE_MIXED_1000_COUNT) {
        return -1;
    }
    if (!micro_has_bytes(len, 8U, LARGE_MIXED_1000_COUNT * 8U)) {
        return -1;
    }

    u64 acc = ((u64)seed << 31) ^ 0xE7037ED1A0B428DBULL;
    for (u32 i = 0; i < LARGE_MIXED_1000_COUNT; i++) {
        u64 value = micro_read_u64_le(data, 8U + i * 8U);
        u64 tmp = value ^ acc ^ ((u64)i << (i & 7U));
        LARGE_MIXED_BLOCK_16(0U);
        acc ^= tmp + value + ((u64)i << 13U);
    }
...
DEFINE_STAGED_INPUT_XDP_BENCH(
    large_mixed_1000_xdp,
    bench_large_mixed_1000,
    large_mixed_1000_input_value,
    LARGE_MIXED_1000_INPUT_SIZE)
