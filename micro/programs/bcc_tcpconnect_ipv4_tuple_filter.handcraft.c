...
static const struct bpf_insn program[] = {
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_2, BPF_REG_3, 0, 0, 1, 0, 784, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_RAW(BPF_JMP | BPF_JNE | BPF_K, BPF_REG_7, 0, -15, 24),
    /* 0x1130: push   rbp [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1131: push   r15 [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1133: push   r14 [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1135: push   rbx [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1136: lea    rsi,[rdx+0x27] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_2, BPF_REG_3, 0, 0, 1, 0, 39, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_1, BPF_REG_1, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_2, 24, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    /* 0x11a9: xor    r9d,r9d [exact-kinsn: xor32 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R9, HC_X86_R9, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_XORL),
    ...
    HC_KINSN(HC_SETCC_CMP_IMM_PAYLOAD(HC_X86_R9, BPF_REG_4, BPF_REG_7, BPF_REG_6, 30000, HC_FLAG_CMP_IMM32), MICRO_HANDCRAFT_BPF_X86_SETGE_R),
    /* 0x11b6: shl    r9d,0x2 [exact-kinsn: shl32 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 2, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_SHLL),
    ...
    /* 0x11e0: mov    r10d,DWORD PTR [rsi-0x13] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R10, BPF_REG_2, -19, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVL_MEM),
    /* 0x11e4: movzx  ebp,BYTE PTR [rsi-0x2] [warning-unmapped: destination register ebp is not in the BPF JIT register file] */
    /* 0x11e8: test   bpl,0x1 [warning-unmapped: only testq reg,same-reg is supported] */
    /* 0x11ec: je     1290 <bcc_tcpconnect_ipv4_tuple_filter_xdp+0x190> [warning-unmapped: standalone x86 branch needs an immediately preceding cmp] */
    /* 0x11f2: test   r9d,r9d [exact-kinsn: testl reg,reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP2_PAYLOAD(HC_X86_R9, HC_X86_R9, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_TESTL_RR),
    /* 0x11f5: je     1290 <bcc_tcpconnect_ipv4_tuple_filter_xdp+0x190> [warning-unmapped: standalone x86 branch needs an immediately preceding cmp] */
    ...
    /* 0x120e: movzx  r11d,WORD PTR [rsi-0x1] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R11, BPF_REG_2, -1, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVZWL_MEM),
    /* 0x1213: shl    r15d,0x18 [exact-kinsn: shl32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_9, 24, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_9, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORL),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_8, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x1224: xor    eax,ecx [exact-kinsn: xor32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x1226: mov    ecx,ebp [warning-unmapped: unsupported mnemonic or operand form: mov    ecx,ebp] */
    /* 0x1228: shl    ecx,0x1e [exact-kinsn: shl32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 30, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x122b: sar    ecx,0x1f [exact-kinsn: sar32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 31, 0), MICRO_HANDCRAFT_BPF_X86_SARL),
    /* 0x122e: lea    r14d,[r10+rbx*1] [exact-kinsn: LEA via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_8, HC_X86_R10, BPF_REG_6, 0, 1, 1, 0, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_LEAL),
    /* 0x1232: and    r14d,ecx [exact-kinsn: and32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x1235: xor    r14d,r11d [exact-kinsn: xor32 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, HC_X86_R11, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x1238: xor    r14d,eax [exact-kinsn: xor32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x123b: lea    ecx,[r9+0x3] [exact-kinsn: LEA via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_4, HC_X86_R9, 0, 0, 1, 0, 3, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_LEAL),
    /* 0x123f: mov    eax,r14d [exact-kinsn: movl register-to-register kinsn] */
    HC_KINSN(HC_REG_REG_PAYLOAD(BPF_REG_0, BPF_REG_8), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    ...
    HC_KINSN(HC_REG_REG_PAYLOAD(BPF_REG_6, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    ...
    HC_KINSN(HC_REG_REG_PAYLOAD(BPF_REG_0, BPF_REG_1), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x1257: and    al,0x7 [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_6, BPF_REG_8, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x126c: xor    rbx,r8 [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_6, BPF_REG_5, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x126f: shl    r9,0x38 [exact-kinsn: shl64 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 56, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1273: shl    r11d,0x10 [exact-kinsn: shl32 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R11, 16, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x1277: or     r11,r9 [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R11, HC_X86_R9, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x127a: mov    r8d,r10d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_5, HC_X86_R10, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x127d: xor    r8,r11 [exact-kinsn: xor64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_5, HC_X86_R11, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x1280: add    r8,rbx [exact-kinsn: add64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_5, BPF_REG_6, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    /* 0x1290: shl    r10,0x20 [exact-kinsn: shl64 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R10, 32, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1294: or     r10,rax [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R10, BPF_REG_0, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1297: add    r8,r10 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_5, HC_X86_R10, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ADDQ),
