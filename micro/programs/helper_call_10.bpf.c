...
#define HELPER_CALL_10_INPUT_SIZE 8U

struct helper_call_10_input_value {
    unsigned char data[HELPER_CALL_10_INPUT_SIZE];
};
...
struct {
    ...
    __type(key, __u32);
    __type(value, struct helper_call_10_input_value);
} input_map SEC(".maps");
...
static __always_inline int bench_helper_call_10(const u8 *data, u32 len, u64 *out)
{
    if (!micro_has_bytes(len, 0, HELPER_CALL_10_INPUT_SIZE)) {
        return -1;
    }

    u32 seed = micro_read_u32_le(data, 0);
    u32 mix = micro_read_u32_le(data, 4);
    u64 acc = ((u64)seed << 32) ^ mix ^ 0xE7037ED1A0B428DBULL;
    HELPER_CALL_BLOCK_10(1U);
    *out = acc;
    return 0;
}
...
DEFINE_STAGED_INPUT_XDP_BENCH(
    helper_call_10_xdp,
    bench_helper_call_10,
    helper_call_10_input_value,
    HELPER_CALL_10_INPUT_SIZE)
