...
static const struct bpf_insn program[] = {
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_3, BPF_REG_5, 0, 0, 1, 0, 1552, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_RAW(BPF_JMP | BPF_JNE | BPF_K, BPF_REG_7, 0, -13, 64),
    /* 0x112b: cmp    DWORD PTR [r8+0xc],0x18 [warning-unmapped: CMP operand form has no current kinsn selector: cmp    DWORD PTR [r8+0xc],0x18] */
    ...
    /* 0x1137: push   r12 [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1139: push   rbx [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x113a: lea    r9,[r8+0x27] [exact-kinsn: LEA via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(HC_X86_R9, BPF_REG_5, 0, 0, 1, 0, 39, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x113e: movabs rdi,0x243f6a8885a308d3 [bpf-jit: 64-bit immediate load] */
    HC_LD_IMM64_RAW(BPF_REG_1, 0, 0x243f6a8885a308d3ULL),
    /* 0x1148: xor    r10d,r10d [exact-kinsn: xor32 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    ...
    /* 0x1150: shl    r11,0x20 [exact-kinsn: shl64 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R11, 32, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    /* 0x1157: or     rax,r11 [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, HC_X86_R11, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    /* 0x1160: add    r9,0x18 [exact-kinsn: add64 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 24, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x1164: cmp    r10,0x40 [exact-kinsn: cmpq reg,imm32 kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R10, BPF_REG_7, 64), MICRO_HANDCRAFT_BPF_X86_CMPQ_IMM32),
    /* 0x1168: je     125c <cilium_socket_lb_service_select_xdp+0x15c> [warning-unmapped: cannot lower cmp    r10,0x40 + je     125c <cilium_socket_lb_service_select_xdp+0x15c> to BPF branch] */
    ...
    /* 0x1177: movzx  r11d,BYTE PTR [r9-0xb] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R11, HC_X86_R9, -11, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x117c: cmp    r11,0x11 [exact-kinsn: cmpq reg,imm32 kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R11, BPF_REG_7, 17), MICRO_HANDCRAFT_BPF_X86_CMPQ_IMM32),
    /* 0x1180: setne  cl [exact-kinsn: setcc kinsn using adjacent cmp reg,imm proof payload] */
    HC_KINSN(HC_SETCC_CMP_IMM_PAYLOAD(BPF_REG_4, HC_X86_R11, BPF_REG_7, BPF_REG_6, 17, HC_FLAG_CMP_IMM64), MICRO_HANDCRAFT_BPF_X86_SETNE_R),
    /* 0x1183: cmp    r11,0x6 [exact-kinsn: cmpq reg,imm32 kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R11, BPF_REG_7, 6), MICRO_HANDCRAFT_BPF_X86_CMPQ_IMM32),
    /* 0x1187: setne  dl [exact-kinsn: setcc kinsn using adjacent cmp reg,imm proof payload] */
    HC_KINSN(HC_SETCC_CMP_IMM_PAYLOAD(BPF_REG_3, HC_X86_R11, BPF_REG_7, BPF_REG_6, 6, HC_FLAG_CMP_IMM64), MICRO_HANDCRAFT_BPF_X86_SETNE_R),
    ...
    /* 0x119d: jne    1150 <cilium_socket_lb_service_select_xdp+0x50> [warning-unmapped: standalone x86 branch needs an immediately preceding cmp] */
    /* 0x119f: movzx  r12d,WORD PTR [r9-0x9] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R12, HC_X86_R9, -9, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVZWL_MEM),
    /* 0x11a4: test   r12w,r12w [warning-unmapped: only testq reg,same-reg is supported] */
    /* 0x11a8: je     1150 <cilium_socket_lb_service_select_xdp+0x50> [warning-unmapped: standalone x86 branch needs an immediately preceding cmp] */
    ...
    /* 0x11b8: xor    edx,DWORD PTR [r9-0x17] [exact-kinsn: xorl memory-source kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_ALU_MEM_PAYLOAD(BPF_REG_3, HC_X86_R9, -23, BPF_REG_7, 0), MICRO_HANDCRAFT_BPF_X86_XORL_MEM),
    /* 0x11bc: xor    edx,ebx [exact-kinsn: xor32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_3, BPF_REG_6, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x11be: xor    edx,DWORD PTR [r9-0x7] [exact-kinsn: xorl memory-source kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_ALU_MEM_PAYLOAD(BPF_REG_3, HC_X86_R9, -7, BPF_REG_7, 0), MICRO_HANDCRAFT_BPF_X86_XORL_MEM),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_2, 2, 0), MICRO_HANDCRAFT_BPF_X86_ANDL),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_0, 16, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_1, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_2, BPF_REG_8, BPF_REG_9, 0, 1, 1, 0, 0), MICRO_HANDCRAFT_BPF_X86_LEAL),
    /* 0x11f6: add    esi,edx [exact-kinsn: add32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, BPF_REG_3, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDL),
    /* 0x11f8: mov    ecx,r11d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_4, HC_X86_R11, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x11fb: and    ecx,0x7 [exact-kinsn: and32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 7, 0), MICRO_HANDCRAFT_BPF_X86_ANDL),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, BPF_REG_3, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_9, 29, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x1211: sar    r15d,0x1f [exact-kinsn: sar32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_9, 31, 0), MICRO_HANDCRAFT_BPF_X86_SARL),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_9, 0, 0), MICRO_HANDCRAFT_BPF_X86_ANDL),
    /* 0x121b: xor    r14d,edx [exact-kinsn: xor32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_3, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x121e: mov    eax,ebp [warning-unmapped: unsupported mnemonic or operand form: mov    eax,ebp] */
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_8, 48, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    /* 0x1235: mov    eax,r10d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_0, HC_X86_R10, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    ...
    /* 0x124b: add    rdi,r11 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_1, HC_X86_R11, BPF_REG_7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ADDQ),
