...
static const struct bpf_insn program[] = {
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_2, BPF_REG_3, 0, 0, 1, 0, 592, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1167: xor    r11,r14 [exact-kinsn: xor64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_8, 8, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x118c: or     r14,rcx [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x118f: cmp    r14w,0x3 [warning-unmapped: CMP operand form has no current kinsn selector: cmp    r14w,0x3] */
    ...
    /* 0x11a3: mov    r12d,DWORD PTR [rsi-0xb] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R12, BPF_REG_2, -11, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_MEM),
    /* 0x11a7: cmp    r12d,0x2001 [exact-kinsn: cmpl reg,imm32 kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R12, BPF_REG_6, 8193), MICRO_HANDCRAFT_BPF_X86_CMPL_IMM32),
    /* 0x11ae: jae    1160 <otel_stack_frame_unwind_scan_xdp+0x60> [warning-unmapped: cannot lower cmp    r12d,0x2001 + jae    1160 <otel_stack_frame_unwind_scan_xdp+0x60> to BPF branch] */
    /* 0x11b0: movzx  ebp,WORD PTR [rsi-0x7] [warning-unmapped: destination register ebp is not in the BPF JIT register file] */
    /* 0x11b4: mov    r9d,DWORD PTR [rsi-0x3] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R9, BPF_REG_2, -3, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_MEM),
    ...
    /* 0x11bc: test   bpl,0x1 [warning-unmapped: only testq reg,same-reg is supported] */
    /* 0x11c0: mov    r13d,r12d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_7, HC_X86_R12, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x11c3: mov    r10d,0x8 [warning-unmapped: movl immediate into HC_X86_R10 needs an immediate-load kinsn] */
    /* 0x11c9: cmove  r13d,r10d [warning-unmapped: cmove needs an adjacent test/cmp proof payload] */
    /* 0x11cd: mov    rbx,r9 [exact-kinsn: movq register-to-register kinsn; verifier instantiate uses temp BPF_REG_7] */
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_7, BPF_REG_9, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDL),
    /* 0x11e2: add    rdi,r13 [exact-kinsn: add64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_1, BPF_REG_7, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x11e5: xor    rax,rdi [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_1, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x11e8: xor    rax,rbx [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_6, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x11eb: test   bpl,0x2 [warning-unmapped: only testq reg,same-reg is supported] */
    /* 0x11ef: je     11f6 <otel_stack_frame_unwind_scan_xdp+0xf6> [warning-unmapped: standalone x86 branch needs an immediately preceding cmp] */
    ...
    /* 0x11f6: mov    ecx,r12d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_4, HC_X86_R12, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x11f9: shl    rcx,0x18 [exact-kinsn: shl64 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 24, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_6, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x1203: test   bpl,0x4 [warning-unmapped: only testq reg,same-reg is supported] */
    ...
    /* 0x1217: and    al,0x7 [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_6, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x122c: add    r14,r11 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    ...
    /* 0x1236: or     r9,rax [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R9, BPF_REG_0, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1239: mov    r11,r9 [exact-kinsn: movq register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    ...
    /* 0x1241: mov    QWORD PTR [rdx],r11 [exact-kinsn: direct memory store via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_STORE_REG_TMP_PAYLOAD(HC_X86_R11, BPF_REG_3, 0, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVQ_MEM_REG),
