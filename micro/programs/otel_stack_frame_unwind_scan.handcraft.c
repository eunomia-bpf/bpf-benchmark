...
static const struct bpf_insn program[] = {
    ...
    HC_RAW(BPF_JMP | BPF_JEQ | BPF_K, BPF_REG_5, 0, 82, 24),
    /* 0x117b: mov    rax,QWORD PTR [rsi-0x17] [exact-kinsn: direct memory load via x86 kinsn selector] */
    HC_KINSN(HC_MEM_PAYLOAD(BPF_REG_0, BPF_REG_2, -23), MICRO_HANDCRAFT_BPF_X86_MOVQ),
    /* 0x117f: movzx  ecx,BYTE PTR [rsi-0x5] [exact-kinsn: direct memory load via x86 kinsn selector] */
    HC_KINSN(HC_MEM_PAYLOAD(BPF_REG_4, BPF_REG_2, -5), MICRO_HANDCRAFT_BPF_X86_MOVZBL),
    /* 0x1183: movzx  r14d,BYTE PTR [rsi-0x4] [exact-kinsn: direct memory load via x86 kinsn selector] */
    HC_KINSN(HC_MEM_PAYLOAD(BPF_REG_8, BPF_REG_2, -4), MICRO_HANDCRAFT_BPF_X86_MOVZBL),
    ...
    HC_RAW(BPF_JMP | BPF_JGT | BPF_K, BPF_REG_8, 0, -24, 3),
    /* 0x1196: mov    r15d,DWORD PTR [rsi-0xf] [exact-kinsn: direct memory load via x86 kinsn selector] */
    HC_KINSN(HC_MEM_PAYLOAD(BPF_REG_9, BPF_REG_2, -15), MICRO_HANDCRAFT_BPF_X86_MOVL),
    ...
    HC_RAW(BPF_JMP | BPF_JGT | BPF_K, BPF_REG_9, 0, -29, 4096),
    /* 0x11a3: mov    r12d,DWORD PTR [rsi-0xb] [exact-kinsn: direct memory load via x86 kinsn selector] */
    HC_KINSN(HC_MEM_PAYLOAD(HC_X86_R12, BPF_REG_2, -11), MICRO_HANDCRAFT_BPF_X86_MOVL),
    ...
    HC_KINSN(HC_MEM_PAYLOAD(HC_X86_R9, BPF_REG_2, -3), MICRO_HANDCRAFT_BPF_X86_MOVL),
    ...
    HC_RAW(BPF_JMP | BPF_JA, 0, 0, -91, 0),
