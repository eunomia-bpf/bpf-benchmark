 ...
 * - 0x1127: jne    110e <cilium_policy_guard_tree_filter_xdp+0xe> [warning-unmapped: jcc has no immediately preceding branchable cmp/test; needs a machine-level branch kinsn]
 ...
 * - 0x112d: push   rbx [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR]
 * - 0x1196: je     11f9 <cilium_policy_guard_tree_filter_xdp+0xf9> [warning-unmapped: jcc has no immediately preceding branchable cmp/test; needs a machine-level branch kinsn]
 ...
 * - 0x11b1: jb     11f5 <cilium_policy_guard_tree_filter_xdp+0xf5> [warning-unmapped: jcc has no immediately preceding branchable cmp/test; needs a machine-level branch kinsn]
 * - 0x11b8: add    cl,r9b [warning-unmapped: add    cl,r9b needs a shadow-aware ALU kinsn]
 * - 0x11be: ja     1204 <cilium_policy_guard_tree_filter_xdp+0x104> [warning-unmapped: jcc has no immediately preceding branchable cmp/test; needs a machine-level branch kinsn]
 * - 0x11cf: jne    1210 <cilium_policy_guard_tree_filter_xdp+0x110> [warning-unmapped: jcc has no immediately preceding branchable cmp/test; needs a machine-level branch kinsn]
 * - 0x1228: je     1141 <cilium_policy_guard_tree_filter_xdp+0x41> [warning-unmapped: jcc has no immediately preceding branchable cmp/test; needs a machine-level branch kinsn]
 * - 0x1274: pop    rbx [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR]
 * - 0x1275: pop    rbp [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR]
 */
...
static const struct bpf_insn program[] = {
    ...
    /* 0x110c: jbe    110f <cilium_policy_guard_tree_filter_xdp+0xf> [bpf-branch: verifier-visible branch from preceding cmp    rdx,rcx; cmp kinsn is preserved] */
    ...
    /* 0x1116: ja     110e <cilium_policy_guard_tree_filter_xdp+0xe> [bpf-branch: verifier-visible branch from preceding cmp    rsi,rcx; cmp kinsn is preserved] */
    ...
    /* 0x1122: ja     110e <cilium_policy_guard_tree_filter_xdp+0xe> [bpf-branch: verifier-visible branch from preceding cmp    rdi,rcx; cmp kinsn is preserved] */
    ...
    /* 0x112a: mov    rbp,rsp [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x112d: push   rbx [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x112e: lea    rdi,[rdx+0x1b] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_PAYLOAD(BPF_REG_1, BPF_REG_3, 0, 0, 1, 0, 27), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    /* 0x1132: movabs rsi,0xe7037ed1a0b428db [bpf-jit: 64-bit immediate load] */
    HC_LD_IMM64_RAW(BPF_REG_2, 0, 0xe7037ed1a0b428dbULL),
    /* 0x113c: xor    r8d,r8d [exact-kinsn: xor32 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_5, BPF_REG_5), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x113f: jmp    117f <cilium_policy_guard_tree_filter_xdp+0x7f> [bpf-branch: lowered direct jmp to verifier-visible BPF jump] */
    HC_RAW(BPF_JMP | BPF_JA, 0, 0, 31, 0),
    /* 0x1141: shl    rcx,0x30 [exact-kinsn: shl64 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 48), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1145: data16 cs nop WORD PTR [rax+rax*1+0x0] [padding: padding is not part of BPF semantics] */
    ...
    /* 0x1179: je     1234 <cilium_policy_guard_tree_filter_xdp+0x134> [bpf-branch: verifier-visible branch from preceding cmp    r8,0x20; cmp kinsn is preserved] */
    HC_RAW(BPF_JMP | BPF_JEQ | BPF_K, BPF_REG_5, 0, 78, 32),
    ...
    /* 0x118b: jb     1150 <cilium_policy_guard_tree_filter_xdp+0x50> [bpf-branch: verifier-visible branch from preceding cmp    rcx,0x21; cmp kinsn is preserved] */
    HC_RAW(BPF_JMP | BPF_JLT | BPF_K, BPF_REG_4, 0, -36, 33),
    ...
    /* 0x11dd: jne    1219 <cilium_policy_guard_tree_filter_xdp+0x119> [bpf-branch: verifier-visible branch from preceding cmp    r9d,0xaa; cmp kinsn is preserved] */
    HC_RAW(BPF_JMP32 | BPF_JNE | BPF_K, HC_X86_R9, 0, 27, 170),
    ...
    HC_RAW(BPF_JMP | BPF_JA, 0, 0, -89, 0),
    ...
    /* 0x1274: pop    rbx [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1275: pop    rbp [warning-unmapped: native stack-frame instruction belongs to ABI/prologue, not BPF verifier IR] */
    /* 0x1276: ret [bpf-jit: BPF exit; kernel JIT emits the real return sequence] */
    HC_EXIT(),
};
