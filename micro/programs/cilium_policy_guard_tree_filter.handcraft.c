...
static const struct bpf_insn program[] = {
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_1, BPF_REG_3, 0, 0, 1, 0, 524, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_1, BPF_REG_3, 0, 0, 1, 0, 27, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    ...
    /* 0x1159: and    al,0x7 [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_IMM_TMP_PAYLOAD(BPF_REG_0, 7, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ANDB_IMM),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, BPF_REG_6, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    /* 0x118d: movzx  r9d,BYTE PTR [rdi-0xe] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R9, BPF_REG_1, -14, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x1192: test   r9b,0x1 [exact-kinsn: testb imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R9, BPF_REG_7, 1), MICRO_HANDCRAFT_BPF_X86_TESTB_IMM),
    /* 0x1196: je     11f9 <cilium_policy_guard_tree_filter_xdp+0xf9> [warning-unmapped: standalone x86 branch needs an immediately preceding cmp] */
    /* 0x1198: movzx  r9d,BYTE PTR [rdi-0xd] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R9, BPF_REG_1, -13, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x119d: test   r9b,r9b [exact-kinsn: testb reg,reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_8] */
    HC_KINSN(HC_REG_REG_TMP2_PAYLOAD(HC_X86_R9, HC_X86_R9, BPF_REG_7, BPF_REG_8), MICRO_HANDCRAFT_BPF_X86_TESTB_RR),
    /* 0x11a0: js     11ef <cilium_policy_guard_tree_filter_xdp+0xef> [warning-unmapped: unsupported mnemonic or operand form: js     11ef <cilium_policy_guard_tree_filter_xdp+0xef>] */
    /* 0x11a2: movzx  r9d,BYTE PTR [rdi-0xc] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R9, BPF_REG_1, -12, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x11a7: mov    r10d,r9d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(HC_X86_R10, HC_X86_R9, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x11aa: xor    r10d,r8d [exact-kinsn: xor32 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_8] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R10, BPF_REG_5, BPF_REG_7, BPF_REG_8), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x11ad: cmp    r10b,0x11 [warning-unmapped: CMP operand form has no current kinsn selector: cmp    r10b,0x11] */
    /* 0x11b1: jb     11f5 <cilium_policy_guard_tree_filter_xdp+0xf5> [warning-unmapped: cannot lower cmp    r10b,0x11 + jb     11f5 <cilium_policy_guard_tree_filter_xdp+0xf5> to BPF branch] */
    /* 0x11b3: movzx  r9d,BYTE PTR [rdi-0xb] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R9, BPF_REG_1, -11, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x11b8: add    cl,r9b [warning-unmapped: add    cl,r9b needs a shadow-aware ALU kinsn] */
    /* 0x11bb: cmp    cl,0xbf [warning-unmapped: CMP operand form has no current kinsn selector: cmp    cl,0xbf] */
    ...
    /* 0x11c4: mov    r9d,ecx [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(HC_X86_R9, BPF_REG_4, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x11c7: and    r9b,0x18 [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_IMM_TMP_PAYLOAD(HC_X86_R9, 24, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ANDB_IMM),
    /* 0x11cb: cmp    r9b,0x10 [warning-unmapped: CMP operand form has no current kinsn selector: cmp    r9b,0x10] */
    /* 0x11cf: jne    1210 <cilium_policy_guard_tree_filter_xdp+0x110> [warning-unmapped: cannot lower cmp    r9b,0x10 + jne    1210 <cilium_policy_guard_tree_filter_xdp+0x110> to BPF branch] */
    /* 0x11d1: movzx  r9d,BYTE PTR [rdi-0x9] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R9, BPF_REG_1, -9, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x11d6: cmp    r9d,0xaa [exact-kinsn: cmpl reg,imm32 kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R9, BPF_REG_7, 170), MICRO_HANDCRAFT_BPF_X86_CMPL_IMM32),
    /* 0x11dd: jne    1219 <cilium_policy_guard_tree_filter_xdp+0x119> [warning-unmapped: cannot lower cmp    r9d,0xaa + jne    1219 <cilium_policy_guard_tree_filter_xdp+0x119> to BPF branch] */
    /* 0x11df: xor    rcx,rax [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_4, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    /* 0x11ef: shl    r9d,0x8 [exact-kinsn: shl32 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    ...
    /* 0x11f5: shl    r9d,0x10 [exact-kinsn: shl32 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 16, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x11f9: add    rax,r9 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_8] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, HC_X86_R9, BPF_REG_7, BPF_REG_8), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    /* 0x1204: shl    r9d,0x18 [exact-kinsn: shl32 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 24, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x1208: xor    rax,r9 [exact-kinsn: xor64 reg kinsn; verifier instantiate uses temps BPF_REG_7/BPF_REG_8] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, HC_X86_R9, BPF_REG_7, BPF_REG_8), MICRO_HANDCRAFT_BPF_X86_XORQ),
    ...
    /* 0x121d: mov    r10d,ecx [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(HC_X86_R10, BPF_REG_4, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x1220: and    r10b,0x3 [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_REG_IMM_TMP_PAYLOAD(HC_X86_R10, 3, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ANDB_IMM),
    /* 0x1224: cmp    r10b,0x1 [warning-unmapped: CMP operand form has no current kinsn selector: cmp    r10b,0x1] */
    /* 0x1228: je     113d <cilium_policy_guard_tree_filter_xdp+0x3d> [warning-unmapped: cannot lower cmp    r10b,0x1 + je     113d <cilium_policy_guard_tree_filter_xdp+0x3d> to BPF branch] */
    /* 0x122e: shl    r9,0x28 [exact-kinsn: shl64 imm kinsn; verifier instantiate uses temp BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 40, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_SHLQ),
