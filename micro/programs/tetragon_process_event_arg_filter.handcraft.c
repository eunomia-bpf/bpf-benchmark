 ...
 * - 0x13c2: cmovne rdx,r13 [warning-unmapped: cmovne needs an adjacent test/cmp proof payload]
...
static const struct bpf_insn program[] = {
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_3, BPF_REG_2, 0, 0, 1, 0, 1040, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_1, BPF_REG_2, 0, 0, 1, 0, 47, 0), MICRO_HANDCRAFT_BPF_X86_LEAQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 32, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1164: mov    eax,r12d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_0, HC_X86_R12, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x1167: or     rax,rcx [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x116a: xor    rbx,rax [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_6, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    ...
    HC_RAW(BPF_JMP | BPF_JEQ | BPF_K, BPF_REG_5, 0, 232, 32),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 8, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x1189: or     rcx,rax [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_4, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x118c: lea    edx,[rcx-0x1] [exact-kinsn: LEA via x86 kinsn selector] */
    HC_KINSN(HC_LEA_TMP_PAYLOAD(BPF_REG_3, BPF_REG_4, 0, 0, 1, 0, -1, 0), MICRO_HANDCRAFT_BPF_X86_LEAL),
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_8, BPF_REG_8, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    ...
    /* 0x11f4: movzx  r9d,BYTE PTR [rdi-0x5] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R9, BPF_REG_1, -5, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x11f9: shl    r9d,0x10 [exact-kinsn: shl32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 16, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x11fd: or     r9,rsi [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R9, BPF_REG_2, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_2, 24, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x1207: or     rsi,r9 [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_2, HC_X86_R9, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x120a: movzx  r11d,BYTE PTR [rdi-0x3] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R11, BPF_REG_1, -3, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x120f: shl    r11,0x20 [exact-kinsn: shl64 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R11, 32, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1213: movzx  r10d,BYTE PTR [rdi-0x2] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R10, BPF_REG_1, -2, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVZBL_MEM),
    /* 0x1218: shl    r10,0x28 [exact-kinsn: shl64 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R10, 40, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x121c: or     r10,r11 [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R10, HC_X86_R11, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x121f: or     r10,rsi [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R10, BPF_REG_2, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    /* 0x1255: movabs r11,0x6e6f68747970 [warning-unmapped: movabs into HC_X86_R11 needs an immediate-load kinsn] */
    /* 0x125f: cmp    r10,r11 [exact-kinsn: cmpq reg,reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_REG_REG_TMP2_PAYLOAD(HC_X86_R10, HC_X86_R11, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_CMPQ_RR),
    /* 0x1262: je     1280 <tetragon_process_event_arg_filter_xdp+0x180> [warning-unmapped: cannot lower cmp    r10,r11 + je     1280 <tetragon_process_event_arg_filter_xdp+0x180> to BPF branch] */
    ...
    /* 0x1266: cmp    r9d,0x6873 [exact-kinsn: cmpl reg,imm32 kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R9, BPF_REG_6, 26739), MICRO_HANDCRAFT_BPF_X86_CMPL_IMM32),
    /* 0x126d: sete   sil [exact-kinsn: setcc kinsn using adjacent cmp reg,imm proof payload] */
    HC_KINSN(HC_SETCC_CMP_IMM_PAYLOAD(BPF_REG_2, HC_X86_R9, BPF_REG_6, BPF_REG_7, 26739, HC_FLAG_CMP_IMM32), MICRO_HANDCRAFT_BPF_X86_SETE_R),
    /* 0x1271: shl    esi,0x2 [exact-kinsn: shl32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_2, 2, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x1274: data16 data16 cs nop WORD PTR [rax+rax*1+0x0] [padding: padding is not part of BPF semantics] */
    /* 0x1280: mov    r12d,DWORD PTR [rdi-0x1f] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R12, BPF_REG_1, -31, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_MEM),
    ...
    /* 0x1299: mov    QWORD PTR [rsp-0x20],rcx [warning-unmapped: store base rsp is not in the BPF JIT register file] */
    /* 0x129e: movzx  r9d,BYTE PTR [rdi-0xe] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    ...
    /* 0x12db: movzx  r11d,WORD PTR [rdi-0x11] [exact-kinsn: direct memory load via x86 kinsn selector; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_MEM_TMP_PAYLOAD(HC_X86_R11, BPF_REG_1, -17, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVZWL_MEM),
    ...
    HC_RAW(BPF_JMP | BPF_JEQ | BPF_K, BPF_REG_3, 0, 25, 66),
    /* 0x12e9: mov    ecx,r9d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(BPF_REG_4, HC_X86_R9, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x12ec: and    ecx,0x24 [exact-kinsn: and32 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 36, 0), MICRO_HANDCRAFT_BPF_X86_ANDL),
    ...
    HC_RAW(BPF_JMP | BPF_JNE | BPF_K, BPF_REG_4, 0, 20, 0),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 16, 0), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x12ff: or     rax,rdx [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_3, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1302: or     rax,rcx [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_0, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1305: shl    r13,0x10 [exact-kinsn: shl64 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_7, 16, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1309: xor    r13,rax [exact-kinsn: xor64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_7, BPF_REG_0, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x130c: mov    rbx,QWORD PTR [rsp-0x38] [warning-unmapped: memory base rsp is not in the BPF JIT register file] */
    /* 0x1311: add    rbx,r13 [exact-kinsn: add64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_6, BPF_REG_7, 0, 0), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    /* 0x1314: jmp    116d <tetragon_process_event_arg_filter_xdp+0x6d> [bpf-branch: lowered direct jmp to verifier-visible BPF jump] */
    HC_RAW(BPF_JMP | BPF_JA, 0, 0, -151, 0),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 40, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1327: or     rcx,r14 [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_4, BPF_REG_8, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x132a: mov    rdx,r11 [exact-kinsn: movq register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_3, 24, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1331: or     rdx,rcx [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_3, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1334: mov    r12d,r12d [exact-kinsn: movl register-to-register kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_REG_TMP_PAYLOAD(HC_X86_R12, HC_X86_R12, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_MOVL_RR),
    /* 0x1337: or     r12,rdx [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R12, BPF_REG_3, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORQ),
    ...
    /* 0x133e: test   r11b,0x2 [exact-kinsn: testb imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R11, BPF_REG_6, 2), MICRO_HANDCRAFT_BPF_X86_TESTB_IMM),
    /* 0x1342: je     1373 <tetragon_process_event_arg_filter_xdp+0x273> [warning-unmapped: standalone x86 branch needs an immediately preceding cmp] */
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_4, 48, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1351: shl    r15,0x38 [exact-kinsn: shl64 imm kinsn] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_9, 56, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    /* 0x1355: or     r15,rcx [exact-kinsn: or64 reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_9, BPF_REG_4, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1358: or     r15,r10 [exact-kinsn: or64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_9, HC_X86_R10, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x135b: and    al,0x7 [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    ...
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_9, BPF_REG_3, 0, 0), MICRO_HANDCRAFT_BPF_X86_ORQ),
    /* 0x1370: xor    r12,r15 [exact-kinsn: xor64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(HC_X86_R12, BPF_REG_9, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_XORQ),
    /* 0x1373: shl    r9d,0x8 [exact-kinsn: shl32 imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(HC_X86_R9, 8, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_SHLL),
    /* 0x1377: add    r9,QWORD PTR [rsp-0x20] [warning-unmapped: ALU memory base rsp is not in the BPF JIT register file] */
    /* 0x137c: mov    eax,DWORD PTR [rsp-0x3c] [warning-unmapped: memory base rsp is not in the BPF JIT register file] */
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_0, 32, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    HC_KINSN(HC_X86_ALU_IMM_PAYLOAD(BPF_REG_0, 56, 0), MICRO_HANDCRAFT_BPF_X86_SHLQ),
    ...
    /* 0x13b3: or     rbp,rax [warning-unmapped: ALU destination rbp is not in the BPF JIT register file] */
    /* 0x13b6: xor    r13,rbp [warning-unmapped: ALU source rbp is not supported] */
    /* 0x13b9: test   r11b,0x4 [exact-kinsn: testb imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_TMP_IMM_PAYLOAD(HC_X86_R11, BPF_REG_6, 4), MICRO_HANDCRAFT_BPF_X86_TESTB_IMM),
    /* 0x13bd: mov    edx,0x0 [exact-kinsn: movl zero via xorl reg,reg kinsn] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_3, BPF_REG_3, 0, 0), MICRO_HANDCRAFT_BPF_X86_XORL),
    /* 0x13c2: cmovne rdx,r13 [warning-unmapped: cmovne needs an adjacent test/cmp proof payload] */
    /* 0x13c6: add    rdx,r12 [exact-kinsn: add64 reg kinsn; verifier instantiate uses temps BPF_REG_6/BPF_REG_7] */
    HC_KINSN(HC_X86_ALU_RR_PAYLOAD(BPF_REG_3, HC_X86_R12, BPF_REG_6, BPF_REG_7), MICRO_HANDCRAFT_BPF_X86_ADDQ),
    ...
    /* 0x13cc: and    al,0x7 [exact-kinsn: andb imm kinsn; verifier instantiate uses temp BPF_REG_6] */
    HC_KINSN(HC_REG_IMM_TMP_PAYLOAD(BPF_REG_0, 7, BPF_REG_6), MICRO_HANDCRAFT_BPF_X86_ANDB_IMM),
