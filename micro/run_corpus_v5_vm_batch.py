...
def normalize_scan(scan: dict[str, Any] | None) -> dict[str, int]:
    ...
    for _, field in FAMILY_FIELDS:
        base[field] = int(scan.get(field, 0) or 0)
    base["total_sites"] = int(scan.get("total_sites", sum(base[field] for _, field in FAMILY_FIELDS)) or 0)
    return base
...
def load_targets(
    ...
    for record in payload["programs"]:
        if record.get("strategy") != "packet_test_run":
            continue
        inventory_scan = normalize_scan(record.get("directive_scan"))
    ...
    return selected, inventory_summary
