...
from directive_census import collect_inputs
...
def list_programs(runner: Path, object_path: Path, timeout_seconds: int) -> dict[str, Any]:
    ...
    lines = [line.strip() for line in stdout.splitlines() if line.strip()]
    try:
        inventory = json.loads(lines[-1]) if lines else []
    except json.JSONDecodeError as exc:
        result["ok"] = False
        result["error"] = f"unable to parse list-programs output: {exc}"
        result["inventory"] = []
        return result
...
def main() -> int:
    ...
    inputs = collect_inputs(ROOT_DIR, corpus_build_report)
