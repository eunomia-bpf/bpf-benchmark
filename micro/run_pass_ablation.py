...
def run_command(command: list[str], cpu: str | None) -> dict[str, Any]:
    full_command = command if cpu is None else ["taskset", "-c", cpu, *command]
...
def ensure_artifacts_built(suite: SuiteSpec) -> None:
    for step in ("micro_exec", "programs"):
...
def build_command(
    ...
    if disabled_pass is not None:
        command.extend(["--llvm-disable-pass", disabled_pass])
    if log_passes:
        command.append("--llvm-log-passes")
    return command
