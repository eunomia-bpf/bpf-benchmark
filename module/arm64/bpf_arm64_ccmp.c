...
/*
 * BpfReJIT arm64 koperation: CMP/CCMP/CSET.
 */
...
#define KOP_CCMP_MAX_TERMS	4
#define KOP_CCMP_MODE_FAIL_EQ	0
#define KOP_CCMP_MODE_FAIL_NE	1
...
{
	*reg = kop_payload_reg(payload, 0);
	*mode = kop_payload_reg(payload, 4);

	if (payload >> 8)
		return -EINVAL;
	if (*reg > BPF_REG_10)
		return -EINVAL;
	if (*mode != KOP_CCMP_MODE_FAIL_EQ &&
	    *mode != KOP_CCMP_MODE_FAIL_NE)
		return -EINVAL;
					       ...
					       struct ccmp_payload *out)
{
	...
	out->dst_reg = kop_payload_reg(payload, 0);
	count_bits = (payload >> 4) & 0x3;
	if (count_bits > KOP_CCMP_MAX_TERMS - 2)
		return -EINVAL;
	...
	if (out->dst_reg > BPF_REG_9)
		return -EINVAL;
	if (out->mode != KOP_CCMP_MODE_FAIL_EQ &&
	    out->mode != KOP_CCMP_MODE_FAIL_NE)
		return -EINVAL;

	for (i = 0; i < KOP_CCMP_MAX_TERMS; i++) {
		out->regs[i] = kop_payload_reg(payload, 8 + 4 * i);
		if (i >= out->count) {
			if (out->regs[i])
				return -EINVAL;
			continue;
...
static int instantiate_cset(u64 payload, struct bpf_insn *insn_buf)
{
	...
	op = decoded.mode == KOP_CCMP_MODE_FAIL_EQ ? BPF_JEQ : BPF_JNE;
			   ...
			   const struct bpf_prog *prog, const u32 *final_ip)
{
	...
	dst_reg = kop_arm64_reg(decoded.dst_reg);
	if (dst_reg == 0xff)
		return -EINVAL;
...
const struct bpf_kop bpf_arm64_cmp_x_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = 1,
	.max_emit_bytes = 4,
...
const struct bpf_kop bpf_arm64_cmp_w_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = 1,
	.max_emit_bytes = 4,
	.instantiate_insn = instantiate_cmp,
...
const struct bpf_kop bpf_arm64_ccmp_x_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = 1,
	.max_emit_bytes = 4,
...
const struct bpf_kop bpf_arm64_ccmp_w_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = 1,
	.max_emit_bytes = 4,
	.instantiate_insn = instantiate_ccmp,
...
static const struct bpf_kop * const bpf_arm64_ccmp_kop_descs[] = {
...
DEFINE_KOP_V2_MODULE(bpf_arm64_ccmp, "BpfReJIT arm64 koperation: CMP/CCMP/CSET",
		       bpf_arm64_ccmp_kfunc_ids, bpf_arm64_ccmp_kop_descs);
