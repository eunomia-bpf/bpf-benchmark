...
/*
 * BpfReJIT x86 koperation: fused CMP + CMOV.
 *
 * Standalone CMP/TEST/SETcc/CMOV koperation need a cross-kop flags channel.
						      ...
						      bool *value32)
{
	...
	*left_reg = kop_payload_reg(payload, 4);
	*right_reg = kop_payload_reg(payload, 8);
	*dst_reg = kop_payload_reg(payload, 12);
	*src_reg = kop_payload_reg(payload, 16);
	*cmp32 = payload & (1ULL << 20);
	*value32 = payload & (1ULL << 21);

	if (!kop_x86_operand_valid(*left_reg) ||
	    !kop_x86_operand_valid(*right_reg) ||
	    !kop_bpf_gpr_valid(*dst_reg) ||
				...
				const struct bpf_prog *prog, u8 cc)
{
	...
	left_reg = kop_x86_reg_for_prog(prog, left_reg);
	right_reg = kop_x86_reg_for_prog(prog, right_reg);
	src_reg = kop_x86_reg_for_prog(prog, src_reg);
	if (!kop_x86_valid(left_reg) || !kop_x86_valid(right_reg) ||
	    !kop_x86_valid(dst_reg) || !kop_x86_valid(src_reg))
		return -EINVAL;

	kop_emit_rex_rr(buf, &len, !cmp32, right_reg, left_reg);
	kop_emit_u8(buf, &len, 0x39);
	...
	kop_emit_u8(buf, &len, 0xC0 |
		(kop_x86_reg_code(dst_reg) << 3) |
		kop_x86_reg_code(src_reg));
...
const struct bpf_kop bpf_x86_cmp_cmove_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = 2,
	.max_emit_bytes = 16,
...
const struct bpf_kop bpf_x86_cmp_cmovne_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = 2,
	.max_emit_bytes = 16,
...
const struct bpf_kop bpf_x86_cmp_cmovb_desc = {
	.owner = THIS_MODULE,
	.max_insn_cnt = 2,
	.max_emit_bytes = 16,
...
static const struct bpf_kop * const bpf_x86_cmov_kop_descs[] = {
	&bpf_x86_cmp_cmovb_desc,
	&bpf_x86_cmp_cmove_desc,
	&bpf_x86_cmp_cmovne_desc,
};

DEFINE_KOP_V2_MODULE(bpf_x86_cmov, "BpfReJIT x86 koperation: fused CMP/CMOV",
		       bpf_x86_cmov_kfunc_ids, bpf_x86_cmov_kop_descs);
