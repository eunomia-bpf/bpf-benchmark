						 ...
						 u8 *right_tmp_reg)
{
	...
	*right_tmp_reg = kinsn_payload_reg(payload, 12);
	...
	if (payload >> 16)
		return -EINVAL;
	if (kinsn_x86_reg_is_shadowed(*left_reg) &&
	    !bpf_gpr_ok(*left_tmp_reg))
		return -EINVAL;
	if (kinsn_x86_reg_is_shadowed(*right_reg) &&
	    !bpf_gpr_ok(*right_tmp_reg))
		return -EINVAL;
	if (kinsn_x86_reg_is_shadowed(*left_reg) &&
	    kinsn_x86_reg_is_shadowed(*right_reg) &&
	...
	if (kinsn_x86_reg_is_shadowed(*left_reg) &&
	    !kinsn_x86_reg_is_shadowed(*right_reg) &&
	    *left_tmp_reg == *right_reg)
		return -EINVAL;
	if (kinsn_x86_reg_is_shadowed(*right_reg) &&
	    !kinsn_x86_reg_is_shadowed(*left_reg) &&
	    *right_tmp_reg == *left_reg)
		return -EINVAL;
						  ...
						  u8 *tmp_reg, s32 *imm)
{
	...
	*imm = kinsn_payload_s32(payload, 8);
	...
	if (kinsn_x86_reg_is_shadowed(*reg)) {
		if (!bpf_gpr_ok(*tmp_reg))
			return -EINVAL;
	} else if (payload & (0xfULL << 4)) {
		if (!bpf_gpr_ok(*tmp_reg) || *tmp_reg == *reg)
			return -EINVAL;
	}
						     ...
						     u8 *rhs_tmp_reg)
{
	...
	*rhs_tmp_reg = kinsn_payload_reg(payload, 40);
	...
	if (!bpf_gpr_ok(*base_reg) || !bpf_gpr_ok(*index_reg) ||
	    !bpf_reg_ok(*rhs_reg) || !bpf_gpr_ok(*addr_tmp_reg) ||
	    !bpf_gpr_ok(*value_tmp_reg))
		return -EINVAL;
	if (*addr_tmp_reg == *base_reg || *addr_tmp_reg == *index_reg ||
	    *addr_tmp_reg == *rhs_reg || *value_tmp_reg == *base_reg ||
	    *value_tmp_reg == *index_reg || *addr_tmp_reg == *value_tmp_reg)
		return -EINVAL;
	if (kinsn_x86_reg_is_shadowed(*rhs_reg)) {
		if (!bpf_gpr_ok(*rhs_tmp_reg))
			return -EINVAL;
		if (*rhs_tmp_reg == *base_reg || *rhs_tmp_reg == *index_reg ||
		    *rhs_tmp_reg == *addr_tmp_reg ||
		    *rhs_tmp_reg == *value_tmp_reg)
			return -EINVAL;
	} else if (payload & (0xfULL << 40)) {
		return -EINVAL;
	}
...
static __always_inline int decode_setcc_emit_payload(u64 payload, u8 *dst_reg)
{
	...
	*dst_reg = kinsn_payload_reg(payload, 0);
	...
	if (kind == X86_FLAG_PROOF_STACK) {
		u8 tmp_high_reg = kinsn_payload_reg(payload, 4);
		u8 tmp_flag_reg = kinsn_payload_reg(payload, 8);

		if (payload >> 20)
			return -EINVAL;
		if (!bpf_reg_ok(*dst_reg) || !bpf_gpr_ok(tmp_high_reg) ||
		    !bpf_gpr_ok(tmp_flag_reg))
			return -EINVAL;
		if (tmp_high_reg == tmp_flag_reg)
			return -EINVAL;
		if (!kinsn_x86_reg_is_shadowed(*dst_reg) &&
		    (tmp_high_reg == *dst_reg || tmp_flag_reg == *dst_reg))
			return -EINVAL;
		return 0;
	}
	if (!bpf_gpr_ok(*dst_reg))
...
static int instantiate_cmp_rr_flags(u64 payload, struct bpf_insn *insn_buf,
				    bool is32)
{
	u8 left_reg, right_reg, left_tmp_reg, right_tmp_reg;
	...
	if (kinsn_x86_reg_is_shadowed(left_reg)) {
		insn_buf[cnt++] = BPF_LDX_MEM(BPF_DW, left_tmp_reg, BPF_REG_10,
					      kinsn_x86_shadow_reg_off(left_reg));
		left_eval_reg = left_tmp_reg;
	}
	if (kinsn_x86_reg_is_shadowed(right_reg)) {
		insn_buf[cnt++] = BPF_LDX_MEM(BPF_DW, right_tmp_reg, BPF_REG_10,
					      kinsn_x86_shadow_reg_off(right_reg));
		right_eval_reg = right_tmp_reg;
	}
...
static int instantiate_cmpq_sib_rr(u64 payload, struct bpf_insn *insn_buf)
{
	...
	u8 addr_tmp_reg, value_tmp_reg, rhs_tmp_reg, rhs_eval_reg;
	s16 offset;
	int add_count;
	int cnt = 0;
	int err;
	...
	if (kinsn_x86_reg_is_shadowed(rhs_reg)) {
		insn_buf[cnt++] = BPF_LDX_MEM(BPF_DW, rhs_tmp_reg, BPF_REG_10,
					      kinsn_x86_shadow_reg_off(rhs_reg));
		rhs_eval_reg = rhs_tmp_reg;
	}
	insn_buf[cnt++] = BPF_ST_MEM(BPF_W, BPF_REG_10,
				     KINSN_X86_SHADOW_ZF_OFF, 0);
...
static int instantiate_test_rr(u64 payload, struct bpf_insn *insn_buf)
{
	...
	insn_buf[cnt++] = BPF_JMP_REG(BPF_JEQ, left_eval_reg, right_eval_reg, 0);
	return cnt;
}

static int instantiate_testb_imm(u64 payload, struct bpf_insn *insn_buf)
{
	u8 reg, tmp_reg, eval_reg, imm;
	int cnt = 0;
	int err;
	...
	if (kinsn_x86_reg_is_shadowed(reg)) {
		...
		insn_buf[cnt++] = BPF_MOV64_REG(tmp_reg, reg);
		eval_reg = tmp_reg;
	} else {
		insn_buf[cnt++] = BPF_JMP_IMM(BPF_JEQ, eval_reg, imm, 0);
		return cnt;
	}
				  ...
				  bool move_on_set, bool is64, s16 flag_off)
{
	...
	insn_buf[cnt++] = BPF_LDX_MEM(BPF_W, tmp_reg, BPF_REG_10, flag_off);
	...
	if (kinsn_x86_reg_is_shadowed(src_reg)) {
		...
		if (kinsn_x86_reg_is_shadowed(dst_reg))
			...
			insn_buf[cnt++] = is64 ? BPF_MOV64_REG(dst_reg, tmp_reg) :
					      BPF_MOV32_REG(dst_reg, tmp_reg);
		return cnt;
	}
	if (kinsn_x86_reg_is_shadowed(dst_reg)) {
		if (is64) {
...
static int instantiate_cmov_eqne(u64 payload, struct bpf_insn *insn_buf,
				 bool move_on_eq, bool is64)
{
	if (kinsn_payload_reg(payload, 12) == X86_FLAG_PROOF_STACK)
				   ...
				   bool set_on_zf)
{
	...
	if (kinsn_x86_reg_is_shadowed(dst_reg))
		insn_buf[cnt++] = BPF_LDX_MEM(BPF_DW, tmp_high_reg, BPF_REG_10,
					      kinsn_x86_shadow_reg_off(dst_reg));
	else
		insn_buf[cnt++] = BPF_MOV64_REG(tmp_high_reg, dst_reg);
	insn_buf[cnt++] = BPF_ALU64_IMM(BPF_AND, tmp_high_reg, -256);
	...
	if (kinsn_x86_reg_is_shadowed(dst_reg))
		insn_buf[cnt++] = BPF_STX_MEM(BPF_DW, BPF_REG_10, tmp_high_reg,
					      kinsn_x86_shadow_reg_off(dst_reg));
	else
		insn_buf[cnt++] = BPF_MOV64_REG(dst_reg, tmp_high_reg);
	return cnt;
}
...
static int instantiate_setcc_dispatch(u64 payload, struct bpf_insn *insn_buf,
				      u8 op)
{
	if (kinsn_payload_reg(payload, 16) == X86_FLAG_PROOF_STACK) {
		if (op == BPF_JNE)
				...
				const struct bpf_prog *prog)
{
	u8 buf[16];
	u8 base_reg, index_reg, scale_log2, rhs_reg;
	u8 addr_tmp_reg, value_tmp_reg, rhs_tmp_reg;
	s16 offset;
	u32 len = 0;
	int err;
